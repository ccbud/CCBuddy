 ...
 *
 * 前置：docker 可用；已运行 `pnpm --filter @ccbuddy/server-cli stage --target <target>`。
...
const HOST_CAPABILITY_HEADER = "x-ccbuddy-rpc-host-capability";
...
const releaseArchive = join(packageRoot, "dist-release", `ccbuddy-server-${target}.tar.gz`);
const containerName = `ccbuddy-server-verify-${process.pid}`;
const imageTag = "ccbuddy-server-verify-sshd:ubuntu22";
...
async function main() {
  ...
  await run("ls", [releaseArchive], { quiet: true }).catch(() => {
    throw new Error(
      `Release archive missing: ${releaseArchive}; run pnpm --filter @ccbuddy/server-cli stage --target ${target}`,
    );
  });

  // 一次性 ssh 密钥，容器仅信任本次运行生成的公钥。
  const workDir = await mkdtemp(join(tmpdir(), "ccbuddy-server-verify-"));
  ...
  await ssh(
    `mkdir -p /root/ccbuddy-server && tar -xzf /root/ccbuddy-server-${target}.tar.gz -C /root/ccbuddy-server --strip-components=1`,
  );

  log("start daemon on remote");
  const { stdout: daemonOutput } = await ssh(
    "/root/ccbuddy-server/bin/ccbuddy serve --daemon --json",
  );
  ...
  await ssh(
    "cd /root/ccbuddy-server/runtime && ./node -e \"const pty=require('node-pty');const p=pty.spawn('/bin/echo',['pty-ok'],{cols:80,rows:24});let o='';p.onData(d=>o+=d);p.onExit(()=>{process.exit(o.includes('pty-ok')?0:1)})\"",
  );
  log("node-pty ok");

  log("verify remote lifecycle status/stop");
  const { stdout: statusOutput } = await ssh("/root/ccbuddy-server/bin/ccbuddy status --json");
  assert(
    JSON.parse(statusOutput.trim().split("\n").pop()).state === "ready",
    "remote status ready",
  );
  await ssh("/root/ccbuddy-server/bin/ccbuddy stop --json");
  const { stdout: stoppedOutput } = await ssh("/root/ccbuddy-server/bin/ccbuddy status --json");
