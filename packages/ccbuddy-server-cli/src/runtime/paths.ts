...
function getDefaultServerDataRoot(): string {
  const configured = process.env.CCBUDDY_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".ccbuddy", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\ccbuddy-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".ccbuddy") {
