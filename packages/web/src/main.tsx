...
import { connectViaWebSocket } from "@ccbuddy/client";
...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@ccbuddy/shared";
import { WEB_DEFAULT_THEME, resolveWebInitialTheme } from "./webThemeSeed.js";

function resolveWebThemePreference(defaultTheme: Theme = WEB_DEFAULT_THEME): Theme {
  const saved = localStorage.getItem("ccbuddy-theme");
...
function renderUnavailableSharePage(): void {
  const chinese = /^zh\b/i.test(navigator.language);
  document.title = chinese ? "CCbuddy 会话分享" : "CCbuddy Conversation Share";
  root.render(
    <main className="flex min-h-dvh items-center justify-center bg-background px-4 text-foreground">
      <section className="max-w-md rounded-xl border border-card-border bg-card p-6 text-center">
        <h1 className="text-lg font-semibold">CCbuddy</h1>
        <p className="mt-3 text-ui-base text-foreground-subtle">
          {chinese
            ? "此版本不提供云端会话分享。"
            : "Cloud conversation sharing is unavailable in this version."}
        </p>
      </section>
    </main>,
  );
...
async function bootstrapWebApp() {
  ...
  try {
    ...
    document.title = "CCbuddy - Web + Server";
