...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const ccbuddyJwtToken = tokenSet.ccbuddyJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (ccbuddyJwtToken) {
      this.localStorage.setItem(CCBUDDY_JWT_TOKEN_KEY, ccbuddyJwtToken);
    } else {
      this.localStorage.removeItem(CCBUDDY_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const ccbuddyJwtToken = this.localStorage.getItem(CCBUDDY_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(ccbuddyJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(ccbuddyJwtToken, this.now()).kind === "expired") {
  ...
  loadCCbuddyJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(CCBUDDY_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(CCBUDDY_JWT_TOKEN_KEY) ||
