...
import { BIGMODEL_PROVIDER_ID, ZAI_PROVIDER_ID } from "@ccbuddy/shared";
import type { ConversationSharePreview } from "@ccbuddy/shared";
import { ConversationShareReadonlyTimeline } from "@ccbuddy/ui/conversation-share-readonly";
import { renderOAuthProviderIcon } from "@ccbuddy/ui/oauth-provider-icon";
import { applyTheme, resolveTheme, type Theme } from "@ccbuddy/ui/useTheme";
...
// 修复原因：旧分享页会把 CCbuddy 用户导向 CCbuddy 官网；发行站点未就绪时只指向本项目。
const CCBUDDY_PROJECT_URL = "https://github.com/ccbud/CCBuddy";
...
const COPY: Record<ConversationShareLandingLocale, Copy> = {
  ...
  "en-US": {
    ...
    projectHome: "View CCbuddy project",
    ...
    outdatedTitle: "Update CCbuddy to continue",
    outdatedDescription:
      "This share was created by a newer version of CCbuddy. Please update to view it.",
    ...
    continueHelp: "If the desktop app did not open, check its link association or try again.",
    openCCbuddyProject: "View CCbuddy project",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {importLink && headerView.continueVisible ? (
                      <a
                        ...
                        href={importLink}
                        aria-label={copy.continueOnDesktop}
                        title={copy.continueOnDesktop}
                        onClick={handleContinue}
                      >
                        <span className={headerView.continueCompact ? "sr-only" : "truncate"}>
                          {copy.continueOnDesktop}
                        </span>
                        <ArrowUpRightIcon
                          className={
                            headerView.continueCompact
                  ...
                  >
                    ...
                    {importLink ? (
                      <span className={SHARE_CONTINUE_MEASURE_CLASS}>
                        <span>{copy.continueOnDesktop}</span>
                        <ArrowUpRightIcon
        ...
        {importLink && showContinueHelp ? (
          ...
          <ShareContentInset>
            ...
            >
              ...
              <a className="text-brand underline underline-offset-2" href={CCBUDDY_PROJECT_URL}>
                {copy.openCCbuddyProject}
              </a>
            </div>
          </ShareContentInset>
        ) : null}
          ...
          <ShareContentInset>
            ...
            >
              <a
                ...
                onClick={handleContinue}
              >
                <span className="truncate">{copy.continueOnDesktop}</span>
...
export function ConversationShareLandingLoader({
  ...
  const handleThemeChange = useCallback((nextTheme: Theme) => {
    localStorage.setItem("ccbuddy-theme", nextTheme);
