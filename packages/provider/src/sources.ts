...
export interface ProviderConfigSnapshot {
  ...
  readonly ccbuddyBuiltinProviders: ProviderConfigMap;
  readonly ccbuddyBuiltinProviderTemplates: ProviderTemplateMap;
  readonly personalProviders: ProviderConfigMap;
  readonly ccbuddyBuiltinModelRules: ModelConfigRules;
...
export function createFailClosedAccountProviderConfigSnapshot(
  ...
  const unentitledProviders = new ProviderConfigMap(
    config.ccbuddyBuiltinProviders.entries().flatMap(([providerId, provider]) =>
  ...
  return createAccountProviderConfigSnapshot(config.ccbuddyBuiltinRevision, unentitledProviders);
}

export function createAccountProviderConfigSnapshot(
  basedOnCCbuddyBuiltinRevision: string,
  providers: ProviderConfigMap,
  states?: AccountProviderStates,
): AccountProviderConfigSnapshot {
  return Object.freeze({
    revision: `account:${JSON.stringify([basedOnCCbuddyBuiltinRevision, providers.toJSON(), states])}`,
...
const EMPTY_ACCOUNT_PROVIDER_CONFIG_SNAPSHOT: AccountProviderConfigSnapshot = Object.freeze({
  ...
  basedOnCCbuddyBuiltinRevision: "uninitialized",
