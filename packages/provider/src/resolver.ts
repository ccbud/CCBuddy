...
import type { completeModelConfigDataSchema } from "@ccbuddy/shared/model-config";
import type {
  ...
  resolve(input: ProviderConfigResolverInput): ProviderConfigResolution {
    const accountProviders = new ProviderConfigMap(
      input.accountProviders
        .entries()
        .filter(([providerId]) => input.ccbuddyBuiltinProviders.has(providerId))
        .map(([providerId, config]) => [providerId, config.withoutGroup()] as const),
    );
    const concreteBuiltinProviders = input.ccbuddyBuiltinProviders.overlay(accountProviders);
    const providerTemplates = input.ccbuddyBuiltinProviderTemplates;
    ...
    const effectiveModelRules = ModelConfigRules.composeEffective(
      input.ccbuddyBuiltinModelRules,
    ...
    for (const providerId of resolveProviderOrder(input, effectiveProviders)) {
      ...
      const models = orderedModelIds.map((modelId): ResolvedProviderModel => {
        ...
        const effectiveBuiltinConfig = input.ccbuddyBuiltinModelRules.resolve({
          providerId,
          templateId,
          modelId,
          apiType: config.api?.type,
...
function resolveProviderOrder(
  ...
  const builtinIds = input.ccbuddyBuiltinProviders
    .keys()
