  ...
  async #runRefreshLoop(): Promise<AccountProviderConfigSnapshot> {
    ...
    while (this.#pendingReasons.size > 0) {
      ...
      try {
        ...
        const configuredProviders = config.ccbuddyBuiltinProviders;
        const { providers, states } = await this.#resolve({
          configRevision: config.ccbuddyBuiltinRevision,
          configuredProviders,
        ...
        if (
          ...
          currentConfig.ccbuddyBuiltinRevision !== config.ccbuddyBuiltinRevision
        ) {
          this.#pendingReasons.add("superseded-resolution");
          continue;
        }
        const basedOnCCbuddyBuiltinRevision = config.ccbuddyBuiltinRevision;
