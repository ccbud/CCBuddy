...
import type { ModelSelection } from "@ccbuddy/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly ccbuddyBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#ccbuddyBuiltinSource = dependencies.ccbuddyBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#ccbuddyBuiltinSource.onDidChange((reason) => this.#emit(`ccbuddyBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [ccbuddyBuiltin, personal] = await Promise.all([
      this.#ccbuddyBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([ccbuddyBuiltin.revision, personal.revision]),
      ccbuddyBuiltinRevision: ccbuddyBuiltin.revision,
      personalRevision: personal.revision,
      ccbuddyBuiltinProviders: ccbuddyBuiltin.providers,
      ccbuddyBuiltinProviderTemplates:
        ccbuddyBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      ccbuddyBuiltinModelRules: ccbuddyBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const ccbuddyBuiltin = await this.#ccbuddyBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = ccbuddyBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = ccbuddyBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 CCbuddy Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(ccbuddyBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = ccbuddyBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? ccbuddyBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...ccbuddyBuiltin.providers.keys(), ...current.providers.keys()]);
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        ccbuddyBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(ccbuddyBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(ccbuddyBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(ccbuddyBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(ccbuddyBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#ccbuddyBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _ccbuddyBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  ccbuddyBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    ccbuddyBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(ccbuddyBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? ccbuddyBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return ccbuddyBuiltin.providers.overlay(personal);
}
