...
const desktopProductIdentity = resolveDesktopProductIdentity({
  ...process.env,
  CCBUDDY_ENV: builtinProviderConfig.environment,
});
...
const shouldEnableMacSigning =
  process.env.CCBUDDY_ENABLE_MAC_SIGN === "1" && Boolean(macSigningIdentity);
...
const desktopDistDir = process.env.CCBUDDY_DESKTOP_DIST_DIR || "dist";
...
const REQUIRED_ASAR_RUNTIME_MODULES = [
  ...
  "pngjs",
  // @ccbuddy/services 的代理连通性探测会动态 require("undici") 取 ProxyAgent。
...
const WINDOWS_INSTALL_MANIFEST_NAME = ".ccbuddy-install-manifest";
...
function resolveElectronDownloadMirror(env = process.env) {
  const existingMirror =
    env.CCBUDDY_ELECTRON_RUNTIME_MIRROR ||
...
if (
  ...
  process.env.CCBUDDY_ENABLE_MAC_SIGN === "1" &&
  !macSigningIdentity
) {
  throw new Error(
    "CCbuddy Preview macOS packaging requires APPLE_SIGNING_IDENTITY or CSC_NAME when CCBUDDY_ENABLE_MAC_SIGN=1",
  );
}
...
function resolveAppAsarPath(context) {
  if (context.electronPlatformName === "darwin") {
    const appName = `${context.packager?.appInfo?.productFilename ?? "CCbuddy"}.app`;
...
async function injectHoistedRuntimeModulesIntoAsar(context) {
  ...
  const stagingDir = mkdtempSync(resolve(tmpdir(), "ccbuddy-app-asar-"));
  try {
...
export default {
  ...
  extraMetadata: {
    ...
    ccbuddyProductFlavor: desktopProductIdentity.flavor,
    // 修复原因：继承的 CCbuddy 主页会让 CCbuddy 安装包的发布者信息指向上游产品。
    homepage: "https://github.com/ccbud/CCBuddy",
    author: {
      name: "CCbuddy contributors",
    },
  },
  ...
  extraResources: [
      ...
      ? [
          {
            ...
            // 只是不吸附，所以这里不做存在性断言。
            from: "resources/macos-window-bounds/ccbuddy-window-bounds",
            to: "macos-window-bounds/ccbuddy-window-bounds",
          },
        ]
      : []),
    {
    ...
    {
      // Provider Registry 的 CCbuddy Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      from: builtinProviderConfig.sourcePath,
      to: "config/provider/ccbuddy-builtin.json",
    },
    {
    ...
    {
      // About 小尺寸徽标沿用侧栏的 128px 原图，直接填满图标区域。
      from: "build/icons/128x128.png",
      to: "ccbuddy-about-icon.png",
    },
      ...
      : []),
    {
      ...
      // 桌面端内置的是 agent 的 JS bundle（glm/ccbuddy.cjs，由 prepare:agent-bundle 生成），
      // Host 进程用 app 自带的 Electron Node runtime（ELECTRON_RUN_AS_NODE）执行 `ccbuddy.cjs app-server --stdio`，
  ...
  protocols: [
    {
      ...
      // 展示名跟随安装包身份；scheme 使用 ccbuddy，与上游产品独立。
  ...
  mac: {
    ...
    // ccbuddy 之前只有本地未签名打包配置，CI 即使注入了证书变量，
  ...
  linux: {
    ...
    // desktop 包名是 scoped package（@ccbuddy/desktop），electron-builder 默认会把
    // Linux executable/Icon 推成 @ccbuddydesktop。部分桌面环境无法按这个 icon name 命中
    // hicolor 图标，最终回退成系统齿轮。这里固定成稳定的小写名称，让 Icon=ccbuddy
    // 与 /usr/share/icons/hicolor/*/apps/ccbuddy.png 保持一致。
    executableName: desktopProductIdentity.linuxExecutableName,
    category: "Development",
    maintainer: "CCbuddy contributors",
  },
  deb: {
