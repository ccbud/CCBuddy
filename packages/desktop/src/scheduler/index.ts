...
function resolveScheduledAt(automation: CCbuddyAutomation, now: number): number {
...
async function handleClaimed(automation: CCbuddyAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: CCbuddyAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: CCbuddyOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: CCbuddyAutomationTrigger =
