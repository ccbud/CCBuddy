...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@ccbuddy/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "ccbuddy-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "ccbuddy.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "ccbuddy.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@ccbuddy/local-ttft", "1");
  const duration = meter.createHistogram("ccbuddy.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("ccbuddy.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("ccbuddy.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "ccbuddy.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("ccbuddy.local_ttft.system.duration", {
    unit: "ms",
  });
  const preparationDuration = meter.createHistogram("ccbuddy.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("ccbuddy.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("ccbuddy.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("ccbuddy.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("ccbuddy.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("ccbuddy.local_ttft.records");
  const drops = meter.createCounter("ccbuddy.local_ttft.dropped");
