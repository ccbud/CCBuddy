...
function buildElectronManifestUrl(options: {
  ...
  const url = options.manifestUrl?.trim()
    ...
    : new URL(ELECTRON_MANIFEST_API_PATH, normalizeCCbuddyEndpointOrigin(options.endpointOrigin));
...
export class ManifestUpdateProvider extends Provider<UpdateInfo> {
  ...
  private resolveBaseUrl = new URL(DEFAULT_CCBUDDY_ENDPOINT_ORIGIN);
  ...
  constructor(
    ...
    this.resolveBaseUrl = new URL(
      normalizeCCbuddyEndpointOrigin(options.endpointOrigin ?? DEFAULT_CCBUDDY_ENDPOINT_ORIGIN),
    );
  }
  ...
  override async getLatestVersion(): Promise<UpdateInfo> {
    ...
    // 修复原因：electron-builder 的 latest-mac.yml 把 ZIP 写成相对文件名；
    // 归档 URL 必须以 manifest 所在目录为基准，不能退到域名根目录。
    ...
    return {
      ...
      ccbuddyReleaseChannel: releaseChannel,
    } as UpdateInfo;
  }
  ...
  private async resolveEndpointOrigin(): Promise<string> {
    const resolved =
      ...
      DEFAULT_CCBUDDY_ENDPOINT_ORIGIN;
    return normalizeCCbuddyEndpointOrigin(resolved);
  }
