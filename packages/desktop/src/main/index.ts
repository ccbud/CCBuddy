...
import { isTrustedHistoryMainWindow, registerHistoryReviewIpc } from "./historyReviewIpc.js";
...
import {
  ...
  updateCCbuddyStdioTapDevMenuState,
...
import { resolveCCbuddyBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveCCbuddyEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopCCbuddyDataSizeTelemetry,
  stopDesktopCCbuddyDataSizeTelemetry,
} from "./desktopCCbuddyDataSizeTelemetry.js";
...
import { mapCCbuddyEnvToArmsRumEnv } from "@ccbuddy/shared";
import {
...
// 会和开发态已打开的 CCbuddy Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.CCBUDDY_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".ccbuddy", "v2", "setting.json");
...
async function resolveCurrentCCbuddyEndpointOrigin() {
  return resolveCCbuddyEndpointOrigin({
    env: CCBUDDY_ENV,
    envBaseOrigin: resolveCCbuddyEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).ccbuddyEndpointOrigin,
  });
}
...
const appTelemetryCore = createTelemetryCore({
  ...
  resolveCCbuddyEndpointOrigin: resolveCurrentCCbuddyEndpointOrigin,
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 CCbuddy 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
// CCbuddy help links are local constants; the copied cloud config endpoint is not queried.
const readHelpConfig = createDesktopHelpConfigReader();
// CCbuddy uses local BYOK configuration; inherited cloud rollout decisions are off.
const localDisabledClientConfigsFetcher = async (_signal: AbortSignal): Promise<unknown> => ({
  code: 0,
  data: { configs: { desktopContextPrompt: { enabled: false }, rendererActionTrace: null } },
});
desktopContextPromptRollout = createDesktopContextPromptRollout({
  fetchConfig: localDisabledClientConfigsFetcher,
  logger,
});
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: CCBUDDY_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    CCBUDDY_TELEMETRY_ENABLED && CCBUDDY_ARMS_RUM_ENDPOINT
...
const primaryWindowCoordinator = createPrimaryWindowCoordinator({
  listWindows: getRegisteredMainApplicationWindows,
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，ccbuddy-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.CCBUDDY_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return CCBUDDY_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${CCBUDDY_ENV}`);
    return true;
  }
  ...
  const dialogOptions = {
    ...
    message: isZh ? "确认退出 CCbuddy?" : "Quit CCbuddy?",
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onCCbuddyEndpointChanged: handleCCbuddyEndpointChanged,
    ccbuddyEndpointEnvBaseOrigin: resolveCCbuddyEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveCCbuddyEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (CCBUDDY_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentCCbuddyEndpointOrigin();
  if (origin === DEFAULT_CCBUDDY_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleCCbuddyEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveCCbuddyEndpointSelection(), mainSettingService.get()]).then(
    ([ccbuddyEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        ccbuddyEndpointSelection,
        openHistoryReview: () => {
          void primaryWindowCoordinator
            .ensurePrimaryWindow("history-menu")
            .then(() => {
              const focusedWindow = BrowserWindow.getFocusedWindow();
              const target =
                (focusedWindow && isMainApplicationWindowWebContents(focusedWindow.webContents.id)
                  ? focusedWindow
                  : getRegisteredMainApplicationWindows()[0]) ?? null;
              if (target) openHistoryInMainWindow(target, "list");
            })
            .catch((error: unknown) => logger.error("[history] failed to open main window", error));
        },
...
function openHistoryInMainWindow(window: BrowserWindow, view: "list" | "timeline") {
  if (!isMainApplicationWindowWebContents(window.webContents.id)) return;
  if (window.isMinimized()) window.restore();
  window.show();
  window.focus();
  const send = () => {
    if (isTrustedHistoryMainWindow(window)) {
      window.webContents.send(PlatformChannels.OpenHistoryViewRequested, view);
    }
  };
...
function getRegisteredMainApplicationWindows(): BrowserWindow[] {
  return getMainApplicationWindows().filter((win) =>
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          ccbuddyBuiltinProviderConfigFilePath: resolveCCbuddyBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.on("open-url", (event, url) => {
  ...
  if (workspacePath && getApplicationWindowsExcludingCuaIndicator().length === 0) {
    // macOS 冷启动收到外部 open-url 时可能还没有首窗。
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(CCBUDDY_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // 设置仍与 CCbuddy 共享；无 CCbuddy 发行源时不能把上游 pending 版本读成可安装更新。
  ...
  // CCbuddy 使用独立版本线。未在构建期配置自有发行源时必须禁用继承的 CCbuddy 更新器，
  // 否则后台检查可能把 CCbuddy 安装包当成 CCbuddy 的新版本。
  void initAutoUpdater({
    enabled: desktopUpdatePolicy.autoUpdateEnabled,
    ...
    locale: currentApplicationLocale,
    deviceMid,
    resolveEndpointOrigin: resolveCurrentCCbuddyEndpointOrigin,
    updateFeedSource: desktopUpdatePolicy.updateFeedSource,
  });
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapCCbuddyEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (CCBUDDY_TELEMETRY_ENABLED && CCBUDDY_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: CCBUDDY_VERSION,
  ...
  registerDesktopCCbuddyDataSizeTelemetry({
    ...
    isCCbuddyBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getCCbuddyDataRootDir(),
    stateFile: join(app.getPath("userData"), "ccbuddy-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // CCbuddy 没有自有的最低版本策略。旧 CCbuddy /client/configs 中的 minimalVersion 属于
  // 另一条发行线，不能再阻止 CCbuddy 创建首窗（例如 0.1.0 与上游 3.5.3）。
  logger.info("[startup] CCbuddy 跳过继承的 CCbuddy 强制升级门槛");
