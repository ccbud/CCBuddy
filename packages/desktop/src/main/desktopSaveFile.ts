...
import type { SaveFileRequest, SaveFileResult } from "@ccbuddy/shared";
import { PlatformChannels } from "@ccbuddy/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "ccbuddy-save-file-"));
