...
import type { ISettingService } from "@ccbuddy/services";
import {
  DEFAULT_LOCALE,
...
const UPDATE_FEED_URL_ENV = "CCBUDDY_UPDATE_FEED_URL";
const UPDATE_FEED_URL_SWITCH = "--ccbuddy-update-feed-url";
const DEV_AUTO_UPDATE_ENV = "CCBUDDY_AUTO_UPDATE_DEV";
const DEV_AUTO_UPDATE_SWITCH = "--ccbuddy-auto-update-dev";
const DEV_AUTO_UPDATE_VERSION_ENV = "CCBUDDY_AUTO_UPDATE_DEV_VERSION";
const DEV_AUTO_UPDATE_VERSION_SWITCH = "--ccbuddy-auto-update-dev-version";
...
type UpdateDownloadedInfoLike = {
  ...
  ccbuddyReleaseChannel?: ElectronReleaseChannel | null;
...
function readUpdateInfoReleaseChannel(
  ...
  return info.ccbuddyReleaseChannel === "preview" || info.ccbuddyReleaseChannel === "stable"
    ? info.ccbuddyReleaseChannel
    : null;
}
...
function applyManifestUpdateProvider(options: InitAutoUpdaterOptions): void {
  ...
  autoUpdater.setFeedURL({
    ...
    endpointOrigin: DEFAULT_CCBUDDY_ENDPOINT_ORIGIN,
    ...(manifestUrl ? { manifestUrl } : {}),
    releasePlatform: getElectronReleasePlatform(),
    deviceMid: options.deviceMid,
    resolveEndpointOrigin:
      options.resolveEndpointOrigin ?? (() => resolveRuntimeCCbuddyEndpointOrigin(process.env)),
...
async function persistPendingPostUpdateReleaseNotes(
  ...
  await settingService.update({ ccbuddyPendingPostUpdateReleaseNotes: payload });
...
async function clearPendingPostUpdateReleaseNotes(
  ...
  await settingService.update({ ccbuddyPendingPostUpdateReleaseNotes: undefined });
...
function clearPersistedPostUpdateReleaseNotesForVersion(version: string, reason: string) {
  ...
  void (async () => {
    try {
      const settings = await settingService.get();
      if (settings.ccbuddyPendingPostUpdateReleaseNotes?.version !== version) {
        return;
      }
...
export async function hydratePendingPostUpdateReleaseNotes(settingService: SettingServiceLike) {
  ...
  const settings = await settingService.get();
  pendingPostUpdateReleaseNotes = settings.ccbuddyPendingPostUpdateReleaseNotes ?? null;
  deliveredPostUpdateReleaseNotesWebContentsId = null;
...
export function syncReadyUpdateToWindow(win: BrowserWindow) {
  if (autoUpdaterDisabledForProductFlavor || !readyUpdateVersion || win.isDestroyed()) {
    return;
  }
...
export function refreshAutoUpdaterReleaseChannel(
  ...
  if (autoUpdaterDisabledForProductFlavor) {
    // 设置同步可能在启动禁用 updater 后发生；此入口不能再检查未配置的上游 feed。
    logger.info(`[auto-update] skip ${reason}: no CCbuddy update feed`);
    return;
  }
...
export function syncPostUpdateReleaseNotesToWindow(win: BrowserWindow) {
  if (autoUpdaterDisabledForProductFlavor || !pendingPostUpdateReleaseNotes || win.isDestroyed()) {
    return;
  }
...
export async function initAutoUpdater(options: InitAutoUpdaterOptions = {}): Promise<void> {
  if (options.enabled === false) {
    ...
    // 共享设置中可能残留 CCbuddy 的 pending 版本。只丢弃 CCbuddy 进程内投影，
    // 不改写磁盘上的原始设置；两种窗口同步出口也必须继续按本策略拒绝发送。
    clearReadyUpdateState();
    clearAvailableUpdateState();
    clearDownloadingUpdateState();
    pendingPostUpdateReleaseNotes = null;
    deliveredPostUpdateReleaseNotesWebContentsId = null;
    // 禁用时同步主进程唯一更新状态，renderer 和原生菜单不能继续显示可用的更新入口。
...
export function requestForceAutoUpdate(
  ...
  if (autoUpdaterDisabledForProductFlavor) {
    // 即使旧强更调用方误触发，也不能绕过 CCbuddy 的发行源策略发起检查。
    onStateChange({ kind: "error", message: "CCbuddy update feed is not configured" });
    return dispose;
  }
