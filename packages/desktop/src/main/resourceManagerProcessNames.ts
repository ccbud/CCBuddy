...
import { formatCCbuddyRendererProcessName } from "@ccbuddy/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatCCbuddyRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatCCbuddyRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatCCbuddyRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
