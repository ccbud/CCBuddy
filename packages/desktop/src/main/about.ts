...
const ABOUT_APPLICATION_NAME = "CCbuddy";
...
> = {
  "zh-CN": {
    aboutTitle: "关于 CCbuddy",
    versionLabel: "版本",
    okButtonLabel: "确定",
    optimizedForAppleSilicon: "已针对 Apple Silicon 优化。",
    copyright: (year) => `版权所有 © ${year} CCbuddy。`,
  },
  "en-US": {
    aboutTitle: "About CCbuddy",
    versionLabel: "version",
    okButtonLabel: "OK",
    optimizedForAppleSilicon: "Optimized for Apple Silicon.",
    copyright: (year) => `Copyright © ${year} CCbuddy.`,
  },
};
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? CCBUDDY_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? CCBUDDY_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? CCBUDDY_BUILD_TIME),
    environment: normalizeValue(options.environment ?? CCBUDDY_ENV),
...
async function readAboutArtworkDataUrl(isPackaged: boolean): Promise<string | null> {
  const artworkPath = isPackaged
    ? join(process.resourcesPath, "ccbuddy-about-icon.png")
    : join(import.meta.dirname, "../../build/icons/128x128.png");
  try {
    return `data:image/png;base64,${(await readFile(artworkPath)).toString("base64")}`;
  } catch {
    // Bugfix: About 曾内嵌旧底座字母图形；图片缺失时只显示产品名，不再回退旧标识。
    return null;
  }
}
...
export async function showAboutDialog(
  ...
  const iconDataUrl = await readAboutArtworkDataUrl(app.isPackaged);
