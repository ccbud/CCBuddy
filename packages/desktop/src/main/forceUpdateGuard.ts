...
const CCBUDDY_CLIENT_CONFIG_API_PATH = "/api/v1/client/configs";
...
function resolveForceUpdateClientConfigUrl(
  ...
  const url = new URL(
    `${buildCCbuddyEndpointUrls(endpointOrigin).origin}${CCBUDDY_CLIENT_CONFIG_API_PATH}`,
  );
...
function getForceUpdateMinimalVersionFromClientConfig(config: unknown): string | undefined {
  ...
  if (typeof envelope.code === "number" && envelope.code !== 0) {
    ...
    throw new Error(`CCbuddy update config failed: ${envelope.code}`);
  }
...
async function resolveDesktopForceUpdateRequirement(options: {
  ...
  const resolveFromConfig = (config: unknown) =>
    resolveForceUpdateRequirement({
      currentVersion: CCBUDDY_VERSION,
      forceUpdate: {
        minimalVersion:
...
function resolveForceUpdateDownloadUrl(
  ...
  const origin = buildCCbuddyEndpointUrls(endpointOrigin).origin;
