...
import type { ChromeBrowserDataImportError } from "@ccbuddy/shared";
...
export async function readChromeCookiesWithHelper(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "ccbuddy-chrome-cookie-helper-"));
...
export async function importChromeLocalStorage(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "ccbuddy-chrome-local-storage-"));
  const userDataPath = join(tempRoot, "User Data");
  const targetProfilePath = join(userDataPath, "Default");
  let originCount = 0;
  try {
