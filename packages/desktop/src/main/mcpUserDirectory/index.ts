...
import { getWorkspaceAppConfigDir } from "@ccbuddy/services/node";
import type {
  CliMcpSource,
...
const CCBUDDY_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  ...
  userConfigDirSegments: [".ccbuddy", "cli"],
  workspaceConfigDirSegments: ["cli"],
...
function buildDirectoryConfigPath(
  ...
  const baseDir =
    scope === "user"
      ? resolveUserHomeDir()
      : workspacePath && descriptor === CCBUDDY_MCP_DESCRIPTOR
...
function getUserCliConfigPath(): string {
  return buildDirectoryConfigPath(CCBUDDY_MCP_DESCRIPTOR, "user");
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  // `.ccbuddy` 是强优先级来源；只要读到 MCP server，同 scope 的 `.agents` 就不再参与。
...
export async function loadCliMcpFromUserDirectory(
  ...
  // 去掉其他 provider 后，CCbuddy Agent 只按目录约定读取；先 workspace，再 user。
