...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@ccbuddy/shared";

import type { CCbuddyDataSizeScanResult } from "./ccbuddyDataSizeScanner.js";
import {
  readCCbuddyDataSizeTelemetryState,
  writeCCbuddyDataSizeTelemetryState,
  type CCbuddyDataSizeTelemetryState,
} from "./ccbuddyDataSizeTelemetryState.js";
import { scanCCbuddyDataDirectoryInWorker } from "./ccbuddyDataSizeWorkerClient.js";

export type { CCbuddyDataSizeTelemetryState } from "./ccbuddyDataSizeTelemetryState.js";

const CCBUDDY_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const CCBUDDY_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const CCBUDDY_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const CCBUDDY_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const CCBUDDY_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const CCBUDDY_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const CCBUDDY_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const CCBUDDY_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const CCBUDDY_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const CCBUDDY_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const CCBUDDY_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const CCBUDDY_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface CCbuddyDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: CCbuddyDataSizeTelemetryTiming = {
  abortedRetryMs: CCBUDDY_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: CCBUDDY_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: CCBUDDY_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: CCBUDDY_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: CCBUDDY_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: CCBUDDY_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: CCBUDDY_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: CCBUDDY_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: CCBUDDY_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: CCBUDDY_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface CCbuddyDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface CCbuddyDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isCCbuddyBusy: () => boolean;
  logger: CCbuddyDataSizeTelemetryLogger;
  readState: () => Promise<CCbuddyDataSizeTelemetryState | null>;
  report: (result: CCbuddyDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<CCbuddyDataSizeScanResult>;
  timing?: Partial<CCbuddyDataSizeTelemetryTiming>;
  writeState: (state: CCbuddyDataSizeTelemetryState) => Promise<void>;
}
...
function buildCCbuddyDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: CCbuddyDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_ccbuddy_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      metric_kind: "ccbuddy_data_bytes",
      ...
      schema_version: 1,
      ccbuddy_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / CCBUDDY_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createCCbuddyDataSizeTelemetryScheduler(
  dependencies: CCbuddyDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: CCbuddyDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isCCbuddyBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[ccbuddy-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<CCbuddyDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: CCbuddyDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[ccbuddy-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("CCbuddy data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("CCbuddy data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[ccbuddy-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[ccbuddy-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: CCbuddyDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[ccbuddy-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createCCbuddyDataSizeTelemetryScheduler> | null = null;

export function registerDesktopCCbuddyDataSizeTelemetry(options: {
  context: Parameters<typeof buildCCbuddyDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isCCbuddyBusy: () => boolean;
  logger: CCbuddyDataSizeTelemetryLogger;
  ...
  stopDesktopCCbuddyDataSizeTelemetry();
  desktopScheduler = createCCbuddyDataSizeTelemetryScheduler({
    ...
    isCCbuddyBusy: options.isCCbuddyBusy,
    logger: options.logger,
    readState: () => readCCbuddyDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildCCbuddyDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanCCbuddyDataDirectoryInWorker(
        {
          maxDurationMs: CCBUDDY_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: CCBUDDY_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeCCbuddyDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[ccbuddy-data-size] scheduler start failed", error);
  });
}

export function stopDesktopCCbuddyDataSizeTelemetry(): void {
