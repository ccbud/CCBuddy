...
import { PlatformChannels, type RendererActionTraceConfigV1 } from "@ccbuddy/shared";
...
function resolveRuntimeConfig(
  ...
  if (isTruthy(env.CCBUDDY_LOCAL_TTFT_ENABLED)) config = { ...config, localTtftEnabled: true };
  if (!isTruthy(env.CCBUDDY_RENDERER_ACTION_TRACE_ENABLED)) return config;
  return {
    ...config,
    enabled: true,
    sampleRatio: 1,
