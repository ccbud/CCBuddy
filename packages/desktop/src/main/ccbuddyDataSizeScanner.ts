...
export type CCbuddyDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type CCbuddyDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: CCbuddyDataSizePartialReason }
);

export interface CCbuddyDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface CCbuddyDataSizeScanOptions extends CCbuddyDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("CCbuddy data size scan aborted", "AbortError");
}
 ...
 */
export async function scanCCbuddyDataDirectory(
  options: CCbuddyDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<CCbuddyDataSizePartialReason, "io_error"> | null = null;
...
export function isCCbuddyDataSizeScanResult(value: unknown): value is CCbuddyDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<CCbuddyDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
