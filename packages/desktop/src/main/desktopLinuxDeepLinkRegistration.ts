...
// 修复原因：CCbuddy 写入 ccbuddy.desktop 会覆盖上游应用的用户级入口，并把其归属标记误认为自身。
const LINUX_DEEP_LINK_DESKTOP_FILE = "ccbuddy.desktop";
const LINUX_DEEP_LINK_MIME_TYPE = "x-scheme-handler/ccbuddy";
const LINUX_DESKTOP_ENTRY_OWNERSHIP_MARKER = "Comment=CCbuddy Desktop App";
...
function resolveLinuxDeepLinkCommand(params: {
  ...
  return {
    ...
    // AppImage 的 ccbuddy:// 回调会由 xdg-open 按 .desktop Exec 二次启动。
...
function createLinuxDeepLinkDesktopEntry(params: {
  ...
  const productName = params.productName ?? "CCbuddy";
  const iconName = params.iconName ?? "ccbuddy";
  const command = {
...
function removeOwnedUserDesktopEntry(
  ...
  if (!isOwnedDesktopEntry(desktopFilePath)) {
    logger.warn("[deep-link] Linux 用户级 ccbuddy.desktop 非本应用写入，保留不清理", {
      desktopFilePath,
    });
    return;
  }
  try {
    ...
    logger.info("[deep-link] 已清理遗留的用户级 ccbuddy.desktop，恢复系统级条目", {
      desktopFilePath,
    });
  } catch (error) {
    logger.warn("[deep-link] 清理遗留用户级 ccbuddy.desktop 失败", { desktopFilePath, error });
  }
}
...
export function registerLinuxDeepLinkProtocol(options: RegisterLinuxDeepLinkProtocolOptions): void {
  ...
  // 用户级 ccbuddy.desktop 在 XDG
  // 解析中永远优先于系统级同名条目。rpm/deb 安装后，旧 AppImage 写入的用户级条目会把
  // /usr/share/applications/ccbuddy.desktop 持续遮蔽，快捷方式和 ccbuddy:// deep link 一直
  ...
  // 用户手写的自定义 ccbuddy.desktop（无归属标记）不受影响，保留不清理。
