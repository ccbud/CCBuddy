...
import type { ConnectOptions } from "@ccbuddy/server/remote";
import { listSSHConfigAliasesFromLocalConfig } from "@ccbuddy/services/node";
...
import {
  ...
  CCBUDDY_DYNAMIC_WORKFLOW_MODE_ENV,
  CCBUDDY_ENV,
  CCBUDDY_PRODUCT_FLAVOR,
  CCBUDDY_RUNTIME_ENV_KEY,
  CCBUDDY_VERSION,
  buildCCbuddyToolEnvPassthroughEnv,
...
export const desktopRuntimeEnv: CCbuddyRuntimeEnv = isLocalDevelopmentRuntime
  ? "development"
  : "production";
// 身份看编译期 flavor 而不是 CCBUDDY_ENV：CCBUDDY_PREVIEW_IDENTITY=1 的生产后端构建同样是 Preview，
// 需要独立的应用名、Electron 数据目录和 Helper 安装子目录才能与正式版并排运行。
const isPreviewPackagedRuntime = !isLocalDevelopmentRuntime && CCBUDDY_PRODUCT_FLAVOR === "preview";
...
// e2e 默认工作区依赖 home 路径，因此提供显式覆盖，避免测试写到开发者真实 ~/CCbuddyProject。
export const runtimeHomePath = readRuntimeEnvOverride("CCBUDDY_DESKTOP_HOME_DIR");
...
export const shouldUseElectronDefaultUserDataPath = isTruthyRuntimeEnvOverride(
  "CCBUDDY_DESKTOP_USE_ELECTRON_DEFAULT_USER_DATA",
);
export const runtimeUserDataPath =
  readRuntimeEnvOverride("CCBUDDY_DESKTOP_USER_DATA_DIR") ??
  (shouldUseElectronDefaultUserDataPath
    ? undefined
    : join(runtimeHomePath ?? homedir(), ".ccbuddy", "electron"));
export const runtimeSessionDataPath =
  readRuntimeEnvOverride("CCBUDDY_DESKTOP_SESSION_DATA_DIR") ??
...
export async function isDockerDaemonAvailable(): Promise<boolean> {
  const { isDockerAvailable } = await import("@ccbuddy/server/remote");
  return isDockerAvailable();
}

export async function listAvailableWSLDistros() {
  const { listWSLDistros } = await import("@ccbuddy/server/remote");
  return listWSLDistros();
}

export async function listAvailableDockerContainers() {
  const { listDockerContainers } = await import("@ccbuddy/server/remote");
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    ...
    return { CCBUDDY_TELEMETRY_RUNTIME_DISTRIBUTION: "packaged" };
  }
  ...
  return applySelectedCCbuddyEnvLinks(merged);
}
...
function resolveAvailableDevelopmentMockCdnDir(): string | undefined {
  ...
  const releaseDir = join(mockCdnDir, "releases", CCBUDDY_VERSION);
...
function shouldUseRemoteCdnInDevelopment(localEnv: LocalRuntimeEnv = {}): boolean {
  return isTruthyEnvFlag(resolveEnvValue("CCBUDDY_DEV_REMOTE_ASSET_USE_CDN", localEnv));
}
...
function resolveRemoteCdnBaseUrls(
  ...
  const raw = resolveEnvValue("CCBUDDY_REMOTE_ASSET_CDN_BASE_URL", localEnv);
  return resolveOrderedRemoteCdnBaseUrls({
    ...options,
    env: CCBUDDY_ENV,
    overrideBaseUrl: raw,
    version: CCBUDDY_VERSION,
  });
}
...
export function resolveCCbuddyEndpointEnvBaseOrigin(
  localEnv: LocalRuntimeEnv = {},
): string | undefined {
  const buildEnv = readProductEndpointEnv();
  // main 进程临时验证更新服务时不会重新写 .env，命令行传入的 endpoint 必须优先于本地文件。
  return (
    process.env["CCBUDDY_BASE_URL"]?.trim() ||
    process.env["CCBUDDY_ENDPOINT_ORIGIN"]?.trim() ||
    localEnv.CCBUDDY_BASE_URL?.trim() ||
    localEnv.CCBUDDY_ENDPOINT_ORIGIN?.trim() ||
    buildEnv.CCBUDDY_BASE_URL?.trim() ||
    buildEnv.CCBUDDY_ENDPOINT_ORIGIN?.trim() ||
    undefined
  );
}
...
function applySelectedCCbuddyEnvLinks(env: Record<string, string>): Record<string, string> {
  const endpointEnv = {
    ...readProductEndpointEnv(),
    ...env,
    CCBUDDY_ENV,
  ...
  return {
    ...
    ...env,
    CCBUDDY_BASE_URL: env.CCBUDDY_BASE_URL ?? resolveRuntimeCCbuddyEndpointOrigin(endpointEnv),
...
function resolveHostProcessNodeEnv(): CCbuddyRuntimeEnv {
  return desktopRuntimeEnv;
}

function resolveRemoteAssetCacheDir(localEnv: LocalRuntimeEnv = {}): string {
  const overrideCacheDir = resolveEnvValue("CCBUDDY_REMOTE_ASSET_CACHE_DIR", localEnv);
  if (overrideCacheDir) {
...
function resolveBundledCCbuddyAgentBinaryPath(): string | undefined {
  const runtime = CCBUDDY_AGENT_RUNTIME;
...
export function resolveBundledGlmBinaryPath(): string | undefined {
  return resolveBundledCCbuddyAgentBinaryPath();
}
...
function resolveHostProcessBinaryEnv(
  ...
  // CCbuddy Agent 与 app 协议适配强绑定版本，生产包必须优先使用随包携带的固定 runtime。
...
function resolveDynamicWorkflowModeHostEnv(options: {
  ...
  if (!options.isPackaged) {
    ...
    return mode ? { [CCBUDDY_DYNAMIC_WORKFLOW_MODE_ENV]: mode } : {};
  }
  if (options.isPreview) {
    return { [CCBUDDY_DYNAMIC_WORKFLOW_MODE_ENV]: "alwaysOn" };
  }
  return {};
}
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
    ...
    process.platform !== "darwin"
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ["1", "true", "on"].includes(
              rawInheritedEnv.CCBUDDY_CUA_HELPER_ALLOW_UNSIGNED_LOCAL?.trim().toLowerCase() ?? "",
            )
          ? rawInheritedEnv.CCBUDDY_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.CCBUDDY_HOME?.trim() || join(homedir(), ".ccbuddy"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
  ...
  const agentTelemetryEnv = readCCbuddyAgentTelemetryEnv(rawInheritedEnv);
  ...
  const inheritedEnv = applySelectedCCbuddyEnvLinks({
    ...sanitizeCCbuddyRuntimeEnv(rawInheritedEnv),
    ...buildCCbuddyToolEnvPassthroughEnv(rawInheritedEnv),
  });
  ...
  if (packagedDesktop) {
    delete inheritedEnv.CCBUDDY_CUA_HELPER_ALLOW_UNSIGNED_LOCAL;
  }
  const dynamicWorkflowModeHostEnv = resolveDynamicWorkflowModeHostEnv({
    inheritedValue: rawInheritedEnv[CCBUDDY_DYNAMIC_WORKFLOW_MODE_ENV],
  ...
  delete inheritedEnv[CCBUDDY_DYNAMIC_WORKFLOW_MODE_ENV];
  ...
  return {
    ...
    ...agentTelemetryEnv,
    // CCbuddy 运行时不再使用 NODE_ENV；它会被用户 shell、包管理器和测试框架复用。
    // 这里显式下发 CCBUDDY_RUNTIME_ENV，并在继承环境里清掉 NODE_ENV，避免 host/agent/Bash 被污染。
    [CCBUDDY_RUNTIME_ENV_KEY]: resolveHostProcessNodeEnv(),
    // 显式注入编译期产品身份，保证主进程与 host 的身份语义一致；地址独立解析。
    // inheritedEnv 从 .env 通用变量补齐 CCbuddy/ZAI 链接，未覆盖时统一使用线上默认值。
    CCBUDDY_ENV,
    // Preview 与生产版共享任务、配置和凭据，但不同版本的 Helper 不能互相覆盖或触发降级保护。
    // 只隔离 computer-use 下的运行组件，不改写 CCBUDDY_HOME / CCBUDDY_DATA_BASE_DIR 业务数据根。
    ...(isPreviewPackagedRuntime ? { CCBUDDY_CUA_HELPER_INSTALL_VARIANT: "preview" } : {}),
    ...
    [CCBUDDY_APP_VERSION_ENV]: CCBUDDY_VERSION,
    ...(dataBaseDir !== homedir() ? { CCBUDDY_DATA_BASE_DIR: dataBaseDir } : {}),
    ...(windowsAppInstallDir
      ? { [CCBUDDY_WINDOWS_APP_INSTALL_DIR_ENV]: windowsAppInstallDir }
      : {}),
    ...(bundledCuaHelperAppPath
      ? { [CCBUDDY_CUA_BUNDLED_HELPER_APP_PATH_ENV]: bundledCuaHelperAppPath }
      : {}),
    ...(resolvedGlmBinaryPath ? { GLM_BINARY_PATH: resolvedGlmBinaryPath } : {}),
    ...(resolvedLarkCliBinaryPath ? { CCBUDDY_LARK_CLI_BINARY: resolvedLarkCliBinaryPath } : {}),
  };
}
