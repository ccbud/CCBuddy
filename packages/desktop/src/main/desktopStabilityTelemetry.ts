...
interface StabilityGlobalContext {
  ...
  appVersion: string;
  armsEnv: ReturnType<typeof mapCCbuddyEnvToArmsRumEnv>;
}
...
function redactRemainingAbsolutePaths(value: string): string {
  ...
  const protectedValue = value.replace(AGENT_CRASH_KNOWN_PATH_PLACEHOLDER_PATTERN, (match) => {
    ...
    return `CCBUDDY_REDACTED_PATH_${index}_TOKEN`;
  });
  ...
  return protectedPaths.reduce(
    (result, protectedPath, index) =>
      result.replace(`CCBUDDY_REDACTED_PATH_${index}_TOKEN`, protectedPath),
    redacted,
  );
}
...
function mapChildProcessGoneToProcessRoleWithName(
  ...
  if (processName?.startsWith("ccbuddy-host")) {
    return "host";
  }
  if (processName?.startsWith("ccbuddy-agent") || processName?.startsWith("ccbuddy-agent")) {
    return "agent";
  }
  switch (type) {
    case "GPU":
...
function shouldReportChildProcessGoneAsCrash(input: ChildProcessGoneInput): boolean {
  ...
  // 子进程会被系统自动重建，不会让 CCbuddy 主窗口或会话不可用；它们只能作为可恢复退出记录，
