...
// - 在官网页主世界挂 window.ccbuddyBridge，暴露三个能力：
...
// - getLang 读 main world 的 window.__ccbuddyLang__（由 App executeJavaScript 注入）；
//   onLangChange 在 main world 监听 ccbuddy-coding-plan-lang-change CustomEvent
...
const PUBLIC_BRIDGE_KEY = "ccbuddyBridge";
const NATIVE_BRIDGE_KEY = "__ccbuddyCodingPlanWebviewNativeBridge__";
const LANG_VAR = "__ccbuddyLang__";
const LANG_CHANGE_EVENT = "ccbuddy-coding-plan-lang-change";
const REPORT_CONTEXT_VAR = "__ccbuddyReportContext__";
  ...
  try {
    ...
    if (
      !isTrustedCodingPlanWebviewOrigin(url.origin, {
        e2eStoreBridgeEnabled: process.env.VITE_CCBUDDY_E2E_STORE_BRIDGE === "1",
      })
    ) {
      return false;
    }
    ...
    // 该页支付成功后仍需 ccbuddyBridge.notifyPurchaseComplete 通知 App 刷新模型设置。
    ...
    notifyPurchaseComplete(payload: NotifyPurchaseCompletePayload) {
      try {
        ipcRenderer.sendToHost(CodingPlanWebviewChannels.PurchaseComplete, {
          ...
          timestamp: Date.now(),
        } satisfies import("@ccbuddy/shared").CodingPlanPurchaseCompletePayload);
      } catch {
  ...
  contextBridge.executeInMainWorld({
    func: (
      ...
      const bridge = {
        ...
        // App locale 变化时用 executeJavaScript 派发 ccbuddy-coding-plan-lang-change 事件。
