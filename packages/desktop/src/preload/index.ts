...
contextBridge.exposeInMainWorld("__CCBUDDY_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  HistoryReviewChannels,
  formatCCbuddyRendererProcessName,
  shouldEnableE2ETestBridge,
  type IPlatformService,
} from "@ccbuddy/shared";
import { createOAuthCallbackHandler } from "./oauthCallbackBridge.js";

// 只给可信主窗口提供版本化只读历史 API；Main 在每次 IPC 时复验发送者。
const historyReviewBridge: NonNullable<IPlatformService["historyReview"]> = {
  protocolVersion: 1,
  list: async () => parseHistorySnapshot(await ipcRenderer.invoke(HistoryReviewChannels.list)),
  load: async (id) =>
    parseHistorySessionDetail(await ipcRenderer.invoke(HistoryReviewChannels.load, id)),
  refresh: async () => {
    const event = parseHistoryRefreshEvent(await ipcRenderer.invoke(HistoryReviewChannels.refresh));
  ...
  onRefreshEvent(listener) {
    const handler = (_event: IpcRendererEvent, payload: unknown) => {
      listener(parseHistoryRefreshEvent(payload));
    };
    ipcRenderer.on(HistoryReviewChannels.refreshEvent, handler);
    return () => ipcRenderer.removeListener(HistoryReviewChannels.refreshEvent, handler);
  },
};
contextBridge.exposeInMainWorld("ccbuddyHistory", historyReviewBridge);

if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__ccbuddyFinalArmsCustomEventsE2E", {
...
const historyViewListeners = new Set<(view: "list" | "timeline") => void>();
let pendingHistoryView: "list" | "timeline" | null = null;
ipcRenderer.on(PlatformChannels.OpenHistoryViewRequested, (_event, payload: unknown) => {
  if (payload !== "list" && payload !== "timeline") return;
  if (historyViewListeners.size === 0) {
    pendingHistoryView = payload;
    return;
  }
  pendingHistoryView = null;
  for (const listener of historyViewListeners) listener(payload);
});
...
function updateRendererProcessTitle(): void {
  process.title = formatCCbuddyRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 ccbuddy-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("ccbuddy", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@ccbuddy/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  /** 打开 CCbuddy Computer Use 完整权限引导 */
  ...
  reportLocalTtftBatch: (batch: import("@ccbuddy/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.ccbuddy/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@ccbuddy/shared").ChromeBrowserDataImportOptions) =>
  ...
  getCCbuddyStdioTapDevState: (): Promise<CCbuddyStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetCCbuddyStdioTapDevState),
  ...
  onOpenHistoryViewRequested: (listener: (view: "list" | "timeline") => void) => {
    historyViewListeners.add(listener);
    if (pendingHistoryView) {
      const view = pendingHistoryView;
      pendingHistoryView = null;
      listener(view);
    }
    return () => historyViewListeners.delete(listener);
  },
  /** 读取自动更新偏好 */
 ...
 */
// Main may deliver the initial port at dom-ready, before the renderer module has
// installed its message listener. Keep the port in preload until the renderer's
// first startup snapshot request proves that listener is ready.
let rendererReadyForLocalServicePort = false;
let pendingLocalServicePort:
  | { port: MessagePort; payload: { databaseStartupId: string } }
  | undefined;
function forwardLocalServicePort(port: MessagePort, payload: { databaseStartupId: string }): void {
...
ipcRenderer.on(InternalChannels.ServicePort, (event, payload: unknown) => {
  ...
  if (rendererReadyForLocalServicePort) {
    forwardLocalServicePort(port, parsed.data);
    return;
  }
  pendingLocalServicePort?.port.close();
  pendingLocalServicePort = { port, payload: parsed.data };
});
...
window.addEventListener("message", (event) => {
  ...
  if (parsed.data.action === "snapshot") {
    rendererReadyForLocalServicePort = true;
    if (pendingLocalServicePort) {
      const { port, payload } = pendingLocalServicePort;
      pendingLocalServicePort = undefined;
      forwardLocalServicePort(port, payload);
    }
  }
