...
import { RendererUserActionTelemetry, setUserActionTelemetry } from "@ccbuddy/ui";
...
export function initializeDesktopUserActionTrace(options: {
  ...
  const telemetry = new RendererUserActionTelemetry({
    ...
    resource: {
      ...
      serviceVersion: CCBUDDY_VERSION || "unknown",
      deploymentEnvironment: options.isLocalDevelopmentRuntime ? "development" : CCBUDDY_ENV,
      rendererInstanceId,
    },
