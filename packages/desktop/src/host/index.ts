...
import {
  ...
  createCCbuddyAgentConnectionScope,
  type CCbuddyAgentV4ClientMode,
...
import {
  ...
  type CCbuddyAutomationRunOutcome,
  type ModelSelection,
} from "@ccbuddy/shared";
import {
...
import type {
  ...
  RemoteConnection,
} from "@ccbuddy/server/remote";
import type { RemoteTarget } from "@ccbuddy/shared";
...
import { createRemoteConnectionProgressContext } from "@ccbuddy/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["CCBUDDY_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 ccbuddy-* title，方便系统进程列表过滤。
process.title = formatCCbuddyHostProcessName(process.env["CCBUDDY_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("ccbuddy-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 ccbuddy-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  ...
  try {
    const snapshot = await params.ccbuddyTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  workspacePath: string;
  workspaceIdentity?: string;
  outcome: CCbuddyAutomationRunOutcome;
  error?: string;
}): Promise<void> {
  ...
  void params.ccbuddyTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.ccbuddyTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        ccbuddyTaskService: params.ccbuddyTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const ccbuddyTaskService = activeServices?.getOptional(ICCbuddyTaskService);
  if (!ccbuddyTaskService) {
    throw new Error("CCbuddy task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        ccbuddyTaskService.listDeletedTaskIds(workspaceScope),
        ccbuddyTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      await ccbuddyTaskService.setConfigOption({
        taskId,
        traceId,
        configId: "mode",
      ...
      const task = await ccbuddyTaskService.createTask({
        ...
        mode: request.permissionMode as CCbuddyTaskMode,
    ...
    await ccbuddyTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const ccbuddyTaskService = targetServices.getOptional(ICCbuddyTaskService);
  if (!ccbuddyTaskService) {
    throw new Error("CCbuddy task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await ccbuddyTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isCCbuddyTaskMeta(value: unknown): value is CCbuddyTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteCCbuddyTaskService<T extends object>(
  service: T,
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: CCbuddyPromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: CCbuddyTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // ccbuddySessionStore 的整条消费链词表都是 CCbuddyStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→CCbuddyStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 ccbuddy-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: CCbuddyTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: CCbuddyTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote CCbuddy task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote CCbuddy task ready event is not subscribable");
    }
  ...
  // remote workspace 的 CCbuddy Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: CCbuddyTaskMeta; started: boolean } | undefined;
...
function warmUpCCbuddyAgent(
  ...
  const ccbuddySessionService = services.getOptional(ICCbuddySessionService);
  if (!ccbuddySessionService) {
    return;
  }
  void ccbuddySessionService
    .initializeWorkspace({
      workspacePath,
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `CCbuddy agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `CCbuddy agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `CCbuddy agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`CCbuddy agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(ICCbuddyAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteCCbuddyTaskService: (service) =>
      createReportingRemoteCCbuddyTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 ccbuddy-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(ICCbuddyTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(ICCbuddyTaskService),
        agentService: services.getOptional(ICCbuddyAgentService),
    ...
    const taskService = activeServices?.getOptional(ICCbuddyTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(ICCbuddyAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<ICCbuddyTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<ICCbuddyTaskService["archiveTask"]>[0]
            | Parameters<ICCbuddyTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<ICCbuddyTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<ICCbuddyTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<ICCbuddyTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<ICCbuddyTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: CCbuddyAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(ICCbuddyAgentService);
  const connectionScope = agentService
    ? createCCbuddyAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(ICCbuddyTaskService);
  if (taskService) {
    overrides.set(
      ICCbuddyTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(ICCbuddyAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as CCbuddyTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!ccbuddyTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "CCbuddy task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    ...
    if (!ccbuddyTaskService) {
      logger.warn(
        "session message delivery result received before CCbuddy task service initialized",
      );
      return;
    }
    void ccbuddyTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              ccbuddyAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              ccbuddyBuiltinProviderConfigFilePath: msg.ccbuddyBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 ccbuddyAgentService 转到这个 executor，
        ...
        const ccbuddyTaskService = services.getOptional(ICCbuddyTaskService);
        if (ccbuddyTaskService) {
          const reportingCCbuddyTaskService = createReportingRemoteCCbuddyTaskService(
            ccbuddyTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(ICCbuddyTaskService, reportingCCbuddyTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@ccbuddy/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 CCBUDDY_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: CCBUDDY_VERSION,
    // 远端 ccbuddy-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
