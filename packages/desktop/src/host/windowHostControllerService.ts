...
interface ResolvedWindowHostControllerSource {
  ...
  taskService?: ICCbuddyTaskService;
...
function taskKey(task: Pick<CCbuddyTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: CCbuddyTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@ccbuddy/shared/ccbuddy-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: CCbuddyTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (
    scope: CCbuddyTaskListWorkspaceScope,
  ...
  const sourceTaskServices = new Map<string, ICCbuddyTaskService>();
  const sourceSnapshotTaskServices = new Map<string, ICCbuddyTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: ICCbuddyTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: ICCbuddyAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: CCbuddyTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@ccbuddy/shared").WindowHostAttachmentScope;
