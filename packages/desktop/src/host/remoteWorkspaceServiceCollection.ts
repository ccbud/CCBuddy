...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteCCbuddyTaskService: <T extends object>(service: T) => T;
  ...
  // Remote workspace 也只允许用户自配模型链路；旧账号 API 端口必须在装配处关闭。
  const localApiClient = createLocalOnlyApiClient();
  ...
  const reportingRemoteCCbuddyTaskService = params.createReportingRemoteCCbuddyTaskService(
    params.connectionServices.ccbuddyTaskService,
  );
  ...
  const remoteCCbuddyTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteCCbuddyTaskService,
  );
  const remoteCCbuddySessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.ccbuddySessionService,
  );
  ...
  params.connectionServices.ccbuddyAgentService.onDynamicSessionRuntimePreferencesRequest()(
    (request) => {
      const startedAt = Date.now();
      const requestContext = {
        event: "ccbuddy_protocol.runtime_preferences.host_request_received",
      ...
      void (async () => {
        ...
        await params.connectionServices.ccbuddyAgentService.respondSessionRuntimePreferences({
          requestId: request.requestId,
          resolution,
        });
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、CCbuddy Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(ICCbuddyTaskService, remoteCCbuddyTaskService)
    .register(ICCbuddyAgentService, params.connectionServices.ccbuddyAgentService)
    .register(ICCbuddySessionService, remoteCCbuddySessionService)
    .register(IConversationShareService, conversationShareService)
    .register(
      IBotsService,
      createBotsService({
        credentialService: localCredentialService,
        ccbuddyTaskService: remoteCCbuddyTaskService,
    ...
    .register(IOAuthService, createDisabledOAuthService())
    ...
    .register(
      ...
      createUsageStatsService({
        ...
        credentialService: localCredentialService,
        ccbuddyAgentService: params.connectionServices.ccbuddyAgentService,
      }),
    )
