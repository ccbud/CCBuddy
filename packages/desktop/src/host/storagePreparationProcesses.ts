...
import {
  ...
  ccbuddyStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@ccbuddy/shared";
import { resolveDefaultCCbuddyAgentCommand } from "@ccbuddy/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultCCbuddyAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = ccbuddyStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
