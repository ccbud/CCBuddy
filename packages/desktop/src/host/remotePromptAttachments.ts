import type { IRemoteBackend, RemoteUploadOptions } from "@ccbuddy/server/remote";
import { quotePosixPathArg } from "@ccbuddy/server/remote/posixShell.js";
import type { TraceId, CCbuddyPromptAttachment } from "@ccbuddy/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.ccbuddy/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".ccbuddy/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: CCbuddyPromptAttachment[] = [];
...
async function resolveRemotePromptAttachmentRoot(
  ...
  return resolveRemotePromptAttachmentRootFromHome(homeDir);
}

export function resolveRemotePromptAttachmentRootFromHome(homeDir: string): string {
...
function getAttachmentLocalPath(attachment: CCbuddyPromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: CCbuddyPromptAttachment[] }).attachments
            : undefined,
        });
