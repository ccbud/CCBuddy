...
type PatchedIpcRenderer = IpcRenderer & { __ccbuddyArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __ccbuddyArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__ccbuddyArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__ccbuddyArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__ccbuddyArmsIpcPatched) {
    return;
  }
  ...
  patched.__ccbuddyArmsIpcPatched = true;
}
