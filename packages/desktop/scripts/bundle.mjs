...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    CCBUDDY_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.CCBUDDY_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  CCBUDDY_TARGET_OS              与 --os 等价
  CCBUDDY_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.CCBUDDY_TARGET_OS ?? null,
    arch: process.env.CCBUDDY_TARGET_ARCH ?? null,
    skipPrepare: process.env.CCBUDDY_SKIP_PREPARE === "1",
    skipBuild: process.env.CCBUDDY_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };
...
async function main() {
  ...
  const buildEnv = {
    CCBUDDY_TARGET_OS: os,
    CCBUDDY_TARGET_ARCH: arch,
