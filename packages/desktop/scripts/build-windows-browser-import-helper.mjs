...
const targetArch = (process.env.CCBUDDY_TARGET_ARCH ?? arch()).toLowerCase();
const targetKey = `win32-${targetArch}`;
const outputDir = resolve(desktopRoot, `bundled-tools/${targetKey}/browser-import`);
const outputPath = resolve(outputDir, "ccbuddy-browser-import-helper.exe");
...
if (targetArch === "arm64" && !process.env.CCBUDDY_CSC_PATH) {
  throw new Error(
    "Windows arm64 browser import helper requires CCBUDDY_CSC_PATH pointing to an arm64-capable Roslyn csc.exe",
  );
}
if (!cscPath) {
  throw new Error("Windows C# compiler not found; set CCBUDDY_CSC_PATH to a trusted csc.exe");
}
