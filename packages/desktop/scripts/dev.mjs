...
if (process.platform === "darwin" && existsSync(electronBinary)) {
  ...
  // ccbuddy:// 交给一个没有项目入口的 Electron 默认壳。给本地启动副本补齐产品
  ...
  const devBundle = await prepareDevElectronAppBundle({
    electronAppPath,
    runtimeRoot: resolve(
      process.env.CCBUDDY_DATA_BASE_DIR?.trim() || process.env.HOME?.trim() || homedir(),
      ".ccbuddy",
      "runtime",
      "desktop-dev",
    ),
  ...
  console.log(`[dev] Prepared macOS CCbuddy Dev bundle: ${devBundle.appPath}`);
}
