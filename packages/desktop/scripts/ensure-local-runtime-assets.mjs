...
const shouldRequireWindowsBrowserImportHelper =
  target.os === "win32" && process.env.CCBUDDY_ENABLE_WINDOWS_BROWSER_IMPORT === "1";
// CUA 权限浮窗靠 ccbuddy-window-bounds 读系统设置窗口 bounds 才能吸附。该 Swift 产物被
    ...
    ? [
        {
          ...
          isReady: () =>
            existsSync(
              join(bundledToolsRoot, "browser-import", "ccbuddy-browser-import-helper.exe"),
            ),
        },
      ]
    : []),
    ...
    ? [
        {
          ...
          isReady: () =>
            existsSync(
              join(desktopRoot, "resources", "macos-window-bounds", "ccbuddy-window-bounds"),
            ),
        },
      ]
    : []),
