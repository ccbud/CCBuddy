...
{
    private const string Protocol = "CCBUDDY_BROWSER_IMPORT_V1";
        ...
        try
        {
            if (HasArgument(args, "--version"))
            {
                ...
                Console.Out.WriteLine(
                    "CCBUDDY_BROWSER_IMPORT_HELPER\t" + HelperVersion + "\t" + GetProcessArchitecture() +
    ...
    private static void RunBrokerCore()
    {
        ...
        try
        {
            ...
            pipeName = "ccbuddy-browser-import-broker-" + Guid.NewGuid().ToString("N");
            token = CreateToken();
    ...
    private static void RunTemporarySystemService(
        ...
        int brokerPid)
    {
        string serviceName = "CCbuddyBrowserImport_" + Guid.NewGuid().ToString("N");
        string pipeName = "ccbuddy-browser-import-system-" + Guid.NewGuid().ToString("N");
