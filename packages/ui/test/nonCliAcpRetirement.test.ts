...
import { ccbuddyTaskMetaSchema } from "../../shared/src/validation.js";
...
test("current task metadata is accepted without upgrading third-party Agent identities", () => {
  assert.equal(ccbuddyTaskMetaSchema.parse(meta).provider, "glm");
  for (const provider of ["claude", "codex", "gemini", "opencode"]) {
    assert.equal(ccbuddyTaskMetaSchema.safeParse({ ...meta, provider }).success, false, provider);
  }
});
...
test("CCbuddy subagent identity wins over retired Codex nicknames", () => {
  const tool = {
    id: "tool-example",
    kind: "Agent",
    title: "Agent",
