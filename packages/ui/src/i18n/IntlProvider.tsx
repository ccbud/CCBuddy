...
import type { Locale, LocalePreference } from "@ccbuddy/shared";
import { DEFAULT_LOCALE } from "@ccbuddy/shared";
import type { BroadcastMessage, IBroadcastService, ISettingService } from "@ccbuddy/services";
import {
...
const LOCALE_PREFERENCE_KEY = "ccbuddy-locale-preference";
 ...
 */
export function CCbuddyIntlProvider({
  children,
  settingService,
  broadcastService,
  initialLocale,
...
/** 获取 intl 上下文 */
export function useCCbuddyIntl(): IntlContextValue {
  const ctx = useContext(IntlContext);
  if (!ctx) {
    throw new Error("CCbuddy 多语言组件必须在 IntlProvider 内使用");
  }
  return ctx;
}
