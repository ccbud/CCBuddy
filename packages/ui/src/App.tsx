...
import type { GitChangeSourceId, WorkspacePurpose } from "@ccbuddy/shared";
import { useCCbuddyStore } from "@/store/StoreProvider.js";
import { getVisibleTaskMetas, useCCbuddySessionStore } from "@/store/ccbuddySessionStore.js";
...
import { CCBUDDY_PRODUCT_DOCS_URL } from "@/lib/productDocs.js";
...
import { useWorkspaceShellCCbuddyState } from "@/app-shell/useWorkspaceShellCCbuddyState.js";
...
export function App({
  ...
  const { intl, locale, setLocale } = useCCbuddyIntl();
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellCCbuddyState, reloadSessionDisabled } = useWorkspaceShellCCbuddyState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellCCbuddyState.activeTaskId;
  ...
  const draftSessionId = useCCbuddySessionStore(
  ...
  const draftFocusVersion = workspaceShellCCbuddyState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useCCbuddyStore((s) => s.notificationEnabled);
  ...
  const theme = useCCbuddyStore((s) => s.theme);
  const setTheme = useCCbuddyStore((s) => s.setTheme);
  const {
    isMacFullscreen,
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellCCbuddyState.selectedProvider,
    intl,
  });
  ...
  const handleOpenProductDocs = useCallback(() => {
    platform.openExternal(CCBUDDY_PRODUCT_DOCS_URL);
  }, [platform]);
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@ccbuddy/shared").SessionCreateSource,
    ) => {
      const store = useCCbuddySessionStore.getState();
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellCCbuddyState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellCCbuddyState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.ccbuddyAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.ccbuddyAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.ccbuddyAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.ccbuddyAgentService.installPlugin(params),
      listPlugins: (params) => services.ccbuddyAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.ccbuddyAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.ccbuddyAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleOpenHistoryViewFromMenu = useCallback(
    (view: "list" | "timeline") => {
      // 设置层可能覆盖仍挂载的工作区；显式菜单导航先保留目标，再返回工作区。
      // 即使目标与进入设置时的视图相同，也不能让退出设置的默认逻辑改回 chat。
      preserveNextSettingsExit();
      runVisibleWorkspaceCommand(() => {
        setWorkspaceMainView(view === "list" ? "history-list" : "history-timeline");
      });
    },
    [preserveNextSettingsExit, runVisibleWorkspaceCommand],
  );
  useEffect(
    () => platform.onOpenHistoryViewRequested?.(handleOpenHistoryViewFromMenu),
    [handleOpenHistoryViewFromMenu, platform],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useCCbuddySessionStore.getState();
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellCCbuddyState={workspaceShellCCbuddyState}
        theme={theme}
