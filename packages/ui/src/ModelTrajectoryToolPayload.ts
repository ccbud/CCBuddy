...
export function trajectoryToolOutputs(message: CCbuddyModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: CCbuddyModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: CCbuddyModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: CCbuddyModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
