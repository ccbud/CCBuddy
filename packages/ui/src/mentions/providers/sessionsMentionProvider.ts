// composer parity：`#` 会话候选的数据源从旧 ccbuddySessionStore/taskQueryCache/remote*
...
import type { CCbuddyProvider, CCbuddyTaskMeta } from "@ccbuddy/shared";
...
function compareSessionTasks(
  left: CCbuddyTaskMeta,
  right: CCbuddyTaskMeta,
...
function getSessionLabel(task: CCbuddyTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: CCbuddyTaskMeta): string {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, CCbuddyTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.ccbuddyAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.ccbuddyAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.ccbuddyAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.ccbuddyAgentService,
        },
      ];
    }
