...
import type { CCbuddyGroupedTaskView, CCbuddyGroupedTaskViewNode } from "@ccbuddy/services";
import type { CCbuddyTaskMeta } from "@ccbuddy/shared";
import type { GroupedDraftTaskPlacement } from "@/store/ccbuddySessionStoreTypes.js";
import { taskKey } from "@/workspace-grouped-tasks/ids.js";

function cloneView(view: CCbuddyGroupedTaskView): CCbuddyGroupedTaskView {
  return {
...
function cloneNodes(view: CCbuddyGroupedTaskView): CCbuddyGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<CCbuddyGroupedTaskViewNode, { type: "group" }>,
): Extract<CCbuddyGroupedTaskViewNode, { type: "group" }> {
...
function filterGroupedViewByTaskKeys(
  ...
  let changed = false;
  const nodes: CCbuddyGroupedTaskView["nodes"] = [];
...
function getGroupedTaskGroupIds(view: CCbuddyGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: CCbuddyGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: CCbuddyGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: CCbuddyGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: CCbuddyGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function getNodeIdAfterGroup(
  view: CCbuddyGroupedTaskView,
  groupId: string,
): CCbuddyGroupedTaskViewNode | null {
...
function insertTaskBeforeTopLevelNode(
  view: CCbuddyGroupedTaskView,
  task: CCbuddyTaskMeta,
  beforeNode: CCbuddyGroupedTaskViewNode | null,
): CCbuddyGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null
