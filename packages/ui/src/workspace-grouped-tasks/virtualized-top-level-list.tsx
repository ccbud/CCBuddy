...
import type { CCbuddyGroupedTaskViewNode } from "@ccbuddy/services";
...
function getTopLevelNodeKey(node: CCbuddyGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: CCbuddyGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: CCbuddyGroupedTaskViewNode, index: number) => ReactNode;
}) {
