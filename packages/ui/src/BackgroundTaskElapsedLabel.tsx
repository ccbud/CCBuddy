...
function createElapsedBaseline(job: CCbuddyBackgroundTaskControlItem) {
  const mountedAt = Date.now();
  return {
    elapsedMs: getCCbuddyBackgroundTaskControlItemElapsedMs(job, mountedAt),
...
function elapsedMsForClock(input: {
  ...
  const elapsedFromJob = getCCbuddyBackgroundTaskControlItemElapsedMs(input.job, input.now);
