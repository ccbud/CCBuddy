...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@ccbuddy/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type CCbuddyAgentCuaPermissionObservation,
...
function cuaPermissionObservationKey(observation: CCbuddyAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.ccbuddyAgentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.ccbuddyAgentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);
