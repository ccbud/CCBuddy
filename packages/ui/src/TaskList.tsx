...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@ccbuddy/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<CCbuddyTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<CCbuddyTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<CCbuddyTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<CCbuddyTaskMeta | null>;
  ...
  const startDraft = useCCbuddySessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: CCbuddyTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareCCbuddyTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
