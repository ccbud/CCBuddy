...
import type { IPlatformService, TaskNotificationPayload } from "@ccbuddy/shared";
import type { ConversationSnapshot, SessionSummary } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, ccbuddyAgentService);
    const syncState = () => {
      setIndexState({
        signature,
