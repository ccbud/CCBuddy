...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly CCbuddyTaskMeta[];
  optimisticTasks: readonly CCbuddyTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareCCbuddyTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useCCbuddySessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useCCbuddySessionStore.getState();
