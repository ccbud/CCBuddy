...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): CCbuddyTaskListWorkspaceScope[] {
  const scopes = new Map<string, CCbuddyTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useCCbuddySessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceCCbuddyState(
            state,
            tab.workspacePath,
