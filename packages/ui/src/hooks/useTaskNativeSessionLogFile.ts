...
import { useCCbuddyTaskService } from "@/hooks/useCCbuddyTaskService.js";
...
function supportsTaskNativeSessionLogFile(_provider: CCbuddyProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 ccbuddyTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: CCbuddyProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const ccbuddyTaskService = useCCbuddyTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, ccbuddyTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);
