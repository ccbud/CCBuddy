...
import type { CCbuddySessionEndedSubagent, CCbuddySessionSubagentsResult } from "@ccbuddy/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: CCbuddySessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly CCbuddySessionEndedSubagent[],
  incoming: readonly CCbuddySessionEndedSubagent[],
): CCbuddySessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const ccbuddyAgentService = services.ccbuddyAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<CCbuddySessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof ccbuddyAgentService?.listSessionSubagents !== "function") {
      ...
      return ccbuddyAgentService.listSessionSubagents({
