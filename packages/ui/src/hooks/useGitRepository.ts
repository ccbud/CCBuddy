...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, CCbuddyPersistedFileSnapshot>,
  file: CCbuddyTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: CCbuddyPersistedFileChange | null;
  summary: CCbuddyTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, CCbuddyPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 CCbuddy Agent 已持久化的单轮文件快照，
