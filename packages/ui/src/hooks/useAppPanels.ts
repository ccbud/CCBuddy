...
import { createUuid } from "@ccbuddy/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@ccbuddy/shared";
...
export function useAppPanels(options: {
  ...
  const { ccbuddyAgentService, ccbuddySessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await ccbuddyAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
