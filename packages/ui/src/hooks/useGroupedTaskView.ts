...
import { mergeTaskWithOptimisticMeta } from "@/lib/ccbuddyTaskMetaMerge.js";
import {
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: CCbuddyTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, CCbuddyTaskMeta>();
  const placementTaskByKey = new Map<string, CCbuddyTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, CCbuddyTaskMeta[]>();
  const topMissingTasks: CCbuddyTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: CCbuddyGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: CCbuddyGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: CCbuddyGroupedTaskView["nodes"][number],
  next: CCbuddyGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: CCbuddyGroupedTaskView,
  next: CCbuddyGroupedTaskView,
): CCbuddyGroupedTaskView {
...
function nodeToTopLevelRef(
  node: CCbuddyGroupedTaskView["nodes"][number],
): CCbuddyGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
const groupedViewCacheBySignature = new Map<string, CCbuddyGroupedTaskView>();

function readCachedGroupedView(signature: string): CCbuddyGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: CCbuddyGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<CCbuddyGroupedTaskView>(
  ...
  const taskListVersionSignature = useCCbuddySessionStore((state) =>
    JSON.stringify(
  ...
  const clearPromotedGroupedDraftTask = useCCbuddySessionStore(
  ...
  const displayedViewRef = useRef<CCbuddyGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: CCbuddyGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.ccbuddyTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.ccbuddyTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: CCbuddyTaskMeta[] = [];
    const settledRootTasks: CCbuddyTaskMeta[] = [];
    ...
    void services.ccbuddyTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.ccbuddyTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.ccbuddyTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<CCbuddyTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.ccbuddyTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.ccbuddyTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: CCbuddyGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.ccbuddyTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.ccbuddyTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: CCbuddyTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.ccbuddyTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: CCbuddyGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.ccbuddyTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.ccbuddyTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: CCbuddyGroupedTaskView = {
      ...
      try {
        ...
        await services.ccbuddyTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.ccbuddyTaskService, view],
  );
