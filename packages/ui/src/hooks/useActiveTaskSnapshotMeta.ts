...
import { ccbuddySessionSnapshotToTaskMeta } from "@/lib/ccbuddySessionProjection.js";

function resolveImmediateActiveTaskSnapshotMeta(
  previousSnapshotMeta: CCbuddyTaskMeta | null,
  taskId: string | null,
  taskMetaFromLists?: CCbuddyTaskMeta | null,
) {
...
export function useActiveTaskSnapshotMeta(
  ...
  const [snapshotMeta, setSnapshotMeta] = useState<CCbuddyTaskMeta | null>(null);
  ...
  useEffect(() => {
    ...
    void ccbuddySessionService
      // active header 只需要 session meta/标题兜底，走 CCbuddy Protocol 的轻量读取，
      ...
      .then((snapshot) => {
        ...
        setSnapshotMeta(ccbuddySessionSnapshotToTaskMeta(snapshot));
      })
      .catch(() => {
        if (cancelled) {
          return;
  ...
  }, [ccbuddySessionService, taskId, taskMetaFromLists, workspaceIdentity, workspacePath]);
