...
import { isRemoteWorkspaceIdentity, CCBUDDY_CUA_OFFICIAL_PLUGIN_ID } from "@ccbuddy/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/ccbuddySessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 ccbuddySessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 ccbuddy-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useCCbuddySessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
