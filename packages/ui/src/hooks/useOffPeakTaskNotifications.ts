...
import type { CCbuddyOffPeakTaskStatus } from "@ccbuddy/shared";
import type { IOffPeakTaskService } from "@ccbuddy/services";
...
function notifiableStatus(status: CCbuddyOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, CCbuddyOffPeakTaskStatus>());
