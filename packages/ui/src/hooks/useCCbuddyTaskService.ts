...
import type { ICCbuddyTaskService } from "@ccbuddy/services";
import type { CCbuddyTaskSnapshot } from "@ccbuddy/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<ICCbuddyTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<CCbuddyTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<ICCbuddyTaskService["getTaskSnapshotWithEtag"]>[0];

const ccbuddyTaskServiceProxyCache = new WeakMap<ICCbuddyTaskService, ICCbuddyTaskService>();
...
const snapshotCacheByService = new WeakMap<
  ICCbuddyTaskService,
  Map<string, { etag: string; snapshot: CCbuddyTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "ccbuddy-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: CCbuddyTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: CCbuddyTaskSnapshot }>();
...
function createCCbuddyTaskServiceProxy(service: ICCbuddyTaskService): ICCbuddyTaskService {
...
/** 获取 CCbuddy task wrapper 服务实例 */
export function useCCbuddyTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): ICCbuddyTaskService {
  // CCbuddy task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.ccbuddyTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = ccbuddyTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createCCbuddyTaskServiceProxy(rawService);
  ccbuddyTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
