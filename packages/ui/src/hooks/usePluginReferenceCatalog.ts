...
interface PluginReferenceCatalogState {
  entries: CCbuddyPluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<CCbuddyPluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<CCbuddyPluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<CCbuddyPluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.ccbuddyAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<CCbuddyPluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<CCbuddyPluginsReferenceCatalogResult>> | undefined;
