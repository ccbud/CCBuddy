 ...
 * （见 ccbuddySessionSettingsToConfigOptions），那种「名字」在这里就当作没有——屏幕上绝不出现
 * providerId（团队套餐的它是一个 UUID）。
 */
function workflowSubagentProviderNameLookup(
  configOptions: readonly CCbuddyConfigOption[] | null | undefined,
...
export function useWorkflowSubagentModelProviderName(
  ...
  const configOptions = useCCbuddySessionStore((state) =>
    workspacePath === undefined
      ? undefined
      : selectWorkspaceCCbuddyState(state, workspacePath, workspaceIdentity).configOptions,
  );
