...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: CCbuddyTaskMeta,
    overrides?: Partial<CCbuddyTaskMeta>,
  ): CCbuddyTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.ccbuddyTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.ccbuddyTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 ccbuddyTaskMetaMerge：归档后只有旧列表状态被更新。
