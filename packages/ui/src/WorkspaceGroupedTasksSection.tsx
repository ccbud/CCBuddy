...
import type { CCbuddyGroupedTaskView, CCbuddyTaskGroupColor } from "@ccbuddy/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type CCbuddyTaskMeta } from "@ccbuddy/shared";
...
function getGroupedTaskViewSignature(view: CCbuddyGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const groupedDraftTask = useCCbuddySessionStore(
    (state) =>
      selectWorkspaceCCbuddyState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useCCbuddySessionStore(
    (state) =>
      selectWorkspaceCCbuddyState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useCCbuddySessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<CCbuddyGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<CCbuddyGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: CCbuddyTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: CCbuddyTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.ccbuddyTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: CCbuddyTaskMeta) => {
      ...
      void workspaceServices.services.ccbuddyTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: CCbuddyTaskGroupColor) => {
  ...
  const setViewWithGroupedTaskAnimation = useCallback(
    (nextView: CCbuddyGroupedTaskView) => {
