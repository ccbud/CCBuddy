...
> &
  Partial<Pick<ICCbuddyTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<CCbuddyTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: CCbuddyTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: CCbuddyTaskMeta[][]): CCbuddyTaskMeta[] {
  const itemByEntityKey = new Map<string, CCbuddyTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, CCbuddyTaskMeta>();
