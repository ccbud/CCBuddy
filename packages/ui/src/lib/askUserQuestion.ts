type CCbuddyUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: CCbuddyUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // CCbuddy Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): CCbuddyUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): CCbuddyUserQuestionAnswers | undefined {
...
function parseCCbuddyAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): CCbuddyUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedCCbuddyOutputAnswers = parseCCbuddyAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedCCbuddyOutputAnswers) {
    return parsedCCbuddyOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawCCbuddyOutputAnswers = parseCCbuddyAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawCCbuddyOutputAnswers) {
      return parsedRawCCbuddyOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawCCbuddyAnswers = parseCCbuddyAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawCCbuddyAnswers) {
      return parsedRawCCbuddyAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: CCbuddyUserQuestionAnswers | undefined,
  noAnswerText: string,
) {
