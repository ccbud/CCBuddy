import type { LaunchMarks } from "@ccbuddy/shared";
...
export function readRendererLaunchTimings(): {
  ...
  const w = window as Window & {
    ...
    __CCBUDDY_REACT_COMMIT_AT__?: number;
  };
  const rendererStart = w.__CCBUDDY_RENDERER_START__;
  const reactCommit = w.__CCBUDDY_REACT_COMMIT_AT__;
  if (typeof rendererStart !== "number" || typeof reactCommit !== "number") {
    return null;
  }
  return { marks: w.__CCBUDDY_LAUNCH_MARKS__ ?? null, rendererStart, reactCommit };
}
