import {
  getCCbuddyToolFamilyForName,
  isTodoPlanToolName,
  isCCbuddyFileContentWriteToolName,
  normalizeCCbuddyToolName,
  type CCbuddyKnownToolName,
  type CCbuddyToolFamily,
} from "@ccbuddy/shared";
...
export interface ToolCallIdentity {
  toolName: CCbuddyKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getCCbuddyToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    ccbuddy: readNestedString(raw, ["_meta", "ccbuddy", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawCCbuddy = normalizeLegacyToken(rawNames.ccbuddy);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.ccbuddy, source: "raw-ccbuddy-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.ccbuddy ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 CCbuddy Agent 投影会用
  ...
  if (
    // 当前 CCbuddy Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isCCbuddyFileContentWriteToolName(identity.toolName)) {
    return true;
  }
