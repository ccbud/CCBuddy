/* oxlint-disable eslint(max-lines) -- CCbuddy session 到当前聊天 projection 的迁移桥需要同时保持 snapshot 和 event 映射一致。 */
import {
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // CCbuddy Protocol 必须收到严格的 providerId/modelId 结构。
    return {
...
export function ccbuddySessionSettingsToConfigOptions(
...
export function ccbuddyWorkspacePresentationToConfigOptions(
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableCCbuddyMode(mode),
      options: getCCbuddyAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: CCbuddySessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // CCbuddy Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
export function ccbuddySessionSnapshotToTaskMeta(
  snapshot: CCbuddySessionStateSnapshot,
): CCbuddyTaskMeta {
  return {
...
function normalizeAvailableCCbuddyMode(mode: CCbuddySessionMode): string {
  return CCBUDDY_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getCCbuddyAgentModeSelectOptions(): NonNullable<CCbuddyConfigOption["options"]> {
