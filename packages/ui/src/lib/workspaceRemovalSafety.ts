import type { FileEntry, CCbuddyTaskMeta, CCbuddyTaskRuntimeStatus } from "@ccbuddy/shared";
import type { IFileService } from "@ccbuddy/services";
import type { WorkspaceCCbuddyUIState } from "@/store/ccbuddySessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceCCbuddyUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<CCbuddyTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: CCbuddyTaskRuntimeStatus): boolean {
