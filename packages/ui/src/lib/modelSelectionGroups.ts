...
function supportsRegistryApiFormat(
  ...
  // 仅剩 glm（CCbuddy Agent）provider；三方 CLI 的 api format 差异已随 provider 下线。
  return isCCbuddyAgentProvider(selectedProvider);
}
...
export function buildRegistryModelSelectGroups(
  ...
  return view.providers.flatMap((provider) => {
    ...
    const accountAccess = ccbuddyProviderAccountAccessSchema.safeParse(provider.config.access);
