...
// sessions-index 只补实时 activity/detail，最终拼出旧 CCbuddyGroupedTaskView 同形结果。
...
function taskKeyOf(task: CCbuddyTaskMeta): string {
...
function taskOrderMapKey(task: CCbuddyTaskMeta): string {
...
function nodeMapKey(node: CCbuddyGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, CCbuddyGroupedTaskViewStructureMember>,
): CCbuddyTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: CCbuddyTaskListItem; addedAt: number; key: string }> = [];
...
interface BuildGroupedTaskViewParams {
  ...
  taskIndexItems: CCbuddyTaskMeta[];
  /** sessions-index 派生的会话 activity/detail，只 enrich 命中的持久行。 */
  sessions: CCbuddyTaskMeta[];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, CCbuddyTaskListItem>();
  ...
  const membersByGroupId = new Map<string, CCbuddyGroupedTaskViewStructureMember[]>();
  ...
  const nodes: CCbuddyGroupedTaskViewNode[] = structure.groups.map((group) => {
