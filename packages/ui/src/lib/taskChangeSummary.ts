import type {
  ...
  CCbuddyTaskChangedFileSummary,
  CCbuddyTaskMeta,
} from "@ccbuddy/shared";
import { computeLineChangeStat } from "@ccbuddy/shared";
...
export function getTaskChangeSummary(
  task: Pick<CCbuddyTaskMeta, "changeSummary"> | null | undefined,
): CCbuddyTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: CCbuddyTaskChangeSummary,
...
export function formatTaskTitleWithChanges(
  title: string,
  summary: CCbuddyTaskChangeSummary | null,
  intl: TaskChangeSummaryIntl,
): string {
  if (!summary) {
    return title;
...
export function buildTaskChangeSummary(
  fileChanges: readonly CCbuddyPersistedFileChange[] | undefined,
): CCbuddyTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: CCbuddyTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: CCbuddyPersistedFileChange | null | undefined,
): CCbuddyTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: CCbuddyTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly CCbuddyPersistedFileChange[] | undefined,
): Map<number, CCbuddyTaskChangeSummary> {
  const result = new Map<number, CCbuddyTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
