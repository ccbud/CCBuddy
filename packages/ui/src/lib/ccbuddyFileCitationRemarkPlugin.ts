...
import { extractCCbuddyFileCitations } from "@/lib/ccbuddyFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractCCbuddyFileCitations(value);
...
export function createCCbuddyFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function ccbuddyFileCitationRemarkPlugin() {
    return (tree: unknown) => {
