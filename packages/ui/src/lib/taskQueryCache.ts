...
import { resolveWorkspaceStateKey } from "@/store/ccbuddySessionStoreSelectors.js";

export type TaskEntityKey = string;
export type TaskListCacheKey = string;
export type TaskListQueryKind = CCbuddyTaskListKind | "workspace";
...
function normalizeTaskListWorkspaceScopes(
  ...
  const uniqueScopes = new Map<string, CCbuddyTaskListWorkspaceScope>();
