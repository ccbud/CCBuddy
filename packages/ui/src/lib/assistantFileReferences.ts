...
import { extractConversationPreviewFileReferences } from "@ccbuddy/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@ccbuddy/shared";
import {
  extractCCbuddyFileCitationDirectives,
  resolveCCbuddyFileCitationPreviewKind,
} from "@/lib/ccbuddyFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractCCbuddyFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 ccbuddy-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
