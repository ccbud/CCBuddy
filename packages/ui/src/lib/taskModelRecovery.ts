import type { CCbuddyConfigOption, CCbuddyTaskMeta } from "@ccbuddy/shared";
import { getCCbuddyAgentModeSelectOptions } from "@ccbuddy/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<CCbuddyTaskMeta, "provider" | "model">,
): string | null {
...
function createRecoveredModelOption(modelValue: string): CCbuddyConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): CCbuddyConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): CCbuddyConfigOption {
  return {
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<CCbuddyTaskMeta, "provider" | "model"> &
    Partial<Pick<CCbuddyTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly CCbuddyConfigOption[];
}): CCbuddyConfigOption[] {
