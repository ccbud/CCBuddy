...
import type {
  ...
  CCbuddyPersistedFileChange,
  CCbuddyProvider,
  CCbuddyStreamEvent,
  CCbuddyTimelineStatus,
  CCbuddyUsage,
} from "@ccbuddy/shared";
import {
...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly CCbuddyPersistedFileChange[];
  usage?: CCbuddyUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 CCbuddy Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 CCbuddy Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly CCbuddyTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: CCbuddyTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: CCbuddyContextCompactionTimelineMeta;
  provider?: CCbuddyProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // CCbuddy Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: CCbuddyUsage | null, right: CCbuddyUsage): CCbuddyUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly CCbuddyPersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<CCbuddyStreamEvent, { type: "task_network_debug_status" }>,
...
export function recordAgentStepTelemetryEvent(input: {
  ...
  switch (input.event.type) {
    ...
    case "agent_message_chunk":
      if (input.event.ccbuddyTimeline) {
        return finalized;
      }
