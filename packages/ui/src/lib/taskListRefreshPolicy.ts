import type { CCbuddyWorkspaceTaskListChanged } from "@ccbuddy/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<CCbuddyWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<CCbuddyWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<CCbuddyWorkspaceTaskListChanged, "reason">,
): boolean {
