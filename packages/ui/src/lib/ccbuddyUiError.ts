import type { CCbuddyError, TraceId } from "@ccbuddy/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@ccbuddy/shared/ccbuddy-protocol-v4";

export interface CCbuddyUiError extends CCbuddyError {
...
interface NormalizeCCbuddyUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_CCBUDDY_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "CCbuddy session failed",
  "CCbuddy session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // CCbuddy Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // ccbuddy-cli 会把模型/网络错误摘要放在 data.ccbuddy.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "ccbuddy", "error", "message"],
    ["data", "ccbuddy", "error", "detail"],
    ["data", "ccbuddy", "error", "details"],
  ];
...
function readFirstAttributionFromPaths(error: unknown): ErrorAttribution | undefined {
  ...
  const paths: Array<readonly string[]> = [
    ...
    ["data", "ccbuddy", "error", "attribution"],
  ];
  for (const path of paths) {
...
export function normalizeCCbuddyUiError(
  error: unknown,
  options: NormalizeCCbuddyUiErrorOptions = {},
): CCbuddyUiError {
  const candidates = collectMessageCandidates(error);
  // ccbuddy-cli 已经把 provider/network 根因放进 detail 或 data.ccbuddy.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_CCBUDDY_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) =>
      candidate !== primaryMessage && !GENERIC_CCBUDDY_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "ccbuddy", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "ccbuddy", "error", "underlyingErrorDetail"],
  ]);
