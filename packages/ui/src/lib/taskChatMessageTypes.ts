// re-home 产物（为删除 ccbuddyChatMessages 铺路，模式同 ccbuddy-task-types-core）。
// 本文件承载旧消息转换层（ccbuddyChatMessages/ccbuddyChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（ccbuddyChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  CCbuddyAssistantMessagePart,
  CCbuddyAssistantMessageFeedback,
  CCbuddyPromptAttachment,
  CCbuddyTimelineMeta,
  CCbuddyTaskSnapshotBodyRef,
  CCbuddyTaskSnapshotToolFieldRef,
  CCbuddyTaskSnapshotToolSlice,
} from "@ccbuddy/shared";
...
export interface TaskChatToolCall {
  ...
  /** CCbuddy 固定工具名；kind 仍保留给旧 CCbuddy Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = CCbuddyAssistantMessagePart;
