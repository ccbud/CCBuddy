import type { CCbuddyTaskRuntimeStatus } from "@ccbuddy/shared";
import { getWorkspaceDisplayedTaskState } from "@/store/ccbuddySessionStore.js";
import type {
...
interface MarkRemoteWorkspaceRunningTasksFailedParams {
  ...
  getWorkspaceState: CCbuddySessionStoreState["getWorkspaceState"];
  setTaskRuntimeState: CCbuddySessionStoreState["setTaskRuntimeState"];
  reason: string;
}

function isRunningRuntimeStatus(status: CCbuddyTaskRuntimeStatus): boolean {
...
function collectRemoteWorkspaceRunningTaskIds(workspaceState: WorkspaceCCbuddyUIState): string[] {
