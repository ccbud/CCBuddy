import type { CCbuddyProvider, CCbuddyTaskRuntimeStatus } from "@ccbuddy/shared";

function isBusyTaskRuntimeStatus(status: CCbuddyTaskRuntimeStatus): boolean {
  return status === "creating" || status === "restoring" || status === "streaming";
}

function buildTaskProviderByTaskId(
  optimisticTaskMetaByTaskId: Record<string, { provider?: CCbuddyProvider }>,
  taskListCache?: Array<{ taskId: string; provider?: CCbuddyProvider }> | null,
): Record<string, CCbuddyProvider | undefined> {
  const providerByTaskId: Record<string, CCbuddyProvider | undefined> = {};
...
export function hasBusyTaskInWorkspaceProvider(
  ...
  taskRuntimeByTaskId: Record<
    string,
    { status: CCbuddyTaskRuntimeStatus; provider?: CCbuddyProvider }
  >,
  optimisticTaskMetaByTaskId: Record<string, { provider?: CCbuddyProvider }>,
  taskListCache?: Array<{ taskId: string; provider?: CCbuddyProvider }> | null,
