...
import type { OnboardingRecordEntry } from "@ccbuddy/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useCCbuddyStore((state) => state.newUserOnboardingOpen);
  const setRequested = useCCbuddyStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useCCbuddyStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useCCbuddyStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useCCbuddyStore((state) => state.requestOnboardingDialog);
