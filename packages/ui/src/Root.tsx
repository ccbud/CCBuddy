...
import { StoreProvider, useCCbuddyStore } from "@/store/StoreProvider.js";
...
function RootInner({
  ...
  const user = useCCbuddyStore((state) => state.user);
  const isRestoringOAuthSession = useCCbuddyStore((state) => state.isRestoringOAuthSession);
  const setUser = useCCbuddyStore((state) => state.setUser);
  const setIsRestoringOAuthSession = useCCbuddyStore((state) => state.setIsRestoringOAuthSession);
  const setOAuthError = useCCbuddyStore((state) => state.setOAuthError);
  const oauthPollingActive = useCCbuddyStore((state) => state.oauthPollingActive);
  const setOAuthPollingActive = useCCbuddyStore((state) => state.setOAuthPollingActive);
  const markOAuthSuccess = useCCbuddyStore((state) => state.markOAuthSuccess);
  const {
    settings: appSettings,
    refresh: refreshAppSettings,
    update: updateAppSettings,
  } = useSettings();
  // CCbuddy only uses user-configured model services; inherited cloud JWT state
  // must never open the account sign-in screen at startup.
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.ccbuddyAgentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  useEffect(() => {
    ...
    void services.ccbuddyAgentService
  ...
  const totalUnreadTaskCount = useCCbuddySessionStore((state) =>
  ...
  const providerAvailabilityLoginEntryGuardEnabled = false;
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 CCbuddy session。
  ...
  const handleReauthenticationRequired = useCallback(() => {
    // A stale cloud-auth callback cannot turn a BYOK workspace into a login gate.
  }, []);
  const {
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 CCbuddy SVG，但 React 接管 root 后旧壳会被整棵替换。
