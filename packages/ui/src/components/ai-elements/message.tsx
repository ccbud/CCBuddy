 ...
 * Modified by CCbuddy: local integration, formatting and adaptations.
...
import type { EditorInfo, FileStat, OpenInEditorOptions } from "@ccbuddy/shared";
...
import { createCCbuddyFileCitationRemarkPlugin } from "@/lib/ccbuddyFileCitationRemarkPlugin.js";
import { windowsFileLinkEscapeRemarkPlugin } from "@/lib/windowsFileLinkEscapeRemarkPlugin.js";
import { projectCCbuddyFileCitations } from "@/lib/ccbuddyFileCitation.js";
import { rewriteMarkdownArtifactImageSources } from "@ccbuddy/shared";
...
export type MessageResponseProps = {
  ...
  /** 仅 Assistant 正文开启：把完整 ccbuddy-file-citation 投影为现有文件链接。 */
  renderCCbuddyFileCitations?: boolean;
};
...
export function buildMessageStreamdownRenderKey(params: {
  ...
  return [
    ...
    params.renderCCbuddyFileCitations ? "citations" : "plain",
...
export const messageResponsePropsAreEqual = (
  ...
  nextProps.renderCCbuddyFileCitations === prevProps.renderCCbuddyFileCitations &&
...
export const MessageResponse = memo(
  ({
    ...
    onOpenFileLink,
    onOpenExternalUrl,
    renderCCbuddyFileCitations = false,
    workspacePath,
    workspaceHomePath,
    workspaceIdentity,
    ...
    const projectedCitationMarkdown = useMemo(
      () =>
        renderCCbuddyFileCitations
          ? projectCCbuddyFileCitations(rawMarkdown, { streaming: renderStreaming }).visibleText
          : rawMarkdown,
      [rawMarkdown, renderStreaming, renderCCbuddyFileCitations],
    );
    ...
    const messageRemarkPlugins = useMemo<PluggableList>(
      () => [
        ...
        ...(renderCCbuddyFileCitations && workspacePath
          ? [createCCbuddyFileCitationRemarkPlugin(workspacePath, workspaceHomePath)]
          : []),
      ],
      [renderCCbuddyFileCitations, workspaceHomePath, workspacePath],
    );
    ...
    const boundaryResetKey = useMemo(
      () =>
        renderStreaming
          ...
          : `${streamdownMode}:${renderCCbuddyFileCitations ? "citations" : "plain"}:${hashMarkdownCacheKey(targetMarkdown)}`,
      [renderStreaming, renderCCbuddyFileCitations, streamdownMode, targetMarkdown],
    );
