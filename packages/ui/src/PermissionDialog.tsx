...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
function readPermissionRuleScopes(option: CCbuddyPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: CCbuddyPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: CCbuddyPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<CCbuddyProvider, Record<string, string>>
> = {
  glm: {
    // GLM/CCbuddy Agent 通过 CCbuddy Agent 发来的项目级记忆授权文案是英文原文。
...
function getPermissionDisplayReason(request: CCbuddyPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: CCbuddyPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: CCbuddyPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  // 当前 CCbuddy Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // CCbuddy Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
