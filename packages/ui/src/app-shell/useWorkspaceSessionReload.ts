...
import type { IServiceAccessor } from "@ccbuddy/services";
...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: CCbuddyProvider | null }) => {
      ...
      const ccbuddySessionStore = useCCbuddySessionStore.getState();
      const latestWorkspaceState = ccbuddySessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: CCbuddyProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      ccbuddySessionStore.setWorkspaceInitState(
        workspaceAbsPath,
        "initializing",
        null,
        workspaceIdentity,
      ...
      if (shouldPrepareWorkspace) {
        ccbuddySessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        ccbuddySessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        ccbuddySessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.ccbuddyTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithCCbuddySessionService({
            ...
            workspaceIdentity,
            provider,
            ccbuddySessionService: services.ccbuddySessionService,
          });

          const latestAfterPrepare = ccbuddySessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = ccbuddySessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              ccbuddySessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              ccbuddySessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              ccbuddySessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        ccbuddySessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        ccbuddySessionStore.setWorkspaceInitState(
          workspaceAbsPath,
          "ready",
          null,
        ...
        if (shouldPrepareWorkspace) {
          ccbuddySessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          ccbuddySessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
    ...
    [
      ...
      services.ccbuddySessionService,
      workspaceAbsPath,
      workspaceIdentity,
    ],
  );
