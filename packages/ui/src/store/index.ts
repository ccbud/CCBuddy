...
import type { IBroadcastService, BroadcastMessage } from "@ccbuddy/services";
import type { OAuthProviderId, UserInfo } from "@ccbuddy/shared";
import type { CodingPlanResetType } from "@ccbuddy/shared";
...
const CODE_PREVIEW_SETTINGS_KEY = "ccbuddy-code-preview-settings";
const PERFORMANCE_MODE_STORAGE_KEY = "ccbuddy-performance-mode";
 ...
 */
export function createCCbuddyStore(
  ...
  const useStore = create<CCbuddyState>()((set, get) => ({
    ...
    theme: normalizeThemePreference((readSafeLocalStorage("ccbuddy-theme") as Theme) || "zai-dark"),
    setTheme: (theme: Theme) => {
      const normalizedTheme = normalizeThemePreference(theme);
      writeSafeLocalStorage("ccbuddy-theme", normalizedTheme);
    ...
    locale: readSafeLocalStorage("ccbuddy-locale") || "zh-CN",
    setLocale: (locale: string) => {
      writeSafeLocalStorage("ccbuddy-locale", locale);
      set({ locale });
    },
...
export type CCbuddyStore = ReturnType<typeof createCCbuddyStore>;
