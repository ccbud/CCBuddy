...
import type { CCbuddyTaskListSortBy, ICCbuddyTaskService } from "@ccbuddy/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, CCbuddyTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    ccbuddyTaskService: Pick<ICCbuddyTaskService, "listTaskList">;
    sortBy: CCbuddyTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: CCbuddyTaskMeta, sortBy?: CCbuddyTaskListSortBy) => void;
...
function sortTimelineTasks(
  ...
  sortBy: CCbuddyTaskListSortBy = "updated",
): CCbuddyTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, ccbuddyTaskService, sortBy, limit }) {
    ...
    try {
      const result = await ccbuddyTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    ccbuddyTaskService: session.services.ccbuddyTaskService,
    sortBy,
    limit,
  });
}
