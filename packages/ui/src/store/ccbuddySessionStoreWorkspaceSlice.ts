...
} from "@/store/ccbuddySessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/ccbuddySessionStoreSelectors.js";

type SetFn = (
  partial:
    | CCbuddySessionStoreState
    | Partial<CCbuddySessionStoreState>
    | ((
        state: CCbuddySessionStoreState,
      ) => CCbuddySessionStoreState | Partial<CCbuddySessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: CCbuddyConfigOption): CCbuddyConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: CCbuddyConfigOption[]): CCbuddyConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly CCbuddyConfigOption[]): CCbuddyConfigOption[] {
...
function isModeConfigOption(option: CCbuddyConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<CCbuddyConfigOption["options"]>;
}): CCbuddyConfigOption {
...
function replaceModeConfigOption(
  currentOptions: CCbuddyConfigOption[] | null,
  nextModeOption: CCbuddyConfigOption | null,
): CCbuddyConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceCCbuddyUIState,
  updater: (options: CCbuddyConfigOption[] | null) => CCbuddyConfigOption[] | null,
): Pick<WorkspaceCCbuddyUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceCCbuddyUIState,
  taskId: string,
): { configOptions: CCbuddyConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: CCbuddyTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as CCbuddyTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToCCbuddyAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToCCbuddyAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToCCbuddyAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
