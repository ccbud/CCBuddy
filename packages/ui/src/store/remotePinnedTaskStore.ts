...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    ccbuddyTaskService: Pick<ICCbuddyTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: CCbuddyTaskMeta) => void;
...
function sortPinnedTasks(tasks: CCbuddyTaskMeta[]): CCbuddyTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, ccbuddyTaskService }) {
    ...
    try {
      const items = await ccbuddyTaskService.listPinnedTasks({
        workspacePath,
