...
// 定时任务(automation)管理 store：走 ccbuddy-agent RPC（列表 / 创建 / 编辑 / 启停 / 重跑 / 删除 + 运行历史）。
...
interface AutomationManagementState {
  ...
  refresh: (agentService: ICCbuddyAgentService) => Promise<void>;
  createAutomation: (
  ...
  deleteAutomation: (automationId: string, agentService: ICCbuddyAgentService) => Promise<void>;
  setEnabled: (
    automationId: string,
    enabled: boolean,
    agentService: ICCbuddyAgentService,
  ) => Promise<void>;
  restartAutomation: (automationId: string, agentService: ICCbuddyAgentService) => Promise<void>;
