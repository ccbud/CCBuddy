import type { McpServerConfig, NativeMcpServerRecord } from "@ccbuddy/shared";
...
function isCCbuddyAgentUserServer(server: NativeMcpServerRecord): boolean {
  return (
    server.source === "ccbuddyagentmcp" &&
    server.scope === "user" &&
    (!server.location || server.location.source === "ccbuddy")
  );
}

export async function importLegacyCommonServersToCCbuddyAgent(
  ...
  const existingNames = new Set(
    nativeServers.filter(isCCbuddyAgentUserServer).map((server) => server.name),
  );
  let importedCount = 0;
  let skippedCount = 0;
...
export async function migrateStoredCommonMcpToCCbuddyAgent(
  platform: McpPlatformService | null,
  nativeServers: NativeMcpServerRecord[],
  ...
  const migration = await importLegacyCommonServersToCCbuddyAgent(
    ...
    legacyServers,
    nativeServers,
    "localStorage:ccbuddy-mcp-config",
  );
  if (migration.completed) {
    // 只有确认写入 ccbuddy agent 目录后才清理旧数据，避免 Web 端没有 desktop bridge 时丢配置。
    clearLegacyCommonMcpServers();
  }
  ...
  logger.info(
    `[mcpStore] migrated ${migration.importedCount} legacy common MCP servers to ccbuddy agent config`,
  );
