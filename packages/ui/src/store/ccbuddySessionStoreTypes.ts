/* oxlint-disable eslint(max-lines) -- CCbuddy Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * CCbuddy Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 ccbuddySessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type CCbuddyWorkspaceInitStatus,
  type CCbuddyTaskRuntimeStatus,
  type CCbuddyProvider,
  type CCbuddyTaskMeta,
  type CCbuddyContextCacheUsage,
  ...
  type CCbuddySessionActiveTurnKind,
  type CCbuddyContextUsageBreakdownItem,
  type InputId,
  type SessionCreateSource,
} from "@ccbuddy/shared";
import type { CCbuddyUiError } from "@/lib/ccbuddyUiError.js";
import type {
...
export interface WorkspaceInitState {
  status: CCbuddyWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: CCbuddyTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 CCbuddy Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: CCbuddyBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: CCbuddyPermissionRequest | null;
  pendingPermissionRequests: CCbuddyPermissionRequest[];
  elicitationRequest: CCbuddyElicitationRequest | null;
  pendingElicitationRequests: CCbuddyElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: CCbuddyUiError | null;
}
...
export interface WorkspaceCCbuddyUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * CCbuddy Agent 的 workspace 初始化状态。
   *
   * 单 CCbuddy Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  draftError: CCbuddyUiError | null;
  ...
  taskConfigOptionsByTaskId: Record<string, CCbuddyConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, CCbuddyTaskMeta>;
  ...
  /** 当前 workspace 选中的 CCbuddy Agent provider */
  selectedProvider: CCbuddyProvider;
  ...
  /** CCbuddy Agent configOptions（模型、模式、思考级别等） */
  configOptions: CCbuddyConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: CCbuddyTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface CCbuddySessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceCCbuddyUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceCCbuddyUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: CCbuddyPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: CCbuddyElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
...
const FALLBACK_PROVIDER: CCbuddyProvider = CCBUDDY_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceCCbuddyUIState {
  // 单 CCbuddy Agent 迁移后默认 provider 必须收敛到 glm。
