...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToCCbuddyAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToCCbuddyAgent(params.provider);
