...
import { getDefaultWorkspaceState } from "@/store/ccbuddySessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly CCbuddyTaskMeta[]): CCbuddyTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: CCbuddyPermissionRequest["options"],
  right: CCbuddyPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: CCbuddyPermissionRequest,
  right: CCbuddyPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
...
): Pick<CCbuddySessionStoreState, "workspaces"> {
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useCCbuddyChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToCCbuddyAgent(params.provider),
