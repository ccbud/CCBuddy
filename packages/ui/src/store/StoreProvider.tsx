...
import type { IBroadcastService } from "@ccbuddy/services";
import { createCCbuddyStore, type CCbuddyStore, type CCbuddyState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<CCbuddyStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<CCbuddyStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createCCbuddyStore(broadcastService, {
 ...
 */
export function useCCbuddyStore<T>(selector: (state: CCbuddyState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useCCbuddyStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useCCbuddyStore（store 耦合剥离配套）。
 *
 ...
 */
export function useCCbuddyStoreWithDefault<T>(
  selector: (state: CCbuddyState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
