...
import type { CCbuddyTaskListItem } from "@ccbuddy/services";
import type { CCbuddyTaskMeta } from "@ccbuddy/shared";
import { matchesTaskListMembershipKind } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import { mergeTaskMetaCandidates } from "@/lib/ccbuddyTaskMetaMerge.js";
import { compareCCbuddyTaskListItems } from "@/lib/taskListOrdering.js";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: CCbuddyTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: CCbuddyTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: CCbuddyTaskMeta;
    nextTask: CCbuddyTaskMeta;
    previousState: TaskListMembershipState;
    nextState: TaskListMembershipState;
  }) => void;
  setTaskUnreadOverlay: (
    task: Pick<CCbuddyTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">,
...
type CachedTaskListItem = CCbuddyTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: CCbuddyTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, CCbuddyTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareCCbuddyTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
...
export function upsertTaskQueryCacheTaskMeta(task: CCbuddyTaskMeta): void {
  useTaskQueryCacheStore.getState().upsertTaskMeta(task);
}

export function updateTaskQueryCacheTaskMetaPreservingMembership(task: CCbuddyTaskMeta): void {
