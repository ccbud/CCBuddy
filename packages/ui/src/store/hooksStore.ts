...
import type { Hook, HookConfig } from "@ccbuddy/shared";
import type { IHooksService } from "@ccbuddy/services";
...
function buildCCbuddyHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildCCbuddyHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "ccbuddy") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildCCbuddyHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
