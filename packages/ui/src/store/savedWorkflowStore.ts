...
import {
  ...
  type CCbuddySavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: CCbuddySavedWorkflowEntry[];
  invalid: CCbuddySavedWorkflowInvalidEntry[];
  runs: CCbuddySavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: CCbuddyAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: CCbuddyAgentSavedWorkflowTarget,
  agentService: ICCbuddyAgentService,
): Promise<{ list: CCbuddyWorkflowsListResult; runs: CCbuddySavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as CCbuddySavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: CCbuddyAgentSavedWorkflowTarget | null,
