...
import type {
  ...
  CCbuddyMcpListMode,
  CCbuddyMcpServerStatusSnapshot,
  CCbuddyMcpServer,
} from "@ccbuddy/shared";
import { convertToCCbuddyAgentMcpServer } from "@ccbuddy/shared";
...
import {
  importLegacyCommonServersToCCbuddyAgent,
  migrateStoredCommonMcpToCCbuddyAgent,
...
interface McpStoreState {
  ...
  servers: CCbuddyMcpServer[];
  statusSnapshots: Record<string, CCbuddyMcpServerStatusSnapshot>;
  ...
  addCCbuddyAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateCCbuddyAgentMcpServer: (
    name: string,
    config: McpServerConfig,
    projectPath?: string,
  ) => void;
  deleteCCbuddyAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  beginServerStatusListRefresh: (mode?: CCbuddyMcpListMode) => number;
  ...
  mergeServerStatusSnapshots: (
    statuses: Record<string, CCbuddyMcpServerStatusSnapshot>,
    requestEpoch: number,
    mode?: "connect" | "status",
  ) => void;
  getServer: (id: string) => CCbuddyMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForCCbuddy: (provider: string) => CCbuddyAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<CCbuddyMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 CCbuddy Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.ccbuddy/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 CCbuddy Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToCCbuddyAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addCCbuddyAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateCCbuddyAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteCCbuddyAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "ccbuddyagentmcp" : source;
    ...
    addCCbuddyAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("ccbuddyagentmcp", name, config, projectPath),
    updateCCbuddyAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("ccbuddyagentmcp", name, config, projectPath),
    deleteCCbuddyAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("ccbuddyagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "ccbuddyagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForCCbuddy: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, CCbuddyMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "ccbuddyagentmcp") continue;
      ...
      const result: CCbuddyAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const ccbuddyAgentServer = convertToCCbuddyAgentMcpServer(server.name, server.config);
        if (ccbuddyAgentServer) result.push(ccbuddyAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "ccbuddyagentmcp";
    ...
    migrateLegacyCommonMcp: async () => {
      ...
      const migration = await importLegacyCommonServersToCCbuddyAgent(
        mcpPlatformService,
        result.servers ?? {},
