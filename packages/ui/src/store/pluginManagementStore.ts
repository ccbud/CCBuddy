...
import type {
  ...
  CCbuddyPluginDiagnostic,
  CCbuddyPluginInfo,
  CCbuddyPluginMarketplaceSummary,
  CCbuddyPluginScope,
  CCbuddyPluginsDescribeResult,
} from "@ccbuddy/shared";
import type { IPluginManagementService } from "@ccbuddy/services";
...
export interface PluginDescribeEntry {
  ...
  data?: CCbuddyPluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 ccbuddy-cli 提供 (list + enable/disable)。
// UI 不再直触 ICCbuddyAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
// 与已 retired 的 marketplace pluginStore 无关, 故单独建一个精简 store。
export interface PluginManagementState {
  workspacePath: string | null;
  workspaceIdentity: string | null;
  configScope: CCbuddyPluginScope | null;
  ...
  diagnostics: CCbuddyPluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
  ...
  initialize: (params: {
    ...
    configScope?: CCbuddyPluginScope;
...
function throwForErrorDiagnostic(diagnostics: CCbuddyPluginDiagnostic[]): void {
