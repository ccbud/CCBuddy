...
import type { AttachmentRef } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
declare global {
  interface Window {
    __ccbuddyComposerAttachmentUploadStoreE2E?: typeof useComposerAttachmentUploadStore;
    __ccbuddyCurrentComposerAttachmentScopeKeyE2E?: string;
  }
}

if (shouldExposeE2EStoreBridge()) {
  // E2E 只暴露当前唯一附件 owner，供 scope 切换用例准备状态；不再把附件塞回旧 Session Store。
  window.__ccbuddyComposerAttachmentUploadStoreE2E = useComposerAttachmentUploadStore;
}

export function exposeComposerAttachmentScopeKeyForE2E(scopeKey: string): void {
  if (window.__ccbuddyComposerAttachmentUploadStoreE2E) {
    window.__ccbuddyCurrentComposerAttachmentScopeKeyE2E = scopeKey;
  }
}
