/**
 * CCbuddy Session Store 选择器与内部辅助函数
 *
 * 从 ccbuddySessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { CCbuddyTaskRuntimeStatus, CCbuddyTaskMeta } from "@ccbuddy/shared";
...
import {
  ...
  type CCbuddySessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceCCbuddyUIState,
} from "./ccbuddySessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceCCbuddyUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceCCbuddyUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: CCbuddySessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceCCbuddyUIState) => WorkspaceCCbuddyUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 CCbuddy Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceCCbuddyUIState,
  taskId: string,
): TaskRuntimeState {
...
export function getTaskUiState(workspaceState: WorkspaceCCbuddyUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceCCbuddyUIState
    | Partial<Pick<WorkspaceCCbuddyUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, CCbuddyTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<CCbuddyTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceCCbuddyState(
  state: CCbuddySessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceCCbuddyUIState, "workspaceInit">,
): WorkspaceInitState {
