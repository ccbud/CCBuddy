...
import {
  CCBUDDY_MODE_OPTION_DESCRIPTION_IDS,
  CCBUDDY_MODE_OPTION_LABEL_IDS,
...
function isConfigSelectValueInOptions(option: CCbuddyConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: CCbuddyConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  option: CCbuddyConfigOption,
  entry: CCbuddyConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 CCbuddy Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<CCbuddyConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: CCbuddyProvider | undefined,
  entry: Pick<CCbuddyConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return CCBUDDY_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return CCBUDDY_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
