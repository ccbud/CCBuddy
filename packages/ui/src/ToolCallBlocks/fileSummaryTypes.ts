...
import type { WorkflowRunState } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
export function readRawToolCallInput(raw: unknown): unknown {
  ...
  // CCbuddy protocol 的 permission/request payload 按 schema 把工具参数放在 input，
...
export interface WorkflowRunCardSummary {
   ...
   *
   * 两个数都出自 `workflowRunStepCounts`（@ccbuddy/shared）：`nodes` 表内 + `usage.nodesUnlisted`
