import type { useCCbuddyIntl } from "@/i18n/IntlProvider.js";
import type { ToolCallBlockRenderContext } from "@/ToolCallBlocks/shared.js";
import { isSubagentColor } from "@/lib/subagentColors.js";

type AgentIntl = ReturnType<typeof useCCbuddyIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "ccbuddy", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "ccbuddy", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "ccbuddy", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "ccbuddy", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const ccbuddy = meta && isPlainRecord(meta.ccbuddy) ? meta.ccbuddy : null;
  const ccbuddyBackgroundAgent =
    ccbuddy && isPlainRecord(ccbuddy.backgroundAgent) ? ccbuddy.backgroundAgent : null;
  const taskNotification =
    ccbuddy && isPlainRecord(ccbuddy.taskNotification) ? ccbuddy.taskNotification : null;
  const backgroundAgent = ccbuddyBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "ccbuddy", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "ccbuddy", "taskNotification", "summary"]);
