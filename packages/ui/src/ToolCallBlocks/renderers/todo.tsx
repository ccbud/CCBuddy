...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@ccbuddy/shared";
import type { CCbuddyPlanStep } from "@ccbuddy/shared";
...
const todoTextClasses: Record<CCbuddyPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): CCbuddyPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: CCbuddyPlanStep["status"] }) {
  if (status === "completed") {
