...
import type { BroadcastMessage } from "@ccbuddy/services";
import { buildTaskWorkspaceKey } from "@/lib/taskQueryCache.js";
import type { WindowTabState } from "@/store/tabStore.js";
import { isWorkspaceTab } from "@/store/tabStore.js";

function isCCbuddyStreamEvent(value: unknown): value is CCbuddyStreamEvent {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const event = value as Partial<CCbuddyStreamEvent>;
...
function isBotTaskStreamBroadcastPayload(
  ...
  return (
    ...
    isCCbuddyStreamEvent(value.event) &&
