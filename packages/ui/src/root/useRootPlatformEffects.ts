...
function isCloudSharingEnabled(): boolean {
  return false;
}
...
export function useRootPlatformEffects({
  ...
  initialWorkspacePurpose?: import("@ccbuddy/shared").WorkspacePurpose;
  ...
  isDesktop?: boolean;
  locale: ReturnType<typeof import("@/i18n/IntlProvider.js").useCCbuddyIntl>["locale"];
  tabs: WindowTabState[];
  ...
  intl: ReturnType<typeof import("@/i18n/IntlProvider.js").useCCbuddyIntl>["intl"];
  ...
  useEffect(() => {
    ...
    const disposeShareImport =
      isCloudSharingEnabled() && platform.onShareImport
    ...
    const disposeNotificationClick = platform.onTaskNotificationClick((taskId: string) => {
      ...
      const workspaces = useCCbuddySessionStore.getState().workspaces;
  ...
  useEffect(() => {
    ...
    void baseServices.conversationShareService
      ...
      .then((result) => {
        ...
        const sessionStore = useCCbuddySessionStore.getState();
  ...
  useEffect(() => {
    ...
    syncActiveSession();
    return useCCbuddySessionStore.subscribe(syncActiveSession);
