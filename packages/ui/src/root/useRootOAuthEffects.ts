...
import {
  ...
  CCBUDDY_JWT_INVALID_BROADCAST_CHANNEL,
} from "@ccbuddy/shared";
...
import { markCCbuddyJwtInvalidRestart } from "@/root/ccbuddyJwtInvalidRestartMarker.js";
...
export function useRootOAuthEffects({
  ...
  useEffect(() => {
    ...
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel !== CCBUDDY_JWT_INVALID_BROADCAST_CHANNEL || disposed) {
        return;
      }
      void (async () => {
        ...
        markCCbuddyJwtInvalidRestart();
        if (typeof window !== "undefined" && !("ccbuddy" in window)) {
