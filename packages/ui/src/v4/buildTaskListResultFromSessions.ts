...
interface BuildTaskListParams {
  ...
  terminalStatusByTaskId?: ReadonlyMap<
    string,
    Extract<CCbuddyTaskMeta["status"], "completed" | "error">
  >;
...
function mergeTaskIndexRowWithSession(
  taskIndexTask: CCbuddyTaskMeta,
  sessionTask: CCbuddyTaskMeta,
): CCbuddyTaskMeta {
...
export function buildTaskListResult(params: BuildTaskListParams): BuildTaskListResult {
  ...
  filtered.sort((a, b) => compareCCbuddyTaskListItems(a, b, params.sortBy));
