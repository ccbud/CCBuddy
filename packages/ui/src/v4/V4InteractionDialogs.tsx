...
import type { ElicitationFormDraft } from "@/store/ccbuddySessionStoreTypes.js";
...
function resolveV4ElicitationRequest(
  projected: CCbuddyElicitationRequest | null,
  botProgress: CCbuddyElicitationRequest | null,
): CCbuddyElicitationRequest | null {
...
function buildV4ElicitationProgressKey(request: CCbuddyElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useCCbuddyStoreWithDefault(
    (state) => state.notificationEnabled,
    true,
  );
  const botElicitationProgress = useCCbuddySessionStore(
    (state) =>
      getTaskUiState(getWorkspaceState(state, workspacePath, workspaceIdentity), sessionId)
        .elicitationRequest,
  );
  const localElicitationDraft = useCCbuddySessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useCCbuddySessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: CCbuddyPermissionOption, feedback?: string) => {
