import type { GitRepositorySummary } from "@ccbuddy/shared";
import type {
...
import { workflowRunStepCounts } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
function buildRunningWorkflowRuns(
  ...
  for (const run of runs ?? []) {
    ...
    // 计数与聊天紧凑卡同源（唯一实现在 @ccbuddy/shared 的 workflowRunStepCounts：表内 + 表外）。
