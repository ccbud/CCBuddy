...
import { isCCbuddyFileLockTimeoutError } from "@ccbuddy/shared";
import { CCBUDDY_AGENT_RUNTIME_UNAVAILABLE_CODE } from "@ccbuddy/services";
...
function isTransientSubscribeError(error: unknown): boolean {
  if (isCCbuddyFileLockTimeoutError(error)) {
    return true;
  }
  const code =
