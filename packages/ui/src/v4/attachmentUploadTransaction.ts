import { BufferWriter, serialize } from "@ccbuddy/rpc";
...
interface AttachmentUploadAgent {
  ...
  attachmentCommitV4(params: CCbuddyAgentAttachmentTerminalParams): Promise<V4AttachmentPutResult>;
...
function measureAttachmentChannelRequestBytes(method: string, params: unknown): number {
  ...
  serialize(writer, [100, 2_147_483_647, ServiceChannels.CCbuddyAgent, method]);
...
export async function uploadAttachmentTransaction(
  ...
  const beginParams: CCbuddyAgentAttachmentBeginParams = {
    ...common,
  ...
  try {
    ...
    for (let chunkIndex = begin.nextChunkIndex; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      const chunkParams: CCbuddyAgentAttachmentChunkParams = {
        ...common,
        chunkIndex,
    ...
    const terminal = common satisfies CCbuddyAgentAttachmentTerminalParams;
