...
import { CCBUDDY_AGENT_PROVIDER, resolveExecutionState } from "@ccbuddy/shared";
...
import type { SessionConfigState } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import type { IModelSelectionService } from "@ccbuddy/services";
...
import { submissionModeSchema } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import { prepareWorkspaceWithCCbuddySessionService } from "@/hooks/useWorkspacePrepare.js";
import { useCCbuddySessionService } from "@/hooks/useCCbuddySessionService.js";
...
function shouldHydrateWorkspaceCatalog(params: {
  configOptions: readonly CCbuddyConfigOption[];
  sessionId: string | null;
  slashCommands: readonly CCbuddySlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const ccbuddySessionService = useCCbuddySessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithCCbuddySessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useCCbuddySessionStore.getState();
