...
// 复用 treemappingActivity 的既有解析规则，不复活 ccbuddyChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@ccbuddy/shared/ccbuddy-protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(ccbuddyAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [ccbuddyAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
