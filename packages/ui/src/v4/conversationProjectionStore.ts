...
const RUNTIME_RECYCLED_ERROR = "CCbuddy Agent runtime 已被回收，重连未成功";
 ...
 *
 * 三种文案都来自同一次回收：transport 关闭时 CCbuddyProtocolClient.rejectAll 打断在途请求
...
function isRuntimeRecycleError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("CCbuddy Agent transport closed") ||
    message.includes("CCbuddy Protocol client disposed") ||
    message.includes("CCbuddy agent transport closed") ||
    message.includes("CCbuddy Protocol client disposed") ||
    message.includes("CCbuddy Protocol client is disposed") ||
    message.includes("CCbuddy Agent runtime is not running") ||
    message.includes("CCbuddy Agent runtime is not running")
  );
}
...
export class ConversationProjectionStore {
  ...
  /**
   * 有界退避重连。重订阅走 start-if-needed（ccbuddyAgentService.subscribeConversationV4），
