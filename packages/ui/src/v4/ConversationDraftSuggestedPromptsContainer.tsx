  ...
  const revalidateAndPrependPlugin = useCallback(
    async (current: DraftSuggestedPluginFlow, requestVersion: number) => {
      await invalidateDeferredDraftSessionForSkillChange({
        ccbuddySessionService: resolution.services.ccbuddySessionService,
        workspacePath,
        workspaceIdentity,
    ...
    [
      ...
      resolution.services.ccbuddySessionService,
      targetParams,
      workspaceIdentity,
      workspacePath,
    ],
      ...
      if (
        ...
        (kind === "install" &&
          (!flow.result?.pluginName ||
            flow.result.marketplace !== CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID ||
