// sessions-index 的 SessionSummary → 侧栏实时 detail CCbuddyTaskMeta 映射。
...
import type { TraceId, CCbuddyProvider, CCbuddyTaskMeta } from "@ccbuddy/shared";
import type { SessionSummary } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): CCbuddyTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
/**
 * SessionSummary → 实时 detail CCbuddyTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as CCbuddyProvider } : {}),
      ...(status ? { status } : {}),
