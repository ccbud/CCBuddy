...
import type { ConversationTelemetryFact } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
export interface ConversationPromptTelemetrySeed {
  localTtft?: import("@ccbuddy/shared").LocalTtftContext;
...
export function resolveSendAckSettlement(ack: {
  ...
  // ccbuddy 没有乐观渲染，要等投影回流出 userInput row 才算发送成功。
...
function toLegacyNetworkEvent(
...
): Extract<CCbuddyStreamEvent, { type: "task_network_debug_status" }> {
  return {
  ...
  } as Extract<CCbuddyStreamEvent, { type: "task_network_debug_status" }>;
}
...
function toToolLifecycleEvent(input: {
  ...
  if (!input.hasStarted && fact.phase !== "completed" && fact.phase !== "failed") {
    ...
    } as Extract<CCbuddyStreamEvent, { type: "tool_call" }>;
  }
  return {
    ...
    raw: {},
    ...(skillMetadata ? { skillMetadata } : {}),
  } as Extract<CCbuddyStreamEvent, { type: "tool_call_update" }>;
}

function toUsage(fact: Extract<ConversationTelemetryFact, { kind: "usage.delta" }>): CCbuddyUsage {
  return {
...
function mergeUsage(left: CCbuddyUsage | null, right: CCbuddyUsage): CCbuddyUsage {
...
export class ConversationTelemetrySupervisor {
  ...
  private processPromptFact(
    ...
    switch (fact.kind) {
      ...
      case "stream.chunk": {
        ...
        } as Extract<CCbuddyStreamEvent, { type: "agent_thought_chunk" | "agent_message_chunk" }>;
  ...
  private handleTerminal(
    ...
    const terminalEvent = // 修复原因：旧 adapter 把所有 TurnComplete（包括 cancelled）先投影成
      ...
      ) as Extract<CCbuddyStreamEvent, { type: "task_complete" | "task_error" }>;
    this.reportFinalizedSteps(
      lifecycle,
      recordAgentStepTelemetryEvent({
        taskId: lifecycle.taskKey,
  ...
  private handleCompaction(
    ...
    const timeline: CCbuddyContextCompactionTimelineMeta = {
      version: 1,
      kind: "synthetic",
