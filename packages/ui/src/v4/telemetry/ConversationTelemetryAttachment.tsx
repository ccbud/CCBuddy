...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@ccbuddy/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@ccbuddy/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.ccbuddyAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.ccbuddyAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.ccbuddyAgentService as object | null | undefined;
