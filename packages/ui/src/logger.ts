import { formatLogPrefix } from "@ccbuddy/shared";
  ...
  return (
    (
      globalThis as typeof globalThis & {
        __CCBUDDY_RENDERER_DISABLE_LOGGING__?: boolean;
      }
    ).__CCBUDDY_RENDERER_DISABLE_LOGGING__ === true
  );
}
...
function log(level: LogLevel, ...args: unknown[]) {
  ...
  // ui 包单独 typecheck 时拿不到 desktop renderer 注入的 window.ccbuddy 声明，
  ...
  if (level !== "debug" && typeof window !== "undefined") {
    ...
    (window as DesktopLogBridgeWindow).ccbuddy?.log?.(level, args);
  }
}
...
export function logMemoryDiagnostics(line: string): void {
  ...
  if (typeof window !== "undefined") {
    const bridge = (window as DesktopLogBridgeWindow).ccbuddy?.log;
    if (bridge) {
      bridge("info", [line]);
      return;
    }
