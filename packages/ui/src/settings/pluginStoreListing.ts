...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<CCbuddyAvailablePluginSummary, "name" | "listing">[],
  name: string,
): CCbuddyPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: CCbuddyPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: CCbuddyPluginStoreListing;
  summary?: CCbuddyAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: CCbuddyPluginInfo;
  installedMeta?: CCbuddyInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<CCbuddyInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly CCbuddyPluginMarketplaceSummary[],
  locale: string,
): CCbuddyPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: CCbuddyPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: CCbuddyAvailablePluginSummary[];
  installedPlugins: CCbuddyInstalledPluginSummary[];
  plugins: CCbuddyPluginInfo[];
  restorableBuiltins: CCbuddyAvailablePluginSummary[];
}): StorePluginItem[] {
