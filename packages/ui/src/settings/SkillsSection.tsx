...
import { CCBUDDY_AGENT_PROVIDER } from "@ccbuddy/shared";
...
export function SkillsSection({
  ...
  const setEnabled = useCallback(
    async (skillId: string, enabled: boolean) => {
      ...
      // 移除三方来源后，技能状态统一写入 CCbuddy Agent 上下文，避免旧 provider 前缀带来分桶漂移。
      const targetSkill = skills.find((skill) => skill.id === skillId);
      const effectiveProvider: CCbuddyProvider = CCBUDDY_AGENT_PROVIDER;
      try {
  ...
  const scopedProviderSkills = useMemo(() => {
    const allProviderSkills = filterSkillsForProvider(skills, CCBUDDY_AGENT_PROVIDER);
