...
import {
  ...
  type CCbuddySavedWorkflowEntry,
  type CCbuddySavedWorkflowMeta,
  type CCbuddySavedWorkflowRun,
  type CCbuddyWorkflowsGetResult,
} from "@ccbuddy/shared";
import type { ICCbuddyAgentService, CCbuddyAgentSavedWorkflowTarget } from "@ccbuddy/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<CCbuddyWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: CCbuddySavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: CCbuddyAgentSavedWorkflowTarget;
  agentService: ICCbuddyAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: CCbuddySavedWorkflowEntry | undefined;
  runs: readonly CCbuddySavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: CCbuddySavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: CCbuddySavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: CCbuddySavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: CCbuddySavedWorkflowMeta = {
      description,
