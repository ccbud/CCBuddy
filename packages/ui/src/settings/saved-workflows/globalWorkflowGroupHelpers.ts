...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type CCbuddySavedWorkflowRun } from "@ccbuddy/shared";
import type { CCbuddyAgentSavedWorkflowTarget } from "@ccbuddy/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: CCbuddyAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: CCbuddySavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
