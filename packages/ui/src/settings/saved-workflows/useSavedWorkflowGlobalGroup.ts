...
// max-lines 400；载体是 `useServices().ccbuddyAgentService`，RPC 一律带 `{ scope: "global" }`。
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { CCbuddySavedWorkflowEntry, CCbuddySavedWorkflowRun } from "@ccbuddy/shared";
...
export function useSavedWorkflowGlobalGroup({
  ...
  const { ccbuddyAgentService: agentService, fileWatcherService } = useServices();
  ...
  const [launchEntry, setLaunchEntry] = useState<CCbuddySavedWorkflowEntry | null>(null);
  const [moveEntry, setMoveEntry] = useState<CCbuddySavedWorkflowEntry | null>(null);
  ...
  // 目录监听：list 回的 CCbuddy 绝对目录可直接 watch。
  ...
  const handleRun = useCallback(
    (entry: CCbuddySavedWorkflowEntry) => {
      launcher.clearError();
  ...
  const handleDelete = useCallback(
    async (entry: CCbuddySavedWorkflowEntry) => {
  ...
  const handleCardDelete = useCallback(
    (entry: CCbuddySavedWorkflowEntry) => void handleDelete(entry),
    [handleDelete],
  );
  const handleCardMove = useCallback((entry: CCbuddySavedWorkflowEntry) => setMoveEntry(entry), []);
  const handleOpen = useCallback(
    (entry: CCbuddySavedWorkflowEntry) => onOpenDetail(entry.name),
    [onOpenDetail],
  );
  // 全局档的目标项目按 `run.cwd` 反查已打开项目；没打开就两个入口都关掉
  // （实例必须开在发起它的项目）。门与实参构造与项目档共用（产物不需要 toolCallId）。
  const resolveRunTarget = useCallback(
    (run: CCbuddySavedWorkflowRun) => findProjectByCwd(localProjects, run.cwd) ?? null,
    [localProjects],
  );
