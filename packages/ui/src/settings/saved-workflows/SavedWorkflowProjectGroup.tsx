...
export function SavedWorkflowProjectGroup({
  ...
  const agentService = services.ccbuddyAgentService;
  ...
  const launch = useCallback(
    async (entry: CCbuddySavedWorkflowEntry, args: Record<string, unknown>) => {
  ...
  const handleCardPromote = useCallback(
    (entry: CCbuddySavedWorkflowEntry) => void handlePromote(entry),
    [handlePromote],
  );
