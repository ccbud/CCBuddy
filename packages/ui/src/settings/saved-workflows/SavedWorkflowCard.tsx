...
function SavedWorkflowLastRunBadge({
  ...
  run: CCbuddySavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: CCbuddySavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: CCbuddySavedWorkflowEntry) => void;
  onRun: (entry: CCbuddySavedWorkflowEntry) => void;
  onRevise: (entry: CCbuddySavedWorkflowEntry) => void;
  onCopyPath: (entry: CCbuddySavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: CCbuddySavedWorkflowEntry) => void;
  onDelete: (entry: CCbuddySavedWorkflowEntry) => void;
}
