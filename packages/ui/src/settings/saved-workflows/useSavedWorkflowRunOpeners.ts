...
import type { CCbuddySavedWorkflowRun } from "@ccbuddy/shared";
import type {
...
export function useSavedWorkflowRunOpeners(options: {
  ...
  resolveTarget: (run: CCbuddySavedWorkflowRun) => SavedWorkflowRunOpenTarget | null;
  ...
  handleOpenRun: (run: CCbuddySavedWorkflowRun, workflowName: string) => void;
  handleOpenArtifact: (run: CCbuddySavedWorkflowRun, artifactId: string) => void;
} {
  const { onOpenWorkflowArtifact, onOpenWorkflowRun, resolveTarget } = options;

  const handleOpenRun = useCallback(
    (run: CCbuddySavedWorkflowRun, workflowName: string) => {
  ...
  const handleOpenArtifact = useCallback(
    (run: CCbuddySavedWorkflowRun, artifactId: string) => {
      if (!run.parentSessionId) return;
      const target = resolveTarget(run);
      if (!target) return;
      // 行上的 chip 载荷（`CCbuddySavedWorkflowRun.artifacts`）带最新版的 `contentType`：
