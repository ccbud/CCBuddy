import type {
  ...
  CCbuddyPluginInfo,
  CCbuddyPluginComponentKind,
  CCbuddyPluginsDescribeResult,
} from "@ccbuddy/shared";
import { isPluginCommand, isUserCommand, CCBUDDY_COMMAND_AGENT_SOURCE } from "@ccbuddy/shared";
...
const COMPONENT_KIND_ORDER: CCbuddyPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: CCbuddyPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<CCbuddyPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: CCbuddyMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: CCbuddyMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === CCBUDDY_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(
  ...
  return servers.filter((server) => {
    if (server.source !== "ccbuddyagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: CCbuddyPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: CCbuddyPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: CCbuddyMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
