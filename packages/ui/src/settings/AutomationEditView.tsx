...
import {
  ...
  type CCbuddyAutomationScheduleRule,
} from "@ccbuddy/shared";
import {
...
function MonthDayPicker({
  ...
  month: number;
  day: number;
  intl: ReturnType<typeof useCCbuddyIntl>["intl"];
...
interface AutomationEditViewProps {
  /** null = 新建；否则编辑。 */
  editing: CCbuddyAutomation | null;
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: CCbuddyAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: CCbuddyAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: CCbuddyAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: CCBUDDY_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<CCbuddyAutomationScheduleRule | undefined>(
    () =>
