...
import type { CodingPlanWebviewLocale } from "@ccbuddy/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "ccbuddy:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeCCbuddyEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeCCbuddyEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeCCbuddyEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__ccbuddyLang__ 供 ccbuddyBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          ccbuddyjwttoken: credentials.ccbuddyJwtToken?.trim() || null,
          "oauth:bigmodel:access_token": null,
        }
      : {
          "oauth:zai:access_token": null,
          // ccbuddyjwttoken 是 ccbuddy-plan 域通用凭证（BigModel OAuth callback 同样落盘），
  ...
  return `(() => {
  ${storageUpdates}
  const ccbuddyTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", ccbuddyTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", ccbuddyTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", ccbuddyTheme === "zai-dark");
  localStorage.setItem("ccbuddy-theme", ccbuddyTheme);
  localStorage.setItem("ccbuddy:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 ccbuddyBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__ccbuddyLang__ = ${JSON.stringify(resolvedLocale)};
  const ccbuddyReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__ccbuddyReportContext__ = ccbuddyReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(ccbuddyReportContext));
  window.dispatchEvent(new CustomEvent("ccbuddy-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: ccbuddyReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__ccbuddyReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "ccbuddy-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__ccbuddyLang__ 并派发 ccbuddy-coding-plan-lang-change 事件，
 * 官网侧（ccbuddyBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__ccbuddyLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("ccbuddy-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  // ccbuddyjwttoken 对两个 provider 都加载：它是 ccbuddy-plan 域通用凭证，
  // BigModel OAuth callback 同样落盘（见 resolveBigModelStartPlanCCbuddyJwt）。
  return provider === "zai"
    ? [`oauth:${ZAI_PROVIDER_ID}:access_token`, "ccbuddyjwttoken"]
    : [`oauth:${BIGMODEL_PROVIDER_ID}:access_token`, "ccbuddyjwttoken"];
}
