...
function ActiveWorkspaceFileSearchEditor({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("[WorkspaceFileSearchSection] 读取 .ccbuddyignore 失败", {
  ...
  const applyTransform = useCallback(
    async (transform: "sync-gitignore" | "reset-defaults") => {
      ...
      try {
        ...
        logger.warn("[WorkspaceFileSearchSection] 应用 .ccbuddyignore 分区操作失败", {
          transform,
          error: error instanceof Error ? error.message : String(error),
        });
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("[WorkspaceFileSearchSection] 保存 .ccbuddyignore 失败", {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  // 保存语义是"把编辑框内容落盘"：template 态（.ccbuddyignore 尚未创建）即使未编辑也允许保存，
