...
import { RENDERER_CCBUDDY_ENDPOINT_URLS } from "@/lib/rendererCCbuddyEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 官网页购买成功后的回调。
   *
   * 官网页通过 preload 注入的 window.ccbuddyBridge.notifyPurchaseComplete({ provider })
   * 发送 ccbuddy:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const theme = useCCbuddyStoreWithDefault((state) => state.theme, "zai-dark");
  const userId = useCCbuddyStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const computedWebviewUrl = useMemo(() => {
    ...
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_CCBUDDY_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_CCBUDDY_E2E_STORE_BRIDGE === "1",
        ...
        const credentials: CodingPlanEmbeddedCredentials =
          provider === "zai"
            ? {
                ...
                ccbuddyJwtToken: values[1],
              }
            : {
                bigmodelAccessToken: values[0],
                // BigModel OAuth callback 同样会落盘 ccbuddy JWT；官网用它在
                // ccbuddy-plan 域查 billing/balance 判定 Start Plan 状态。
                ccbuddyJwtToken: values[1],
              };
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // ipc-message: 官网页通过 preload 的 window.ccbuddyBridge.notifyPurchaseComplete
      // 发回购买完成信号（ccbuddy:coding-plan-purchase-complete）。
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:ccbuddy-coding-plan"
            src={webviewUrl}
            className={cn(
