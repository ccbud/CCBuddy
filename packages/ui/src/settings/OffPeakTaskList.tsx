...
interface OffPeakTaskListProps {
  ...
  onOpen: (task: CCbuddyOffPeakTask) => void;
  onPause: (task: CCbuddyOffPeakTask) => void;
  onContinue: (task: CCbuddyOffPeakTask) => void;
  onCancel: (task: CCbuddyOffPeakTask) => void;
  onDelete: (task: CCbuddyOffPeakTask) => void;
  onOpenSession: (task: CCbuddyOffPeakTask) => void;
}

// 按状态分组会让任务在运行和终态切换时跳位，破坏用户对已有卡片位置的预期；
// 列表只按不可变的创建时间倒序，状态变化不再影响顺序。
function sortOffPeakTasksByCreatedAt(tasks: readonly CCbuddyOffPeakTask[]): CCbuddyOffPeakTask[] {
