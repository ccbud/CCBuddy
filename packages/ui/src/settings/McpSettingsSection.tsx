 ...
 * Supports the unified CCbuddy Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToCCbuddyAgentMcpServer,
...
const DEFAULT_MCP_SOURCE: ServerScope = "ccbuddyagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 ccbuddyAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isCCbuddyAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "ccbuddyagentmcp" || !server.enabled) {
      continue;
    }
    ...
    if (!ccbuddyAgentServer) {
      continue;
    }
    seenNames.add(server.name);
    result.push(ccbuddyAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: CCbuddyAgentMcpServer[]): {
  mode: "status";
  mcpServers: CCbuddyAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: CCbuddyMcpServer[]): string {
  return servers
    .filter((server) => server.source === "ccbuddyagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: CCbuddyPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, CCbuddyAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<CCbuddyMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForCCbuddy("ccbuddy");
  ...
  async function handleSave(form: FormState, prev?: CCbuddyMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: CCbuddyMcpServer) {
  ...
  function handleEdit(server: CCbuddyMcpServer) {
  ...
  function handleOpenAuthorization(server: CCbuddyMcpServer) {
