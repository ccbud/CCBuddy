import type { CCbuddyCommand, CCbuddySlashCommand } from "@ccbuddy/shared";
...
function commandToSlashCommand(command: CCbuddyCommand): CCbuddySlashCommand | null {
  if (!command.enabled) {
    return null;
  }
...
export function mergeSlashCommandsAfterCommandRefresh(
  currentSlashCommands: readonly CCbuddySlashCommand[],
  refreshedCommands: readonly CCbuddyCommand[],
): CCbuddySlashCommand[] {
  ...
  const customCommands: CCbuddySlashCommand[] = [];
