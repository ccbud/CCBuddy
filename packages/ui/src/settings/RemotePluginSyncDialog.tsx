...
import type {
  ...
  RemoteTarget,
  CCbuddyPluginOptionValue,
  CCbuddyPluginInfo,
  CCbuddyPluginMarketplaceSummary,
  CCbuddyPluginUserConfigOption,
  CCbuddyPluginsOverviewResult,
} from "@ccbuddy/shared";
import type { IPluginSyncService, ICCbuddyAgentService } from "@ccbuddy/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, CCbuddyPluginOptionValue>;
  userConfig: Record<string, CCbuddyPluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localCCbuddyAgentService?: RemotePluginSyncAgentService | null;
  remoteCCbuddyAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: CCbuddyPluginsOverviewResult,
  plugins: readonly CCbuddyPluginInfo[],
): RemotePluginSyncCandidate[] {
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: CCbuddyPluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: CCbuddyPluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: CCbuddyPluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: CCbuddyPluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localCCbuddyAgentService || !params.remoteCCbuddyAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localCCbuddyAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localCCbuddyAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteCCbuddyAgentService.getPluginsOverview({
...
function isPathPluginOption(option: CCbuddyPluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: CCbuddyPluginOptionValue,
  localOption: CCbuddyPluginUserConfigOption | undefined,
  remoteOption: CCbuddyPluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: CCbuddyPluginInfo,
): { options: Record<string, CCbuddyPluginOptionValue>; skippedCount: number } {
  const options: Record<string, CCbuddyPluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, CCbuddyPluginOptionValue>,
  right: Record<string, CCbuddyPluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteCCbuddyAgentService = params.remoteCCbuddyAgentService;
  if (!remoteCCbuddyAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteCCbuddyAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteCCbuddyAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteCCbuddyAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteCCbuddyAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteCCbuddyAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteCCbuddyAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteCCbuddyAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteCCbuddyAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteCCbuddyAgentService.cancelPluginOperation({ operationId });
          },
