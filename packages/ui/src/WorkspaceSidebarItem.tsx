...
import { invalidateDeferredDraftSessionForSkillChange } from "@/lib/ccbuddyDraftSkillInvalidation.js";
...
const EMPTY_PINNED_TASKS: CCbuddyTaskMeta[] = [];
...
export const WorkspaceSidebarItem = memo(function WorkspaceSidebarItem({
  ...
  const workspaceCCbuddyState = useCCbuddySessionStore((state) =>
    selectWorkspaceCCbuddyState(state, tab.workspacePath, tab.workspaceIdentity),
  );
  const activeTaskId = workspaceCCbuddyState.activeTaskId;
  const removeTaskState = useCCbuddySessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useCCbuddySessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const removeOptimisticTaskListItem = useCCbuddySessionStore(
    (state) => state.removeOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useCCbuddySessionStore((state) => state.setTaskUnreadIndicator);
  ...
  const ccbuddyTaskService = services.ccbuddyTaskService;
  const taskItemsRef = useRef(taskItems);
  taskItemsRef.current = taskItems;
  const workspaceCCbuddyStateRef = useRef(workspaceCCbuddyState);
  workspaceCCbuddyStateRef.current = workspaceCCbuddyState;
  ...
  const handleRemoveWorkspace = useCallback(async () => {
    ...
    if (
      hasRunningWorkspaceChat({
        workspaceState: workspaceCCbuddyStateRef.current,
  ...
  const handleRenameTask = useCallback(
    async (taskId: string, title: string) => {
      ...
      let meta: CCbuddyTaskMeta;
      try {
        meta = await ccbuddyTaskService.renameTask({
          taskId,
          workspacePath: tab.workspacePath,
          title,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleSetTaskPinned = useCallback(
    async (taskId: string, pinned: boolean) => {
      ...
      try {
        const meta = await ccbuddyTaskService.setTaskPinned({
          taskId,
          workspacePath: tab.workspacePath,
          pinned,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleArchiveTask = useCallback(
    async (taskId: string) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await ccbuddyTaskService.archiveTask({
        taskId,
        workspacePath: tab.workspacePath,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
      });
  ...
  const handleSetTaskUnread = useCallback(
    async (taskId: string, unread: boolean) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await ccbuddyTaskService.setTaskUnread({
        taskId,
        workspacePath: tab.workspacePath,
        unread,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  return (
    <li ref={itemRef} style={itemStyle} className="space-y-2">
      ...
      >
        ...
        <CollapsibleContent>
          <TaskList
            ...
            activeTaskId={isActiveWorkspace && showActiveTask ? activeTaskId : null}
      ...
      <RemoteSyncDialogs
        ...
        localCCbuddyAgentService={baseServices.ccbuddyAgentService}
        remoteCCbuddyAgentService={services.ccbuddyAgentService}
        remoteTarget={tab.remoteTarget}
        ...
        onPluginsSynced={async () => {
          await refreshWorkspacePluginCapabilitiesAfterRemoteSync({
            ...
            workspaceIdentity: tab.workspaceIdentity,
            workspacePath: tab.workspacePath,
            ccbuddyAgentService: services.ccbuddyAgentService,
            ccbuddySessionService: services.ccbuddySessionService,
          });
        }}
      />
