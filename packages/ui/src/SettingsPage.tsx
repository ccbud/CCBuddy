...
export function SettingsPage({
  ...
  const { intl, localePreference, setLocalePreference } = useCCbuddyIntl();
  ...
  >([]);
  const interfaceMode = useCCbuddyStore((state) => state.interfaceMode);
  const setInterfaceMode = useCCbuddyStore((state) => state.setInterfaceMode);
  const theme = useCCbuddyStore((state) => state.theme);
  const setTheme = useCCbuddyStore((state) => state.setTheme);
  const codePreviewSettings = useCCbuddyStore((state) => state.codePreviewSettings);
  const setCodePreviewSettings = useCCbuddyStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useCCbuddyStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useCCbuddyStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useCCbuddyStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useCCbuddyStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useCCbuddyStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useCCbuddyStore((state) => state.setNotificationSoundEnabled);
  ...
  // CCbuddy does not mount the inherited account entitlement or pricing hooks.
  const usageZaiEntitlement = { snapshot: null as UsageEntitlementSnapshot | null };
  const usageBigmodelEntitlement = { snapshot: null as UsageEntitlementSnapshot | null };
  ...
  const setNewUserOnboardingOpen = useCCbuddyStore((state) => state.setNewUserOnboardingOpen);
  ...
  const [ccbuddyInteractionBehavior, setCCbuddyInteractionBehavior] =
    useState<CCbuddyInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setCCbuddyInteractionBehavior(settings.ccbuddyInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setCCbuddyInteractionBehavior(sharedSettings.ccbuddyInteractionBehavior ?? "queue");
  ...
  const handleCCbuddyInteractionBehaviorChange = useCallback(
    async (behavior: CCbuddyInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ ccbuddyInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setCCbuddyInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          <GeneralSectionContent
                            ...
                            ccbuddyInteractionBehavior={ccbuddyInteractionBehavior}
                            ...
                            onCCbuddyInteractionBehaviorChange={
                              handleCCbuddyInteractionBehaviorChange
                            }
