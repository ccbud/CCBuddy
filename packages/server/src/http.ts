...
import {
  ...
  CCBUDDY_RPC_HOST_CAPABILITY_HEADER,
  CCBUDDY_VERSION,
  type BotProvider,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("ccbuddy-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    ...
    readTrimmedEnv("CCBUDDY_SERVER_ID") ||
    hostname() ||
    "ccbuddy-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("CCBUDDY_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("CCBUDDY_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("CCBUDDY_SERVER_NAME") }
      : {}),
    version: CCBUDDY_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("CCBUDDY_SERVER_TOKEN")),
...
const ccbuddyLiteTokenCookieName = "ccbuddy_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${ccbuddyLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(ccbuddyLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(CCBUDDY_RPC_HOST_CAPABILITY_HEADER);
  ...
  const handleBotCallback = async (c: Context) => {
    ...
    const webhookSecret = c.req.header("x-ccbuddy-bot-secret");
