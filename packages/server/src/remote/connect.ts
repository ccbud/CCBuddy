import { SocketProtocol, ChannelClient } from "@ccbuddy/rpc";
import type { IServiceAccessor } from "@ccbuddy/services";
import { RemoteServiceAccess } from "@ccbuddy/client";
import {
...
/**
 * Connect to a remote ccbuddy server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 ccbuddy-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'CCBUDDY_SERVER_RUNTIME_ROOT="$HOME/.ccbuddy/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${CCBUDDY_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${CCBUDDY_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${CCBUDDY_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${CCBUDDY_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.ccbuddy/server/node ~/.ccbuddy/server/ccbuddy-server.cjs`;
}
