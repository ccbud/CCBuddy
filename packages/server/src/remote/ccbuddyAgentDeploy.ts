...
import type { IRemoteBackend, RemoteEnvironment } from "@ccbuddy/server/remote/backend.js";
import {
  REMOTE_BASE,
  type DeployLoggers,
...
import type { RemoteAssetInstaller } from "@ccbuddy/server/remote/remoteAssetInstaller.js";
import { buildWriteLiteralFileCommand } from "@ccbuddy/server/remote/posixShell.js";
import { deployDevelopmentCCbuddyAgentRuntime } from "@ccbuddy/server/remote/ccbuddyAgentDevDeploy.js";
import {
...
} from "@ccbuddy/server/remote/remoteAssetLiveIdentity.js";

const REMOTE_AGENT_RUNTIME_BASE = `${REMOTE_BASE}/agents`;

export interface DeployCCbuddyAgentRuntimeOptions extends RemoteAssetDeployOptions {
  platformArch: string;
...
function isSelectedCCbuddyAgentComponent(
  componentId: string,
...
async function shouldSkipCCbuddyAgentDeploy(params: {
  ...
  // wrapper 在、但 ccbuddy.cjs 缺失（被清理 / 旧原生二进制部署残留）时也要重新部署。
  ...
  params.loggers.log(
    `[ccbuddy-agent-deploy] ${CCBUDDY_AGENT_PROVIDER}: 制品 SHA ${params.expectedArtifactSha256} 已部署，跳过`,
  );
  return true;
}
...
/**
 * 部署 CCbuddy Agent runtime 到远程机器。
 *
 * 生产态只用 manifest SHA 判断制品是否变化；语义版本不参与跳过决策。
 */
export async function deployCCbuddyAgentRuntime(
  backend: IRemoteBackend,
  env: RemoteEnvironment,
  options: DeployCCbuddyAgentRuntimeOptions,
  loggers: DeployLoggers,
): Promise<void> {
  const provider = CCBUDDY_AGENT_PROVIDER;
  const runtime = CCBUDDY_AGENT_RUNTIME;
  const componentId = provider;
  if (!isSelectedCCbuddyAgentComponent(componentId, options.selectedResourcePackageIds)) {
    loggers.log(`[ccbuddy-agent-deploy] ${provider}: 未选择资源包 ${componentId}，跳过检查和部署`);
    return;
  }

  // binaryName 指 wrapper 可执行文件名（如 ccbuddy-agent / ccbuddy-agent.exe）——
  // 一个调用远端 node 执行 ccbuddy.cjs 的壳脚本。
  const binaryName = runtime.resolveEntrySegments(env.platform).at(-1);
  if (!binaryName) {
    loggers.logWarn(`[ccbuddy-agent-deploy] ${provider}: 无法解析 agent 入口名称，跳过部署`);
    return;
  }
  ...
  if (
    await deployDevelopmentCCbuddyAgentRuntime(
      backend,
      {
  ...
  try {
    ...
    loggers.logWarn(
      `[ccbuddy-agent-deploy] ${provider}: 读取 manifest SHA 失败，将重新部署: ${String(error)}`,
    );
  }

  if (
    await shouldSkipCCbuddyAgentDeploy({
      backend,
      remoteBinaryPath,
      remoteBundlePath,
      runtimeResourceDir: runtime.bundledResourceDir,
  ...
  loggers.log(`[ccbuddy-agent-deploy] ${provider}: 开始部署 v${runtime.version}...`);
  ...
  if (permissionRepairSucceeded) {
    ...
    await installOfficialPluginPackages();
    // 2) packages 替换成功后再安装编译产物 ccbuddy.cjs（跨平台同一份，glm 组件里就是它）。
    await installBundle();
  }
  // 3) 写入 wrapper（即 resolver 期望的 ccbuddy-agent），用远端已部署的 node 执行 ccbuddy.cjs。
  ...
  loggers.log(`[ccbuddy-agent-deploy] ${provider}: 部署完成 v${runtime.version}`);
}
