import type { HelloMessage, HelloAckMessage } from "@ccbuddy/shared";
import { CCBUDDY_VERSION, formatZodError, helloMessageSchema } from "@ccbuddy/shared";
 ...
 * 1. Read lines from stdout until we find a ccbuddy-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a ccbuddy-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no ccbuddy-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "ccbuddy-hello"
            ) {
              rejectWithDiagnostics(`Invalid ccbuddy-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
