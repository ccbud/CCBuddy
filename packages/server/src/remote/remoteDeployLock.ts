...
import { REMOTE_BASE } from "@ccbuddy/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `ccbuddy-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `ccbuddy-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
