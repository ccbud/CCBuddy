...
import { deployCCbuddyAgentRuntime } from "./ccbuddyAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@ccbuddy/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { quotePosixPathArg } from "@ccbuddy/server/remote/posixShell.js";
import { checkServerBundleRequiredMarkers } from "@ccbuddy/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@ccbuddy/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@ccbuddy/server/remote/ccbuddyAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@ccbuddy/server/remote/remoteAssetCache.js";
import {
...
} from "@ccbuddy/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@ccbuddy/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@ccbuddy/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@ccbuddy/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@ccbuddy/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the ccbuddy server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/ccbuddy-server.cjs 可启动，不代表随包工具仍存在。
      ...
      await deployCCbuddyAgentRuntime(
        backend,
        env,
        {
          ...assetDeployOptions,
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/ccbuddy-server.cjs",
      remotePath: `${REMOTE_BASE}/ccbuddy-server.cjs`,
    ...
    // 部署 CCbuddy Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployCCbuddyAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/ccbuddy-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", CCBUDDY_VERSION);
    if (version !== CCBUDDY_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${CCBUDDY_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", CCBUDDY_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/ccbuddy-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/ccbuddy.cjs`);
