...
const { content: ccbuddyBuiltinProviderConfigJson } = await loadBuiltinProviderConfig();
...
const buildResult = await build({
  ...
  bundle: true,
  outfile: "dist/remote/ccbuddy-server.cjs",
  platform: "node",
  format: "cjs",
  target: "node22",
  ...
  define: {
    ...
    __CCBUDDY_BUILTIN_PROVIDER_CONFIG_JSON__: JSON.stringify(ccbuddyBuiltinProviderConfigJson),
  },
  metafile: true,
});

const remoteBundleSource = readFileSync("dist/remote/ccbuddy-server.cjs", "utf-8");
...
console.log("Built dist/remote/ccbuddy-server.cjs");
