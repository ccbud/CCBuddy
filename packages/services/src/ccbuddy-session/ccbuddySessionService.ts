/* eslint-disable max-lines -- ccbuddySessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { ICCbuddyAgentService } from "#src/ccbuddy-agent/ccbuddyAgent.js";
import type { CCbuddyTaskIndexSyncer } from "#src/ccbuddy-agent/ccbuddyTaskIndexSyncer.js";
...
import type {
  ...
  CCbuddySessionEventsParams,
  CCbuddySessionListParams,
  CCbuddySessionMessagesParams,
  CCbuddySessionReadParams,
  CCbuddySessionReadWorkspacePresentationParams,
  CCbuddySessionResumeParams,
  CCbuddySessionSetModeParams,
  CCbuddySessionSetModelParams,
  CCbuddySessionSetThoughtLevelParams,
  CCbuddyTaskTarget,
  CCbuddySessionWorkspaceTarget,
} from "#src/ccbuddy-session/ccbuddySession.js";
import { formatModelPickerValue } from "#src/ccbuddy-agent/ccbuddyConfigOptions.js";
import { createCCbuddySessionApiRetryRuntimeTracker } from "#src/ccbuddy-session/ccbuddySessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/ccbuddy-session/importedClaudeSessionRepair.js";
import { createCCbuddyDeferredDraftRegistry } from "#src/ccbuddy-session/ccbuddySessionDraftRegistry.js";
...
interface CreateCCbuddySessionServiceOptions {
...
export function createCCbuddySessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateCCbuddySessionServiceOptions): ICCbuddySessionService {
  const { withApiRetryRuntime } = createCCbuddySessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createCCbuddyDeferredDraftRegistry();

  function notifySyncer(
    target: CCbuddyTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: CCbuddySessionStateSnapshot) {
  ...
  // ccbuddySessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[ccbuddy-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: CCbuddySessionStateSnapshot,
    params: CCbuddySessionResumeParams | CCbuddySessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends CCbuddySessionCreateParams | CCbuddySessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: CCbuddySessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: CCbuddySessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: CCbuddySessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: CCbuddySessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[ccbuddy-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[ccbuddy-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 CCbuddy task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[ccbuddy-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: CCbuddySessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[ccbuddy-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[ccbuddy-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[ccbuddy-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: CCbuddySessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: CCbuddySessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[ccbuddy-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: CCbuddySessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: CCbuddySessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: CCbuddyTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[ccbuddy-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: CCbuddyTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: CCbuddyTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[ccbuddy-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: CCbuddySessionSetModelParams) {
    ...
    async setThoughtLevel(params: CCbuddySessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: CCbuddySessionSetModeParams) {
