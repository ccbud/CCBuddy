import {
  ...
  resolveWorkspaceKey,
  type CCbuddySessionApiRetryStatus,
...
export function createCCbuddySessionApiRetryRuntimeTracker(): {
  trackApiRetryFromSessionEvent: (
    params: CCbuddyTaskTarget,
    event: CCbuddySessionServiceEvent,
  ) => CCbuddySessionServiceEvent;
  withApiRetryRuntime: (snapshot: CCbuddySessionStateSnapshot) => CCbuddySessionStateSnapshot;
} {
  const apiRetryBySessionKey = new Map<string, CCbuddySessionApiRetryStatus | null>();
  ...
  function trackApiRetryFromSessionEvent(
    ...
    if (
      ...
      if (apiRetry !== undefined) {
        ...
        isCCbuddyModelRetryRecoveryProgressPayload(asRecord(event.event.payload))
      ) {
  ...
  function withApiRetryRuntime(snapshot: CCbuddySessionStateSnapshot): CCbuddySessionStateSnapshot {
  ...
  function sessionKey(params: CCbuddyTaskTarget): string {
...
): CCbuddySessionApiRetryStatus | null | undefined {
