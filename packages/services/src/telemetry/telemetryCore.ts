...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["ccbuddy:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".ccbuddy", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".ccbuddy", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? CCBUDDY_VERSION;
  ...
  async function sendReport(
    ...
    if (!CCBUDDY_TELEMETRY_ENABLED || !CCBUDDY_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteCCbuddyEndpointUrl(
        CCBUDDY_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveCCbuddyEndpointOrigin?.()) ?? CCBUDDY_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildCCbuddySourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? CCBUDDY_ENV,
