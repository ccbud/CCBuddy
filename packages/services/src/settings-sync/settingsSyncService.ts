...
import { getWorkspaceAppConfigDir, getCCbuddyDataRootDir } from "../paths.js";
...
const CCBUDDY_PLUGIN_MANIFEST_PATH = [".ccbuddy-plugin", "plugin.json"] as const;
...
function getWorkspaceCCbuddySkillRoot(workspacePath: string): string {
  return join(getWorkspaceAppConfigDir(workspacePath), "skills");
}

function getUserCCbuddySkillRoot(): string {
  return join(getCCbuddyDataRootDir(), "skills");
}

function getWorkspaceCCbuddyCommandRoot(workspacePath: string): string {
  return join(getWorkspaceAppConfigDir(workspacePath), "commands");
}

function getUserCCbuddyCommandRoot(): string {
  return join(getCCbuddyDataRootDir(), "commands");
}

function getWorkspaceCCbuddyPluginRoot(workspacePath: string): string {
  return join(getWorkspaceAppConfigDir(workspacePath), "plugins");
}

function getUserCCbuddyPluginRoot(): string {
  return join(getCCbuddyDataRootDir(), "plugins");
}

function getUserCCbuddyCliConfigPath(): string {
  return join(getCCbuddyDataRootDir(), "cli", "config.json");
}

function getWorkspaceCCbuddyConfigPath(workspacePath: string): string {
  return join(getWorkspaceAppConfigDir(workspacePath), "config.json");
}
...
function getUserCCbuddyAgentsFilePath(): string {
  return join(getCCbuddyDataRootDir(), "AGENTS.md");
}
...
function resolveTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserCCbuddySkillRoot();
  }
  return workspacePath ? getWorkspaceCCbuddySkillRoot(workspacePath) : null;
}
...
function resolveCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceCCbuddySkillRoot(workspacePath)
      : null
    : getUserCCbuddySkillRoot();
}
...
function resolveCommandTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserCCbuddyCommandRoot();
  }
  return workspacePath ? getWorkspaceCCbuddyCommandRoot(workspacePath) : null;
}
...
function resolveCommandCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceCCbuddyCommandRoot(workspacePath)
      : null
    : getUserCCbuddyCommandRoot();
}
...
function resolvePluginTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserCCbuddyPluginRoot();
  }
  return workspacePath ? getWorkspaceCCbuddyPluginRoot(workspacePath) : null;
}
...
function resolvePluginCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceCCbuddyPluginRoot(workspacePath)
      : null
    : getUserCCbuddyPluginRoot();
}
...
function resolvePluginConfigPathForScope(
  ...
  if (targetScope === "global") {
    return getUserCCbuddyCliConfigPath();
  }
  return workspacePath ? getWorkspaceCCbuddyConfigPath(workspacePath) : null;
}
...
async function findPluginManifestPath(pluginPath: string): Promise<string | null> {
  const ccbuddyManifestPath = join(pluginPath, ...CCBUDDY_PLUGIN_MANIFEST_PATH);
  if (await pathExists(ccbuddyManifestPath)) {
    return ccbuddyManifestPath;
  }
...
function readCCbuddyMcpServers(parsed: Record<string, unknown>): Record<string, McpServerConfig> {
...
async function collectExistingMcpServerNameKeys(
  ...
  if (targetConfigPath) {
    for (const name of Object.keys(
      readCCbuddyMcpServers(await readJsonFileOrEmpty(targetConfigPath)),
    )) {
...
async function addMcpServerToCCbuddyConfig(
  filePath: string,
  name: string,
  config: McpServerConfig,
): Promise<void> {
  ...
  const servers = readCCbuddyMcpServers(parsed);
...
async function importMcpServersForAgent(
  ...
  for (const candidate of candidates) {
    ...
    try {
      await addMcpServerToCCbuddyConfig(selectedConfigPath, candidate.name, candidate.config);
      importedCount += 1;
...
async function getClaudeAgentsFileMigrationStatus(): Promise<SettingsSyncClaudeAgentsFileMigrationStatus> {
  ...
  const targetPath = getUserCCbuddyAgentsFilePath();
...
async function copyClaudeAgentsFileToCCbuddyAgentsFile(params: {
  overwrite?: boolean;
...
export function createSettingsSyncService(
  ...
  return {
    ...
    async copyClaudeAgentsFileToCCbuddyAgentsFile(
      request = {},
    ): Promise<SettingsSyncClaudeAgentsFileCopyResult> {
      const result = await copyClaudeAgentsFileToCCbuddyAgentsFile({
