...
import { BIGMODEL_PROVIDER_ID, isCredentialDecryptError, ZAI_PROVIDER_ID } from "@ccbuddy/shared";
...
export class OAuthCredentialRepo {
    ...
    try {
      ...
      const ccbuddyJwtToken =
        provider === ZAI_PROVIDER_ID || provider === BIGMODEL_PROVIDER_ID
          ? await this.credentialService.load(CCBUDDY_JWT_TOKEN_KEY)
          : null;
  ...
  async saveTokenSet(provider: OAuthProviderId, tokenSet: OAuthTokenSet): Promise<void> {
    ...
    if (provider === ZAI_PROVIDER_ID || provider === BIGMODEL_PROVIDER_ID) {
      if (tokenSet.ccbuddyJwtToken) {
        // BigModel Start Plan 与 Z.ai Start Plan 一样消费 ccbuddy JWT。
        // JWT 必须在 OAuth callback 阶段随 tokenSet 落盘，后续 balance/runtime 只读取它，
        // 不能再拿 BigModel access token 拼另一个 /oauth/token body 临时兑换。
        await this.credentialService.save(CCBUDDY_JWT_TOKEN_KEY, tokenSet.ccbuddyJwtToken);
      } else {
        await this.credentialService.delete(CCBUDDY_JWT_TOKEN_KEY);
      }
    }
  }
  ...
  async clearProvider(provider: OAuthProviderId): Promise<void> {
    ...
    await this.credentialService.delete(refreshTokenKey(provider));
    await this.credentialService.delete(userInfoKey(provider));
    if (shouldClearCCbuddyJwtOnLogout(provider)) {
      await this.credentialService.delete(CCBUDDY_JWT_TOKEN_KEY);
    }
  }
  ...
  private async clearCorruptOAuthSession(): Promise<void> {
    ...
    // 等价于强制登出已注册 OAuth provider：先清 provider 命名空间与共享 ccbuddy JWT，
...
function shouldClearCCbuddyJwtOnLogout(provider: OAuthProviderId): boolean {
