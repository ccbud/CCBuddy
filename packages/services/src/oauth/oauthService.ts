...
export class OAuthService implements IOAuthService {
  ...
  async restoreCachedSessionState(): Promise<OAuthCachedSessionRestoreResult> {
    ...
    // 启动缓存恢复只需要检查共享 ccbuddy JWT；若通过 loadActiveTokenSet 连带读取
    // provider access token，会把原本后台执行的 BigModel profile 迁移重新阻塞到首屏恢复链路。
    const ccbuddyJwtToken =
      (await this.credentialService.load(CCBUDDY_JWT_TOKEN_KEY))?.trim() ?? "";
    if (ccbuddyJwtToken && resolveJwtExpiration(ccbuddyJwtToken, this.now()).kind === "expired") {
      serviceLog.info("cached session invalidated because ccbuddy JWT expired", {
    ...
    if (activeProvider === ZAI_PROVIDER_ID) {
      if (!ccbuddyJwtToken) {
        // sidebar 登录入口之前只看缓存 user_info，会把“缺少 ccbuddyjwttoken”的状态误判成已登录。
        // 这里补充 ccbuddyjwttoken 门槛，确保没有后端 JWT 时统一按未登录处理。
        log("restoreCachedSession skipped: missing ccbuddyjwttoken:", activeProvider);
        return { status: "signed-out" };
      }
    }
  ...
  private async invalidateExpiredCachedSession(
    ...
    const invalidated = await this.runSessionMutation(async () => {
      ...
      const currentJwt = (await this.credentialService.load(CCBUDDY_JWT_TOKEN_KEY))?.trim() ?? "";
      if (
  ...
  private async persistOAuthSession(
    ...
    if (inactiveProvider) {
      ...
      // 再保存当前 token；反序会让 clearProvider 误删共享的 ccbuddyjwttoken。
  ...
  async startOAuthWithPolling(provider: OAuthProviderId): Promise<OAuthStartResponse> {
    ...
    const initUrl = buildCCbuddyApiUrlFromEnv(this.env, "/api/v1/oauth/cli/init");
    ...
    if (provider === BIGMODEL_PROVIDER_ID) {
      ...
      // flow 仍由 Host 轮询，但浏览器回调恢复到官网中转页，再透传到 ccbuddy://oauth/callback。
      ...
      // Desktop 统一改写为官网中转页，再由官网透传到 ccbuddy://oauth/callback。
    ...
    this.pendingState = {
      ...
      polling: {
        ...
        pollIntervalMs,
        pollToken,
        pollUrl: buildCCbuddyApiUrlFromEnv(
          this.env,
  ...
  async pollPendingOAuth(): Promise<OAuthCallbackResult | null> {
    ...
    try {
      const result = await this.runPendingSessionCompletion(pending, async () => {
        ...
        const ccbuddyJwtToken = readTrimmedString(ready.token);
        const userId = readTrimmedString(user?.user_id);
        if (!ccbuddyJwtToken || !providerAccessToken || !userId) {
          throw new Error("OAuth flow 查询响应无效");
        }
