...
const DESKTOP_OAUTH_CALLBACK_URI = "ccbuddy://oauth/callback";
...
export function buildCCbuddyApiUrlFromEnv(env: NodeJS.ProcessEnv, path: string): string {
  // OAuth provider 是运行时配置，必须跟随传入 env.CCBUDDY_ENV；
  // 地址来自 .env 的通用变量，默认线上；登录与 token 交换必须使用同一配置来源。
  return buildRuntimeCCbuddyApiUrl(env, path);
}

export function buildDesktopOAuthRedirectUriFromEnv(env: NodeJS.ProcessEnv): string {
  const url = new URL("/app/oauth/login", buildRuntimeCCbuddyEndpointUrls(env).origin);
