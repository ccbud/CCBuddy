import { BIGMODEL_PROVIDER_ID, buildBigModelApiUrl } from "@ccbuddy/shared";
...
const BIGMODEL_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  tokenUrl: "https://ccbuddy.z.ai/api/v1/oauth/token",
  userinfoUrl: buildBigModelApiUrl({ CCBUDDY_ENV: "production" }, BIGMODEL_USERINFO_PATH),
  appId: "ccbuddy",
  redirectUri: "ccbuddy://oauth/callback",
};
...
export function createBigModelProviderRuntimeConfig(
  ...
  return {
    ...
    tokenUrl:
      ...
      buildCCbuddyApiUrlFromEnv(env, "/api/v1/oauth/token"),
    ...
    // 当前 BigModel callback 只消费 ccbuddy OAuth token 路由，显式 appSecret 仅保留给
