...
function getCachedProfileSchemaVersion(profile: OAuthUserProfile): number | null {
  ...
  const version = (rawProfile as { ccbuddyProfileSchemaVersion?: unknown })
    .ccbuddyProfileSchemaVersion;
...
function getCachedProfileMigrationRetryAfter(profile: OAuthUserProfile): number | null {
  ...
  const retryAfter = (rawProfile as { ccbuddyProfileMigrationRetryAfter?: unknown })
    .ccbuddyProfileMigrationRetryAfter;
...
export function withProviderProfileSchema(
  ...
  delete nextRawProfile.ccbuddyProfileMigrationRetryAfter;

  return {
    ...profile,
    rawProfile: {
      ...nextRawProfile,
      ccbuddyProfileSchemaVersion: BIGMODEL_PROFILE_SCHEMA_VERSION,
    },
  };
}
...
function withBigModelProfileMigrationRetryAfter(
  ...
  return {
    ...
    rawProfile: {
      ...
      ccbuddyProfileMigrationRetryAfter: now + BIGMODEL_PROFILE_MIGRATION_RETRY_DELAY_MS,
    },
  };
}
...
export async function refreshLegacyBigModelCachedProfile(
  ...
  try {
    ...
    if (isBigModelUserInfoFallback(refreshedProfile)) {
      // 旧版本可能把 ccbuddy JWT 写进 BigModel access token。
