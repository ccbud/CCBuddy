...
const CCBUDDY_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "ccbuddy",
  directorySource: "ccbuddy",
  userConfigDirSegments: [],
  workspaceConfigDirSegments: [],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserCCbuddyMcpConfigPath()));
    },
...
function buildDirectoryConfigPath(
  ...
  if (descriptor.directorySource === "ccbuddy") {
    if (scope === "user") {
      return join(getCCbuddyDataRootDir(), "cli", descriptor.fileName);
    }
    if (!workspacePath) {
      throw new Error("Missing workspace path for ccbuddy workspace MCP config");
    }
    return join(getWorkspaceAppConfigDir(workspacePath), descriptor.fileName);
  }
...
function getUserCCbuddyMcpConfigPath(): string {
  return buildUserConfigPath(CCBUDDY_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const ccbuddyRecords = await readUserMcpRecordsFromFile(CCBUDDY_MCP_DESCRIPTOR);
  if (ccbuddyRecords.length > 0) {
    return sortMcpRecords(ccbuddyRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(CCBUDDY_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeCCbuddyServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const ccbuddyServers = await readDirectoryServersFromFile(
    CCBUDDY_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (ccbuddyServers.length > 0) {
    return ccbuddyServers;
  }
...
async function writeCCbuddyServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(CCBUDDY_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, CCBUDDY_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserCCbuddyMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserCCbuddyMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserCCbuddyMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, CCBUDDY_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, CCBUDDY_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
