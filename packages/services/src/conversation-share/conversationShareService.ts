...
import type { ConversationRow } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import {
  PROTOCOL_V4_LIMITS,
  CCBUDDY_ATTACHMENT_FAULT_CODES,
  readCCbuddyAttachmentFaultCode,
} from "@ccbuddy/shared/ccbuddy-protocol-v4";
import { Emitter } from "@ccbuddy/rpc";

import type { ICCbuddyAgentService } from "../ccbuddy-agent/ccbuddyAgent.js";
import type { ICCbuddySessionService } from "#src/ccbuddy-session/ccbuddySession.js";
...
interface ConversationShareServiceOptions {
  ccbuddyAgentService: ConversationShareAgentService;
  ...
  ccbuddySessionService?: Pick<ICCbuddySessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^ccbuddy-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 ccbuddy-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readCCbuddyAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readCCbuddyAttachmentFaultCode(error);
  return (
    faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === CCBUDDY_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly ccbuddyAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly ccbuddySessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.ccbuddyAgentService = options.ccbuddyAgentService;
    ...
    this.ccbuddySessionService = options.ccbuddySessionService;
    ...
    // 兜底写死生产站 https://ccbuddy.z.ai/cn/share，于是测试环境（API base 走
    // 配置的 CCbuddy origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeCCbuddyApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > CCBUDDY_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.CCBUDDY_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeCCbuddyEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".ccbuddy-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.ccbuddyAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".ccbuddy-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".ccbuddy-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.ccbuddySessionService.listSessions({
          workspacePath,
          limit: 100,
        });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.ccbuddy-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.ccbuddySessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .ccbuddy-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".ccbuddy-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.ccbuddyAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.ccbuddyAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      ccbuddyAgentService: agentService,
