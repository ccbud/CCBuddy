...
import { redactFeedbackText, CCBUDDY_VERSION, CCBUDDY_COMMIT } from "@ccbuddy/shared";
...
export async function createFeedbackDiagnosticArchive(options: {
  ...
  const path = join(outputDir, "ccbuddy-diagnostic-logs.zip");
  ...
  try {
    ...
    zip.addBuffer(
      Buffer.from(
        [
          "CCbuddy diagnostic logs",
          `timestamp: ${now.toISOString()}`,
          `appVersion: ${CCBUDDY_VERSION}`,
          `commit: ${CCBUDDY_COMMIT}`,
