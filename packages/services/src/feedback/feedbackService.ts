...
import type { ApiClient, FeedbackDeviceInfo } from "@ccbuddy/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.CCBUDDY_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeCCbuddyApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: CCBUDDY_COMMIT,
    buildTime: CCBUDDY_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getCCbuddyJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(CCBUDDY_JWT_TOKEN_KEY))?.trim() || undefined;
  }

  async function hasCCbuddyJwtToken(): Promise<boolean> {
    return Boolean(await getCCbuddyJwtToken());
  }
  ...
  const httpClient = new FeedbackHttpClient({
    ...
    getAuthHeaders: async () => {
      ...
      const jwtToken = await getCCbuddyJwtToken();
      if (jwtToken) {
