import { redactFeedbackText } from "@ccbuddy/shared";
...
function toFeedbackEnvironment(input: CreateFeedbackTicketInput): Record<string, unknown> {
  ...
  assignDefined(environment, "release_channel", process.env.CCBUDDY_ENV?.trim() || "stable");
...
async function uploadOssForm(
  ...
  return new Promise((resolve, reject) => {
    ...
    const boundary = `----ccbuddy-feedback-${randomUUID()}`;
