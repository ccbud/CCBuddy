...
async function readAll(credentialsFile = getCredentialsFile()): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read CCbuddy credentials: ${credentialsFile}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`CCbuddy credentials are corrupt: ${credentialsFile}`, { cause: error });
  }
}
