...
import { parseWorkspaceHookTrustStoreContent } from "@ccbuddy/shared/workspace-hook-trust-store-file";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "ccbuddy") {
    return workspacePath
      ? getWorkspaceAppConfigDir(workspacePath)
      : join(getCCbuddyDataRootDir(), "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "ccbuddy" ? CCBUDDY_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserCCbuddySource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("ccbuddy");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("ccbuddy"))) ?? {};
...
async function loadHooksImpl(
  ...
  const projectConfigPath = getConfigPath("ccbuddy", workspacePath);
  const [projectConfig, userSource] = await Promise.all([
    readJsonFile<CCbuddyConfigFile>(projectConfigPath),
    readUserCCbuddySource(),
  ]);
  const projectHooks = workspaceHooksConfigSchema.safeParse(projectConfig?.hooks);
  const projectSources: WorkspaceHookSourceInput[] = projectHooks.success
    ? [
        {
          ...createWorkspaceHookSourceInput({
  ...
  const hooks = [
    ...
    ...fromUserCCbuddySource({
      ...
      runtimeRoot,
      workspacePath,
      location: buildLocation("ccbuddy"),
    }),
...
async function writeCCbuddyHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("ccbuddy", workspacePath);
  const existingConfig = (await readJsonFile<CCbuddyConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toCCbuddyHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = getConfigPath("ccbuddy", params.workspacePath);
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "ccbuddy" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "ccbuddy" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeCCbuddyHooksConfig(undefined, userHooks);
  await writeCCbuddyHooksConfig(params.workspacePath, projectHooks);
}
