...
export interface ProviderConfigRuntimeOptions {
  ...
  readonly ccbuddyBuiltinRemote?: NodeProviderConfigRuntimeOptions["ccbuddyBuiltinRemote"];
  readonly ccbuddyBuiltinEnvironment?: NodeProviderConfigRuntimeOptions["ccbuddyBuiltinEnvironment"];
 ...
 * 配置迁移保留 CCbuddy 用户的供应商数据，文件运行时由 @ccbuddy/provider-node 唯一实现。
 */
export class ProviderConfigRuntime {
  ...
  constructor(options: ProviderConfigRuntimeOptions) {
    const runtimeOptions: NodeProviderConfigRuntimeOptions = {
      ccbuddyBuiltinFilePath: options.ccbuddyBuiltinFilePath,
      ccbuddyBuiltinActiveFilePath: options.ccbuddyBuiltinActiveFilePath,
      ccbuddyBuiltinRemote: options.ccbuddyBuiltinRemote,
      ccbuddyBuiltinEnvironment: options.ccbuddyBuiltinEnvironment,
      onCCbuddyBuiltinRefreshError: options.onCCbuddyBuiltinRefreshError,
  ...
  resolveCCbuddyBuiltinActiveFilePath(): Promise<string> {
    return this.#runtime.resolveCCbuddyBuiltinActiveFilePath();
  }

  refreshCCbuddyBuiltin(options?: { readonly force?: boolean }) {
    return this.#runtime.refreshCCbuddyBuiltin(options);
  }

  onDidCheckCCbuddyBuiltin(listener: () => Promise<void>): () => void {
    return this.#runtime.onDidCheckCCbuddyBuiltin(listener);
  }
