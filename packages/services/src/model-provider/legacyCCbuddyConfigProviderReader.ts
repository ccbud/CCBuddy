/* oxlint-disable eslint(max-lines) -- 已发布旧 CCbuddy config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  const fallbackBaseUrl =
    resolveRuntimeCCbuddyEnv(env) === "production"
  ...
  if (resolveRuntimeCCbuddyEnv(env) === "production") {
...
function getCCbuddyConfigFilePath(): string {
...
interface CCbuddyConfigFile {
  $schema?: string;
  provider?: Record<string, CCbuddyOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface CCbuddyOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, CCbuddyOpenCodeModelConfig>;
  ccbuddy?: CCbuddyProviderConfigExtension;
  [key: string]: unknown;
}

interface CCbuddyOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | CCbuddyReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  ccbuddy?: CCbuddyModelConfigExtension;
  [key: string]: unknown;
}

interface CCbuddyReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface CCbuddyProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface CCbuddyModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const ccbuddyModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const ccbuddyProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const ccbuddyReasoningConfigExtensionSchema = z
  .object({
...
const ccbuddyModelConfigExtensionSchema = z
  .object({
...
const ccbuddyOpenCodeModelConfigSchema: z.ZodType<CCbuddyOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), ccbuddyReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(ccbuddyModelProviderModalitySchema).optional(),
        output: z.array(ccbuddyModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    ccbuddy: ccbuddyModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const ccbuddyProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: ccbuddyProviderModelMappingsSchema.optional(),
...
const ccbuddyOpenCodeProviderConfigSchema: z.ZodType<CCbuddyOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), ccbuddyOpenCodeModelConfigSchema).optional(),
    ccbuddy: ccbuddyProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const ccbuddyConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), ccbuddyOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: CCbuddyOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: CCbuddyOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/ccbuddy 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolvePresetProviderKindFromRuntimeBaseUrl(
  ...
  if (path.includes("/api/anthropic") || path.includes("/ccbuddy-plan/anthropic")) {
    return "anthropic";
  }
  return undefined;
}
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.ccbuddy?.endpoints ?? {};
}

function stripCCbuddyPrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("ccbuddy"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: CCbuddyOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.ccbuddy?.reasoning) {
    return stripModelProviderReasoningPatches(model.ccbuddy.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const ccbuddy = options.preferOpenCodeFields ? undefined : model.ccbuddy;
  const ccbuddyExtensions = model.ccbuddy;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(ccbuddyExtensions?.priority),
    disabledReason: model.disabledReason ?? ccbuddy?.disabledReason,
    supportsTools: model.supportsTools ?? ccbuddy?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? ccbuddy?.supportsStructuredOutput,
    modified: model.modified ?? ccbuddyExtensions?.modified,
    deleted: model.deleted ?? ccbuddyExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  const ccbuddy = provider.ccbuddy;
  ...
  for (const deletedModelId of normalizeDeletedModelIds(ccbuddy?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? ccbuddy?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripCCbuddyPrefixedProviderMappings(provider.providerMappings ?? ccbuddy?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : ccbuddy?.enabled !== undefined
        ? { enabled: ccbuddy.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? ccbuddy?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? ccbuddy?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ...
      : (provider.apiFormat ??
        ccbuddy?.apiFormat ??
        resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? ccbuddy?.source)
      ? { source: provider.source ?? ccbuddy?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? ccbuddy?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? ccbuddy?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? ccbuddy?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? ccbuddy?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? ccbuddy?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? ccbuddy?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? ccbuddy?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? ccbuddy?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? ccbuddy?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? ccbuddy?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? ccbuddy?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? ccbuddy?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? ccbuddy?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? ccbuddy?.updatedAt) : undefined) ?? now,
  });
}

async function readRawCCbuddyConfigFile(): Promise<CCbuddyConfigFile | null> {
  const filePath = getCCbuddyConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = ccbuddyConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readCCbuddyConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawCCbuddyConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripCCbuddyPrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 CCbuddy Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyCCbuddyConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readCCbuddyConfigProviders();
