...
export interface AccountRequestAuthInput {
  providerId: string;
  modelId?: string;
  accountAccess: CCbuddyProviderAccountAccess | CCbuddyAccountAccess;
...
export interface AccountRequestAuthResolver {
  resolveAccessCurrent(access: CCbuddyProviderAccountAccess): Promise<CCbuddyAccountAccess | null>;
...
interface AccountProviderRequestAuthServiceOptions {
  ...
  resolveTeamPlanApiKey(
    access: Extract<CCbuddyAccountAccess, { planKind: "team-coding-plan" }>,
  ): Promise<string | null>;
}
...
class AccountProviderRequestAuthService implements AccountRequestAuthResolver {
  ...
  async resolveCurrent(input: AccountRequestAuthInput): Promise<AccountRequestAuthMaterial> {
    ...
    if (access.planKind === "start-plan") {
      ...
      return { apiKey: requireApiKey(tokenSet?.ccbuddyJwtToken, providerId) };
    }
  ...
  #resolveAccess(
    access: CCbuddyProviderAccountAccess | CCbuddyAccountAccess,
  ): Promise<CCbuddyAccountAccess | null> {
