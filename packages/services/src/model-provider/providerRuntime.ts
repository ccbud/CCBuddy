...
export class EmptyAccountProviderConfigSource implements ProviderSource<AccountProviderConfigSnapshot> {
  ...
  /** CCbuddy has no account-backed model service; refresh remains a local read. */
  refresh(_reason = "explicit"): Promise<AccountProviderConfigSnapshot> {
    return this.read();
  }

  onDidRefreshError(
    _listener: (event: { readonly error: unknown; readonly reasons: readonly string[] }) => void,
  ): () => void {
    return () => {};
  }
...
export class ProviderRuntime {
  ...
  constructor(dependencies: ProviderRuntimeDependencies) {
    ...
    this.#disposeBuiltinRecovery = this.#configRuntime.onDidCheckCCbuddyBuiltin(async () => {
      ...
      if (
        !this.#disposed &&
        config.ccbuddyBuiltinRevision !== account.basedOnCCbuddyBuiltinRevision
      ) {
  ...
  return {
    ...
    refreshSources: async (reason) => {
      const sourceResults = await Promise.allSettled([
        configRuntime.refreshCCbuddyBuiltin({ force: true }),
