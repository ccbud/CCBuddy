import { BIGMODEL_PROVIDER_ID, resolveBigModelApiOrigin, ZAI_PROVIDER_ID } from "@ccbuddy/shared";
import { ZAI_API_HOST } from "../providers/api/apiEndpoints.js";
import {
  DEFAULT_ORG_NAME,
  DEFAULT_PROJECT_NAME,
  CCBUDDY_API_KEY_NAME,
...
export class AccountProviderApiKeyResolver {
  ...
  private async resolveBizApiKey(
    ...
    let apiKeyEntry = apiKeys.find((item) => item.name === CCBUDDY_API_KEY_NAME) ?? null;

    if (!apiKeyEntry) {
      apiKeyEntry = await this.fetchRemoteData<RemoteApiKeySummary>(listUrl, {
        method: "POST",
        headers: createBizAuthHeaders(authorization),
        body: JSON.stringify({ name: CCBUDDY_API_KEY_NAME }),
      });
    }
