...
export function resolveUserAppRoot(options?: SubagentStorageOptions): string {
  return options?.homeDir?.trim()
    ? join(resolveUserHomeDir(options), ".ccbuddy")
    : getCCbuddyDataRootDir();
}

export async function resolveUserSubagentRoot(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveCCbuddyStorageRoot(options), "agents");
}

export function resolveWorkspaceSubagentRoot(workspacePath: string): string {
  return join(getWorkspaceAppConfigDir(workspacePath), "agents");
}

export async function resolveSubagentStateFile(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveCCbuddyStorageRoot(options), "v2", "agents-state.json");
}

export async function resolveCCbuddyStorageRoot(options?: SubagentStorageOptions): Promise<string> {
...
async function readUserCliConfig(
  ...
  try {
    const raw = await readFile(join(resolveUserAppRoot(options), "cli", "config.json"), "utf8");
