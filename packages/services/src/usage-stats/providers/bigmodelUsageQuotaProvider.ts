...
import type { ProviderFamilyDomain } from "@ccbuddy/shared";
...
} from "./ccbuddyMcpQuotaProvider.js";
...
export class BigModelUsageQuotaProvider {
  ...
  ): Promise<CCbuddyAccountAccess | undefined> {
  ...
  private async getStartPlanSnapshot(
    providerId: string,
    accountAccess: CCbuddyAccountAccess | undefined,
  ...
  async getCodingPlanResetStatus(
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeCCbuddyApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/status`),
      {
        method: "GET",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  async useCodingPlanReset(request: CodingPlanResetUseRequest): Promise<CodingPlanResetUseResult> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeCCbuddyApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/use`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
        headers: {
  ...
  async requestCodingPlanResetOpportunity(
    ...
    try {
      payload = await readCodingPlanResetApiJson(
        this.apiClient,
        buildRuntimeCCbuddyApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/opportunity`),
        {
          method: "POST",
          timeoutMs: REQUEST_TIMEOUT_MS,
          headers: {
  ...
  async markCodingPlanResetHistoryRead(request: CodingPlanResetScopeRequest): Promise<void> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeCCbuddyApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/history/read`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  private async resolveCodingPlanResetAuthorization(
    ...
    const ccbuddyJwt = (await this.credentialService?.load(CCBUDDY_JWT_TOKEN_KEY))?.trim() ?? "";
    if (!ccbuddyJwt) {
      throw new Error("coding_plan_reset_ccbuddy_jwt_required");
    }
    ...
    return {
      ccbuddyAuthorization: /^Bearer\s/i.test(ccbuddyJwt) ? ccbuddyJwt : `Bearer ${ccbuddyJwt}`,
  ...
  private async resolveAuthorization(
    ...
    if (request.allowEnvApiKey !== false && request.requirePreferredProvider !== true) {
      const envApiKey =
        readEnv(this.env, "CCBUDDY_BIGMODEL_USAGE_API_KEY") ??
...
function createCodingPlanResetHeaders(
  ...
  const headers: Record<string, string> = {
    Authorization: authorization.ccbuddyAuthorization,
...
function resolveTeamPlanContext(accountAccess: CCbuddyAccountAccess): TeamPlanContext | null {
...
function resolveQuotaUrlFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    readEnv(env, "CCBUDDY_BIGMODEL_USAGE_QUOTA_URL") ?? readEnv(env, "BIGMODEL_USAGE_QUOTA_URL")
  );
}
