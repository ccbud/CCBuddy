...
import { isCodingPlanModelProviderId } from "@ccbuddy/shared";
...
import type { OfficialMcpCredentialSource } from "./providers/ccbuddyMcpQuotaProvider.js";
...
interface UsageStatsServiceDependencies {
  ...
  /** App Usage 经 CCbuddy Protocol 读取 agent 数据库真实统计。 */
  ccbuddyAgentService: Pick<ICCbuddyAgentService, "getAppUsageStats">;
  /**
...
export function createUsageStatsService(
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 CCbuddy Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.ccbuddyAgentService.getAppUsageStats({
        range: request.range,
