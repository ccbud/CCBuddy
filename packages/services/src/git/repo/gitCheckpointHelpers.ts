...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@ccbuddy/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/ccbuddy/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "CCbuddy Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@ccbuddy.local",
    GIT_COMMITTER_NAME: "CCbuddy Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@ccbuddy.local",
  };
}
