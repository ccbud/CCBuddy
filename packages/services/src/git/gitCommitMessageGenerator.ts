...
interface GitCommitMessageCurrentModelProvider {
  ...
  }): Promise<CCbuddyWorkspaceGenerateTextParams["selection"] | null>;
}
...
interface GitCommitMessageTextGenerator {
  generateText(params: {
    ...
    prompt: string;
    querySource: string;
  }): Promise<{ text: string; selection: CCbuddyWorkspaceGenerateTextParams["selection"] }>;
}
...
export class GitCommitMessageGenerator {
  ...
  private async resolveCurrentModel(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<CCbuddyWorkspaceGenerateTextParams["selection"]> {
