...
// Conversation share 的具体实现依赖 Node 文件系统，只能从 @ccbuddy/services/node 引入；
...
// 若 value 导出 createOnboardingRecordService，会连带 fs/atomicFileUtils → @ccbuddy/shared/node →
...
// CCbuddy task wrapper service — task 列表/置顶/归档等 app 侧包装状态入口。
export { ICCbuddyTaskService } from "./session/ccbuddyTaskService.js";
export type {
  CCbuddyArchivedTaskDeletionResult,
  CCbuddyModelTrajectory,
  CCbuddyModelTrajectoryCallSource,
  CCbuddyModelTrajectoryCallSourceKind,
  CCbuddyModelTrajectoryContentPart,
  CCbuddyModelTrajectoryMessage,
  CCbuddyModelTrajectoryRecord,
  CCbuddyModelTrajectoryUsage,
  CCbuddyTaskListKind,
...
export type { CCbuddyTaskListItem } from "./session/ccbuddyTaskListTypes.js";
...
// CCbuddy agent service — ICCbuddyAgentService is both a type (interface) and value (descriptor)
export {
  ICCbuddyAgentService,
  type CCbuddyAgentLocalRuntimeChildProcesses,
  CCBUDDY_AGENT_RUNTIME_UNAVAILABLE_CODE,
} from "./ccbuddy-agent/ccbuddyAgent.js";
export {
  isCCbuddyAgentMcpStatusModeUnsupportedError,
  CCBUDDY_AGENT_MCP_STATUS_MODE_UNSUPPORTED_ERROR_CODE,
  CCbuddyAgentMcpStatusModeUnsupportedError,
} from "./ccbuddy-agent/ccbuddyAgentErrors.js";
export {
  createCCbuddyAgentConnectionScope,
  readTrustedCCbuddyAgentV4Connection,
} from "./ccbuddy-agent/ccbuddyAgentConnectionScope.js";
export type {
  CCbuddyAgentConnectionScope,
  CCbuddyAgentV4ClientMode,
...
export type {
  ...
  CCbuddyAgentStorageStartupSnapshot,
  CCbuddyAgentRuntimeLifecycleEvent,
  CCbuddyAgentRuntimePolicy,
  CCbuddyAgentReadSessionParams,
  CCbuddyAgentResumeSessionParams,
  CCbuddyAgentRunAutomationNowResult,
...
// Storage（资源管理器「存储」tab）：数据类型在 @ccbuddy/shared；这里只导出服务接口与卷分组纯函数
...
// 设置页插件管理薄服务（UI 平台能力面不再直触 ccbuddyAgentService）
