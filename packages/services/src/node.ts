...
import { getAppConfigDir as resolveAppConfigDir, getCCbuddyDataRootDir } from "./paths.js";
import {
...
export {
  ...
  CCBUDDY_E2E_FS_FAULTS_ALLOW_ENV,
  CCBUDDY_E2E_FS_FAULTS_ENV,
...
export {
  ...
  CCBUDDY_WINDOWS_APP_INSTALL_DIR_ENV,
} from "./paths.js";
...
export { createCCbuddyAgentService } from "./ccbuddy-agent/ccbuddyAgentService.js";
export { createCCbuddyTaskServiceAdapter } from "./ccbuddy-agent/ccbuddyTaskServiceAdapter.js";
export { createCCbuddySessionService } from "./ccbuddy-session/ccbuddySessionService.js";
export {
  resolveDefaultCCbuddyAgentCommand,
  CCbuddyAgentProcessManager,
} from "./ccbuddy-agent/ccbuddyAgentProcessManager.js";
export type {
  CCbuddyAgentCommand,
  CCbuddyAgentCommandResolver,
  CCbuddyAgentCommandResolverContext,
  CCbuddyAgentProcessManagerOptions,
} from "./ccbuddy-agent/ccbuddyAgentProcessManager.js";
export { CCbuddyProtocolClient } from "./ccbuddy-agent/ccbuddyProtocolClient.js";
export type { CCbuddyProtocolTransport } from "./ccbuddy-agent/ccbuddyProtocolTransport.js";
export { CCbuddyStdioTransport } from "./ccbuddy-agent/ccbuddyStdioTransport.js";
export {
  getCCbuddyStdioTapDevLogDir,
  readCCbuddyStdioTapDevState,
  setCCbuddyStdioTapDevEnabled,
} from "./ccbuddy-agent/ccbuddyStdioTapDevConfig.js";
export type { CCbuddyStdioTapDevState } from "@ccbuddy/shared";
export {
...
export { createDisabledOAuthService } from "./oauth/disabledOAuthService.js";
...
export { createLocalOnlyApiClient } from "./providers/api/localOnlyApiClient.js";
export {
...
import { ICCbuddyTaskService } from "./session/ccbuddyTaskService.js";
import { ICCbuddyAgentService } from "./ccbuddy-agent/ccbuddyAgent.js";
import type { CuaOperationStateReporter } from "./ccbuddy-agent/cuaOperationTurnTracker.js";
import { ICCbuddySessionService } from "./ccbuddy-session/ccbuddySession.js";
import {
...
import { createCCbuddyAgentService } from "./ccbuddy-agent/ccbuddyAgentService.js";
import type { CCbuddyAgentCommandResolver } from "./ccbuddy-agent/ccbuddyAgentProcessManager.js";
import { buildAgentTelemetrySpawnEnv } from "./ccbuddy-agent/agentTelemetryEnv.js";
import { resolveCCbuddyAgentPresentationSurface } from "./ccbuddy-agent/ccbuddyAgentPresentationSurface.js";
import { createCCbuddyTaskServiceAdapter } from "./ccbuddy-agent/ccbuddyTaskServiceAdapter.js";
import { createCCbuddySessionService } from "./ccbuddy-session/ccbuddySessionService.js";
import { createCCbuddyTaskIndexSyncer } from "./ccbuddy-agent/ccbuddyTaskIndexSyncer.js";
...
import { createDisabledOAuthService } from "./oauth/disabledOAuthService.js";
...
import { createLocalOnlyApiClient } from "./providers/api/localOnlyApiClient.js";
import {
  createHostApiNetworkTransport,
  type HostApiNetworkTransport,
} from "./providers/api/nodeApiNetwork.js";
...
} from "@ccbuddy/ccbuddy-cua/broker/server";
...
import {
  ...
  isPotentialCCbuddyCuaAgentMcpServer,
...
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@ccbuddy/ccbuddy-cua/broker/helperConstants";
import { resolveBrokerSocketPath } from "@ccbuddy/ccbuddy-cua/broker/socketPath";
import {
  ...
  type BrowserCommand,
  isCCbuddyCuaMcpCommand,
  isCCbuddyCuaMcpPackageArg,
  isCCbuddyCuaInternalFeatureEnabled,
  CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  type CCbuddyAutomation,
  type CCbuddyAutomationRun,
  getCapturedCCbuddyAgentTelemetryEnv,
  CCBUDDY_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV,
  ZAI_PROVIDER_ID,
  ccbuddyAccountAccessSchema,
  ccbuddyProviderAccountAccessSchema,
  CCBUDDY_VERSION,
} from "@ccbuddy/shared";

// 这些 conversation-share 实现依赖 Node 文件系统；仅通过 @ccbuddy/services/node 暴露，
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isCCbuddyCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isCCbuddyCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialCCbuddyCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error("CCbuddy Computer Use is not enabled (plugin off or not product mode).");
      }
...
export const CCBUDDY_CUA_BUNDLED_HELPER_APP_PATH_ENV = "CCBUDDY_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[CCBUDDY_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliCCbuddyCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const appRoot =
    env === process.env
      ? getCCbuddyDataRootDir()
      : join(env.CCBUDDY_DATA_BASE_DIR?.trim() || env.HOME?.trim() || homedir(), ".ccbuddy");
  const configPath = join(appRoot, "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliCCbuddyCuaServer(name, config),
  );
}

function isGlobalCliCCbuddyCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @ccbuddy/shared 的单一事实源，避免第三处
  // 判定漂移：git/.git/本地路径形态的 ccbuddy-cua 若这里漏判，全局 CLI env 注入不会带 broker
  // socket/token，agent 会回退成 Python/uvx 自己持有 macOS TCC（违反 product broker 边界）。
  if (typeof config.command === "string" && isCCbuddyCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isCCbuddyCuaMcpPackageArg(arg))
  );
}
...
export async function buildCuaProductHelperAgentEnv(
  ...
  //
  // fail-closed 理由：CLI 全局配置里存在 ccbuddy-cua 时，返回空 env 会让 agent 按原始
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
    ...
    logger.warn(
      undefined,
      `Computer Use Helper broker_unavailable; disabling workspace ccbuddy-cua MCP server for this agent spawn (${cuaHelperStartErrorDetail(error)})`,
    );
    return {
...
export function createLocalServices(options: {
  ...
  ccbuddyAgentCommandResolver?: CCbuddyAgentCommandResolver;
  ...
  ccbuddyAgentSpawnFallbackCwd?: string;
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 CCbuddy Built-in Provider Config 物理路径。 */
  ccbuddyBuiltinProviderConfigFilePath: string;
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 CCbuddy Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 CCbuddy Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[CCBUDDY_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  const resolveCurrentCCbuddyEndpointOrigin = async () =>
  ...
  // 继承的账号、订阅和公开配置服务共享这一出口。CCbuddy 不提供云账号，
  // 在 DI 边界拒绝所有请求，不能依赖 UI 隐藏来保证旧 RPC 不发送网络流量。
  const apiClient = createLocalOnlyApiClient();
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    ccbuddyBuiltinFilePath: options.ccbuddyBuiltinProviderConfigFilePath,
    ccbuddyBuiltinActiveFilePath: join(resolveAppConfigDir(), "provider", "builtin.json"),
  ...
  // Bundled templates require user credentials. No account overlay may add a
  // cloud model or trigger subscription queries when the app starts.
  ...
  let providerConnectivityAgentService:
    | Pick<ICCbuddyAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 ccbuddyAgentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => ccbuddyAgentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 ccbuddyAgentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 ccbuddyAgentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（CCBUDDY_CUA_DEV_MODE=1 或
    // CCBUDDY_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isCCbuddyCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isCCbuddyCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，ccbuddyAgentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@ccbuddy/ccbuddy-cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // CCBUDDY_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 CCBUDDY_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __CCBUDDY_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const home = process.env.CCBUDDY_HOME?.trim() || getCCbuddyDataRootDir();
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（CCbuddy 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "CCbuddy Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "CCbuddy Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@ccbuddy/ccbuddy-cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "CCbuddy Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "CCbuddy Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart CCbuddy Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const ccbuddyAgentService = createCCbuddyAgentService({
    ...
    commandResolver: options?.ccbuddyAgentCommandResolver,
    presentationSurface: resolveCCbuddyAgentPresentationSurface({
    ...
    // CCbuddyAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.ccbuddyAgentSpawnFallbackCwd,
    ...
    // adapter 的 fetch wrapper。判定实现与 CLI 侧共用 @ccbuddy/shared 的同一份，避免分叉。
    // origin 解析复用 resolveCurrentCCbuddyEndpointOrigin——与闲时任务同口径（含 settings
    ...
    officialMcpTrustedOrigins: createOfficialMcpTrustedOriginRegistry({
      ...
      resolveCCbuddyApiOrigin: resolveCurrentCCbuddyEndpointOrigin,
    }),
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // CCbuddy 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      const telemetryEnv = getCapturedCCbuddyAgentTelemetryEnv();
      ...
      return {
        ...
        // broker 凭据（socket/token）注入 agent spawn env，让内置 ccbuddy-cua plugin 的
        // computer-use MCP server 经 __ccbuddy-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          ccbuddyBuiltinFilePath: await providerConfigRuntime.resolveCCbuddyBuiltinActiveFilePath(),
    ...
    ...(isDesktopAttachedRemote
      ...
      : {
          ...
          resolveSessionRuntimePreferences: async (scope) => {
            ...
            const settings = await settingService.get();
            const modelContextBudgetStrategy = DEFAULT_CCBUDDY_MODEL_CONTEXT_BUDGET_STRATEGY;
            return {
  ...
  providerConnectivityAgentService = ccbuddyAgentService;
  ...
  hasActiveTurnRef = () => ccbuddyAgentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 ccbuddySessionService，绕开 CCbuddy task adapter 的
  ...
  const ccbuddyTaskIndexSyncer = createCCbuddyTaskIndexSyncer({
    agentService: ccbuddyAgentService,
    taskIndexRepo,
  });
  ...
  const ccbuddySessionService = createCCbuddySessionService({
    agentService: ccbuddyAgentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await ccbuddyAgentService.generateWorkspaceText({
  ...
  // task wrapper 由 CCbuddy task service adapter 提供；核心 session 状态由 CCbuddy agent server 维护。
  const ccbuddyTaskService = createCCbuddyTaskServiceAdapter({
    ccbuddyAgentService,
    taskIndexRepo,
    taskIndexSyncer: ccbuddyTaskIndexSyncer,
    settingService,
  ...
  const oauthService = createDisabledOAuthService();
  ...
  const offPeakCredentialResolverDeps = {
    ...
    resolveAccountProvider: async () => {
      ...
      const staticAccess = ccbuddyProviderAccountAccessSchema.parse(config.access.toJSON());
      const access = await accountRequestAuthService.resolveAccessCurrent(staticAccess);
      if (!access) return null;
      return {
        providerId: provider.providerId,
        access: ccbuddyAccountAccessSchema.parse(access),
  ...
  // CCbuddy 保留本地会话查看；分享会把会话写入项目目录并上传到上游服务，
  // 因此所有运行模式都在装配层关闭分享，旧 RPC 调用明确失败。
  ...
  const services = new ServiceCollection()
    ...
    .register(ICCbuddyTaskService, ccbuddyTaskService)
    .register(ICCbuddyAgentService, ccbuddyAgentService)
    .register(ICCbuddySessionService, ccbuddySessionService)
    ...
    .register(
      ...
      createClientConfigService({
        ...
        resolveRequestContext: async () => ({
          endpointOrigin: await resolveCurrentCCbuddyEndpointOrigin(),
          appVersion: CCBUDDY_VERSION,
      ...
      (() => {
        ...
        const offPeakTaskService = new OffPeakTaskService({
          ...
          stopRunningTask: async (params) => {
            await ccbuddyTaskService.stopGeneration({
        ...
        // 回写前向引用，供 ccbuddyAgentService 的 offPeak/create、offPeak/list 协议 handler 调用。
    ...
    .register(IPluginManagementService, createPluginManagementService({ ccbuddyAgentService }))
...
/** 仅给同一事件账号返回当前 CCbuddy JWT；不缓存、不修改登录凭据。 */
export function createTelemetryAuthorizationLoader(
  ...
  return async (userId) => {
    ...
    try {
      ...
      const jwt = (await credentialService.load("ccbuddyjwttoken"))?.trim();
...
): () => Promise<import("@ccbuddy/shared").OAuthLoginAttribution | null> {
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    ...
    services.getOptional(ICCbuddyTaskService),
    services.getOptional(ICCbuddyAgentService),
    services.getOptional(ICCbuddySessionService),
