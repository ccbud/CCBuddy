import { formatLogPrefix, type TraceId } from "@ccbuddy/shared";
...
export function createServiceLogger(
  ...
  function write(
    ...
    // 服务层日志过去常被复用到 CCbuddy Agent 命名 logger，导致新 CCbuddy 路径继续依赖 CCbuddy Agent 目录。
    // 这里把通用分级日志抽到独立模块，后续删除 CCbuddy Agent runtime 时不会牵连非 CCbuddy Agent 服务。
