export const CCBUDDY_E2E_FS_FAULTS_ENV = "CCBUDDY_E2E_FS_FAULTS";
export const CCBUDDY_E2E_FS_FAULTS_ALLOW_ENV = "CCBUDDY_E2E_FS_FAULTS_ALLOW";
...
function createInjectedFsFaultError(input: {
  ...
  error.ccbuddyFsFaultId = input.id;
  return error;
}
...
export function isInjectedFsFaultError(error: unknown): error is InjectedFsFaultError {
  return (
    typeof error === "object" &&
    error !== null &&
    "ccbuddyFsFaultId" in error &&
    typeof (error as { ccbuddyFsFaultId?: unknown }).ccbuddyFsFaultId === "string"
  );
}
...
export function parseFsFaultRulesFromEnvValue(rawValue: string): FsFaultRuleConfig[] {
  ...
  try {
    ...
    throw new Error(
      `Invalid ${CCBUDDY_E2E_FS_FAULTS_ENV}: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
  ...
  if (!Array.isArray(parsed)) {
    throw new Error(`Invalid ${CCBUDDY_E2E_FS_FAULTS_ENV}: expected a JSON array`);
  }
...
function createFsFaultInjectorFromEnv(
  ...
  const rawValue = env[CCBUDDY_E2E_FS_FAULTS_ENV]?.trim();
  if (!rawValue) {
    return createFsFaultInjector();
  }

  // 测试故障注入必须默认被生产环境隔离，避免用户机器残留环境变量后误伤真实配置和会话落盘。
  if (env.CCBUDDY_ENV !== "test" && env[CCBUDDY_E2E_FS_FAULTS_ALLOW_ENV] !== "1") {
    return createFsFaultInjector();
  }
