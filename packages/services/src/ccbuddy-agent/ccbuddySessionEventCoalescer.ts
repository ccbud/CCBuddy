import type { CCbuddySessionEvent } from "@ccbuddy/shared";
import type { CCbuddyAgentServiceEvent } from "#src/ccbuddy-agent/ccbuddyAgent.js";
...
type SessionServiceEvent = Extract<CCbuddyAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: CCbuddyAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: CCbuddySessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: CCbuddySessionEvent,
  next: CCbuddySessionEvent,
): CCbuddySessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: CCbuddyAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
