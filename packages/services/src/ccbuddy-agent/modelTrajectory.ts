...
} from "#src/ccbuddy-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/ccbuddy-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.ccbuddy/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - CCbuddy Agent 把 taskId 当作 sessionId（见 ccbuddyTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): CCbuddyModelTrajectoryRecord {
  ...
  const mapped: CCbuddyModelTrajectoryRecord = {
...
function mapMessages(value: unknown): CCbuddyModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): CCbuddyModelTrajectoryContentPart {
...
function mapUsage(value: unknown): CCbuddyModelTrajectoryUsage | undefined {
