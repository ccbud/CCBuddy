...
class CCbuddyV4CommandRejectedError extends Error {
  readonly code = "CCBUDDY_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "CCbuddyV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new CCbuddyV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new CCbuddyV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
