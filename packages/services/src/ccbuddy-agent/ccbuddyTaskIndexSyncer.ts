...
import { repairSubagentTaskIndex } from "#src/ccbuddy-agent/repairSubagentTaskIndex.js";
import {
  deriveCCbuddyTaskStatusFromSessionSnapshot,
  generateTraceId,
  getCCbuddyUserVisibleMessages,
  isCCbuddyGoalContinuationReminderText,
  isCCbuddyModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@ccbuddy/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { CCbuddyWorkspaceEventSubscriptionParams } from "#src/session/ccbuddyTaskListTypes.js";
import type {
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./ccbuddyConfigOptions.js";

const logger = createServiceLogger("ccbuddy-task-index-syncer");
...
export interface CCbuddyTaskIndexTerminalEvent {
...
export interface CCbuddyTaskIndexReadyEvent {
  target: CCbuddyAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface CCbuddyTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: CCbuddyAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，ccbuddySessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: CCbuddyWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: CCbuddyWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: CCbuddyAgentSessionTarget, model: string): Promise<CCbuddyTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: CCbuddyTaskMeta | undefined,
    reason: CCbuddyWorkspaceTaskListChanged["reason"],
    options?: Pick<CCbuddyWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): CCbuddyWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): CCbuddyTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: CCbuddyAgentWorkspaceTarget,
...
export function createCCbuddyTaskIndexSyncer(
  options: CreateCCbuddyTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<CCbuddyWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<CCbuddyTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<CCbuddyTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === CCBUDDY_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<CCbuddyWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<CCbuddyWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: CCbuddyAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: CCbuddyAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 ccbuddySessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: CCbuddyAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 ccbuddySessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 ccbuddySessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 ccbuddySessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useCCbuddyConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 CCbuddyConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useCCbuddyConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: CCbuddyAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: CCbuddySessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isCCbuddyGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isCCbuddyModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: CCbuddySessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getCCbuddyUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<CCbuddyMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromCCbuddyGoal(
  goal: NonNullable<CCbuddySessionStateSnapshot["projection"]["target"]>,
): CCbuddyTaskGoal {
  return {
