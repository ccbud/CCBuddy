...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@ccbuddy/rpc";
import {
...
export type CCbuddyAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface CCbuddyAgentV4ConnectionContext {
  connectionId: string;
  clientMode: CCbuddyAgentV4ClientMode;
...
const TRUSTED_CONNECTION_FIELD = "__ccbuddyTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__ccbuddyTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: CCbuddyAgentV4ConnectionContext;
};

interface TrustedCCbuddyAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedCCbuddyAgentV4UnsubscribeRoute;
};

export function readTrustedCCbuddyAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedCCbuddyAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedCCbuddyAgentV4Connection(
  value: unknown,
): CCbuddyAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: CCbuddyAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedCCbuddyAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: CCbuddyAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: CCbuddyAgentWorkspaceTarget): CCbuddyAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: CCbuddyAgentWorkspaceTarget;
  connection: CCbuddyAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: CCbuddyAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: CCbuddyAgentV4ConnectionContext): HelloMessage {
...
export interface CCbuddyAgentConnectionScope {
  readonly service: ICCbuddyAgentService;
...
export function createCCbuddyAgentConnectionScope(
  base: ICCbuddyAgentService,
  context: CCbuddyAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): CCbuddyAgentV4ConnectionContext => {
    const downstream = readTrustedCCbuddyAgentV4Connection(params);
  ...
  const routedEventKey = (kind: SubscriptionKind, target: CCbuddyAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: CCbuddyAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies CCbuddyAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: CCbuddyAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedCCbuddyAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedCCbuddyAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: CCbuddyAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: CCbuddyAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: CCbuddyAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<ICCbuddyAgentService> = {
