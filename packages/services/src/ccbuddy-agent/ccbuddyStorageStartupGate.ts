...
export class CCbuddyStorageStartupGate {
  private current?: CCbuddyStorageStartupState;
  private terminalError?: Error;
  private pending?: Promise<void>;
  private resolve?: () => void;
  private reject?: (error: Error) => void;
  private timer?: ReturnType<typeof setTimeout>;
  private readonly changed = new Emitter<CCbuddyStorageStartupState>();
  ...
  get snapshot(): CCbuddyStorageStartupState | undefined {
    return this.current;
  }

  accept(input: unknown): boolean {
    const parsed = ccbuddyStorageStartupStateSchema.safeParse(input);
