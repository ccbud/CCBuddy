/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 CCbuddy session 的协议适配。 */
import { createHash } from "node:crypto";
import { existsSync } from "node:fs";
import { join } from "node:path";
import { getLegacyTaskSessionSnapshotPath, getCCbuddyDataRootDir } from "../paths.js";
import {
  Emitter,
  Event,
...
import {
  coalesceConsecutiveCCbuddyAssistants,
  createSessionTraceId,
  decodeCustomModelValue,
  deriveCCbuddyTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  generateTraceId,
  getCCbuddyGoalActiveIterationCount,
  getCCbuddyGoalIterationByAssistantMessageId,
  getCCbuddyUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeCCbuddyApiRetryStatus,
  attachCCbuddyBackgroundTaskNotificationToRaw,
  collectCCbuddyBackgroundTaskNotificationsByToolUseId,
  mergeCCbuddyBackgroundTaskControlItems,
  parseCCbuddyBackgroundTaskControlItems,
  parseCCbuddyBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveCCbuddyVisibleSessionTitle,
  textFromCCbuddyMessageParts,
  CCBUDDY_AGENT_PROVIDER,
  ccbuddyBackgroundTaskNotificationToolUpdateStatus,
  appendCCbuddyStreamingToolInputDelta,
  buildCCbuddyStreamingToolInputPreview,
  createCCbuddyToolProjectionMemory,
  finalizeCCbuddyToolProjectionInput,
  forgetCCbuddyToolProjectionMetadata,
  isCCbuddyModelRetryRecoveryProgressPayload,
  markCCbuddyStreamingToolInputPreviewMaterialized,
  resolveCCbuddyToolProjectionMetadata,
  ccbuddyApiRetryFromModelNetworkStatusPayload,
  ccbuddyApiRetryFromStreamRecoveryPayload,
  ccbuddyTaskNetworkDebugStatusFromPayload,
  shouldMaterializeCCbuddyStreamingToolInputPreview,
  type CCbuddyApiRetryStatus,
  type CCbuddyAssistantMessageFeedback,
  type CCbuddyBackgroundTaskNotificationInfo,
  type CCbuddyBackgroundTaskControlItem,
  type CCbuddyBackgroundTurnAttribution,
  type CCbuddyAutomationBotDeliveryTarget,
  type CCbuddyCancelTaskCommandResult,
  type CCbuddyConfigOption,
  ...
  type CCbuddyUsage,
  type CCbuddyPersistedMessage,
  type CCbuddyPersistedMessagePart,
  type CCbuddyPersistedToolCall,
  type CCbuddyPromptAttachment,
  type CCbuddyProvider,
  type CCbuddySessionFile,
  type CCbuddyTaskGoal,
  type CCbuddyTaskGoalStats,
  type CCbuddyTaskMode,
  type CCbuddyPlanStep,
  type CCbuddySlashCommand,
  type CCbuddyStreamEvent,
  type CCbuddyTaskCreateResult,
  type CCbuddyTaskMeta,
  type CCbuddyTaskClientMode,
  type CCbuddyTaskRuntimeCommand,
  type CCbuddyTaskSnapshot,
  type CCbuddyTaskSnapshotBody,
  type CCbuddyTaskSnapshotRefContent,
  type CCbuddyTaskSnapshotToolCallsSlice,
  type CCbuddyTaskTokenUsageResult,
  type CCbuddyTodoGroup,
  type CCbuddyTurnSteerCommandKind,
  type CCbuddyTurnSteerSource,
  type CCbuddyWorkspaceEvent,
  type CCbuddyWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  ccbuddyContextUsageBreakdownSchema,
  ccbuddySessionSettingsStateSchema,
  type CCbuddyDeliveryKind,
  type CCbuddyMessagePart,
  type CCbuddyMessageWithParts,
  type ModelSelection,
  type CCbuddyPermissionOption,
  type CCbuddyPermissionRequestParams,
  type CCbuddyPermissionRequest,
  type CCbuddySessionEvent,
  type CCbuddySessionMode,
  type CCbuddySessionSettingsState,
  type CCbuddySessionStateSnapshot,
  type CCbuddyStateUpdatedNotification,
  type CCbuddyContextCompactionTimelineMeta,
  type CCbuddyTimelineMeta,
  type CCbuddyTimelineStatus,
  type CCbuddyTimelineTrigger,
  type CCbuddyToolProjectionMemory,
  type CCbuddyUserInputRequestParams,
  type CCbuddyUserInputResponse,
  type CCbuddyAgentMcpServer,
} from "@ccbuddy/shared";
import type {
  CCbuddyTaskListQuery,
  CCbuddyTaskListResult,
  CCbuddyWorkspaceEventSubscriptionParams,
  ICCbuddyTaskService,
  CCbuddyArchivedTaskDeletionResult,
  CCbuddyTaskReadyOutcome,
  CCbuddyTaskTerminalOutcome,
} from "../session/ccbuddyTaskService.js";
...
import type {
  CCbuddyTaskIndexReadyEvent,
  CCbuddyTaskIndexSyncer,
  CCbuddyTaskIndexTerminalEvent,
} from "./ccbuddyTaskIndexSyncer.js";
...
import {
  ...
  getCCbuddyAgentAvailableModes,
  normalizeAvailableCCbuddyMode,
  settingsToConfigOptions,
} from "./ccbuddyConfigOptions.js";
...
interface CreateCCbuddyTaskServiceAdapterOptions {
  ccbuddyAgentService: ICCbuddyAgentService;
  ...
  taskIndexSyncer: CCbuddyTaskIndexSyncer;
...
type WorkspaceEventInput = string | CCbuddyWorkspaceEventSubscriptionParams;
type CCbuddySendPromptRuntimeCommand = Extract<CCbuddyTaskRuntimeCommand, { type: "send_prompt" }>;
type CCbuddyTerminalStreamEvent =
  | Extract<CCbuddyStreamEvent, { type: "task_complete" }>
  | Extract<CCbuddyStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: CCbuddyProvider = CCBUDDY_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: CCbuddySlashCommand[] = [];
const logger = createServiceLogger("ccbuddy-task-service");
...
function formatCCbuddyAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveCCbuddyAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.CCBUDDY_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(getCCbuddyDataRootDir(), "cli", "log");
  return join(logDir, `ccbuddy-${formatCCbuddyAgentLogDate(now)}.jsonl`);
}

export function createCCbuddyTaskServiceAdapter(
  options: CreateCCbuddyTaskServiceAdapterOptions,
): ICCbuddyTaskService {
  const errorEmitter = new Emitter<CCbuddyError>();
  const taskEmitters = new Map<string, Emitter<CCbuddyStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<CCbuddyStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, CCbuddyTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, CCbuddyApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, CCbuddyBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, CCbuddyToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`CCbuddy task service adapter does not support ICCbuddyTaskService.${name} yet.`),
      {
        code: "CCBUDDY_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: CCbuddyBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: CCbuddyAgentMcpServer[] | undefined,
  ): Promise<CCbuddyAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    params: {
      ...
      messageId?: string;
      content: string;
      attachments?: CCbuddyPromptAttachment[];
      toolDenylist?: string[];
    ...
    // CCbuddy task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 CCbuddy session 事件回投 CCbuddy Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "CCbuddy task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.ccbuddyAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.ccbuddyAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "CCbuddy task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "CCbuddy task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: CCbuddyTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: CCbuddySendPromptRuntimeCommand,
  ): CCbuddySendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: CCbuddySendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: CCbuddySendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "CCbuddy task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: CCbuddyTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is CCbuddySendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "CCbuddy task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "CCbuddy task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "CCbuddy task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<CCbuddyStreamEvent, { type: "tool_call_update" }>["status"],
  ): CCbuddyPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: CCbuddyStreamEvent): void {
    ...
    const nextTool: CCbuddyPersistedToolCall =
  ...
  function persistedToolId(tool: CCbuddyPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: CCbuddyPersistedToolCall | undefined,
    liveTool: CCbuddyPersistedToolCall,
  ): CCbuddyPersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly CCbuddyPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly CCbuddyPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly CCbuddyPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): CCbuddyPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: CCbuddyPersistedMessage,
    toolIndex: number,
    tool: CCbuddyPersistedToolCall,
  ): CCbuddyPersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "CCbuddy snapshot 合并 live tool projection", {
        event: "ccbuddy_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`CCbuddy session target is not loaded: ${taskId}`), {
        code: "CCBUDDY_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): CCbuddyToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createCCbuddyToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<CCbuddyWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<CCbuddyStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<CCbuddyStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<CCbuddyStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: CCbuddyStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: CCbuddyTaskMeta | undefined,
    reason: CCbuddyWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.ccbuddyAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.ccbuddyAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 CCbuddySessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.ccbuddyAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.ccbuddyAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: CCbuddySessionStateSnapshot): CCbuddyTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        mode: fromCCbuddyMode(snapshot.session.mode),
        ...
        status: deriveCCbuddyTaskStatusFromSessionSnapshot(snapshot),
        ...
        target: snapshot.projection.target
          ? fromCCbuddyGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: CCbuddyTaskMeta): CCbuddyTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 CCbuddy session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: CCbuddyTaskMeta): Promise<CCbuddyTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 CCbuddy session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: CCbuddyStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: CCbuddyStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToCCbuddy(
    ...
    const goalIterationByAssistantMessageId = getCCbuddyGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectCCbuddyBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveCCbuddyAssistants(
              getCCbuddyUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: CCbuddyPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<CCbuddyStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseCCbuddyBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: CCbuddyAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 CCbuddy snapshot 到 task index 失败", error);
      });
      const snapshotEvent: CCbuddyStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: CCbuddyTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: CCbuddyTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeCCbuddyAgentServiceAndWait(): Promise<void> {
    const agentService = options.ccbuddyAgentService as ICCbuddyAgentService & {
  ...
  const service: ICCbuddyTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.ccbuddyAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.ccbuddyAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<CCbuddyTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: CCbuddySessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.ccbuddyAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.ccbuddyAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.ccbuddyAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        if (params.v4Create === true) {
          ...
          const ack = assertV4CommandAckOk(
            "createSession",
            await options.ccbuddyAgentService.sendConversationCommandV4({
              ...target,
              envelope: createHostCommandEnvelope({
                type: "createSession",
                sessionId: null,
                payload: {
                  ...
                  config: {
                    ...
                    ...(model?.options?.reasoningLevel
                      ...
                      : {}),
                    ...(params.mode ? { mode: toCCbuddyMode(params.mode) } : {}),
                  },
                  ...(mcpServers ? { mcpServers } : {}),
                },
              }),
          ...
          snapshot = await options.ccbuddyAgentService.createSession({
            ...target,
            sessionTraceId: createSessionTraceId(),
            mode: toCCbuddyMode(params.mode),
            model: requestedSelection,
            thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<CCbuddyEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: CCbuddyTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<CCbuddyEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<CCbuddyCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "CCbuddy task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "CCbuddy task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "CCbuddy task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "CCbuddy task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "CCbuddy task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.ccbuddyAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[ccbuddy-task-service] goalSession start", {
      ...
      const result = await options.ccbuddyAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[ccbuddy-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[ccbuddy-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.ccbuddyAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<CCbuddyTaskMeta> {
    ...
    async listTasks(params): Promise<CCbuddyTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<CCbuddyTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: CCbuddyTaskListQuery): Promise<CCbuddyTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<CCbuddyTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<CCbuddyTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<CCbuddyTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<CCbuddyTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const ccbuddySnapshot = limitTaskSnapshotMessages(
        snapshotToCCbuddy(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(ccbuddySnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[ccbuddy-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(ccbuddySnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...ccbuddySnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[ccbuddy-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[ccbuddy-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[ccbuddy-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<CCbuddyTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<CCbuddyTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<CCbuddyTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<CCbuddyTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<CCbuddyConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.ccbuddyAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<CCbuddySessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as CCbuddyAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<CCbuddyImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          ...
          // 导入后的任务必须是真实 CCbuddy session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toCCbuddyMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<CCbuddyConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.ccbuddyAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toCCbuddyMode(params.value as CCbuddyTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<CCbuddyConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.ccbuddyAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<CCbuddyConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveCCbuddyAgentCurrentLogFilePath();
      // 返回 CCbuddy Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // CCbuddy Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[CCbuddyTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<CCbuddyTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 CCbuddy Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.ccbuddyAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: getLegacyTaskSessionSnapshotPath(
    ...
    async deleteArchivedTasks(params): Promise<CCbuddyArchivedTaskDeletionResult> {
      const result: CCbuddyArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<CCbuddyTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[CCbuddyTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[CCbuddyTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[CCbuddyTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[CCbuddyTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[CCbuddyTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<CCbuddyTaskMeta> {
    ...
    async setTaskUnread(params): Promise<CCbuddyTaskMeta> {
    ...
    async archiveTask(params): Promise<CCbuddyTaskMeta> {
    ...
    async unarchiveTask(params): Promise<CCbuddyTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<CCbuddyTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<CCbuddyStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<CCbuddyTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<CCbuddyTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<CCbuddyStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 CCbuddyStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.ccbuddyAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toCCbuddyDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<CCbuddyWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.ccbuddyAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 ccbuddy-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeCCbuddyAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: CCbuddyTaskMeta, overlay: TaskOverlay): CCbuddyTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: CCbuddySessionStateSnapshot): string {
  return resolveCCbuddyVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<CCbuddyTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: CCbuddyTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 ccbuddy-cli。
...
function toCCbuddyMode(mode: CCbuddyTaskMode | undefined): CCbuddySessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function fromCCbuddyMode(mode: CCbuddySessionMode): CCbuddyTaskMode {
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `ccbuddy-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<CCbuddyPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<CCbuddyGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `ccbuddy-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `ccbuddy-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: CCbuddyPersistedMessage[],
  timelineMessages: CCbuddyPersistedMessage[],
): CCbuddyPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: CCbuddySessionStateSnapshot): number {
  ...
  return getCCbuddyGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toCCbuddyDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<CCbuddyStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseCCbuddyBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = ccbuddyBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachCCbuddyBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: CCbuddyMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, CCbuddyBackgroundTaskNotificationInfo>,
): CCbuddyPersistedMessage {
  const tools: CCbuddyPersistedToolCall[] = [];
  const parts: CCbuddyPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: CCbuddyTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly CCbuddyMessagePart[],
): CCbuddyPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<CCbuddyMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is CCbuddyPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<CCbuddyMessagePart, { type: "file" }>,
): CCbuddyPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<CCbuddyMessagePart, { type: "text" }>,
): CCbuddyTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<CCbuddyMessagePart, { type: "compaction" }>,
): CCbuddyTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<CCbuddyMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, CCbuddyBackgroundTaskNotificationInfo>,
): CCbuddyPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // CCbuddy Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachCCbuddyBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? ccbuddyBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly CCbuddyMessagePart[]): string {
  return textFromCCbuddyMessageParts(parts);
}

function reasoningFromParts(parts: readonly CCbuddyMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<CCbuddyMessagePart, { type: "reasoning" }> =>
        part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function fromCCbuddyGoal(goal: unknown): CCbuddyTaskGoal {
  ...
  return {
    ...
    status: isCCbuddyTaskGoalStatus(status) ? status : "active",
...
function isCCbuddyTaskGoalStatus(status: string | undefined): status is CCbuddyTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: CCbuddySessionStateSnapshot["todos"],
): CCbuddyPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: CCbuddySessionStateSnapshot["goalStats"],
): CCbuddyTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: CCbuddySessionStateSnapshot["todoGroups"],
): CCbuddyTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = ccbuddySessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableCCbuddyMode(parsedSettings.data.mode.current),
      availableModes: getCCbuddyAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: CCbuddyToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, CCbuddyBackgroundTaskControlItem[]>,
  ...
  // CCbuddy Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 CCbuddy prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: CCbuddyStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: CCbuddyStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "CCbuddy compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "CCbuddy session failed",
...
): Extract<CCbuddyStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isCCbuddyModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendCCbuddyStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeCCbuddyStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildCCbuddyStreamingToolInputPreview(state.rawInput);
    markCCbuddyStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildCCbuddyStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeCCbuddyToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "CCbuddy streaming tool input projected", {
    ...details,
    event: "ccbuddy.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<CCbuddyStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<CCbuddyMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): CCbuddyContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<CCbuddyStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveCCbuddyToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeCCbuddyToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: CCbuddyStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // CCbuddy Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 CCbuddy Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetCCbuddyToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // CCbuddy Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: CCbuddyPermissionRequestParams,
): CCbuddyPermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: CCbuddySessionStateSnapshot["projection"]["pendingPermissions"][number],
): CCbuddyPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: CCbuddyUserInputRequestParams,
): CCbuddyStreamEvent {
  const questions =
...
): Extract<CCbuddyStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: CCbuddySessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<CCbuddyStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: CCbuddySessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<CCbuddyStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, CCbuddyBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: CCbuddyBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): CCbuddyBackgroundTaskControlItem[] | null {
  const parsedJobs = parseCCbuddyBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeCCbuddyBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): CCbuddyPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as CCbuddyPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<CCbuddyStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = ccbuddyTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromCCbuddyGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromCCbuddyGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseCCbuddyBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<CCbuddyStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): CCbuddyApiRetryStatus | null | undefined {
  if ("apiRetry" in payload) {
    return normalizeCCbuddyApiRetryStatus(payload.apiRetry);
  }

  const runtimeRetry = normalizeCCbuddyApiRetryStatus(asRecord(payload.runtime).apiRetry);
  if (runtimeRetry !== undefined) {
    return runtimeRetry;
  }

  const metaRetry = normalizeCCbuddyApiRetryStatus(
    asRecord(asRecord(payload._meta).ccbuddy).apiRetry,
  );
  ...
  return (
    ccbuddyApiRetryFromStreamRecoveryPayload(payload) ??
    ccbuddyApiRetryFromModelNetworkStatusPayload(payload)
  );
}

function recordAgentModelNetworkTelemetry(event: CCbuddySessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `ccbuddy_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<CCbuddyStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: CCbuddySessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: CCbuddySessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // CCbuddy Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = ccbuddyContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): CCbuddyUsage | undefined {
...
function timelineStatusValue(value: unknown): CCbuddyTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): CCbuddyTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): CCbuddyTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): CCbuddyTurnSteerCommandKind | undefined {
