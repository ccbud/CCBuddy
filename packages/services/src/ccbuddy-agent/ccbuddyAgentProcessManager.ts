import { resolveCCbuddyAgentSpawnCwd } from "#src/ccbuddy-agent/ccbuddyAgentSpawnCwd.js";
import type { CCbuddyAgentStorageStartupSnapshot } from "#src/ccbuddy-agent/ccbuddyAgent.js";
/* eslint-disable max-lines -- ccbuddyAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  ...
  CCBUDDY_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  CCBUDDY_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  CCBUDDY_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@ccbuddy/shared/process-diagnostic";
import {
  CCBUDDY_AGENT_RUNTIME,
  CCBUDDY_AGENT_PROVIDER,
  CCBUDDY_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveCCbuddyRuntimeEnv,
  sanitizeCCbuddyRuntimeEnv,
} from "@ccbuddy/shared";
import {
  findCCbuddyAgentRuntimeBinary,
  findCCbuddyAgentRuntimeNodeBundle,
...
import { CCbuddyProtocolClient } from "./ccbuddyProtocolClient.js";
import { CCbuddyStdioTransport } from "./ccbuddyStdioTransport.js";
import { readCCbuddyStdioTapDevState } from "./ccbuddyStdioTapDevConfig.js";
import type { CCbuddyAgentPresentationSurface } from "./ccbuddyAgentPresentationSurface.js";
...
export interface CCbuddyAgentCommand {
...
export interface CCbuddyAgentCommandResolverContext {
  presentationSurface?: CCbuddyAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type CCbuddyAgentCommandResolver = (
  context: CCbuddyAgentCommandResolverContext,
) => Promise<CCbuddyAgentCommand | null> | CCbuddyAgentCommand | null;

export interface CCbuddyAgentProcessManagerOptions {
  commandResolver?: CCbuddyAgentCommandResolver;
  presentationSurface?: CCbuddyAgentPresentationSurface;
...
interface ManagedCCbuddyAgentProcess {
  ...
  runtimeIdentity: CCbuddyAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.CCBUDDY_E2E_ARTIFACT_DIR?.trim();
  if (env.CCBUDDY_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "ccbuddy-e2e-coverage-preload.cjs");
 ...
 */
interface CCbuddyAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: CCbuddyAgentRuntimeIdentity;
}

export interface CCbuddyAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: CCbuddyAgentRuntimeIdentity;
...
interface CCbuddyAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface CCbuddyAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("ccbuddy-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("CCBUDDY_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildCCbuddyAgentSpawnPreflight(
  command: CCbuddyAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<CCbuddyAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveCCbuddyAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceCCbuddyAgentCommand(
  context: CCbuddyAgentCommandResolverContext,
): CCbuddyAgentCommand | null {
  const distEntrypoint = findUpward("apps/ccbuddy-cli/packages/cli/dist/ccbuddy.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.CCBUDDY_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "ccbuddy.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 ccbuddy-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/ccbuddy-cli/packages/cli/src/main.ts");
...
function resolveDeployedCCbuddyAgentBinaryCommand(
  context: CCbuddyAgentCommandResolverContext,
): CCbuddyAgentCommand | null {
  // 旧 resolver 只识别 CCBUDDY_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 ccbuddy-server.cjs 单文件部署到 ~/.ccbuddy/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 agent 已经部署到 ~/.ccbuddy/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "CCbuddy Agent server command is not configured"。
  // 这里复用 findCCbuddyAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.ccbuddy/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findCCbuddyAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: CCBUDDY_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeCCbuddyAgentCommand(
  context: CCbuddyAgentCommandResolverContext,
): CCbuddyAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 ccbuddy-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 ccbuddy.cjs，
  ...
  const bundlePath = findCCbuddyAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...CCBUDDY_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultCCbuddyAgentCommand(
  context: CCbuddyAgentCommandResolverContext,
): CCbuddyAgentCommand | null {
  const command = process.env.CCBUDDY_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.CCBUDDY_AGENT_SERVER_ARGS_JSON) ?? [
          "app-server",
          "--stdio",
        ],
        cwd: process.env.CCBUDDY_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 ccbuddy.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceCCbuddyAgentCommand(context) ??
    resolveElectronRuntimeCCbuddyAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedCCbuddyAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: CCbuddyAgentCommand | null,
  presentationSurface: CCbuddyAgentCommandResolverContext["presentationSurface"],
): CCbuddyAgentCommand | null {
...
function wrapCCbuddyAgentCommandWithStdioTapDevProxy(
  command: CCbuddyAgentCommand,
  workspaceKey: string,
): CCbuddyAgentCommand {
  const tapState = readCCbuddyStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/ccbuddy-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("CCbuddy stdio tap proxy enabled but script not found");
    return command;
  }
...
export class CCbuddyAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedCCbuddyAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedCCbuddyAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<CCbuddyProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: CCbuddyAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): CCbuddyAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: CCbuddyAgentCommandResolver;
  private readonly presentationSurface: CCbuddyAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: CCbuddyAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: CCbuddyAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<CCbuddyAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<CCbuddyAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 CCbuddyAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: CCbuddyAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultCCbuddyAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("CCbuddy Agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedCCbuddyAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedCCbuddyAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("CCbuddy Agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedCCbuddyAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedCCbuddyAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedCCbuddyAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("CCbuddy Agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("CCbuddy Agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`CCbuddy Agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<CCbuddyProtocolClient> {
    if (this.disposed) {
      throw new Error("CCbuddy Agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("CCbuddy Agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("CCbuddy Agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): CCbuddyProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "CCbuddy Agent server command is not configured. Set CCBUDDY_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("CCbuddy Agent process start was cancelled.");
    }
    const effectiveCommand = wrapCCbuddyAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("CCbuddy Agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("CCbuddy Agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildCCbuddyAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 CCBUDDY_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 CCbuddy 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveCCbuddyRuntimeEnv(process.env);
    log("CCbuddy Agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeCCbuddyRuntimeEnv(process.env),
    ...
    const transport = new CCbuddyStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseCCbuddyProcessDiagnostic(line);
    ...
    const client = new CCbuddyProtocolClient(transport, {
    ...
    const runtimeIdentity: CCbuddyAgentRuntimeIdentity = {
    ...
    const managed: ManagedCCbuddyAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("CCbuddy Agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `CCbuddy Agent process error${this.processLifecycleReporter?.onError ? ` ${CCBUDDY_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("CCbuddy Agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `CCbuddy Agent process exited unexpectedly${this.processLifecycleReporter ? ` ${CCBUDDY_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "CCbuddy Agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("CCbuddy Agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("CCbuddy Agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<CCbuddyAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("CCbuddy Agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "CCBUDDY_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("CCbuddy Agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("CCbuddy Agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，ccbuddy-cli 会残留为孤儿进程。
