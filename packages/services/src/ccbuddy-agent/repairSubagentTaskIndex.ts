import { resolveWorkspaceKey, CCBUDDY_AGENT_PROVIDER } from "@ccbuddy/shared";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<ICCbuddyAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: CCBUDDY_AGENT_PROVIDER });
  const ids = rows
