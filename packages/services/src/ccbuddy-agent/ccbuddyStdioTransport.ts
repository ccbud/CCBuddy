...
import { ccbuddyProtocolMessageSchema } from "@ccbuddy/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  CCbuddyProtocolTransport,
  CCbuddyProtocolTransportClosedEvent,
} from "./ccbuddyProtocolTransport.js";
import {
...
interface CCbuddyStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("ccbuddy-agent-process-tree");

export class CCbuddyStdioTransport implements CCbuddyProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<CCbuddyProtocolMessage>();
  private readonly closeEmitter = new Emitter<CCbuddyProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: CCbuddyStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // CCbuddy Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: CCbuddyProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("CCbuddy Agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.CCBUDDY_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = ccbuddyProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // ccbuddy-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: CCbuddyProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
