import { CCbuddyStorageStartupGate } from "#src/ccbuddy-agent/ccbuddyStorageStartupGate.js";
...
import type {
  ...
  CCbuddyProtocolTrace,
} from "@ccbuddy/shared";
import type { V4Method } from "@ccbuddy/shared/ccbuddy-protocol-v4";
import type { z } from "zod";
import type { CCbuddyProtocolTransport } from "./ccbuddyProtocolTransport.js";

/** 客户端可发的方法名：旧 ccbuddyProtocolMethods + v4/*（并存，收敛为 v4）。 */
type CCbuddyProtocolClientMethod = CCbuddyProtocolMethod | V4Method;

interface CCbuddyProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface CCbuddyProtocolRequestTimeoutEvent {
  method: CCbuddyProtocolClientMethod;
  requestId: CCbuddyProtocolRequestId;
  timeoutMs: number;
}
...
interface CCbuddyProtocolClientRequestOptions {
...
const DEFAULT_CCBUDDY_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class CCbuddyProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "CCbuddyProtocolClientError";
  }
}

export class CCbuddyProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: CCbuddyProtocolClientMethod,
    readonly requestId: CCbuddyProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`CCbuddy Protocol request timed out: ${method}`);
    this.name = "CCbuddyProtocolRequestTimeoutError";
  }
}

export class CCbuddyProtocolClient implements IDisposable {
  readonly storageStartup: CCbuddyStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<CCbuddyProtocolNotification>();
  private readonly requestEmitter = new Emitter<CCbuddyProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<CCbuddyProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "CCbuddy Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: CCbuddyProtocolTransport,
    options?: CCbuddyProtocolClientOptions,
  ) {
    this.storageStartup = new CCbuddyStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs =
      options?.requestTimeoutMs ?? DEFAULT_CCBUDDY_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`CCbuddy Agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new CCbuddyProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: CCbuddyProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: CCbuddyProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: CCbuddyProtocolRequestId; result: unknown }).id,
    ...
    if ("error" in message && "id" in message) {
      ...
      this.rejectResponse(
        errorMessage.id,
        new CCbuddyProtocolClientError(
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as CCbuddyProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as CCbuddyProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as CCbuddyProtocolNotification);
    }
  }

  private resolveResponse(id: CCbuddyProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`CCbuddy Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: CCbuddyProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("CCbuddy Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("CCbuddy Protocol client is disposed");
    }
  }
}
