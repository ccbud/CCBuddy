...
function parseWebhookElicitationResponse(
  ...
  if (payload.type !== "ccbuddy.bot.elicitation_response") {
    return undefined;
  }
  const requestId =
...
export function createWebhookBotProvider(deps: WebhookProviderDeps): BotProviderAdapter {
  return {
      ...
      if (bot.webhookUrl) {
        ...
        const response = await postWebhookWithRetry(bot.webhookUrl, {
          ...
          headers: {
            ...
            [bot.webhookAuthHeaderName || "x-ccbuddy-bot-secret"]: secret ?? "",
          },
          body: JSON.stringify({
            type: "ccbuddy.bot.test",
            botId: bot.id,
            provider: "webhook",
            sentAt: Date.now(),
          }),
    ...
    async send(bot, message) {
      ...
      if (secret) {
        headers[bot.webhookAuthHeaderName || "x-ccbuddy-bot-secret"] = secret;
      }
      const response = await postWebhookWithRetry(bot.webhookUrl, {
        method: "POST",
        headers,
        body: JSON.stringify({
          type: "ccbuddy.bot.message",
          botId: bot.id,
          provider: "webhook",
          ...
          ...(message.elicitation ? { type: "ccbuddy.bot.elicitation_request" } : {}),
          sentAt: Date.now(),
        }),
      });
      if (!response.ok) {
