...
export function taskStatus(task: CCbuddyTaskMeta): string {
...
function readRunningTaskStartedAt(
  snapshot: CCbuddySessionFile | null,
...
export function formatStatusTaskLine(task: CCbuddyTaskMeta, label = "Task"): string {
...
export function readLatestAssistantTurnChangeSummary(
  snapshot: CCbuddySessionFile | null,
): CCbuddyTaskMeta["changeSummary"] | null {
...
function formatStatusToolProgress(tool: CCbuddyPersistedToolCall | undefined): string | null {
  if (!tool) {
    return null;
  }
...
export function formatStatusStreamToolProgress(
  event: Extract<CCbuddyStreamEvent, { type: "tool_call" | "tool_call_update" }>,
): string | null {
...
export function readLatestTaskProgress(snapshot: CCbuddySessionFile | null): string | null {
  const messages = snapshot?.messages ?? [];
