/* eslint-disable max-lines -- Bots 服务仍复用原 RPC 文件名，先把鉴权、命令路由、CCbuddy Agent 桥接收口集中在同一服务内。 */
...
import {
  ...
  buildCCbuddyAssistantPresentation,
  ...
  type CCbuddyElicitationRequest,
  type CCbuddyElicitationQuestion,
...
import type { ICCbuddyTaskService } from "../session/ccbuddyTaskService.js";
...
import type { CCbuddyAgentAppRuntimePreferences } from "../ccbuddy-agent/ccbuddyAgent.js";
...
function formatBotModelSelectionValue(selection: ModelSelection | undefined): string | undefined {
  ...
  return selection.providerId === CCBUDDY_AGENT_PROVIDER
    ? selection.modelId
...
function parseBotModelOptionValue(value: string): ModelSelection | undefined {
  ...
  return value.trim() ? { providerId: CCBUDDY_AGENT_PROVIDER, modelId: value.trim() } : undefined;
}
...
interface BotsServiceDeps {
  ...
  ccbuddyTaskService: ICCbuddyTaskService;
...
interface BotRemoteWorkspaceService {
  ...
  getCCbuddyTaskService?(target: BotRemoteWorkspaceTarget): Promise<ICCbuddyTaskService | null>;
...
interface PreparedBotMessageContent {
  content: string;
  ccbuddyAttachments: CCbuddyPromptAttachment[];
}
...
function resolveAutomationBotDeliveryTarget(
  actor: BotActor,
): CCbuddyAutomationBotDeliveryTarget | undefined {
...
function getBotPermissionOptionDisplayKind(
  option: CCbuddyPermissionOption,
...
function sortBotPermissionOptions(
  options: readonly CCbuddyPermissionOption[],
): CCbuddyPermissionOption[] {
...
function formatBotPermissionOptionLabel(option: CCbuddyPermissionOption, locale?: Locale): string {
...
function formatBotPermissionOptionDescription(
  option: CCbuddyPermissionOption,
  request: Pick<CCbuddyPermissionRequest, "title" | "description" | "kind" | "raw">,
  locale?: Locale,
): string | undefined {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
function isBotPermissionRejectOption(option: CCbuddyPermissionOption): boolean {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
const DEFAULT_BOT_CCBUDDY_PROVIDER: CCbuddyProvider = CCBUDDY_AGENT_PROVIDER;
...
export function createBotsService(
  ...
  async function prepareBotMessageContent(
    ...
    const ccbuddyAttachments: CCbuddyPromptAttachment[] = [];
    ...
    for (const rawAttachment of rawAttachments) {
      ...
      if (cached.kind === "image" || cached.kind === "audio") {
        ccbuddyAttachments.push({
          kind: cached.kind,
          filename: cached.filename,
          mimeType: cached.mimeType,
          dataBase64,
          // Bugfix：Bot 已把附件缓存到本地，CCbuddyPromptAttachment 也必须携带该路径。
          // 只在 prompt 文本里描述路径会让下游附件策略无法选择本地文件读取。
          localPath: cached.localPath,
        });
        // Bugfix: bot 附件已经被 gateway 下载并缓存到本地。只把图片作为 CCbuddy Agent image block 传入时，
  ...
  async function resolveCCbuddyTaskServiceForContext(
    context: Pick<BotContextState, "workspacePath" | "workspaceIdentity">,
  ): Promise<ICCbuddyTaskService> {
    if (!context.workspaceIdentity) {
      return deps.ccbuddyTaskService;
    }
    const remoteCCbuddyTaskService = await deps.remoteWorkspaceService?.getCCbuddyTaskService?.({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    if (remoteCCbuddyTaskService) {
      return remoteCCbuddyTaskService;
    }
    // Bugfix: 远端 workspace 的 bot 请求不能缺 runtime 时静默走本地 ccbuddyTaskService。
  ...
  async function listActiveTaskConfigOptions(
    ...
    const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(context);
    return ccbuddyTaskService.getTaskConfigOptions({ taskId });
  }

  function findSelectConfigOption(
    options: readonly CCbuddyConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
  ): (CCbuddyConfigOption & { type: "select" }) | undefined {
    const category = configId === "thoughtLevel" ? "thought_level" : configId;
    return options.find(
      (item): item is CCbuddyConfigOption & { type: "select" } =>
  ...
  function listConfigSelectOptions(
    options: readonly CCbuddyConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
    context: { locale?: Locale; provider?: CCbuddyProvider } = {},
  ): BotModelOption[] {
  ...
  function getModeDisplayLabel(
    locale: Locale | undefined,
    provider: CCbuddyProvider | undefined,
    ...
    const labels: Partial<Record<CCbuddyProvider, Record<string, string>>> = {
      glm: {
  ...
  async function listModelProviderOptionsForActiveTask(
    task: Pick<CCbuddyTaskMeta, "model" | "workspacePath" | "workspaceIdentity">,
    _activeProvider: CCbuddyProvider,
  ): Promise<BotModelProviderOption[]> {
  ...
  async function listModelOptionsForProviderFromActiveTask(
    task: Pick<CCbuddyTaskMeta, "model" | "workspacePath" | "workspaceIdentity">,
    activeProvider: CCbuddyProvider,
    providerId: string,
  ...
  function readCurrentActiveTaskModel(
    task: Pick<CCbuddyTaskMeta, "model">,
    options: readonly CCbuddyConfigOption[],
  ): string | undefined {
  ...
  function readCurrentActiveTaskMode(
    task: Pick<CCbuddyTaskMeta, "mode">,
    options: readonly CCbuddyConfigOption[],
  ): string | undefined {
  ...
  async function listProviderConfigOptionsForActiveTask(
    task: Pick<CCbuddyTaskMeta, "workspacePath" | "workspaceIdentity">,
    activeProvider: CCbuddyProvider,
  ): Promise<CCbuddyConfigOption[]> {
  ...
  function normalizeBotDraftOptions(draftOptions: BotDraftOptions): BotDraftOptions {
    ...
    // 如果直接复用，/new 后首条消息会重新创建第三方 runtime，绕过 CCbuddy Agent 单一事实源。
    return {
      ...draftOptions,
      provider: normalizeAgentProviderToCCbuddyAgent(draftOptions.provider),
    };
  }
  ...
  async function buildInitializedDraftOptions(
    ...
    const requestedProvider = normalizeAgentProviderToCCbuddyAgent(
      provider ?? DEFAULT_BOT_CCBUDDY_PROVIDER,
    );
    if (context.workspaceIdentity && !(await isRemoteWorkspaceConnected(context))) {
      // Bugfix: 远端断连时初始化草稿也不能偷偷申请远端 CCbuddy Agent runtime。
  ...
  async function buildActiveTaskDraftOptions(context: BotContextState): Promise<BotDraftOptions> {
    ...
    const resolvedProvider = normalizeAgentProviderToCCbuddyAgent(activeTask.provider);
  ...
  async function applyDraftConfigOptions(
    ...
    if (modeOption?.id && forcedDraftMode) {
      const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(context);
      await ccbuddyTaskService.setMode({
        taskId,
        mode: forcedDraftMode as CCbuddyTaskMode,
      });
    } else if (modeOption?.id) {
      // provider 不支持 yolo（非 CCbuddy Agent）：保持其自身默认模式，避免首条消息回调失败。
      botsLogger.debug(
        traceId,
  ...
  function updateLiveStatusProgress(event: CCbuddyStreamEvent): void {
  ...
  async function readTaskMeta(
    context: Pick<BotContextState, "workspacePath" | "workspaceIdentity">,
    taskId: string,
  ): Promise<CCbuddyTaskMeta | null> {
    const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(context);
    const tasks = await ccbuddyTaskService.listTasks({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    ...
    const entries = (
      await Promise.all(
        workspaces.map(async (workspace) => {
          const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(workspace);
          const tasks = await ccbuddyTaskService
            .listTasks({
              workspacePath: workspace.workspacePath,
              workspaceIdentity: workspace.workspaceIdentity,
            })
  ...
  async function readContextActiveTaskMeta(
    ...
    return (
      (
        await (
          await resolveCCbuddyTaskServiceForContext(context)
        )
          .getTaskSnapshot({
  ...
  async function processProviderCallback(
    ...
    const preparedPayload = callbackBot
      ? ((await adapter.prepareCallbackPayload?.(callbackBot, payload).catch((error: unknown) => ({
          ccbuddyCallbackPrepareError: error instanceof Error ? error.message : String(error),
        }))) ?? payload)
      : payload;
    if (
      isRecord(preparedPayload) &&
      typeof preparedPayload.ccbuddyCallbackPrepareError === "string"
    ) {
      return {
        ok: false,
        replies: [],
        responseBody: { error: preparedPayload.ccbuddyCallbackPrepareError },
        status: 401,
      };
    }
    ...
    const parsePayload =
      isFeishuBotProvider(provider) && isRecord(preparedPayload)
        ? { ccbuddyProvider: provider, ...preparedPayload }
        : preparedPayload;
    ...
    for (const inbound of parsedInboundMessages) {
      ...
      if (bot) {
        ...
        const handledByFeishuSynchronousCardAction =
          ...
          isRecord(preparedPayload) &&
          preparedPayload.ccbuddyFeishuSynchronousCardAction === true &&
          Boolean(outbound[0]);
  ...
  function createAssistantReplyBlocks(
    ...
    changeSummary: CCbuddyTaskMeta["changeSummary"] | null | undefined,
  ): BotAssistantReplyBlock[] {
    const blocks: BotAssistantReplyBlock[] = [];
    const resolvedMode = mode ?? getDefaultBotReplyGranularity();
    const presentation = buildCCbuddyAssistantPresentation({
      content: "",
  ...
  function normalizeBotElicitationQuestions(
    event: Extract<CCbuddyStreamEvent, { type: "elicitation_request" }>,
    locale: Locale | undefined,
  ): CCbuddyElicitationQuestion[] {
  ...
  function mergeElicitationFormValues(
    question: CCbuddyElicitationQuestion,
  ...
  async function submitPendingElicitation(
    ...
    const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(auth.context);
    const submitted = await ccbuddyTaskService.respondElicitation({
      taskId: pending.taskId,
      workspacePath: auth.context.workspacePath,
  ...
  async function handlePendingElicitationValue(
    ...
    if (question.multiSelect) {
      ...
      await writeContext({ ...auth.context, pendingElicitation: nextPending });
      // Bugfix: 多选题在 Bot 里 toggle 后不会触发 CCbuddy Agent response，必须主动同步草稿给 UI。
      await broadcastPendingElicitationProgress(auth.context, nextPending);
      return createElicitationReply(actor, nextPending, auth.locale);
    }
    return advancePendingElicitation(auth, actor, pending, {
  ...
  async function handleElicitationRequest(
    ...
    // Bugfix: Bot 原先只消费 permission_request，没有把 CCbuddy Agent 的
  ...
  async function watchTaskStream(
    ...
    let assistantParts: CCbuddyAssistantMessagePart[] = [];
    ...
    const handleStreamEvent = async (
      event: CCbuddyStreamEvent | TaskStreamMirrorableEvent,
      ...
      if (event.type === "permission_request") {
        ...
        // Bugfix: UI 会把 CCbuddy Agent 原始权限选项规整成“允许/始终允许/拒绝”的固定顺序和文案；
      ...
      if (event.type === "task_complete" || event.type === "task_error") {
        ...
        // Bugfix: CCbuddy Agent 终态事件可能先于 task index/meta 落盘广播到 Bots。
        ...
        const mode = getMode();
        const locale = await readMessageLocale();
        const completedSnapshot = await ccbuddyTaskService
          .getTaskSnapshot({
            taskId: event.taskId,
            workspacePath: context.workspacePath,
            workspaceIdentity: context.workspaceIdentity,
    ...
    const enqueueStreamEvent = (
      ...
      // Bugfix: CCbuddy Agent 事件分发不保证等待 async listener。微信这类离散消息如果并发发送，
    ...
    const dynamicTaskEvent = (
      ccbuddyTaskService as Partial<Pick<ICCbuddyTaskService, "onDynamicTaskEvent">>
    ).onDynamicTaskEvent;
    ...
    const streamDisposable = dynamicTaskEvent
      ...
      : ccbuddyTaskService.onDynamicStreamEvent(context.activeTaskId)(enqueueStreamEvent);
  ...
  function sendPromptInBackground(
    ...
    void resolveCCbuddyTaskServiceForContext(context)
      .then((ccbuddyTaskService) =>
        ccbuddyTaskService.sendPrompt({
          taskId,
          traceId,
          content,
  ...
  async function handleMessage(message: BotInboundMessage): Promise<BotOutboundMessage[]> {
    ...
    if (auth.context.mode === "task" && auth.context.activeTaskId) {
      const taskService = await resolveCCbuddyTaskServiceForContext(auth.context);
    ...
    if (auth.context.mode === "draft" || !auth.context.activeTaskId) {
      ...
      const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(auth.context);
      const task = await ccbuddyTaskService.createTask({
        workspacePath: auth.context.workspacePath,
        workspaceIdentity: auth.context.workspaceIdentity,
      ...
      const taskTitle = deriveTaskTitle(
        ...
        preparedMessage.ccbuddyAttachments,
      );
      ...
      await broadcastTaskListChange(context, task.taskId, "prompt_sent", {
        ...
        prompt: {
          ...
          attachments:
            preparedMessage.ccbuddyAttachments.length > 0
              ? preparedMessage.ccbuddyAttachments
              : undefined,
    ...
    const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(auth.context);
    await ccbuddyTaskService.resumeTask({
      taskId: auth.context.activeTaskId,
      workspacePath: auth.context.workspacePath,
      workspaceIdentity: auth.context.workspaceIdentity,
    });
    ...
    const originalSelection = await ccbuddyTaskService.getTaskModelSelection({
      taskId: auth.context.activeTaskId,
    });
  ...
  async function resolveTaskSelectionEntry(
    ...
    const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(context);
    const snapshot = await ccbuddyTaskService
      .getTaskSnapshot({
        taskId: value.trim(),
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
  ...
  async function isContextActiveTaskRunning(context: BotContextState): Promise<boolean> {
    ...
    const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(context);
    const activeTaskSnapshot = await ccbuddyTaskService
      .getTaskSnapshot({
        taskId: context.activeTaskId,
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
    ...
    if (
      ...
      // CCbuddy Agent 历史任务的 status 为空也可能只是旧数据，不代表 UI 仍在运行；只有本进程确实发起
  ...
  function warnAutomationDeliveryOnce(params: {
    target: CCbuddyAutomationBotDeliveryTarget;
    reason: string;
  }): void {
      ...
      return enqueueInboundProcessing(message.actor, async () => {
        ...
        switch (command.type) {
          ...
          case "model.list": {
            ...
            const activeProvider = normalizeAgentProviderToCCbuddyAgent(active.task.provider);
            if (!activeProvider) {
              return [createOutbound(message.actor, msg(auth.locale, "modelProviderMissing"))];
            }
            const providers = await listModelProviderOptionsForActiveTask(
          ...
          case "model.set": {
            ...
            const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(active.task);
            const configOptions = await ccbuddyTaskService.setModel({
            ...
            const optionSource =
              commandName === "mode" && active.task.provider
                ? await listProviderConfigOptionsForActiveTask(
                    active.task,
                    normalizeAgentProviderToCCbuddyAgent(active.task.provider),
                  )
                : active.configOptions;
            const currentValue =
              commandName === "mode"
            ...
            const currentLabel =
              commandName === "mode"
                ? readConfigSelectLabelForValue(optionSource, commandName, currentValue, {
                    locale: auth.locale,
                    provider: normalizeAgentProviderToCCbuddyAgent(active.task.provider),
                  })
          ...
          case "thoughtLevel.set": {
            ...
            const traceId = generateTraceId(active.taskId);
            const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(auth.context);
            const configOptions = await ccbuddyTaskService.setConfigOption({
              taskId: active.taskId,
              traceId,
          ...
          case "permission.respond": {
            ...
            const ccbuddyTaskService = await resolveCCbuddyTaskServiceForContext(auth.context);
            const submitted = await ccbuddyTaskService.respondPermission({
              taskId: auth.context.activeTaskId,
