...
import { type ICCbuddyTaskService as ICCbuddyTaskServiceShape } from "../session/ccbuddyTaskService.js";
...
export function createBotRemoteWorkspaceService(params: {
  ...
  return {
    ...
    async getCCbuddyTaskService(target: {
      workspacePath: string;
      workspaceIdentity: string;
    }): Promise<ICCbuddyTaskServiceShape | null> {
      return (await getRuntimeServices(target))?.ccbuddyTaskService ?? null;
    },
    ...
    async syncAppRuntimePreferences(preferences: CCbuddyAgentAppRuntimePreferences): Promise<void> {
      ...
      await Promise.all(
        Array.from(runtimeServicesByWorkspaceKey.values()).map((services) =>
          services.ccbuddyAgentService.syncAppRuntimePreferences(preferences),
        ),
      );
    },
    dispose(): void {
  ...
  async function getRuntimeServices(target: {
    ...
    // task wrapper 命令走 ICCbuddyTaskService，session 主状态走 CCbuddy session facade。
    ...
    while (true) {
      ...
      const preferences: CCbuddyAgentAppRuntimePreferences = cachedPreferences
        ? cachedPreferences
      ...
      await services.ccbuddyAgentService.syncAppRuntimePreferences(preferences);
