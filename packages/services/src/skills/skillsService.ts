...
import { DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS } from "@ccbuddy/shared";
...
import { getAppConfigDir, getWorkspaceAppConfigDir, getCCbuddyDataRootDir } from "../paths.js";
...
function getSkillCliSettingsDir(): string {
  return join(getCCbuddyDataRootDir(), "cli");
}
...
const HOME_PREFIX = "~/";
const CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE = "ccbuddy-plugins-official";
const CCBUDDY_INLINE_PLUGIN_MARKETPLACE = "inline";
const CCBUDDY_PLUGIN_MANIFEST_PATH = join(".ccbuddy-plugin", "plugin.json");
...
/** 对齐 apps/ccbuddy-cli/packages/adapters/src/skills/index.ts:19 */
...
async function isUserAgentsSkillCoveredByCCbuddy(params: {
  skillPath: string;
  rootPath: string;
  userCCbuddySkillNameKeys: Set<string>;
}): Promise<boolean> {
  const { skillPath, rootPath, userCCbuddySkillNameKeys } = params;
  if (rootPath !== getUserAgentsSkillRoot()) {
    return false;
  }
  if (
    await exists(join(getUserCCbuddySkillRoot(), basename(dirname(skillPath)), SKILL_FILE_NAME))
  ) {
    return true;
  }
  return userCCbuddySkillNameKeys.has(await readSkillNameKey(skillPath));
}

/**
 * 从 workspacePath 向上走到 worktree 根（含 .git 标记），把每一层的
 * CCbuddy workspace skills 与 `.agents/skills` 都收集起来。
 * 对齐 apps/ccbuddy-cli/packages/adapters/src/skills/roots.ts:60-72。
...
async function resolveAncestorWorkspaceRoots(workspacePath: string): Promise<string[]> {
  ...
  for (const dir of baseDirectories) {
    ...
    // 当成 CCbuddy 目录的 fallback，导致同层自有目录只要有一个技能，`/`、`$` 和设置页
    // 就会整根漏掉 `.agents` 技能，形成“模型可执行但 UI 无法引用”的发现语义分裂。
    roots.push(getWorkspaceCCbuddySkillRoot(dir));
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, CCBUDDY_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginSkillRootDescriptors(): Promise<SkillRootDescriptor[]> {
  ...
  const candidates: PluginRootCandidate[] = [
    ...config.dirs.map((dir) => ({
      defaultEnabled: true,
      marketplace: CCBUDDY_INLINE_PLUGIN_MARKETPLACE,
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献技能。
    if (
      candidate.marketplace === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE &&
...
async function discoverSkills(params: {
  ...
  if (params.includeUserSkills) {
    // 用户级技能是全局资源，CCbuddy skills 里只要存在一个技能就截断
    // `.agents/skills` 会导致外部 Agent 的全局技能在导入后从设置页消失。
    roots.push({
      scope: "user" as const,
      rootPath: getUserCCbuddySkillRoot(),
    });
    roots.push({
      scope: "user" as const,
  ...
  const userCCbuddySkillNameKeys = params.includeUserSkills
    ? await collectSkillNameKeysInRoot(getUserCCbuddySkillRoot())
    : new Set<string>();
  ...
  for (const rootPath of scanRootPaths) {
    ...
    for (const skillPath of skillPaths) {
      if (
        await isUserAgentsSkillCoveredByCCbuddy({
          skillPath,
          rootPath: root.rootPath,
          userCCbuddySkillNameKeys,
        })
      ) {
        continue;
      }
      ...
      // 这些字段不影响 CCbuddy 读取 name/description，继续报 warning 只会制造无操作价值的噪音。
...
function resolveCapabilities(options?: SkillsServiceOptions): SkillsCapability {
  const isDesktopRuntime = options?.isDesktopRuntime ?? Boolean(process.env.CCBUDDY_PROCESS_LABEL);
  if (isDesktopRuntime) {
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async copyToCommon(params: {
      ...
      const commonRoot =
        skill.scope === "workspace"
          ? getWorkspaceCCbuddySkillRoot(params.workspacePath)
    ...
    async removeFromCommon(params: {
      ...
      const userCommonRoot = getUserCCbuddySkillRoot().replaceAll("\\", "/").toLowerCase();
      const workspaceCommonRoot = getWorkspaceCCbuddySkillRoot(params.workspacePath)
        .replaceAll("\\", "/")
        .toLowerCase();
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.ccbuddy/skills/<name>` 下的目录项本身。
      ...
      // 收集工作区各层级（沿 worktree 向上）的 CCbuddy skills 与 .agents/skills，外加用户级两根。
      const allowedRootCandidates = await resolveAncestorWorkspaceRoots(params.workspacePath);
      allowedRootCandidates.push(getUserCCbuddySkillRoot());
