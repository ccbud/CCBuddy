...
import { getMediaPreviewFormat } from "@ccbuddy/shared";
import { packWorkspaceFileEntries } from "@ccbuddy/shared/workspaceFileEntriesCodec";
import type { IFileService, WorkspaceFileSearchParams } from "./file.js";
import { WORKSPACE_FILE_SEARCH_DISPLAY_CAP } from "@ccbuddy/shared/workspaceFileSearch";
...
import {
  resolveWorkspaceFileSearchIgnorePath,
...
const SCRATCH_WORKSPACE_ROOT_NAME = "CCbuddyProject";
 ...
 * 校验 CCbuddy workspace 配置中的 .ccbuddyignore mtime/size 指纹；规则编辑后缓存失效，
 * 保持"编辑规则后下次使用生效"的契约。
 */
...
async function statWorkspaceFileSearchIgnoreFingerprint(rootPath: string): Promise<string> {
  try {
    const fileStat = await stat(resolveWorkspaceFileSearchIgnorePath(rootPath));
...
export function createFileService(options: CreateFileServiceOptions = {}): IFileService {
  ...
  // 全量扫描 + 打包（带 60s TTL / .ccbuddyignore 指纹缓存）。分块 RPC 共享同一份 packed。
