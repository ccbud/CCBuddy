...
export interface HostResourceUsageAgent {
  ...
  children: readonly CCbuddyProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: CCbuddyProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatCCbuddyAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
