...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = CCBUDDY_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = CCBUDDY_COMMAND_AGENT_SOURCES;
...
const CCBUDDY_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "ccbuddyAgent",
  directorySource: "ccbuddy",
  userDirectorySegments: [],
  workspaceDirectorySegments: [],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
function getWorkspaceCommandsRootForDescriptor(
  ...
  return join(workspacePath, ...descriptor.workspaceDirectorySegments);
}
...
function getCommandsRootForStorage(params: {
  ...
  if (params.storageLevel === "project") {
    ...
    return {
      commandsRoot: getWorkspaceCommandsRootForDescriptor(params.descriptor, params.workspacePath),
      scope: "project",
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // CCbuddy Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让用户级 CCbuddy 命令抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === CCBUDDY_COMMAND_AGENT_SOURCE
      ...
      if (params.workspacePath) {
        ...
        await discoverCommandsFromDirectorySources({
          ...
          commandsRootForDescriptor: (descriptor) =>
            getWorkspaceCommandsRootForDescriptor(descriptor, workspacePath),
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === CCBUDDY_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as CCbuddyCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(CCBUDDY_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        CCBUDDY_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, CCBUDDY_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // CCbuddy 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "ccbuddy" && discoveredCount > 0) {
      break;
    }
  }
}
