...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildCCbuddyToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildCCbuddyToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. CCbuddy/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 CCBUDDY_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeCCbuddyRuntimeEnvInPlace(process.env);
