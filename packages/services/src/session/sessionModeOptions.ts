import type { CCbuddyConfigOption, CCbuddyProvider, CCbuddyTaskMode } from "@ccbuddy/shared";

const CANONICAL_SESSION_MODES = new Set<CCbuddyTaskMode>([
  "yolo",
  "plan",
  "edit",
  "auto",
...
): CCbuddyConfigOption | undefined {
  return options.find((option) => option.category === "mode" && option.type === "select");
}

function normalizePersistedSessionMode(
  modeId: string | null | undefined,
  _provider?: CCbuddyProvider,
): CCbuddyTaskMode | undefined {
  const trimmedModeId = readTrimmedString(modeId);
  if (!trimmedModeId) {
    return undefined;
  }

  if (CANONICAL_SESSION_MODES.has(trimmedModeId as CCbuddyTaskMode)) {
    return trimmedModeId as CCbuddyTaskMode;
  }
