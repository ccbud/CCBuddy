...
import type {
  ...
  CCbuddyTaskListItem,
} from "#src/session/ccbuddyTaskListTypes.js";
import type {
  CCbuddyGroupedTaskRef,
  CCbuddyGroupedTaskView,
  CCbuddyGroupedTaskViewNode,
  CCbuddyGroupedTaskViewOrderInput,
  CCbuddyGroupedTaskViewQuery,
  CCbuddyGroupedTaskViewStructure,
  CCbuddyGroupedTaskViewStructureMember,
  CCbuddyGroupedTaskViewStructureTopOrder,
  CCbuddyGroupedTaskViewTopLevelNodeRef,
  CCbuddyTaskGroup,
  CCbuddyTaskGroupColor,
} from "#src/session/ccbuddyTaskListTypes.js";
...
function appendCCbuddyAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: CCbuddyTaskMeta["status"];
  lastError?: CCbuddyTaskMeta["lastError"];
  target?: CCbuddyTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY =
  "__ccbuddy_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: CCbuddyTaskGroupColor = "gray";
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies CCbuddyTaskGroupColor[];
...
function isTerminalTaskStatus(status: CCbuddyTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: CCbuddyTaskMeta | null,
  incomingMeta: CCbuddyTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): CCbuddyTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = ccbuddyTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as CCbuddyTaskMeta),
  ...
  return {
    ...
    mode: row.mode as CCbuddyTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === CCBUDDY_AGENT_PROVIDER ? CCBUDDY_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as CCbuddyTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as CCbuddyTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: CCbuddyTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@ccbuddy/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): CCbuddyTaskListItem {
...
function isTaskGroupColor(value: string): value is CCbuddyTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): CCbuddyTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): CCbuddyTaskGroupColor {
...
function groupedTopNodeOrderRef(node: CCbuddyGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: CCbuddyGroupedTaskViewNode,
  right: CCbuddyGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: CCbuddyTaskListItem, right: CCbuddyTaskListItem): number {
...
function compareGroupTasks(
  left: CCbuddyTaskListItem,
  right: CCbuddyTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendCCbuddyAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): CCbuddyTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: CCbuddyTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: CCbuddyTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 CCbuddy session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 CCbuddy 任务。
   ...
   */
  private ensureCronGroupMembership(meta: CCbuddyTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<CCbuddyTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: CCbuddyTaskMeta): Promise<CCbuddyTaskMeta> {
    await this.ensureReady();
  ...
  async clearTaskUnreadIfMatches(params: {
  ...
  }): Promise<{ meta: CCbuddyTaskMeta; cleared: boolean }> {
    await this.ensureReady();
    return this.enqueueWrite(params, () => {
      ...
      try {
        ...
        const nextMeta: CCbuddyTaskMeta = {
          ...current,
          unreadAt: undefined,
        };
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<CCbuddyTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: CCbuddyTaskListQuery & { provider?: CCbuddyProvider },
  ): Promise<CCbuddyTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<CCbuddyTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: CCbuddyGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: CCbuddyGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: CCbuddyGroupedTaskViewQuery & { provider?: CCbuddyProvider },
  ): Promise<CCbuddyGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 CCbuddy Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendCCbuddyAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
  ...
  }): Promise<CCbuddyGroupedTaskViewStructure> {
    await this.ensureReady();
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: CCbuddyGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: CCbuddyGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: CCbuddyGroupedTaskViewOrderInput & { provider?: CCbuddyProvider },
  ): Promise<CCbuddyGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: CCbuddyGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 CCbuddy Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: CCbuddyGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: CCbuddyGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: CCbuddyGroupedTaskRef[] = [];
