import type {
  CCbuddyAutomation,
  CCbuddyAutomationCreateParams,
  CCbuddyAutomationRun,
  CCbuddyAutomationScheduleRule,
  CCbuddyAutomationUpdateParams,
} from "@ccbuddy/shared";
import { resolveWorkspaceKey } from "@ccbuddy/shared";
...
function assertValidAutomationScheduleRule(rule: CCbuddyAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: CCbuddyAutomationCreateParams): Promise<CCbuddyAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: CCbuddyAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: CCbuddyAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: CCbuddyAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: CCbuddyAutomation; run: CCbuddyAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;
