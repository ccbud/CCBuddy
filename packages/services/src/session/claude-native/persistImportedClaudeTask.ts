...
export function buildSearchableTextFromMessages(messages: CCbuddySessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function persistImportedClaudeTask(params: {
  ...
  const indexMeta: CCbuddyTaskMeta = {
    ...meta,
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.ccbuddy/v2/sessions/{hash}/{taskId}.json。现在真实 session 承担续聊，legacy snapshot
