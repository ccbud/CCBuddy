...
import type { CCbuddyImportableSessionCandidate } from "@ccbuddy/shared";
...
class ClaudeNativeSessionImportRepo {
  private getNativeProjectsRoots(): string[] {
    ...
    // 关键业务逻辑：扫描 Claude Code 原生历史目录 ~/.claude/projects，不是 ccbuddy 自己的数据目录。
    // 当 CCBUDDY_DATA_BASE_DIR 把 .ccbuddy 放到别处时，原生 .claude 往往仍在真实用户 HOME 下。
  ...
  async scanImportableSessions(params: {
    ...
    const candidates: CCbuddyImportableSessionCandidate[] = [];
  ...
  async findImportableSession(params: {
    workspacePath?: string;
    sessionId: string;
  }): Promise<CCbuddyImportableSessionCandidate | null> {
    const workspaceKey = params.workspacePath
      ? normalizePathForComparison(params.workspacePath)
      : null;
  ...
  async copySessionFileToWorkspace(params: {
    ...
    // 导入副本使用 ~/.ccbuddy/v2/agent-config/claude/{workspaceHash}/projects；
