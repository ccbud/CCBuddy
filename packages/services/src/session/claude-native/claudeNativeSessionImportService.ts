...
import type { CCbuddyImportSessionsResult, CCbuddyTaskMeta } from "@ccbuddy/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<CCbuddyTaskMeta>;
  onTaskImported: (meta: CCbuddyTaskMeta) => void;
}): Promise<CCbuddyImportSessionsResult> {
  const normalizedSessionIds = [
    ...new Set(params.sessionIds.map((item) => item.trim()).filter(Boolean)),
  ];
  const result: CCbuddyImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
