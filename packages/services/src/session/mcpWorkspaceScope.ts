...
import type { CCbuddyAgentMcpServer } from "@ccbuddy/shared";
...
function isFilesystemServer(
  server: CCbuddyAgentMcpServer,
): server is Extract<CCbuddyAgentMcpServer, { command: string }> {
  return (
    "command" in server &&
...
export function appendWorkspaceToFilesystemMcpServers(
  mcpServers: CCbuddyAgentMcpServer[] | undefined,
  workspacePath: string,
): CCbuddyAgentMcpServer[] | undefined {
