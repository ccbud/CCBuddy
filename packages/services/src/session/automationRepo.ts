...
import {
  ...
  resolveWorkspaceKey,
  ccbuddyAutomationBotDeliveryTargetSchema,
  modelSelectionSchema,
  ccbuddyTaskModeSchema,
  type CCbuddyAutomation,
  type CCbuddyAutomationBotDeliveryTarget,
  type CCbuddyAutomationCreateParams,
  type CCbuddyAutomationDispatchStatus,
  type CCbuddyAutomationLifecycleStatus,
  type ModelSelection,
  type CCbuddyAutomationRun,
  type CCbuddyAutomationRunDispatchStatus,
  type CCbuddyAutomationRunOutcome,
  type CCbuddyAutomationTrigger,
  type CCbuddyAutomationUpdateParams,
} from "@ccbuddy/shared";
...
function rowToAutomation(row: AutomationRow): CCbuddyAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as CCbuddyAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as CCbuddyAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as CCbuddyAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): CCbuddyAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: CCbuddyAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): CCbuddyAutomation["mode"] | undefined {
  const parsed = ccbuddyTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!ccbuddyTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): CCbuddyAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as CCbuddyAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as CCbuddyAutomationRunDispatchStatus,
    outcome: (row.outcome as CCbuddyAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.ccbuddy/v2（历史脏数据
  ...
  async create(
    params: CCbuddyAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: CCbuddyAutomationLifecycleStatus },
  ): Promise<CCbuddyAutomation> {
  ...
  async getBotDeliveryTarget(
    automationId: string,
    workspaceKey?: string,
  ): Promise<CCbuddyAutomationBotDeliveryTarget | undefined> {
    await this.ensureReady();
    const raw = this.getRow(automationId, workspaceKey)?.bot_delivery_target;
    if (!raw) return undefined;
    try {
      const parsed = ccbuddyAutomationBotDeliveryTargetSchema.safeParse(JSON.parse(raw));
      return parsed.success ? parsed.data : undefined;
    } catch {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<CCbuddyAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: CCbuddyAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: CCbuddyAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<CCbuddyAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<CCbuddyAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: CCbuddyAutomation[] = [];
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: CCbuddyAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: CCbuddyAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<CCbuddyAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<CCbuddyAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
