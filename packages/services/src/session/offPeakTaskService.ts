...
import {
  ...
  type CCbuddyOffPeakTaskCreateParams,
} from "@ccbuddy/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@ccbuddy/provider";
...
function isValidCreateParams(params: CCbuddyOffPeakTaskCreateParams): boolean {
  return (
...
> {
  ...
  if (failureStage === "client_validation") {
    ...
    errorCode = error.biccBuddy === undefined ? "" : String(error.biccBuddy);
    if (error.biccBuddy === 3101) errorCategory = "eligibility_3101";
    else if (error.biccBuddy === 3103) errorCategory = "quota_3103";
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: CCbuddyOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: CCbuddyOffPeakTaskCreateParams = {
      ...params,
    ...
    let created: CCbuddyOffPeakTask;
    try {
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null> {
  ...
  ): Promise<CCbuddyOffPeakTask | null> {
    const existing = await this.deps.repo.get(offPeakTaskId);
  ...
  async list(): Promise<CCbuddyOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null> {
  ...
  ): Promise<CCbuddyOffPeakTask[]> {
    const repaired: CCbuddyOffPeakTask[] = [];
  ...
  private async settleOne(task: CCbuddyOffPeakTask): Promise<void> {
