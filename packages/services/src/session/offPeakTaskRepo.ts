...
/* off-peak 任务仓库：off_peak_tasks 的 sqlite schema、状态机守卫写入与调度认领。
   ...
   禁止往 automations 表或 CCbuddyAutomation 类型上加字段。 */
...
function rowToTask(row: OffPeakTaskRow): CCbuddyOffPeakTask {
  ...
  return {
    ...
    prompt: row.prompt,
    permissionMode: row.permission_mode as CCbuddyTaskMode,
    ...
    status: row.status as CCbuddyOffPeakTaskStatus,
...
function readOffPeakModelSelection(
  row: OffPeakTaskRow,
): CCbuddyOffPeakTask["modelSelection"] | null {
...
function serializeOffPeakModelSelection(
  selection: NonNullable<CCbuddyOffPeakTask["modelSelection"]>,
): string {
...
export class OffPeakTaskRepo {
   ...
   */
  async create(
    params: CCbuddyOffPeakTaskCreateParams,
    options?: {
      /** 测试注入用；缺省 Date.now()。 */
      now?: number;
  ...
  async invalidateModelSelection(
    offPeakTaskId: string,
    modelSelection: NonNullable<CCbuddyOffPeakTask["modelSelection"]>,
    options?: { now?: number },
  ...
  async updateEditableFields(
    ...
    params: {
      ...
      modelSelection?: CCbuddyOffPeakTask["modelSelection"] | null;
    },
    options?: { now?: number },
  ): Promise<CCbuddyOffPeakTask | null> {
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<CCbuddyOffPeakTask[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: CCbuddyOffPeakTask[] = [];
  ...
  async listNonTerminal(): Promise<CCbuddyOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
  ...
  async listUnsettledTerminal(): Promise<CCbuddyOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
