...
import type { CCbuddyAutomation, CCbuddyAutomationScheduleRule } from "@ccbuddy/shared";
...
export function scheduleRuleDefinition(rule: CCbuddyAutomationScheduleRule): string {
...
export function isOneShotAutomation(
  automation: Pick<CCbuddyAutomation, "recurring" | "maxRuns">,
): boolean {
...
export function buildRelativeDelaySchedule(
  delayMinutes: number,
  from = Date.now(),
): Pick<CCbuddyAutomation, "cronExpr" | "scheduleRule"> {
...
export function computeInitialAutomationNextRunAt(
  automation: Pick<CCbuddyAutomation, "cronExpr" | "recurring" | "scheduleRule">,
  from = Date.now(),
): number | null {
...
export function computeScheduleRuleNextRunAt(
  rule: CCbuddyAutomationScheduleRule,
  from = Date.now(),
): number | null {
...
export function inferMinuteIntervalScheduleRule(
  cronExpr: string,
  anchorAt: number,
): CCbuddyAutomationScheduleRule | undefined {
...
export function buildIntervalScheduleRule(
  intervalUnit: CCbuddyAutomationScheduleRule["unit"],
  interval: number,
  cronExpr: string,
  anchorAt: number,
...
export function computeAutomationNextRunAt(
  automation: Pick<CCbuddyAutomation, "cronExpr" | "scheduleRule">,
  from?: number,
): number | null {
