import type {
  ...
  CCbuddyOffPeakTask,
  CCbuddyOffPeakTaskCreateParams,
  ModelSelection,
} from "@ccbuddy/shared";
...
export interface IOffPeakTaskService {
  ...
  createTask(params: CCbuddyOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<CCbuddyOffPeakTask | null>;
  list(): Promise<CCbuddyOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<CCbuddyOffPeakTask | null>;
}
