/* eslint-disable max-lines -- CCbuddy task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
...
import type {
  ...
  CCbuddyTaskCreateResult,
  CCbuddyTaskMeta,
  CCbuddyStreamEvent,
  CCbuddyTaskMode,
  CCbuddyConfigOption,
  CCbuddyError,
  CCbuddyAssistantMessageFeedback,
  CCbuddyPromptAttachment,
  CCbuddyProvider,
  CCbuddyWorkspaceEvent,
  TraceId,
  CCbuddySessionFile,
  CCbuddyAgentMcpServer,
  CCbuddyTaskSnapshot,
  CCbuddyTaskSnapshotBody,
  CCbuddyTaskSnapshotToolCallsSlice,
  CCbuddyTaskSnapshotRefContent,
  CCbuddyEnqueueTaskCommandResult,
  CCbuddyCancelTaskCommandResult,
  CCbuddyTaskClientMode,
  CCbuddyTaskTokenUsageResult,
...
import type {
  ...
  CCbuddyTaskListSortBy,
  CCbuddyTaskListWorkspaceScope,
...
export interface CCbuddyTaskSnapshotWithEtagResult {
  snapshot: CCbuddyTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface CCbuddyArchivedTaskDeletionResult {
...
export type CCbuddyModelTrajectoryContentPart =
...
export interface CCbuddyModelTrajectoryMessage {
  role: string;
  parts: CCbuddyModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface CCbuddyModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type CCbuddyModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface CCbuddyModelTrajectoryCallSource {
  kind: CCbuddyModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface CCbuddyModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: CCbuddyModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: CCbuddyModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: CCbuddyModelTrajectoryContentPart[];
    usage?: CCbuddyModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface CCbuddyModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 CCbuddy Agent 落盘 model-io）。 */
  available: boolean;
  records: CCbuddyModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface CCbuddyTaskTerminalOutcome {
  taskId: string;
...
export interface CCbuddyTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * ICCbuddyTaskService — CCbuddy task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * CCbuddy Agent server 维护，新功能应优先走 ICCbuddySessionService。
 */
export interface ICCbuddyTaskService {
  // ---- 生命周期 ----
  ...
  /** 创建 CCbuddy session 并同步 task 索引。 */
  createTask(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    provider?: CCbuddyProvider;
    ...
    v4Create?: boolean;
  }): Promise<CCbuddyTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<CCbuddyEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: CCbuddyTaskClientMode;
  }): Promise<CCbuddyCancelTaskCommandResult>;
  ...
  }): Promise<CCbuddySessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<CCbuddySessionGoalResult>;
   ...
   */
  listTaskList(params: CCbuddyTaskListQuery): Promise<CCbuddyTaskListResult>;
  ...
  applyGroupedTaskViewOrder(
    params: CCbuddyGroupedTaskViewOrderInput,
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<CCbuddyTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<CCbuddyTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<CCbuddyTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<CCbuddyTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<CCbuddyTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<CCbuddyConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: CCbuddyAssistantMessageFeedback | null;
  }): Promise<CCbuddySessionFile>;
  ...
  /** 获取 CCbuddy Agent 当前结构化日志文件路径。 */
  getTaskNativeSessionLogFile(params: {
    taskId: string;
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<{
    provider: CCbuddyProvider | null;
    path: string | null;
    exists: boolean;
  }>;

  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.ccbuddy/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 CCbuddy Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<CCbuddyModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<CCbuddyTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 CCbuddy Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  deleteArchivedTasks(params: {
    ...
    workspaceIdentity?: string;
    taskIds: string[];
  }): Promise<CCbuddyArchivedTaskDeletionResult>;
  ...
  onDynamicWorkspaceEvent(
    workspace: string | CCbuddyWorkspaceEventSubscriptionParams,
  ): Event<CCbuddyWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<CCbuddyError>;
}

export const ICCbuddyTaskService = createServiceDescriptor<ICCbuddyTaskService>(
  ServiceChannels.CCbuddyTask,
);
