...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserCCbuddySkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserCCbuddySkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userCCbuddySkillRoot = getUserCCbuddySkillRoot();
  const rawCCbuddyCandidates = await collectUserSkillCandidatesInRoot(userCCbuddySkillRoot);
  const ccbuddyDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userCCbuddySkillRoot);
  const ccbuddySkillNameKeys = new Set(
    rawCCbuddyCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const ccbuddyCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawCCbuddyCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !ccbuddyDirectoryNames.has(candidate.directoryName) &&
        !ccbuddySkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...ccbuddyCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserCCbuddySkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserCCbuddySkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 CCbuddy skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 CCbuddy 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
