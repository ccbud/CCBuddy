import type { AppSettings } from "@ccbuddy/shared";
...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "ccbuddyEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.ccbuddyEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedCCbuddyEndpointOrigin = normalizedPatch.ccbuddyEndpointOrigin.trim();
    normalizedPatch.ccbuddyEndpointOrigin =
      trimmedCCbuddyEndpointOrigin.length > 0 ? trimmedCCbuddyEndpointOrigin : undefined;
  }
