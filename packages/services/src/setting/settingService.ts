...
const DEFAULT_PROJECT_NAME = "CCbuddyProject";
...
const debugLog = (...args: unknown[]) => {
  // NODE_ENV 来自用户 shell 时会误导服务层 debug 开关；统一使用 CCBUDDY_RUNTIME_ENV。
...
function resolveUserHomeDir() {
  ...
  const envHome =
    process.env.CCBUDDY_DESKTOP_HOME_DIR?.trim() ||
...
function getSettingsDir() {
  return join(resolveUserHomeDir(), ".ccbuddy", "v2");
}
...
export function createSettingServiceWithMigrations(): {
  ...
  const service: ISettingService = {
    ...
    async updateDataBaseDir(newDir: string | undefined): Promise<void> {
      ...
      if (!validation.ok) {
        // Windows 安装目录由安装器/自动更新管理，把 .ccbuddy/v2 放进去可能在升级时被覆盖。
