...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@ccbuddy/shared";

let _dataBaseDir: string | null = null;
export const CCBUDDY_WINDOWS_APP_INSTALL_DIR_ENV = "CCBUDDY_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.CCBUDDY_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env CCBUDDY_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.ccbuddy. Keep the exported name for existing callers. */
export function getCCbuddyDataRootDir(): string {
  return join(getDataBaseDir(), ".ccbuddy");
}

/** 非项目对话共享的真实工作目录；默认 ~/.ccbuddy/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getCCbuddyDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.ccbuddy/v2 */
export function getAppConfigDir(): string {
  return join(getCCbuddyDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, CCBUDDY_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "CCbuddy") : null,
    programFilesX86 ? win32.join(programFilesX86, "CCbuddy") : null,
    programW6432 ? win32.join(programW6432, "CCbuddy") : null,
    localAppData ? win32.join(localAppData, "Programs", "CCbuddy") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getCCbuddyDataRootDir(), "export-log-stage");
}

export function getExportLogDir(): string {
  return join(getCCbuddyDataRootDir(), "export-log");
}

export function getFeedbackRootDir(): string {
  return join(getCCbuddyDataRootDir(), "feedback");
}
...
export function getGitCheckpointIndexRootDir(): string {
  return join(getCCbuddyDataRootDir(), "git-checkpoint-index");
}

/** ~/.ccbuddy/v2/tasks-index.sqlite */
...
/** 与 CCbuddy session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** CCbuddy owns workspace-scoped application state outside the selected project. */
export function getWorkspaceAppConfigDir(
...
/** ~/.ccbuddy/v2/sessions/{workspaceHash} */
...
/** ~/.ccbuddy/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.ccbuddy/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .ccbuddy/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".ccbuddy", "v2");
  const newDir = join(newBaseDir, ".ccbuddy", "v2");
