...
import type { CCbuddyArchivedTaskDeletionResult } from "#src/session/ccbuddyTaskService.js";
import type {
  CCbuddyTaskListItem,
  CCbuddyTaskListQuery,
  CCbuddyTaskListResult,
} from "../session/ccbuddyTaskListTypes.js";
...
export type WindowHostControllerTaskListItem = CCbuddyTaskListItem & {
...
export interface WindowHostControllerTaskListResult extends Omit<CCbuddyTaskListResult, "items"> {
...
export interface IWindowControllerService {
  ...
  listTaskList(params: CCbuddyTaskListQuery): Promise<WindowHostControllerTaskListResult>;
  mutateTask(params: {
    address: WindowHostTaskAddress;
