...
// to @ccbuddy/ccbuddy-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @ccbuddy/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @ccbuddy/services root
...
// at compile time; Vite never resolves @ccbuddy/ccbuddy-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@ccbuddy/ccbuddy-cua/broker";
...
} from "@ccbuddy/ccbuddy-cua/broker/ports";
