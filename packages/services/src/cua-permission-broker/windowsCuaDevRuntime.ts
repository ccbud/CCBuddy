...
const DEV_ROOT_ENV = "CCBUDDY_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@ccbuddy/ccbuddy-cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.ccbuddyCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid ccbuddyCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
