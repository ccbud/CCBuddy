...
import { readLegacyCCbuddyConfigProviders } from "../src/model-provider/legacyCCbuddyConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyCCbuddyConfigProviders();
    },
...
test("startup migrates published CCbuddy config into personal config without changing the source", async () => {
