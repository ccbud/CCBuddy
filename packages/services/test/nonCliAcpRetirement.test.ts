...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  const service = createCCbuddyTaskServiceAdapter({
    ccbuddyAgentService: {
      ...
      disposeAll() {},
    } as unknown as Options["ccbuddyAgentService"],
    taskIndexRepo: {
      async getTaskMeta() {
        return meta;
      },
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-provider-current-"));
  ...
  try {
    ...
    assert.ok(config.ccbuddyBuiltinRevision);
