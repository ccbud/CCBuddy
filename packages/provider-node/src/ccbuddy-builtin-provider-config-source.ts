...
import type { ProviderConfigLayerSnapshot, ProviderSource } from "@ccbuddy/provider";
import { atomicWritePrivateTextFile, withFileLock } from "@ccbuddy/shared/node";
import {
  decodeCCbuddyBuiltinRelease,
  encodeCCbuddyBuiltinRelease,
  serializeCCbuddyBuiltinRelease,
  type CCbuddyBuiltinRelease,
} from "./ccbuddy-builtin-release.js";

export interface NodeCCbuddyBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyCCbuddyBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeCCbuddyBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeCCbuddyBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("CCbuddy Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: CCbuddyBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(
    release: CCbuddyBuiltinRelease,
  ): Promise<ApplyCCbuddyBuiltinReleaseResult> {
    this.#assertNotDisposed();
    await this.#ensureWatcher();
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeCCbuddyBuiltinRelease(release) === serializeCCbuddyBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`CCbuddy Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<CCbuddyBuiltinRelease> {
  ...
  async #writeActiveLocked(release: CCbuddyBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeCCbuddyBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeCCbuddyBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeCCbuddyBuiltinProviderConfigSource(
  options: NodeCCbuddyBuiltinProviderConfigSourceOptions,
): NodeCCbuddyBuiltinProviderConfigSource {
  return new NodeCCbuddyBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: CCbuddyBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeCCbuddyBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
...
): CCbuddyBuiltinRelease {
  if (
    bundled?.release &&
    active?.release &&
    bundled.release.revision === active.release.revision &&
    serializeCCbuddyBuiltinRelease(bundled.release) !==
      serializeCCbuddyBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is CCbuddyBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active CCbuddy Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  ...
  return Object.freeze({
    revision: `ccbuddy-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: CCbuddyBuiltinRelease): string {
  return `${release.revision}:${serializeCCbuddyBuiltinRelease(release)}`;
}
