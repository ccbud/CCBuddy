...
import type { NodeCCbuddyBuiltinProviderConfigSource } from "./ccbuddy-builtin-provider-config-source.js";
import type { CCbuddyBuiltinRelease } from "./ccbuddy-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type CCbuddyBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface CCbuddyBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: CCbuddyBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface CCbuddyBuiltinRefreshEvent {
  readonly result: CCbuddyBuiltinRefreshResult;
...
export class CCbuddyBuiltinRemoteSynchronizer {
  readonly #options: CCbuddyBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<CCbuddyBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: CCbuddyBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<CCbuddyBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<CCbuddyBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("CCbuddy Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: CCbuddyBuiltinRefreshEvent): void {
