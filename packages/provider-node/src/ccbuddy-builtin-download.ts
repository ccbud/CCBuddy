...
export interface CCbuddyBuiltinDownloadOptions {
...
export async function downloadCCbuddyBuiltinRelease(
  options: CCbuddyBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeCCbuddyBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`CCbuddy Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
