import { ProxyChannel, type IChannelClient } from "@ccbuddy/rpc";
import {
  IFileService,
...
export class RemoteServiceAccess implements IServiceAccessor {
  ...
  readonly ccbuddyTaskService: ICCbuddyTaskService;
  readonly windowControllerService: IWindowControllerService;
  readonly ccbuddyAgentService: ICCbuddyAgentService;
  readonly ccbuddySessionService: ICCbuddySessionService;
  ...
  constructor(channelClient: IChannelClient) {
    ...
    this.ccbuddyTaskService = ProxyChannel.toService<ICCbuddyTaskService>(
      channelClient.getChannel(ICCbuddyTaskService.channelName),
    );
    ...
    this.ccbuddyAgentService = ProxyChannel.toService<ICCbuddyAgentService>(
      channelClient.getChannel(ICCbuddyAgentService.channelName),
    );
    this.ccbuddySessionService = ProxyChannel.toService<ICCbuddySessionService>(
      channelClient.getChannel(ICCbuddySessionService.channelName),
    );
