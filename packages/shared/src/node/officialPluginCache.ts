...
import { CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID } from "../plugin-marketplaces.js";
...
/**
 * 扫描 `<plugins storage>/cache/ccbuddy-plugins-official/<name>/<version>/`。
...
export async function scanOfficialPluginCacheRoots(
  ...
  const cacheRoot = join(pluginStorageRoot, "cache", CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID);
  let pluginEntries;
  try {
