export type CCbuddyAgentBinaryKind = "native-binary";

export interface CCbuddyAgentRuntimeDescriptor {
  binaryKind: CCbuddyAgentBinaryKind;
  binaryEnvVar: string;
  ...
  /**
   * 桌面端把 agent 的 JS bundle（ccbuddy.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const CCBUDDY_AGENT_RUNTIME: CCbuddyAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".ccbuddy/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[CCbuddy Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("ccbuddy-agent", platform)],
  nodeBundleEntryFile: "ccbuddy.cjs",
  resolveNodeBundleSegments() {
    return [this.nodeBundleEntryFile];
  },
};

export function getCCbuddyAgentRuntime(): CCbuddyAgentRuntimeDescriptor {
  return CCBUDDY_AGENT_RUNTIME;
}
