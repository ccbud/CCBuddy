...
import { normalizeCCbuddyEndpointOrigin } from "./ccbuddyEndpoint.js";
import {
...
const ccbuddyInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const ccbuddyEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeCCbuddyEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeCCbuddyEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("ccbuddyEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = ccbuddyEndpointOriginSchema.safeParse(raw.ccbuddyEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, ccbuddyEndpointOrigin: parsed.data };
  }
  const { ccbuddyEndpointOrigin: _ccbuddyEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  ccbuddyInteractionBehavior: ccbuddyInteractionBehaviorSchema.default("queue"),
  ...
  ccbuddyEndpointOrigin: ccbuddyEndpointOriginSchema.optional(),
});
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeCCbuddyEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  ccbuddyInteractionBehavior: ccbuddyInteractionBehaviorSchema.optional(),
