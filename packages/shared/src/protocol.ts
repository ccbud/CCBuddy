...
/** CCbuddy 运行中继续输入时的交互行为 */
export type CCbuddyInteractionBehavior = "queue" | "guide";
...
export interface ResourceUsageProcess {
  pid: number;
  /** 进程显示名，如 ccbuddy-main / ccbuddy-agent-ccbuddy-demo / node_repl */
  name: string;
...
export interface AppSettings {
   ...
   * 只影响内置浏览器出口，不影响 CCbuddy 自身对后端与模型 API 的请求。默认关闭，重启后生效。
   */
  ...
  /** CCbuddy 运行中继续输入时，是排队到下一轮，还是引导到下一次工具调用后运行 */
  ...
  /** 数据目录的根路径（替代 homedir），默认为 os.homedir()；应用目录为 .ccbuddy */
  dataBaseDir?: string;
  ...
  /** CCbuddy 自有发行线的待展示版本说明；与 CCbuddy 的 pending 字段隔离。 */
  ccbuddyPendingPostUpdateReleaseNotes?: {
    version: string;
    title: string;
    markdown: string;
    releaseDate?: string;
  ...
  /** 设置页里的临时 endpoint override；正式/测试默认 base url 由 CCBUDDY_BASE_URL env 管理。 */
