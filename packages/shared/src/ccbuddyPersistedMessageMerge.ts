...
// CCbuddy runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveCCbuddyAssistants(
  messages: readonly CCbuddyPersistedMessage[],
): CCbuddyPersistedMessage[] {
  const merged: CCbuddyPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: CCbuddyPersistedMessage,
  next: CCbuddyPersistedMessage,
): CCbuddyPersistedMessage {
  ...
  const shiftedNextParts: CCbuddyPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
