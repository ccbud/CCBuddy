...
import { CCBUDDY_AGENT_PROVIDER, CCBUDDY_AGENT_PROVIDER_LABEL } from "./ccbuddy-agent-policy.js";
import type {
  CCbuddyConfigOption,
 ...
 */
export const ccbuddyAutomationBotDeliveryTargetSchema = z
  .object({
...
export type CCbuddyAutomationBotDeliveryTarget = z.infer<
  typeof ccbuddyAutomationBotDeliveryTargetSchema
>;
...
export interface BotTaskSummary {
  taskId: string;
  title: string;
  status: CCbuddyTaskRuntimeStatus | "persisted-completed" | "persisted-error" | "unknown";
  workspacePath: string;
  workspaceIdentity?: string;
  provider?: CCbuddyProvider;
  model?: string;
...
export interface BotTaskBroadcastPayload {
  ...
  permissionRequest?: CCbuddyPermissionRequest;
  elicitationRequest?: CCbuddyElicitationRequest;
  requestId?: string;
  error?: string;
}
...
export const botCurrentOptionsSchema = z
  .object({
    ...
    // 兼容旧 bot-config.json；CLI provider 现在统一由 CCbuddy Protocol 侧配置决定。
    cli: z.literal(CCBUDDY_AGENT_PROVIDER).optional(),
  })
  .strict();

export const botDraftOptionsSchema = z
  .object({
    provider: z.literal(CCBUDDY_AGENT_PROVIDER),
    modelSelection: modelSelectionSchema.optional(),
    mode: z.string().min(1).optional(),
  })
  .strict();
...
export const BOT_CCBUDDY_PROVIDER_OPTIONS: Array<{
  id: CCbuddyProvider;
  label: string;
}> = [{ id: CCBUDDY_AGENT_PROVIDER, label: CCBUDDY_AGENT_PROVIDER_LABEL }];
