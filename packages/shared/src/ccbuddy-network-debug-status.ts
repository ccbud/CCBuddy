import type {
  ...
  TraceId,
  CCbuddyStreamEvent,
  CCbuddyTaskNetworkDebugStatusType,
} from "./ccbuddy-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<CCbuddyTaskNetworkDebugStatusType>([
...
export function ccbuddyTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<CCbuddyStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): CCbuddyTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as CCbuddyTaskNetworkDebugStatusType)
    : undefined;
}
