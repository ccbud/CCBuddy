...
import { z } from "zod";
import type { CCbuddyTaskMigrationSource, CCbuddyTaskMode } from "./ccbuddy-task-types-core.js";
import { ccbuddyAgentProviderSchema } from "./ccbuddy-agent-policy.js";
import { ccbuddyPermissionResponseSchema } from "./ccbuddy-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./ccbuddy-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./ccbuddy-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const ccbuddyTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 CCbuddyTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(ccbuddyTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: ccbuddyAgentProviderSchema.optional(),
  migrationSource: z.enum(ccbuddyTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: ccbuddyTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const ccbuddyPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const ccbuddyTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const ccbuddyTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  ccbuddyTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: ccbuddyTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: ccbuddyTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
