export type CCbuddyBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface CCbuddyBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: CCbuddyBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<CCbuddyBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = CCbuddyBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveCCbuddyBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): CCbuddyBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // CCbuddy Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseCCbuddyBackgroundTaskControlItems(
  value: unknown,
): CCbuddyBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, CCbuddyBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveCCbuddyBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: CCbuddyBackgroundTaskControlItem = {
      jobId,
...
export function isActiveCCbuddyBackgroundTaskControlItem(
  job: CCbuddyBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getCCbuddyBackgroundTaskControlItemElapsedMs(
  job: CCbuddyBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleCCbuddyBackgroundTaskControlItems(
  jobs: readonly CCbuddyBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<CCbuddyBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveCCbuddyBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getCCbuddyBackgroundTaskControlItemElapsedMs(job, now),
    }))
