...
// 禁止往 CCbuddyAutomation 上加字段。sqlite 列名 snake_case，此处为跨域 camelCase 领域类型。
 ...
 */
export type CCbuddyOffPeakTaskStatus =
  | "queued"
  | "paused"
  | "running"
  | "completed"
...
export function isOffPeakTerminalStatus(
  status: CCbuddyOffPeakTaskStatus,
 ...
 * 稳定错误标记。ccbuddy-cli 适配层把该业务码分类为不可重试失败并在错误消息里嵌入本标记；
 * host 终态回写据此改走"同 task_id 重新取号 → resume 续跑"而非落 failed。
 * 跨进程只能靠错误文本传递，标记必须全链路唯一且稳定，勿改；与
 * apps/ccbuddy-cli/packages/adapters/src/model/offpeak-retry.ts 的同名常量跨包同值。
 */
...
export interface CCbuddyOffPeakTask {
  ...
  /** 表单 Instructions。 */
  prompt: string;
  /** 权限四档全开放，映射现有 CCbuddyTaskMode，默认 "default"（Ask for approval）。 */
  permissionMode: CCbuddyTaskMode;
  /**
...
export interface CCbuddyOffPeakTaskCreateParams {
  title: string;
  prompt: string;
  permissionMode: CCbuddyTaskMode;
  modelSelection: ModelSelection;
