...
// 消费者：ccbuddyTaskServiceAdapter/ccbuddySessionProjection/ccbuddyTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromCCbuddyMessageParts 已迁
// ccbuddy-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { CCbuddySessionGoal } from "./ccbuddy-protocol/index.js";
...
export function isCCbuddyGoalContinuationReminderText(text: string): boolean {
...
export function isCCbuddyGoalContinuationReminderMessage(
  ...
  return (
    ...
    (message.info.source === "goal-continuation" ||
      ...
      isCCbuddyGoalContinuationReminderText(textFromCCbuddyMessageParts(message.parts)))
  );
}

export function isCCbuddyGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isCCbuddyGoalModelOnlyReminderMessage(message: CCbuddyMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isCCbuddyGoalContinuationReminderMessage(message) ||
      isCCbuddyGoalStateReminderText(textFromCCbuddyMessageParts(message.parts)))
  );
}

export function isCCbuddyModelOnlySyntheticUserMessage(message: CCbuddyMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isCCbuddyCompactSummaryMessage(message: CCbuddyMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getCCbuddyUserVisibleMessages(
  messages: readonly CCbuddyMessageWithParts[],
  _options: { target?: CCbuddySessionGoal | null } = {},
): CCbuddyMessageWithParts[] {
  const visibleMessages: CCbuddyMessageWithParts[] = [];
  for (const message of messages) {
    if (
      isCCbuddyModelOnlySyntheticUserMessage(message) ||
      isCCbuddyCompactSummaryMessage(message)
    ) {
...
export function getCCbuddyGoalIterationByAssistantMessageId(
  messages: readonly CCbuddyMessageWithParts[],
  options: { maxGoalIteration?: number; target?: CCbuddySessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareCCbuddyMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isCCbuddyGoalContinuationReminderMessage(message)) {
...
export interface CCbuddyGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getCCbuddyGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly CCbuddyGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareCCbuddyMessagesByCreatedTime(
  left: CCbuddyMessageWithParts,
  right: CCbuddyMessageWithParts,
) {
...
export function resolveCCbuddyVisibleSessionTitle(input: {
  title?: string;
  messages: readonly CCbuddyMessageWithParts[];
  target?: CCbuddySessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isCCbuddyGoalContinuationReminderText(normalizedTitle) &&
    !isCCbuddyGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isCCbuddyGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromCCbuddyMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
