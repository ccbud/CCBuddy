...
// 透传到 JSON-RPC `error.data.code`；客户端读回时用 readCCbuddyAttachmentFaultCode。

export const CCBUDDY_ATTACHMENT_FAULT_CODES = {
  /** host 未实现 stat 能力。 */
...
export type CCbuddyAttachmentFaultCode =
  (typeof CCBUDDY_ATTACHMENT_FAULT_CODES)[keyof typeof CCBUDDY_ATTACHMENT_FAULT_CODES];

const KNOWN_FAULT_CODES = new Set<string>(Object.values(CCBUDDY_ATTACHMENT_FAULT_CODES));

export function isCCbuddyAttachmentFaultCode(value: unknown): value is CCbuddyAttachmentFaultCode {
 ...
 */
export class CCbuddyAttachmentFaultError extends Error {
  readonly code: CCbuddyAttachmentFaultCode;

  constructor(code: CCbuddyAttachmentFaultCode, options?: { cause?: unknown; message?: string }) {
    // message 默认就是 fault 码本身，旧版本客户端的文本匹配仍能命中。
    super(
      options?.message ?? code,
      options?.cause === undefined ? undefined : { cause: options.cause },
    );
    this.name = "CCbuddyAttachmentFaultError";
    this.code = code;
  }
}
...
/**
 * 从任意错误中读出附件 fault 码：
 * - 同进程抛出的 CCbuddyAttachmentFaultError / 带 code 的 Error；
 * - 跨 JSON-RPC 回传的 `error.data.code`；
 * - 旧版本 CLI 只有消息文本时的兼容兜底（见下方注释）。
 */
export function readCCbuddyAttachmentFaultCode(
  error: unknown,
): CCbuddyAttachmentFaultCode | undefined {
  if (!error || typeof error !== "object") return undefined;
  const candidate = error as { code?: unknown; data?: unknown };
  if (isCCbuddyAttachmentFaultCode(candidate.code)) return candidate.code;
  if (candidate.data && typeof candidate.data === "object") {
    const data = candidate.data as { code?: unknown };
    if (isCCbuddyAttachmentFaultCode(data.code)) return data.code;
  }
  // 兼容兜底：桌面端可能连接尚未带结构化 code 的旧 ccbuddy-cli。仅在消息「整体等于」
  // 某个 fault 码时命中，不做模糊匹配，避免把包装后的任意文本误判成确定分类。
  if (error instanceof Error && isCCbuddyAttachmentFaultCode(error.message)) return error.message;
  return undefined;
}
