...
// 下发，host 侧 ccbuddyTaskIndexSyncer 转成 workspace_config_options_update 广播给 UI；
...
// 与 host 侧 CCbuddyConfigSelectValue（ccbuddy-task-types-core）结构对齐：
// syncer 转发 workspace_config_options_update 时零映射直通，下游 useCCbuddyConfig 消费面不改。
...
// 与 host 侧 CCbuddyConfigOption 结构对齐（id: model / mode / thought_level / 自定义）。
...
// slash 命令目录（workspace 级；与 host 侧 CCbuddySlashCommand 对齐）。
