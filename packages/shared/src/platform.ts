...
export const BROWSER_VIEW_RESTORE_BOOTSTRAP_URL = "ccbuddy-browser-restore://pending";
...
export const LOCAL_MEDIA_PREVIEW_SCHEME = "ccbuddy-media";
...
export interface CCbuddyStdioTapDevState {
  enabled: boolean;
  visible: boolean;
  logDir: string;
  statePath: string;
...
export const DesktopCommandIds = {
  ...
  ToggleCCbuddyStdioTapDevProxy: "toggleCCbuddyStdioTapDevProxy",
  SetCCbuddyEndpointProduction: "setCCbuddyEndpointProduction",
  SetCCbuddyEndpointTest: "setCCbuddyEndpointTest",
  SetCCbuddyEndpointCustom: "setCCbuddyEndpointCustom",
  ResetCCbuddyEndpoint: "resetCCbuddyEndpoint",
...
export interface IPlatformService {
  ...
  /** 打开 CCbuddy Computer Use 的完整权限引导。Desktop only。 */
  ...
  /** 注册 `ccbuddy://share/import?code=...` 导入意图。 */
  ...
  /** 导出日志：打包 ~/.ccbuddy/v2 及外部 agent 日志为 zip 并在系统文件浏览器中显示 */
  ...
  /** Native menu requests a history view in the main workspace region. */
  onOpenHistoryViewRequested?(listener: (view: "list" | "timeline") => void): () => void;

  /** Versioned, validated, read-only history data for trusted desktop main windows. */
  historyReview?: {
    readonly protocolVersion: 1;
    list(): Promise<unknown>;
    load(id: string): Promise<unknown>;
    refresh(): Promise<unknown>;
    onRefreshEvent(listener: (event: unknown) => void): () => void;
  };
