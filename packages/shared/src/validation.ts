...
import { z } from "zod";
import { ccbuddyProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { ccbuddyProviderSchema } from "./providers.js";
import { ccbuddyAgentProviderSchema } from "./ccbuddy-agent-policy.js";
...
} from "./ccbuddy-protocol/index.js";
import { ccbuddyTaskModeSchema } from "./ccbuddy-task-mode-schema.js";
import { PROTOCOL_V4_LIMITS } from "./ccbuddy-protocol-v4/core.js";
import { errorAttributionSchema } from "./ccbuddy-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./ccbuddy-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { ccbuddyTaskModeSchema } from "./ccbuddy-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("ccbuddy-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  ccbuddyBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 CCbuddyTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export const hostAgentProcessSpawnedResponseSchema = z.object({
  ...
  provider: ccbuddyProviderSchema,
  workspacePath: nonEmptyStringSchema,
  command: z.string(),
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: ccbuddyProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = ccbuddyProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `ccbuddyProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: ccbuddyMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: ccbuddyToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: ccbuddyMcpTelemetryEventSchema,
  })
  .strict();
...
export const ccbuddyTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const ccbuddyPromptImageAttachmentSchema = z.object({
...
export const ccbuddyPromptVideoAttachmentSchema = z.object({
...
export const ccbuddyPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const ccbuddyPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const ccbuddyPromptAttachmentSchema = z.discriminatedUnion("kind", [
  ccbuddyPromptImageAttachmentSchema,
  ccbuddyPromptVideoAttachmentSchema,
  ccbuddyPromptPdfAttachmentSchema,
  ccbuddyPromptFileAttachmentSchema,
]);

export const ccbuddyPersistedToolCallSchema = z.object({
...
const ccbuddyPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const ccbuddyPersistedMessageSchema = z.object({
  ...
  attachments: z.array(ccbuddyPromptAttachmentSchema).optional(),
  tools: z.array(ccbuddyPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(ccbuddyPersistedMessagePartSchema).optional(),
...
export const ccbuddyTaskGoalStatusSchema = z.enum([
  "active",
  "paused",
  "budget_limited",
  "complete",
...
export const ccbuddyTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const ccbuddyTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const ccbuddyTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: ccbuddyTaskGoalStatusSchema,
...
export const ccbuddyTaskGoalChangedPatchSchema = z.object({
  action: ccbuddyTaskTargetChangedActionSchema,
  source: ccbuddyTaskTargetChangedSourceSchema,
  target: ccbuddyTaskGoalSchema.nullable(),
  previousTarget: ccbuddyTaskGoalSchema.nullable().optional(),
});

export const ccbuddyTaskTargetStatusSchema = ccbuddyTaskGoalStatusSchema;
export const ccbuddyTaskTargetSchema = ccbuddyTaskGoalSchema;
export const ccbuddyTaskTargetChangedPatchSchema = ccbuddyTaskGoalChangedPatchSchema;

export const ccbuddyTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: ccbuddyTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: ccbuddyAgentProviderSchema.optional(),
  migrationSource: ccbuddyTaskMigrationSourceSchema.optional(),
  ...
  status: ccbuddyTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: ccbuddyTaskGoalSchema.nullable().optional(),
});

export const ccbuddyTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const ccbuddyPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(ccbuddyTaskIndexEntrySchema),
});

const ccbuddyPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const ccbuddyPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(ccbuddyPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const ccbuddyPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const ccbuddySessionFileSchema = z.object({
  meta: ccbuddyTaskMetaSchema,
  messages: z.array(ccbuddyPersistedMessageSchema),
  fileChanges: z.array(ccbuddyPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(ccbuddyPersistedTurnCheckpointSchema).optional(),
});
