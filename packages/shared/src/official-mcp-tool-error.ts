 ...
 * 见 ccbuddy-server `internal/domain/servermcp/toolerror.go` 的 `ToolError.Error()`。
 *
 * 放在 shared 是因为有三个分属不同包的消费者：
 * - `apps/ccbuddy-cli/packages/core`：解析 MCP 结果，把 code 带进 tool result display；
 * - `packages/shared/src/ccbuddy-protocol-v4/rows.ts`：row schema 校验该 code；
