export const CCBUDDY_KNOWN_TOOL_NAMES = [
  "Read",
  "Write",
  "Edit",
  "ApplyPatch",
...
export type CCbuddyKnownToolName = (typeof CCBUDDY_KNOWN_TOOL_NAMES)[number];

export type CCbuddyToolFamily =
  | "file-read"
  | "file-write"
  | "shell"
  | "search"
...
const TOOL_FAMILY_BY_NAME: Record<CCbuddyKnownToolName, CCbuddyToolFamily> = {
  Read: "file-read",
  Write: "file-write",
  Edit: "file-write",
...
const TOOL_NAME_BY_LOWER = new Map<string, CCbuddyKnownToolName>(
  CCBUDDY_KNOWN_TOOL_NAMES.map((toolName) => [toolName.toLowerCase(), toolName]),
);

export function normalizeCCbuddyToolName(
  value: string | null | undefined,
): CCbuddyKnownToolName | null {
...
export function getCCbuddyToolFamilyForName(
  value: string | null | undefined,
): CCbuddyToolFamily | null {
  const toolName = normalizeCCbuddyToolName(value);
  return toolName ? TOOL_FAMILY_BY_NAME[toolName] : null;
}

export function isCCbuddyToolFamily(
  value: string | null | undefined,
  family: CCbuddyToolFamily,
): boolean {
  return getCCbuddyToolFamilyForName(value) === family;
}

export function isCCbuddyFileContentWriteToolName(value: string | null | undefined): boolean {
  return normalizeCCbuddyToolName(value) === "Write";
}
