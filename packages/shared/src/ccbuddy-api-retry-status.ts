import type { CCbuddyApiRetryStatus } from "./ccbuddy-task-types-core.js";

export function normalizeCCbuddyApiRetryStatus(
  value: unknown,
...
export function ccbuddyApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): CCbuddyApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeCCbuddyApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = ccbuddyApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isCCbuddyModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function ccbuddyApiRetryFromStreamRecoveryPayload(
  value: unknown,
): CCbuddyApiRetryStatus | undefined {
  const record = asRecord(value);
