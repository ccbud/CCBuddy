...
export type FeedbackTicketFramework = "ccbuddy-agent";
...
export interface FeedbackDeviceInfo {
  ...
  osVersion?: string;
  osArch?: string;
  /** 提交反馈时的当前 Agent。单 CCbuddy Agent 模式下固定为 CCbuddy Agent。 */
  agentProvider?: string;
  /** 提交反馈时归一到反馈平台的框架标识，当前固定为 ccbuddy-agent。 */
...
export const DEFAULT_FEEDBACK_TICKET_FRAMEWORK: FeedbackTicketFramework = "ccbuddy-agent";

export const FEEDBACK_TICKET_FRAMEWORK_OPTIONS: {
  value: FeedbackTicketFramework;
  label: string;
}[] = [{ value: "ccbuddy-agent", label: "CCbuddy Agent" }];
