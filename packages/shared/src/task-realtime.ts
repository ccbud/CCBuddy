...
// ccbuddy-task-types-core.ts。本文件与旧 realtime 总线组同生命周期。
import type {
...
import type { CCbuddyPermissionResponse } from "./ccbuddy-protocol-legacy-types.js";
import type { WorkspaceHookReviewDecision } from "./ccbuddy-protocol-v4/workspace-hook-review.js";
...
export type TaskOwnerCommandRequest =
  ...
  | {
      ...
      taskId: string;
      runId: string;
      taskCommand: Extract<CCbuddyTaskRuntimeCommand, { type: "send_prompt" }>;
    }
  | {
      commandRequestId: string;
      ...
      runId: string;
      commandId: string;
      clientMode: Extract<CCbuddyTaskClientMode, "web-remote-replayable">;
    }
  | {
      commandRequestId: string;
...
export type TaskOwnerCommandResult =
  | { commandRequestId: string; success: true; taskCommand?: CCbuddyTaskRuntimeCommand }
  | {
      commandRequestId: string;
      success: false;
      error: string;
