...
- **内置 tgrep，移除 SQLite FTS**：以独立压缩的正文块缩小候选范围，精确核对 Unicode、摘要和消息位置；先显示可打开的命中，再补齐出现次数。SQLite 仅保留元数据与压缩正文，加速不可用时按块进行字面搜索。搜索面板呈现真实阶段与实测延迟。[架构与真实历史实测](docs/search-performance.md)。
