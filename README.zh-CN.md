...
- **三种都配**：每个客户端都会被交给协议相同的那个上游，原样直通，不做任何转换。
- **只配一种或两种**：协议已配置的客户端照样直通；协议未配置的客户端由网关完成协议转换后再发出。

两种情况下，若某个上游失败，队列中的其余上游都会按顺序接管。它能一键配置 **Claude Code 和 Codex**；其他兼容客户端可手动使用本地端点。内置约七十个服务商预设，也支持自定义与插件服务，均可切换并映射模型。若服务商提供 `/v1/models` 或 `/models`，可在编辑器中一键拉取并自动建立模型绑定；没有该接口时按钮会置灰而不是报错。
