...
fn gemini_thought_signature_maps_between_openai_wire_and_ir() {
    ...
    assert_eq!(
        tool_call_thought_signature(call).as_deref(),
        Some(signature)
    );
    assert_eq!(
        json_thought_signature(&json!({
    ...
    let mut message =
        llm_connector::types::Message::new(llm_connector::types::Role::Assistant, vec![]);
    ...
    .unwrap();
    let assistant = outgoing["messages"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert_eq!(
        outgoing_call["extra_content"]["google"]["thought_signature"],
        signature
    );
