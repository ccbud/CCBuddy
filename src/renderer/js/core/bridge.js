/*
 * Tauri IPC bridge — the single module that talks to the backend. Exposes the same
 * `window.ccbud` API object as before (the Rust self-check evals scripts that read it),
 * and exports it for ES-module consumers.
 *
 * Backend commands are snake_case Tauri commands (see src-tauri/src/lib.rs); event names
 * keep their original "namespace:event" form so the view modules' onX handlers are stable.
 */
const T = window.__TAURI__;
if (!T) console.error('[ccbud] Tauri API not found — window.__TAURI__ missing');
const invoke = T ? T.core.invoke : () => Promise.reject(new Error('no tauri'));
const listen = T ? T.event.listen : () => {};
...
export const api = {
  ...
  skillsList: () => inv('skills_list'),
  skillsStatus: () => inv('skills_status'),
  skillsDetail: (id) => inv('skills_detail', { id }),
  skillsReadFile: (id, filePath) => inv('skills_read_file', { id, filePath }),
  skillsTools: () => inv('skills_tools'),
  skillsPickLocal: () => inv('skills_pick_local'),
  skillsScanLocal: (path) => inv('skills_scan_local', { path }),
  skillsImportLocal: (path) => inv('skills_import_local', { path }),
  skillsImportGit: (url) => inv('skills_import_git', { url }),
...
// The Rust self-check (CCBUD_SELFCHECK) evals scripts against window.ccbud — keep the global.
window.ccbud = api;

// Window dragging: map `.drag-region` elements onto Tauri's `data-tauri-drag-region`
...
// Re-apply for any drag bars the views inject after first paint (lazy view mounts, etc.).
try {
