...
const defaultEntryPath = join(repoRoot, "apps/ccbuddy-cli/packages/cli/dist/ccbuddy.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.CCBUDDY_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "ccbuddy.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `ccbuddy.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `ccbuddy.bytecode-runtime-${runtimeHash}.cjs`;
