...
import {
  ...
  stageTuiRuntime,
} from "./ccbuddy-distribution/assets.mjs";
import { installScriptSource } from "./ccbuddy-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "ccbuddy");
const defaultBaseUrl = (await loadEndpointEnv()).CCBUDDY_DIST_BASE_URL?.trim() || "";
const packageDirName = "ccbuddy";
const usage = `Usage:
  pnpm build:ccbuddy
  node scripts/build-ccbuddy.mjs --skip-build
  node scripts/build-ccbuddy.mjs --version 3.3.3-dev.1
  node scripts/build-ccbuddy.mjs --out-dir dist/ccbuddy
  node scripts/build-ccbuddy.mjs --base-url http://host/ccbuddy/deps/ccbuddy/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/ccbuddy.
...
function run(command, args, options = {}) {
  console.log(`[ccbuddy] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[ccbuddy] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@ccbuddy/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@ccbuddy/server", "build"]);
  run("pnpm", ["--filter", "@ccbuddy/web", "build"]);
}

async function stageCCbuddyPackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(
    root,
    "apps",
    "ccbuddy-cli",
    "packages",
  ...
  const agentProvider = resolve(root, "apps/ccbuddy-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "ccbuddy-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "ccbuddy.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/ccbuddy-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "ccbuddy.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "ccbuddy.mjs");
  await cp(resolve(root, "scripts/ccbuddy-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure CCBUDDY_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve CCbuddy version.");
  }
  ...
  console.log(`[ccbuddy] release directory: ${outDir}`);
  console.log(`[ccbuddy] tarball: ${tarball}`);
  console.log(`[ccbuddy] sha256: ${sha256}`);
}
