...
export async function packageNativeSearchTools({
  platform = process.env.CCBUDDY_TARGET_OS || process.platform,
  arch = process.env.CCBUDDY_TARGET_ARCH || process.arch,
...
async function main() {
  await packageNativeSearchTools({
    platform: readOption("platform") ?? process.env.CCBUDDY_TARGET_OS ?? process.platform,
    arch: readOption("arch") ?? process.env.CCBUDDY_TARGET_ARCH ?? process.arch,
