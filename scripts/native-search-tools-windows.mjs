...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "ccbuddy-native-search-build-"));
  ...
  console.log("==> CCbuddy native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DCCBUDDY_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DCCBUDDY_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DCCBUDDY_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DCCBUDDY_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DCCBUDDY_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DCCBUDDY_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DCCBUDDY_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DCCBUDDY_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
