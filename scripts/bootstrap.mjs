...
function resolveBootstrapWithRemoteEnv(baseEnv = process.env) {
  ...
  return {
    ...
    CCBUDDY_BOOTSTRAP_WITH_REMOTE: "1",
...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @ccbuddy/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@ccbuddy/rpc", "@ccbuddy/web", "@ccbuddy/formal-proof"]) {
