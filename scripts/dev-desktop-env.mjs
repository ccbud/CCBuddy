...
function run(command, args) {
  return new Promise((resolveRun, rejectRun) => {
    // Windows 下 shell:true 只按空格拼接参数；仓库路径含空格（如 E:\CCbuddy\...）时
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          CCBUDDY_ENV: requestedEnv,
          CCBUDDY_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@ccbuddy/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@ccbuddy/desktop", "dev:runtime"]);
} catch (error) {
