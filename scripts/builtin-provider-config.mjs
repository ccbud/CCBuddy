...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.CCBUDDY_ENV !== undefined) value = parsed.CCBUDDY_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid CCBUDDY_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.CCBUDDY_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/ccbuddy-builtin.json",
  );
  try {
    ...
    const { decodeCCbuddyBuiltinRelease } = await tsImport(
      pathToFileURL(
        resolve(repositoryRoot, "packages/provider-node/src/ccbuddy-builtin-release.ts"),
      ).href,
      import.meta.url,
    );
    decodeCCbuddyBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "ccbuddy-builtin.json"), config.content, "utf8");
  return config;
}
