...
const agentEntry = join(root, "agent", "ccbuddy.cjs");

function usage() {
  return `Usage:
  ccbuddy --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  ccbuddy --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      CCBUDDY_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      CCBUDDY_AGENT_SERVER_COMMAND: process.execPath,
      CCBUDDY_SERVER_HOST: options.host,
      CCBUDDY_SERVER_WORKSPACE: options.workspace,
      CCBUDDY_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      CCBUDDY_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("CCbuddy Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: ccbuddy --web [options] (ccbuddy --web --help for details)\n");
    }
