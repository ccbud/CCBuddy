...
# 修复原因：继承的 CCbuddy 默认路径会验收错误的应用包，CCbuddy 发布检查必须指向自己的安装包。
APP_PATH="${1:-${CCBUDDY_MACOS_RELEASE_APP_PATH:-/Applications/CCbuddy.app}}"
# 安装包身份与后端环境分轴：CCBUDDY_PREVIEW_IDENTITY=1 让生产后端的构建仍是 CCbuddy Preview。
# 只认 "1"，与 CI workflow / release 门的精确比较同一套语义（其它拼写一律视为未开启）。
is_preview_identity_requested() {
  [[ "${CCBUDDY_PREVIEW_IDENTITY:-}" = "1" ]]
}
# 修复原因：macOS basename 会把 --help 当作自身参数报错，帮助选项必须先被入口处理。
if [ "${APP_PATH:-}" = "--help" ] || [ "${APP_PATH:-}" = "-h" ]; then
  cat <<'USAGE'
Usage:
  bash scripts/doctor-macos-release-app.sh /Applications/CCbuddy.app
  CCBUDDY_MACOS_RELEASE_APP_PATH=/Applications/CCbuddy.app pnpm run doctor:macos-release
...
APP_EXECUTABLE_NAME="${CCBUDDY_APP_EXECUTABLE_NAME:-$APP_DISPLAY_NAME}"
...
run_quiet_validation() {
  ...
  shift
  local output_file
  output_file="$(mktemp "${TMPDIR:-/tmp}/ccbuddy-macos-release-doctor.XXXXXX")"
