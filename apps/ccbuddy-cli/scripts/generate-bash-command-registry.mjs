...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "ccbuddy-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/ccbuddy-cli registry:generate`.",
      );
    }
  } finally {
