...
- `npm run build` produces `dist/ccbuddy.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
ccbuddy plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.ccbuddy/cli/plugins`:
...
- `marketplaces/ccbuddy-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and CCbuddy Guide content plugins are default-enabled and appear as `browser-use@ccbuddy-plugins-official`, `document-skills@ccbuddy-plugins-official`, `skill-creator@ccbuddy-plugins-official`, and `ccbuddy-guide@ccbuddy-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@ccbuddy-plugins-official`, `android-emulator@ccbuddy-plugins-official`, and `restore-legacy-sessions@ccbuddy-plugins-official`, are discovered by ccbuddy but stay disabled until the user enables them.

```sh
ccbuddy plugins list
ccbuddy plugins enable ios-simulator
ccbuddy plugins disable browser-use
ccbuddy plugins enable restore-legacy-sessions
ccbuddy plugins disable ios-simulator
```
...
MCP config can live directly in `.ccbuddy-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current ccbuddy plugin surface:
...
Example `.ccbuddy-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${CCBUDDY_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${CCBUDDY_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${CCBUDDY_PLUGIN_DATA}",
...
Only environment variables with the `CCBUDDY_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.

### Recommended Layout

```txt
my-plugin/
  .ccbuddy-plugin/plugin.json
  .mcp.json
  skills/
    my-skill/SKILL.md
  commands/
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting ccbuddy-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

ccbuddy reads MCP servers from the main JSON config. The default user config path is `~/.ccbuddy/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits ccbuddy's environment plus any `env` overrides.
...
ccbuddy reads hooks from the same main JSON config file as MCP, usually `~/.ccbuddy/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
