# @ccbuddy/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting ccbuddy-cli prompt assembly.
...
```bash
pnpm --filter @ccbuddy/bootstrap^... build
pnpm --filter @ccbuddy/bootstrap build

pnpm --filter @ccbuddy/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/ccbuddy-prompt-trajectory/basic-live

pnpm --filter @ccbuddy/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @ccbuddy/prompt-trajectory derive -- \
  --out /tmp/ccbuddy-prompt-trajectory/basic-live

pnpm --filter @ccbuddy/prompt-trajectory model-io -- \
  --input ~/.ccbuddy/cli/debug/model-io-<session>.jsonl \
  --out /tmp/ccbuddy-prompt-trajectory/model-io-session
```
...
uses the same ccbuddy model config resolution as the CLI. The upstream request is
...
`model-io` reads a real CCbuddy `model-io-*.jsonl` file and turns the main
