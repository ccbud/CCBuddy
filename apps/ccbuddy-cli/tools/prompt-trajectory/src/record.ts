...
interface CCbuddyBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createCCbuddyApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface CCbuddyContractsModule {
...
interface CCbuddyProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadCCbuddyModules();
    ...
    const app = await bootstrap.createCCbuddyApp({
...
async function loadCCbuddyModules(): Promise<{
  bootstrap: CCbuddyBootstrapModule;
  contracts: CCbuddyContractsModule;
  provider: CCbuddyProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<CCbuddyBootstrapModule>,
      import(contractsPath) as Promise<CCbuddyContractsModule>,
      import(providerConfigPath) as Promise<Omit<CCbuddyProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<CCbuddyProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built ccbuddy-cli packages. Run `pnpm --filter @ccbuddy/bootstrap^... build && pnpm --filter @ccbuddy/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
