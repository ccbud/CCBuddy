...
`${CCBUDDY_SKILL_DIR}/examples.md` §5. Read it when you want to see a whole script's arc; §7
...
an inline `script` is written under CCbuddy's `workspaces/<key>/workflow-drafts/` before it is even compiled and
...
**Editing a draft asks nothing.** `~/.ccbuddy/workspaces/<key>/workflow-drafts/` is machine-owned
and outside the repository, so an `Edit` or `Write` under it is pre-approved and opens no confirmation
...
- `${CCBUDDY_SKILL_DIR}/patterns.md` — the topology catalogue: fan-out/fan-in, review sweeps,
...
- `${CCBUDDY_SKILL_DIR}/examples.md` — complete worked scripts, including a two-subagent
...
- `script`: a one-off workflow written inline. It is saved to a file under
  `~/.ccbuddy/workspaces/<key>/workflow-drafts/` and the result names that file, whether the script compiled or
...
- `path`: a script file on disk, relative to the working directory or absolute — normally
  ...
  arguments in its `/* ccbuddy-workflow` block.
...
`~/.ccbuddy/workspaces/<key>/workflows/<name>.dwf.ts`, visible only in that logical workspace;
global definitions go in `~/.ccbuddy/workflows/<name>.dwf.ts` and are available from every
project on this machine.

**When to call it.**

- Never unsolicited. Saving creates persistent application data; that is the user's
  decision, not yours.
...
- `script` or `script_path`, never both. `script_path` saves a draft a `CreateWorkflow` or
  `AmendWorkflow` result named without re-emitting it; a `/* ccbuddy-workflow` block in that
...
The saved file is valid TypeScript: a `/* ccbuddy-workflow` block of YAML metadata, then the
