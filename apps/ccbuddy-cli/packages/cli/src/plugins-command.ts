...
import type { SetCCbuddyPluginEnabledResult, CCbuddyPluginInstallData } from "@ccbuddy/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: ccbuddy plugins <command> [options]
...
Scopes: user (default), project. \`ccbuddy plugin\` is an alias of \`ccbuddy plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart ccbuddy to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: CCbuddyPluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetCCbuddyPluginEnabledResult; error?: string }> = [];
