...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@ccbuddy/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot =
    processEnv.CCBUDDY_STORAGE_DIR?.trim() ||
    resolveCcBuddyDataRoot(processEnv.CCBUDDY_DATA_BASE_DIR?.trim() || homedir());
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "ccbuddy-clipboard-"));
