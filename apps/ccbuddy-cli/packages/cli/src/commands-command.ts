...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listCCbuddyCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: ccbuddy commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listCCbuddyCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@ccbuddy/bootstrap"));
  return (await bootstrap()).inspectCCbuddyCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: CCbuddyCustomCommandInspection,
  options: GlobalOptions,
): string {
