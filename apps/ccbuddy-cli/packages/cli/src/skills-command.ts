...
type BootstrapModule = typeof import("@ccbuddy/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listCCbuddySkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: ccbuddy skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listCCbuddySkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@ccbuddy/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectCCbuddySkill;
...
function formatHumanSkillInspection(
  inspection: CCbuddySkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: CCbuddySkillInspection, cwd: string): string =>
  formatJson({
    cwd,
