...
import { formatJson, type PresentationSurface } from "@ccbuddy/core";
import type { RunContext, GlobalOptions } from "@ccbuddy/shared-types";
...
export const runPrompt = async (
  ...
  if (/^\/(?:login|logout)(?:\s|$)/iu.test(prompt.trim())) {
    ctx.stderr.write("Account login is unavailable. Configure your own model provider.\n");
    return 1;
  }
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createCCbuddyApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createCCbuddyApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createCCbuddyApp ?? bootstrapModule?.createCCbuddyApp;
    if (!createApp) throw new Error("CCbuddy app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareCCbuddyTelemetryEnv ?? bootstrapModule?.prepareCCbuddyTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownCCbuddyTelemetry ?? bootstrapModule?.shutdownCCbuddyTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.CCBUDDY_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `ccbuddy -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveCCbuddyCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedCCbuddySlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listCCbuddyCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadCCbuddyCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: ccbuddy hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
