import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@ccbuddy/adapters/config";
import type { SessionEvent } from "@ccbuddy/contracts";
import type { CCbuddyAppOptions } from "@ccbuddy/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@ccbuddy/i18n";
import type { TuiRequestPermission } from "@ccbuddy/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createCCbuddyApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<CCbuddyAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createCCbuddyApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createCCbuddyApp"]>>>;
    try {
