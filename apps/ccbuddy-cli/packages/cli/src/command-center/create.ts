import type { TuiSubmitPrompt } from "@ccbuddy/tui";
import {
...
export function createCommandCenter(deps: CommandCenterDeps): TuiSubmitPrompt {
  return async (input, options) => {
    ...
    if (command.type === "unknown") {
      if (command.rawName === "login" || command.rawName === "logout") {
        return {
          mode: deps.getMode?.(),
          response: "Account login is unavailable. Configure your own model provider.",
        };
      }
