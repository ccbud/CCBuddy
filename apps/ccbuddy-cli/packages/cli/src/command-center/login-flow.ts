import type { TuiSelection, TuiSubmitPrompt } from "@ccbuddy/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getCCbuddyCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getCCbuddyCopy(locale).tui.loginSetup.response;
}
