...
import { createDenyPermissionBroker } from "@ccbuddy/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  CCbuddyAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `ccbuddySessionEventTypeSchema`（`shared/src/ccbuddy-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `ccbuddySessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createCCbuddyApp` 注入点边界）。
 *
