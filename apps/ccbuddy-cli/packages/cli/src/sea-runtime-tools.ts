...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@ccbuddy/shared/runtime-tool-runtime";
...
const assetPrefix = "ccbuddy-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".ccbuddy-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.CCBUDDY_STORAGE_DIR?.trim();
  const storageRoot =
    configuredStorageRoot || resolveCcBuddyDataRoot(env.CCBUDDY_DATA_BASE_DIR?.trim() || homedir());
