...
import { CCBUDDY_PLUGIN_HOST_COMMAND } from "@ccbuddy/contracts/plugins";
import {
  getCapturedCCbuddyCuaBrokerCredentials,
  CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY,
  CCBUDDY_CUA_NODE_REPL_HOST_ENV_KEY,
} from "@ccbuddy/shared/runtime-env";
import { CCBUDDY_CUA_OFFICIAL_PLUGIN_ID, CCBUDDY_PLUGIN_ID_ENV_KEY } from "@ccbuddy/shared/mcp";
import type { RunContext } from "@ccbuddy/shared-types";

const HOST_USAGE = `${CCBUDDY_PLUGIN_HOST_COMMAND} <server-path> [-- <server-arg>...]`;
...
export function isPluginHostInvocation(argv: readonly string[]): boolean {
  return argv[0] === CCBUDDY_PLUGIN_HOST_COMMAND;
}

// __ccbuddy-plugin-host 在 agent 子进程里运行 official plugin 的 MCP server（server.js）。
...
// Agent 进程曾收到过 Helper 凭据，不能证明当前传入的 server 就是官方 ccbuddy-cua：
...
export async function runPluginHostCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  try {
    ...
    const capturedBrokerCredentials = getCapturedCCbuddyCuaBrokerCredentials();
    ...
    const originalBrokerSocket = process.env[CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY];
    // shared node_repl 把同一凭据组恢复到环境，由 broker bridge 读取；旧的独立 CUA
    // MCP 不再拥有执行入口。
    process.argv = [process.execPath, serverPath, ...serverArgs];
    if (capturedBrokerCredentials.socket && process.env[CCBUDDY_CUA_NODE_REPL_HOST_ENV_KEY] === "1") {
      process.env[CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY] = capturedBrokerCredentials.socket;
    }
    try {
      await module.main();
    } finally {
      ...
      if (originalBrokerSocket === undefined) {
        delete process.env[CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY];
      } else {
        process.env[CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY] = originalBrokerSocket;
      }
    }
...
type CapturedBrokerCredentials = ReturnType<typeof getCapturedCCbuddyCuaBrokerCredentials>;
...
function assertCapturedBrokerLaunchIsAuthorized(credentials: CapturedBrokerCredentials): void {
  ...
  const pluginId = process.env[CCBUDDY_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase();
  ...
  // 残留读取方只能靠 CLI 自己的 typecheck 发现（根 `pnpm typecheck` 不含 apps/ccbuddy-cli）。
  ...
  if (
    ...
    pluginId !== CCBUDDY_CUA_OFFICIAL_PLUGIN_ID ||
    process.env[CCBUDDY_CUA_NODE_REPL_HOST_ENV_KEY] !== "1"
  ) {
    throw new Error(
      "Captured CCbuddy CUA broker credentials may only launch the trusted shared node_repl host",
    );
  }
}
