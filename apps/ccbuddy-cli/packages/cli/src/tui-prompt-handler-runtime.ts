...
// 品牌类型（SessionId）和 createCCbuddyApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createCCbuddyApp ?? bootstrapModule?.createCCbuddyApp;
  if (!createAppFactory) throw new Error("CCbuddy app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareCCbuddyTelemetryEnv ?? bootstrapModule?.prepareCCbuddyTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownCCbuddyTelemetry ?? bootstrapModule?.shutdownCCbuddyTelemetry;
  }
