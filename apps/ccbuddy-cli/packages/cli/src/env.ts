...
import {
  ...
  normalizeCCbuddyRuntimeEnv,
  sanitizeCCbuddyRuntimeEnv,
  sanitizeCCbuddyRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeCCbuddyRuntimeEnv(env),
    ...buildCCbuddyToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildCCbuddyToolEnvPassthroughEnv(env);
  sanitizeCCbuddyRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[CCBUDDY_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): CCbuddyRuntimeEnv {
  const explicit = normalizeCCbuddyRuntimeEnv(env[CCBUDDY_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.CCBUDDY_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.CCBUDDY_BETA === "1" || env.CCBUDDY_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])ccbuddy-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.CCBUDDY_STORAGE_DIR = join(
    resolveCcBuddyDataRoot(env.CCBUDDY_DATA_BASE_DIR?.trim() || homedir()),
    "beta",
  );
}
