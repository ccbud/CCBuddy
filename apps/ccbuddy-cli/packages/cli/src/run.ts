...
import { createNodeLoggerFactory } from "@ccbuddy/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@ccbuddy/core";
import { color, formatJson, supportsColor } from "@ccbuddy/core";
import { getCCbuddyCopy, isUiLocale, type UiLocale } from "@ccbuddy/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@ccbuddy/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getCCbuddyCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "ccbuddy_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("ccbuddy doctor", colors)}\n`);
...
const runCCbuddyProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 CCbuddy agent transport closed。
    ...
    const runProtocolAgent =
      deps.runCCbuddyProtocolAgent ?? (await loadBootstrapModule()).runCCbuddyProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("ccbuddy").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runCCbuddyProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
