...
export const resolveBuildExternal = () => ["@ccbuddy/tui", "playwright-core", "koffi"];
  ...
  return {
    ...
    setup(builder) {
      ...
      builder.onResolve({ filter: /^zod(?:\/|$)/ }, async (args) => {
        if (args.pluginData?.ccbuddyZodResolving) return;
        const resolved = await builder.resolve(args.path, {
          importer: args.importer,
          resolveDir: args.resolveDir,
          kind: args.kind,
          pluginData: { ccbuddyZodResolving: true },
        });
...
export const resolveBuildOptions = (args = [], env = process.env) => {
  ...
  const e2eCoverage = env.CCBUDDY_E2E_COVERAGE === "1";
...
export const resolveBuildAliases = ({
  ...
  "@ccbuddy/shared-types": resolve(cliDirectory, "../shared-types/dist/index.js"),
  // plugin-host 启动只需这些独立入口，不能经通用 alias 重新求值 shared 总入口。
  "@ccbuddy/shared/runtime-env": resolve(rootDirectory, "../../packages/shared/src/runtimeEnv.ts"),
  "@ccbuddy/shared/mcp": resolve(rootDirectory, "../../packages/shared/src/mcp.ts"),
  "@ccbuddy/shared/runtime-tool-runtime": resolve(
    rootDirectory,
  ...
  "@ccbuddy/shared/ccbuddy-protocol-v4": resolve(
    rootDirectory,
    "../../packages/shared/src/ccbuddy-protocol-v4/index.ts",
  ),
  ...
  "@ccbuddy/shared/model-selection": resolve(
    rootDirectory,
    "../../packages/shared/src/model-selection.ts",
  ),
  // 共享 Model Schema 新增的子路径不能被通用 alias 拼到 index.ts 后面。
  "@ccbuddy/shared/model-config": resolve(rootDirectory, "../../packages/shared/src/model-config.ts"),
  // 进程异常边界在 bootstrap 之前使用该轻量契约，不能落入 shared 的通用前缀 alias。
  "@ccbuddy/shared/process-diagnostic": resolve(
    rootDirectory,
    "../../packages/shared/src/process-diagnostic.ts",
  ),
  "@ccbuddy/shared/config-schema": resolve(
    rootDirectory,
    "../../packages/shared/src/config-schema.ts",
  ),
  "@ccbuddy/shared/workspace-hook-discovery": resolve(
    rootDirectory,
  ...
  "@ccbuddy/shared/workspace-hook-mutation": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-mutation.ts",
  ),
  // verdict 直连 import 需要本精确 alias；漏声明会被通用
  // "@ccbuddy/shared" 前缀改写成 `src/index.ts/workspace-hook-review-monotonicity`，
  // Desktop agent/SEA 打包直接失败。
  "@ccbuddy/shared/workspace-hook-review-monotonicity": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-review-monotonicity.ts",
  ),
  // trust store 文件 schema 单源下沉后的新 subpath；漏声明会被通用
  // "@ccbuddy/shared" 前缀改写成 `src/index.ts/workspace-hook-trust-store-file`，
  // Desktop agent/SEA 打包失败（同上两类既有规则）。
  "@ccbuddy/shared/workspace-hook-trust-store-file": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-trust-store-file.ts",
  ),
  "@ccbuddy/shared/ccbuddyEndpoint": resolve(
    rootDirectory,
...
export const buildCli = async ({
  ...
  const outfile = resolve(cliDirectory, "dist/ccbuddy.cjs");
  ...
  await build({
    banner: {
      ...
      js: `#!/usr/bin/env node\n"use strict";\nif (process.argv.length === 3 && process.argv[2] === "--licenses") { const sea = require("node:sea"); const nodeNotice = sea.isSea() ? "\\n\\n## Bundled Node.js runtime\\n\\n" + sea.getAsset("ccbuddy-node-license", "utf8") : ""; process.stdout.write(${JSON.stringify(notices.toString("utf8"))} + nodeNotice, () => process.exit(0)); } else {`,
    },
    footer: { js: "}" },
    bundle: true,
    define: {
    ...
    format: "cjs",
    // 桌面 app 集成只内置 ccbuddy.cjs，旧 desktop-agent 构建复用 CLI 调试产物，
    ...
    // 远端 SSH 复用已部署的独立 Node v22.16 跑同一份 ccbuddy.cjs。降到 node22 保证这份产物
