...
const cliBundle = resolve(dist, "ccbuddy.cjs");
const seaBlobForTarget = (target) => resolve(dist, `ccbuddy-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "ccbuddy-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/ccbuddy.cjs",
        output: `dist/ccbuddy-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "ccbuddy-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        CCBUDDY_STORAGE_DIR: storageRoot,
      };
      delete env.CCBUDDY_BFS_BINARY;
      delete env.CCBUDDY_RG_BINARY;
      delete env.CCBUDDY_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/ccbuddy.cjs. Run `pnpm build` first.");
  }
