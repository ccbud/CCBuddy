...
export const seaTuiAssetPrefix = "ccbuddy-tui-runtime/";
...
const workspacePackageParentDirectories = (root) => {
  ...
  return [
    ...
    // TUI runtime 闭包会经由 @ccbuddy/contracts 依赖仓库根的 @ccbuddy/shared；
    // 只扫描 apps/ccbuddy-cli 子 workspace 会把这个合法 workspace 误判为缺失。
    resolve(repositoryRoot, "packages"),
  ];
};

const runtimePackageNames = async ({ root, target, workspacePackageDirectories }) => {
  const tuiDirectory = workspacePackageDirectories.get("@ccbuddy/tui");
  if (!tuiDirectory) {
    throw new Error("Missing @ccbuddy/tui workspace package.");
  }
  const queue = [
    { fromDirectory: tuiDirectory, packageName: "@ccbuddy/tui" },
  ...
  while (queue.length > 0) {
    ...
    const assetPackagePath = placeRuntimePackage({
      ...
      packageDirectory,
      placements,
      fromAssetPath: entry.fromAssetPath ?? "node_modules/@ccbuddy/tui",
    });
    ...
    // SEA 缓存目录无法解析 pnpm workspace 链接；旧的 @ccbuddy 白名单漏掉 i18n
