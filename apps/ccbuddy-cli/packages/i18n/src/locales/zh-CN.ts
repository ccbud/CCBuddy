...
不传 command 时，ccbuddy 会打开全屏 TUI。

命令:
  app-server 运行 CCbuddy stdio app server
...
`,
  ...
  tui: {
    ...
    loginRequired: {
      help: "配置自己的模型服务后，输入 /model 选择模型。",
      message: "没有可用模型，请配置自己的模型服务。",
      status: "没有可用模型，请配置自己的模型服务。",
      title: "需要配置模型",
    },
    loginSetup: {
    ...
    terminal: {
      starting: "正在启动 CCbuddy… Ctrl+C 退出",
