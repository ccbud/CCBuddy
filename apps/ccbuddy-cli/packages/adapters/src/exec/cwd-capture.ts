...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `ccbuddy-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__ccbuddy_status=$?",
    `if [ "$__ccbuddy_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__ccbuddy_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__ccbuddy_status=%ERRORLEVEL%"',
    `if "%__ccbuddy_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__ccbuddy_status%",
  ].join("\r\n");
}
