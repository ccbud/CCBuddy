...
import { windowsPathToGitBashPath } from "@ccbuddy/contracts";
import type { ExecutionShellDialect } from "@ccbuddy/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_CCBUDDY_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_CCBUDDY_SHELL_INIT_SNAPSHOT",
  ];
}
