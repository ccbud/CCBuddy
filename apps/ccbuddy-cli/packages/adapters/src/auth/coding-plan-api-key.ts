import type { HttpClientPort, HttpClientRunOptions, TraceContext } from "@ccbuddy/contracts";
import { resolveBigModelApiOrigin } from "@ccbuddy/shared";

const ZAI_API_HOST = "https://api.z.ai";
const JSON_CONTENT_TYPE = "application/json";
const CCBUDDY_API_KEY_NAME = "ccbuddy-api-key";
...
async function resolveBizApiKey(
  ...
  const keyEntry =
    keys.find((item) => item.name === CCBUDDY_API_KEY_NAME) ??
    (await requestRemoteData<RemoteApiKeySummary>(
      input.httpClient,
      {
        body: new TextEncoder().encode(JSON.stringify({ name: CCBUDDY_API_KEY_NAME })),
        headers: createBizAuthHeaders(input.authorization),
        method: "POST",
        trace: input.trace,
        url: listUrl,
