...
const CREDENTIAL_SECRET_ENV_KEY = "CCBUDDY_CREDENTIAL_SECRET";

export interface CCbuddyCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface CCbuddyCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createCCbuddyCredentialCipher(
  options: CCbuddyCredentialCipherOptions = {},
): CCbuddyCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedCCbuddyCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedCCbuddyCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `ccbuddy-credential-fallback:${platform()}:${homedir()}:${username}`;
}
