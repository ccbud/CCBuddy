import {
  ...
  CCBUDDY_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readCCbuddyToolEnvPassthroughEnv,
} from "@ccbuddy/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [CCBUDDY_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [CCBUDDY_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, CCBUDDY_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readCCbuddyToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const ccbuddyProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [CCBUDDY_HTTP_PROXY_ENV_KEY], platform),
  );
  if (ccbuddyProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, ccbuddyProxy, platform);
    }
    return;
  }
}
