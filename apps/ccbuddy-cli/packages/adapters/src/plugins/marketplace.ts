...
import type { PluginDiagnostic, PluginManifest, PluginStoreListing } from "@ccbuddy/contracts";
import { isOfficialMarketplaceId, CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE } from "@ccbuddy/contracts";
import { DEFAULT_PLUGIN_MARKETPLACES, sanitizeCCbuddyRuntimeEnv } from "@ccbuddy/shared";
...
const CCBUDDY_MANIFEST_PATH = join(".ccbuddy-plugin", "plugin.json");
...
function buildMarketplaceGitEnv(
  ...
  const env = sanitizeCCbuddyRuntimeEnv(sourceEnv);
  // marketplace 安装会启动 Git 子进程，不能只依赖父进程继承的 shell 代理。
  // 这里统一从 CCbuddy 显式网络环境恢复 HTTP(S)/NO_PROXY/CA，避免安装按钮卡到协议超时。
...
export function ensureDefaultPluginMarketplaces(storageRoot: string): KnownMarketplaceRecord[] {
  ...
  const defaults = DEFAULT_PLUGIN_MARKETPLACES.map((marketplace) => ({
    definition: marketplace,
    source: defaultMarketplaceSourceFromString(marketplace.source, storageRoot),
  }));
  // 旧的 CCbuddy 预览版可能留下指向 CCbuddy CDN 的 official record。
  // 统一收敛到本地 bundled source，避免打开商店或刷新时继续访问上游。
  let changed = false;
  const normalized = known.map((record) => {
    const local = defaults.find(({ definition }) => definition.id === record.id);
    if (!local || record.id !== CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) return record;
    if (
      record.source.source === "file" &&
      record.source.path === (local.source.source === "file" ? local.source.path : undefined) &&
      record.name === local.definition.name &&
  ...
  const missing = defaults
    ...
    .map(
      ({ definition, source }): KnownMarketplaceRecord => ({
        id: definition.id,
        source,
        name: definition.name,
        description: definition.description,
        addedAt: now,
        ...(definition.lastUpdated ? { lastUpdated: definition.lastUpdated } : {}),
        pluginCount: definition.pluginCount,
      }),
    );
  if (missing.length === 0 && !changed) return known;
...
export async function ensureMarketplaceManifestAvailable(input: {
  ...
  // 官方插件只能从随包资产 seed。缺少本地 manifest 时不回退联网。
  if (record.id === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) return null;
...
export async function addMarketplace(input: {
  ...
  try {
    ...
    if (
      input.trustedId === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE &&
      loaded.manifest.name !== CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE
    ) {
      throw new Error(
        `Official marketplace source must provide ${CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}, received ${loaded.manifest.name}`,
      );
    }
    ...
    } else if (loaded.manifest.name !== CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function updateMarketplace(input: {
  ...
  for (const record of selected) {
    ...
    if (record.id === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
      // 官方目录是随包本地资产，刷新只返回已有记录，不读取旧 CDN source。
      updated.push(record);
      continue;
    }
...
export async function uninstallMarketplacePlugin(input: {
  ...
  storageRoot: string;
  removeCache?: boolean;
  /** `ccbuddy plugins uninstall --keep-data`：删安装缓存但保留 data/<plugin-id> 用户数据目录。 */
  keepData?: boolean;
...
async function clonePluginSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-plugin-src-"));
  const args = ["clone"];
...
async function cloneMarketplaceSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "ccbuddy-marketplace-src-"));
...
async function execGitCommand(args: string[], signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const gitBinary = process.env.CCBUDDY_GIT_BINARY?.trim() || "git";
...
function defaultMarketplaceSourceFromString(
  source: string,
  storageRoot: string,
): MarketplaceSource {
  const trimmed = source.trim();
  if (trimmed === "bundled:ccbuddy") {
    return {
      source: "file",
      path: join(storageRoot, "marketplaces", CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE, MARKETPLACE_FILE),
    };
  }
...
function findPluginManifestPath(rootPath: string): string | null {
  for (const candidate of [CCBUDDY_MANIFEST_PATH, CLAUDE_MANIFEST_PATH, CODEX_MANIFEST_PATH]) {
  ...
  for (const key of UNSUPPORTED_MANIFEST_FIELDS) {
    if (key in input.manifest) {
      input.diagnostics.push({
        ...
        message: `Plugin component is diagnostic-only in this CCbuddy runtime: ${key}`,
