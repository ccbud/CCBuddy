// Env Config Adapter - Parse the intentionally small CCBUDDY_* environment surface.
...
const DEFAULT_PREFIX = "CCBUDDY_";

/**
 * Parse CCBUDDY_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.CCBUDDY_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
