...
import type { RuntimeConfigPatch, UiLocale } from "@ccbuddy/contracts";
...
const DEFAULT_CONFIG_DIRECTORY = "cli";

function defaultConfigBaseDir(): string {
  return join(resolveCcBuddyDataRoot(), DEFAULT_CONFIG_DIRECTORY);
}
...
export function loadFileConfig(filePath?: string, options: FileConfigOptions = {}): LoadedConfig {
  const resolvedPath = filePath
    ...
    : join(
        resolvePath(options.baseDir ?? defaultConfigBaseDir()),
  ...
  try {
    ...
    if (migrated && options.persistMigration !== false) {
...
export function getDefaultConfigPath(): string {
  return join(defaultConfigBaseDir(), DEFAULT_CONFIG_FILE);
}
