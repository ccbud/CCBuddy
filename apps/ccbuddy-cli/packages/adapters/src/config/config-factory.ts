...
export interface ConfigFactoryOptions {
  /** Path to user config file (default: ~/.ccbuddy/cli/config.json) */
 ...
 * 1. System defaults
 * 2. User config file (~/.ccbuddy/cli/config.json)
 * 3. Project config files (root to cwd, then explicit projectConfigPath)
 * 4. Environment variables (CCBUDDY_*)
 * 5. CLI overrides
 */
export function createConfig(options: ConfigFactoryOptions = {}): ConfigResult {
  ...
  const discoveredProjectConfigs: ProjectConfigDiscovery = options.workingDirectory
    ? loadProjectConfigs(options.workingDirectory, options.projectConfigPath, options.workspaceIdentity)
