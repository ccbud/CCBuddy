/* eslint-disable max-lines -- ccbuddy-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const LEGACY_CUA_PLUGIN_ID = "ccbuddy-cua@ccbuddy-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@ccbuddy-plugins-official";
...
export const CCbuddyConfigFileSchema = z
  .object({
...
export type CCbuddyConfigFile = z.infer<typeof CCbuddyConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. CCbuddy does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；CCbuddy 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = CCbuddyConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: CCbuddyConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: CCbuddyConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: CCbuddyConfigFile["skills"],
