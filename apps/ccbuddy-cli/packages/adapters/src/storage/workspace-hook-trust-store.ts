...
import type { WorkspaceHookTrustRecord, WorkspaceHookTrustStoreFile } from "@ccbuddy/contracts";
import {
...
export async function resolveWorkspaceHookTrustStorePath(
  ...
  const userConfigPath = resolve(
    options.userConfigPath ?? join(home, ".ccbuddy", "cli", "config.json"),
  );
  ...
  const storageRoot = configured ? resolveTrustedUserPath(configured, home) : join(home, ".ccbuddy");
