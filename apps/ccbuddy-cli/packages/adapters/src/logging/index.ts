...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@ccbuddy/contracts";
import { LogLevel, LogLevelName } from "@ccbuddy/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.CCBUDDY_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.CCBUDDY_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(resolveCcBuddyDataRoot(), "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeCCbuddyRuntimeEnv(env[CCBUDDY_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `ccbuddy-${formatLocalLogDate(new Date())}.jsonl`;
}
