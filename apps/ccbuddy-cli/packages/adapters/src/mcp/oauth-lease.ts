...
import { acquireFileLock } from "@ccbuddy/shared/node";
import { CCBUDDY_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@ccbuddy/shared";
import type { SharedCCbuddyCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === CCBUDDY_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
