...
import { CCBUDDY_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type CCbuddyMcpResourceSample } from "@ccbuddy/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: CCbuddyMcpResourceSample["arch"];
  platform: CCbuddyMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: CCbuddyMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const intervalMs =
        previousAt === undefined
          ? CCBUDDY_MCP_RESOURCE_SAMPLE_INTERVAL_MS
          : Math.max(1, sampledAt - previousAt);
      const next = new Map<string, Map<number, number>>();
      const groups = new Map<string, CCbuddyMcpResourceSample>();
