...
import { sanitizeCCbuddyRuntimeEnv } from "@ccbuddy/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeCCbuddyRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.ccbuddy/server/node 启动，但登录环境 PATH 不含该目录，
