...
import { CCBUDDY_MCP_SERVER_REQUEST_ID_META_KEY } from "@ccbuddy/contracts";
...
import {
  ...
  CCBUDDY_OFFICIAL_MCP_AUTH_TYPE,
...
import {
  createSharedCCbuddyCredentialStore,
  type SharedCCbuddyCredentialStore,
...
export interface CreateMcpAdapterOptions {
  ...
  officialMcpAuth?: {
    ...
    /**
     * 当前 CCbuddy API origin。stdio 形态没有 `url` 可供校验，targetOrigin 只能由宿主给出
     * ——插件因此无法把身份头导向别的 origin。
     * 与 trustedOrigins 的 `resolveCCbuddyApiOrigin` 必须同源，否则两侧判定会分叉。
     */
    resolveCCbuddyApiOrigin?: () => string;
 ...
 * 由 adapter 发起无身份的 tools/call，让 CCbuddy server 返回权威结构化错误。把 reason 交给 stdio
...
class NodeMcpAdapter implements McpPort {
  ...
  private credentialStore?: SharedCCbuddyCredentialStore;
  /**
  ...
  constructor(options: CreateMcpAdapterOptions) {
    this.clientName = options.clientName ?? "ccbuddy";
  ...
  private async resolveOfficialStdioAuthMeta(
    ...
    const resolveCCbuddyApiOrigin = this.officialMcpAuth?.resolveCCbuddyApiOrigin;
    const logBase = {
    ...
    if (!authHeadersPort || !trustedOrigins || !resolveCCbuddyApiOrigin) {
    ...
    try {
      targetOrigin = resolveCCbuddyApiOrigin();
      ...
      return {
        ...
        _meta:
          isError && serverRequestId
            ? { ...meta, [CCBUDDY_MCP_SERVER_REQUEST_ID_META_KEY]: serverRequestId }
            : meta,
      };
    } catch (error) {
      const durationMs = Date.now() - startedAt;
    ...
    try {
      ...
      const tools = listed.tools.map((tool) =>
        normalizeMcpToolDescriptor(
          ...
          //   - http：结果来自 CCbuddy 后端。fetch wrapper 对每次请求校验 origin；登录态只在
          ...
          config.type === "http" && config.auth?.type === CCBUDDY_OFFICIAL_MCP_AUTH_TYPE,
        ),
      );
  ...
  private async runInteractiveOAuthAuthorization(input: {
    ...
    try {
      ...
      const credentialStore = oauthOptions?.credentialStore ?? createSharedCCbuddyCredentialStore();
  ...
  private createOAuthClientProvider(
    ...
    // 官方鉴权与 OAuth 互斥：官方 MCP 的失败只能由 CCbuddy 登录/套餐解决，
  ...
  private resolveCredentialStore(): SharedCCbuddyCredentialStore {
    this.credentialStore ??= this.mcpOAuth?.credentialStore ?? createSharedCCbuddyCredentialStore();
...
function mcpRequestMeta(request: McpCallToolRequest): Record<string, unknown> {
  // nodeRepl.requestMeta 暴露。CCbuddy 所有 MCP server 都可忽略这些扩展键；node_repl browser
  ...
  return {
    ...requestContext,
    "com.ccbuddy/request-context": requestContext,
  };
}
...
function resolveVersionNegotiation(
  ...
  return {
    ...
    probe: {
      ...
      // CCbuddy 的总连接超时可能先结束并让 probe 残留，也不给 legacy initialize 留预算。
...
function resolveAuthorizationCodeOAuthConfig(
  ...
  // 只能由 CCbuddy 自身的登录/套餐解决，不可能由目标 MCP 的 OAuth 授权解决。
  if (isOfficialAuthConfig(config)) return undefined;
...
function isOfficialAuthConfig(config: McpServerConfig): boolean {
  return (
    ...
    config.auth?.type === "ccbuddy_official" &&
