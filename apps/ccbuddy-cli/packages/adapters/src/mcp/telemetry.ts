...
import type { CCbuddyMcpTelemetryEvent } from "@ccbuddy/shared";
import {
...
type McpTelemetryEvent = CCbuddyMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: CCbuddyMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: CCbuddyMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as CCbuddyMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as CCbuddyMcpTelemetryEvent["platform"]);
