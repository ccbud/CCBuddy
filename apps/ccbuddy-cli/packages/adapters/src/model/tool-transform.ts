...
import { ModelErrorCode, type JsonSchema, type ModelToolContract } from "@ccbuddy/contracts";
...
function hoistLocalJsonSchemaRefsToDefs(toolName: string, schema: JsonSchema): JsonSchema {
  ...
  for (const ref of refs) {
    let defKey = `ccbuddy_ref_${nextDefIndex}`;
    while (usedDefKeys.has(defKey)) {
      nextDefIndex += 1;
      defKey = `ccbuddy_ref_${nextDefIndex}`;
    }
    nextDefIndex += 1;
...
function providerOptionsForClientTool(options: AiSdkToolTransformOptions): {
  ...
  // Anthropic-compatible gateways reject that extra tool field, so CCbuddy opts out
