import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@ccbuddy/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getCCbuddyCopy(result.locale ?? input.fallback.locale).tui;
