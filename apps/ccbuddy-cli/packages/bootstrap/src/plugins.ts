...
import { CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@ccbuddy/contracts";
import { resolveWorkspaceStateRoot } from "@ccbuddy/contracts/workspace-state";
import { CCBUDDY_CUA_OFFICIAL_PLUGIN_ID, isCCbuddyCuaInternalFeatureEnabled } from "@ccbuddy/shared";
...
export interface ResolveCCbuddyPluginsOptions {
...
export interface ListCCbuddyPluginsOptions extends ResolveCCbuddyPluginsOptions {}

export interface SetCCbuddyPluginEnabledOptions extends ResolveCCbuddyPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetCCbuddyPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface CCbuddyMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface CCbuddyAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface CCbuddyInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface CCbuddyPluginsOverviewData {
  marketplaces: CCbuddyMarketplaceSummaryData[];
  availablePlugins: CCbuddyAvailablePluginData[];
  installedPlugins: CCbuddyInstalledPluginData[];
  restorableBuiltins: CCbuddyAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface CCbuddyMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: CCbuddyMarketplaceSummaryData[];
}

export interface AddCCbuddyMarketplaceOptions extends ResolveCCbuddyPluginsOptions {
...
export interface RemoveCCbuddyMarketplaceOptions extends ResolveCCbuddyPluginsOptions {
  marketplace: string;
}

export interface UpdateCCbuddyMarketplaceOptions extends ResolveCCbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallCCbuddyMarketplacePluginOptions extends ResolveCCbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallCCbuddyMarketplacePluginOptions extends ResolveCCbuddyPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`ccbuddy plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateCCbuddyMarketplacePluginOptions extends ResolveCCbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateCCbuddyPluginPathOptions extends ResolveCCbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface CCbuddyPluginUpdateData extends CCbuddyPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveCCbuddyPluginsOptions {
  pluginId: string;
}

interface ConfigureCCbuddyPluginOptions extends ResolveCCbuddyPluginsOptions {
...
interface ResetCCbuddyPluginConfigOptions extends ResolveCCbuddyPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateCCbuddyPluginOptions extends ResolveCCbuddyPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeCCbuddyPluginOptions extends ResolveCCbuddyPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface CCbuddyPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveCCbuddyPlugins(options: ResolveCCbuddyPluginsOptions = {}): PluginLoadOutcome {
...
export function getCCbuddyPluginsOverview(
  options: ResolveCCbuddyPluginsOptions = {},
  ...
  // ccbuddy-plugins-official 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: CCbuddyMarketplaceSummaryData;
  ...
  const restorableBuiltins: CCbuddyAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isCCbuddyCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
      marketplace: CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE,
      version: def.version,
      installed: false,
      ...(listing ? { listing } : {}),
    };
...
export function listCCbuddyPlugins(options: ListCCbuddyPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveCCbuddyPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setCCbuddyPluginEnabled(
  options: SetCCbuddyPluginEnabledOptions,
): Promise<SetCCbuddyPluginEnabledResult> {
...
export async function addCCbuddyPluginMarketplace(
  options: AddCCbuddyMarketplaceOptions,
): Promise<CCbuddyMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeCCbuddyPluginMarketplace(
  options: RemoveCCbuddyMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateCCbuddyPluginMarketplace(
  options: UpdateCCbuddyMarketplaceOptions,
): Promise<CCbuddyMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installCCbuddyMarketplacePlugin(
  options: InstallCCbuddyMarketplacePluginOptions,
): Promise<CCbuddyPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallCCbuddyMarketplacePlugin(
  options: UninstallCCbuddyMarketplacePluginOptions,
): Promise<CCbuddyInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 ccbuddy-plugins-official id 空间，且其缓存
...
/**
 * `ccbuddy plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateCCbuddyMarketplacePlugin(
  options: UpdateCCbuddyMarketplacePluginOptions,
): Promise<CCbuddyPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateCCbuddyPluginMarketplace({
    ...options,
  ...
  const installed = await installCCbuddyMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `ccbuddy plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateCCbuddyPluginPath(
  options: ValidateCCbuddyPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const ccbuddyCuaPluginId = CCBUDDY_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === ccbuddyCuaPluginId &&
    !isCCbuddyCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires CCBUDDY_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureCCbuddyPlugin(options: ConfigureCCbuddyPluginOptions): Promise<void> {
...
export async function resetCCbuddyPluginConfig(
  options: ResetCCbuddyPluginConfigOptions,
...
export async function validateCCbuddyPlugin(
  options: ValidateCCbuddyPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeCCbuddyPlugin(
  options: DescribeCCbuddyPluginOptions,
...
function resolvePluginContext(options: ResolveCCbuddyPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallCCbuddyMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveCCbuddyPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // 项目 .ccbuddy 是兼容读取源；CCbuddy 自己的插件开关只写用户数据根下的 workspace 配置。
  const workspaceConfigPath = join(
    resolveWorkspaceStateRoot(workingDirectory, { workspaceIdentity: options.workspaceIdentity }),
    "config.json",
  );
