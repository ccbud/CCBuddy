...
import { getModelUsageTotalTokens, SessionEventType } from "@ccbuddy/contracts";
import { parseAutomationRunId } from "@ccbuddy/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const ccbuddy = recordValue(meta.ccbuddy);
  return (
    ...
    optionalString(ccbuddy.parentToolCallId) ??
    optionalString(ccbuddy.parentToolUseId)
  );
}
