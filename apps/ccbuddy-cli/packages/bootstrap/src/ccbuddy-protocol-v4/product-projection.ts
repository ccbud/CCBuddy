import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@ccbuddy/shared/ccbuddy-protocol-v4";
...
import { verdictWorkspaceHookReviewRequest } from "@ccbuddy/shared/workspace-hook-review-monotonicity";
import {
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  isCCbuddyModelRetryRecoveryProgressPayload,
  isCCbuddyFileStreamingToolInputPreviewTool,
  parseCCbuddyBackgroundTaskNotificationText,
  resolveCCbuddyBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseCCbuddyBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = ccbuddyBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isCCbuddyFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < CCBUDDY_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveCCbuddyBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @ccbuddy/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
