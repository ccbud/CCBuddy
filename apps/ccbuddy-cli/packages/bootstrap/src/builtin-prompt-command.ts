...
interface ResolveCCbuddyBuiltinPromptCommandOptions {
  /**
...
export function resolveCCbuddyBuiltinPromptCommand(
  input: string,
  options: ResolveCCbuddyBuiltinPromptCommandOptions = {},
): string | undefined {
...
function buildInitAgentsPrompt(params: {
  ...
  return [
    "You are running CCbuddy's built-in /init command.",
    "",
    "Your task is to create or update a concise workspace instruction file for future CCbuddy agents.",
    "",
    "Target:",
    `- Workspace directory: ${params.workingDirectory}`,
    `- Instruction file: ${params.targetPath}`,
    `- Existing hidden instruction candidate: ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.ccbuddy/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .agents/AGENTS.md exists in the workspace. If it exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
    ...
    "6. Include only project-specific facts future CCbuddy agents would otherwise miss.",
