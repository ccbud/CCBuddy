...
import {
  getCapturedCCbuddyCuaBrokerCredentials,
  isCCbuddyCuaMcpCommand,
  isCCbuddyCuaMcpPackageArg,
  CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY,
  CCBUDDY_CUA_NODE_REPL_HOST_ENV_KEY,
  CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
  CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  CCBUDDY_PLUGIN_ID_ENV_KEY,
} from "@ccbuddy/shared";

export { CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY as CCBUDDY_CUA_BROKER_SOCKET_ENV } from "@ccbuddy/shared";
// CLI 入口会先清理 broker 凭据；shared node_repl 的可信配置随后从进程内捕获快照恢复它们。
function resolveCCbuddyCuaBrokerSocket(): string | undefined {
  // captured 优先；运行时残留的 stale socket 不能覆盖可信快照。
  return (
    getCapturedCCbuddyCuaBrokerCredentials().socket ||
    process.env[CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY]?.trim()
  );
}

function resolveCCbuddyCuaBrokerToken(): string | undefined {
  return undefined;
}

const NODE_REPL_SERVER_NAME = "node_repl";
const REFRESH_MARKER_ENV = "CCBUDDY_CUA_PERMISSION_BROKER_REFRESH_MARKER";
    ...
    Object.entries(pluginServers)
      .filter(
        ([name, config]) =>
          ...
          config.type === "stdio" &&
          config.env?.[CCBUDDY_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() ===
            CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
      )
      .map(([name]) => name),
  );
...
export async function listMcpServerStatuses(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // 迁移后旧的 ccbuddy-cua MCP 只是历史配置，不再进入状态投影；CUA
...
export function omitMcpServers(
  ...
  return injectCCbuddyCuaBrokerMcpServers(
    kept,
    resolveCCbuddyCuaBrokerSocket(),
    resolveCCbuddyCuaBrokerToken(),
    trustedOfficialCuaServerNames,
  );
}

function injectCCbuddyCuaBrokerMcpServers(
  servers: Record<string, McpServerConfig>,
...
function injectCuaCredentialsIntoNodeRepl(
  ...
  const captured = getCapturedCCbuddyCuaBrokerCredentials();
  ...
  return {
    ...
    env: {
      ...config.env,
      [CCBUDDY_CUA_BROKER_SOCKET_ENV_KEY]: socketPath,
      ...(refreshMarker ? { [REFRESH_MARKER_ENV]: refreshMarker } : {}),
      ...(pluginAuthority ? { [CCBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: pluginAuthority } : {}),
      [CCBUDDY_CUA_NODE_REPL_HOST_ENV_KEY]: "1",
      [CCBUDDY_PLUGIN_ID_ENV_KEY]: CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
    },
  };
}

function isCCbuddyCuaStdioServer(
  name: string,
  config: McpServerConfig,
): config is McpStdioServerConfig {
  if (config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 内置 official ccbuddy-cua plugin 的 MCP server 走 __ccbuddy-plugin-host，command 是 Helper
  // (非 ccbuddy-cua)、args 是 [ccbuddy.cjs, __ccbuddy-plugin-host, server.js]（非 ccbuddy-cua package arg），
  // 上面的 name/command/args 三条都匹配不到。_plugin id 由 adapters resolver 权威写入 env
  // （manifest/user env 不可覆盖），用它识别 official plugin server。
  if (
    config.env?.[CCBUDDY_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() === CCBUDDY_CUA_OFFICIAL_PLUGIN_ID
  ) {
    return true;
  }
  // 判定与 desktop/services 共用 @ccbuddy/shared 的单一事实源，避免两条注入入口漂移。
  if (isCCbuddyCuaMcpCommand(config.command)) return true;
  return (config.args ?? []).some(isCCbuddyCuaMcpPackageArg);
}
...
function isRetiredCuaMcpServer(name: string, config: McpServerConfig | undefined): boolean {
  ...
  return (
    name !== NODE_REPL_SERVER_NAME && config !== undefined && isCCbuddyCuaStdioServer(name, config)
  );
}
