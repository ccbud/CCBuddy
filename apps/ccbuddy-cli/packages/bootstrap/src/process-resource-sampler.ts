...
import {
  CCBUDDY_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type CCbuddyProcessResourceSample,
} from "@ccbuddy/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const CCBUDDY_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = CCBUDDY_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateCCbuddyProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: CCbuddyProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: CCbuddyProcessResourceSample["platform"];
  arch?: CCbuddyProcessResourceSample["arch"];
...
export interface CCbuddyProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createCCbuddyProcessResourceSampler(
  options: CreateCCbuddyProcessResourceSamplerOptions,
): CCbuddyProcessResourceSampler {
  const platform = options.platform ?? (process.platform as CCbuddyProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as CCbuddyProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: CCbuddyProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, CCBUDDY_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
