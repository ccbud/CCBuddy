...
import { workspaceHookPolicySchema } from "@ccbuddy/contracts";
import type { WorkspaceHookPolicyProvider } from "@ccbuddy/core";
import {
  ccbuddyWorkspaceHookTrustGrantParamsSchema,
  ccbuddyWorkspaceHookTrustGrantReasonCodeSchema,
  ccbuddyWorkspaceHookTrustGrantResultSchema,
  type CCbuddyWorkspaceHookTrustGrantReasonCode,
  type CCbuddyWorkspaceHookTrustGrantResult,
} from "@ccbuddy/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = ccbuddyWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return ccbuddyWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): CCbuddyWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): CCbuddyWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = ccbuddyWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
