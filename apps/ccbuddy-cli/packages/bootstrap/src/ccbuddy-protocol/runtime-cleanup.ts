import type { Logger, McpPort } from "@ccbuddy/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@ccbuddy/adapters/mcp";
import type { SqliteSessionStore } from "@ccbuddy/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@ccbuddy/telemetry";
...
import type { CCbuddyProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<CCbuddyProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<CCbuddyProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`CCbuddy Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `ccbuddy_protocol.${resource}.shutdown.failed`,
