...
import type { McpTelemetryTracker } from "@ccbuddy/adapters";
import type { WorkspaceHookPolicyProvider } from "@ccbuddy/core";
import type { AccountProviderConfigSnapshot } from "@ccbuddy/provider";
import {
  ...
  type CCbuddyProtocolMessage,
  type CCbuddyProtocolMethod,
  type CCbuddyProtocolNotification,
  type CCbuddyProtocolRequest,
  type CCbuddyProtocolRequestId,
  type CCbuddyProtocolTrace,
  type CCbuddySessionMode,
  type CCbuddySessionPersistence,
  type CCbuddyWorkspaceRef,
} from "@ccbuddy/shared";
import type { CCbuddyApp, CCbuddyAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../ccbuddy-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../ccbuddy-protocol-v4/v4-gateway.js";
...
export interface CCbuddyProtocolAgentDependencies {
  createCCbuddyApp(options?: Omit<CCbuddyAppOptions, "providerRegistry">): CCbuddyApp | Promise<CCbuddyApp>;
  /**
...
export type CCbuddyProtocolAgentResolvedDependencies = CCbuddyProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface CCbuddyProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface CCbuddyProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface CCbuddyProtocolSessionRecord {
  app: CCbuddyApp;
  ...
  protocolEventSequences: Map<string, CCbuddyProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, CCbuddyProtocolToolInputTransmissionState>;
...
export interface CCbuddyProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: CCbuddyProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: CCbuddyProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: CCbuddyProtocolMessage): message is CCbuddyProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: CCbuddyProtocolMessage,
): message is CCbuddyProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: CCbuddyProtocolMessage,
): message is { id: CCbuddyProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: CCbuddyProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): CCbuddyProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: CCbuddyDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("CCbuddy Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "ccbuddy_protocol.session.require_missing",
...
export function createProtocolLogger(deps: CCbuddyProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("ccbuddy").child({
    module: "bootstrap.ccbuddy_protocol",
  });
}
