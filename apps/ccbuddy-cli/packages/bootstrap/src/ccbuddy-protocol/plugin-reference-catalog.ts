...
import {
  CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID,
  ccbuddyProtocolNotifications,
  ccbuddyPluginsReferenceCatalogParamsSchema,
  ccbuddyPluginsResolveSuggestedReferenceParamsSchema,
  type CCbuddyPluginReferenceCatalogEntry,
  type CCbuddyPluginDiagnostic as SharedPluginDiagnostic,
  type CCbuddyPluginsReferenceCatalogResult,
  type CCbuddyPluginsResolveSuggestedReferenceResult,
} from "@ccbuddy/shared";
import type { PluginReferenceCatalogEntry } from "@ccbuddy/contracts";
import { buildPluginReferenceCatalog } from "@ccbuddy/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(ccbuddyPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<CCbuddyPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(ccbuddyPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 ccbuddy-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveCCbuddyPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: ccbuddyProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateCCbuddyPluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 ccbuddy-plugins-official 超时（10000 ms）");
  ...
  const overview = getCCbuddyPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getCCbuddyPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): CCbuddyPluginReferenceCatalogEntry {
