...
import { ccbuddyProtocolMessageSchema, ccbuddyProtocolMethods } from "@ccbuddy/shared";
import type { Logger } from "@ccbuddy/contracts";

type CCbuddyProtocolOutgoingMessage =
  | CCbuddyProtocolError
  | CCbuddyProtocolNotification
  | CCbuddyProtocolRequest
  | CCbuddyProtocolResponse;

type CCbuddyProtocolMessageHandler = (
...
interface CCbuddyProtocolNdjsonConnectionOptions {
  ...
  handleMessage: CCbuddyProtocolMessageHandler;
  ...
  ) => readonly CCbuddyProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: CCbuddyProtocolRequestId) => {
    readonly messages: readonly CCbuddyProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class CCbuddyProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: CCbuddyProtocolNdjsonConnectionOptions) {
  ...
  send(message: CCbuddyProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("CCbuddy Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("CCbuddy Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): CCbuddyProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("CCbuddy Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "ccbuddy_protocol.parse.failed",
    ...
    const parsed = ccbuddyProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid CCbuddy Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: CCbuddyProtocolMessage): boolean {
  ...
  private async handleMessage(message: CCbuddyProtocolMessage): Promise<void> {
    if (this.terminal) return;
