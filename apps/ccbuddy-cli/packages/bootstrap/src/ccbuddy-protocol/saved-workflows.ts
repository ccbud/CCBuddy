...
// 解析器与序列化器只从 @ccbuddy/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.ccbuddy/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@ccbuddy/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  CCBUDDY_WORKFLOWS_RUNS_MAX_LIMIT,
  ccbuddyWorkflowsDeleteParamsSchema,
  ccbuddyWorkflowsGetParamsSchema,
  ccbuddyWorkflowsListParamsSchema,
  ccbuddyWorkflowsMoveParamsSchema,
  ccbuddyWorkflowsRunsParamsSchema,
  ccbuddyWorkflowsUpdateMetaParamsSchema,
  type CCbuddySavedWorkflowRun,
  type CCbuddySavedWorkflowScope,
  type CCbuddyWorkflowsDeleteResult,
  type CCbuddyWorkflowsGetResult,
  type CCbuddyWorkflowsListResult,
  type CCbuddyWorkflowsMoveResult,
  type CCbuddyWorkflowsRunsResult,
  type CCbuddyWorkflowsUpdateMetaResult,
} from "@ccbuddy/shared";
import type { JournalStorePort } from "@ccbuddy/dynamic-workflow";
...
function scopeOf(params: { scope?: CCbuddySavedWorkflowScope }): CCbuddySavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  const params = parseParams(ccbuddyWorkflowsListParamsSchema, rawParams);
  ...
  const workspaceIdentity = params.workspace.workspaceIdentity;
  const listed = listSavedWorkflows({ cwd, scope, workspaceIdentity });
  // 扫过的目录（即使不存在也回）：GUI 的文件监听靠它 watch。
  return {
    workflows: listed.entries,
    invalid: listed.invalid,
    dir: savedWorkflowRoot(cwd, scope, { workspaceIdentity }).dir,
  };
}
...
export async function getSavedWorkflowOp(
  _context: CCbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<CCbuddyWorkflowsGetResult> {
  const params = parseParams(ccbuddyWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(ccbuddyWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(ccbuddyWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(ccbuddyWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(CCBUDDY_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: CCbuddySavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, CCbuddySavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, CCbuddySavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  ...
  const params = parseParams(ccbuddyWorkflowsMoveParamsSchema, rawParams);
  const result = moveSavedWorkflow({
    cwd: params.workspace.workspacePath,
    name: params.name,
    workspaceIdentity: params.workspace.workspaceIdentity,
  });
