...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@ccbuddy/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  ccbuddyProtocolNotifications,
  ccbuddyStorageStartupStateSchema,
  type CCbuddyStorageStartupState,
} from "@ccbuddy/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: CCbuddyStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = ccbuddyStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: ccbuddyProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { ccbuddyStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@ccbuddy/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = ccbuddyStoragePathReadySchema.parse(JSON.parse(line));
