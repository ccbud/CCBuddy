import type { CCbuddyMessagePart, CCbuddyMessageWithParts, CCbuddyToolState } from "@ccbuddy/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@ccbuddy/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): CCbuddyMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): CCbuddyMessagePart {
  const base = {
...
function mapToolState(state: ToolState): CCbuddyToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // CCbuddy protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
