import type { McpServerConfig } from "@ccbuddy/contracts";
import type { CCbuddyProtocolMcpServer } from "@ccbuddy/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: CCbuddyProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // CCbuddy Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
