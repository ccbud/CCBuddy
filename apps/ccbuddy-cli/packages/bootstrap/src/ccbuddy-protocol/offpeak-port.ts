import type { OffPeakPort, OffPeakTaskSummary } from "@ccbuddy/contracts";
import {
  ccbuddyOffPeakCreateResultSchema,
  ccbuddyOffPeakListResultSchema,
  ccbuddyProtocolMethods,
  type CCbuddyOffPeakTaskProtocolSnapshot,
} from "@ccbuddy/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            ccbuddyProtocolMethods.offPeakList,
            {},
            ccbuddyOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        ccbuddyProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: CCbuddyOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
