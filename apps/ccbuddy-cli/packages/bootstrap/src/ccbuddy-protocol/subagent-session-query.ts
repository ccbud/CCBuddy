...
import type { CCbuddySessionEndedSubagent, CCbuddySessionRunningSubagent } from "@ccbuddy/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: CCbuddySessionRunningSubagent[];
  ended: CCbuddySessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): CCbuddySessionRunningSubagent["status"] | undefined {
...
): CCbuddySessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): CCbuddySessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: CCbuddySessionRunningSubagent[] = [];
  const ended: CCbuddySessionEndedSubagent[] = [];
...
function encodeCursor(item: CCbuddySessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly CCbuddySessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: CCbuddySessionEndedSubagent[]; nextCursor?: string } {
