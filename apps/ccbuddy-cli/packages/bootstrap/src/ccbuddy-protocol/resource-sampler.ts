...
import {
  createCCbuddyProcessResourceSampler,
  type CCbuddyProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: CCbuddyProtocolAgentServer,
  send: (message: CCbuddyProtocolNotification) => void,
  logger: Logger,
): CCbuddyProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createCCbuddyProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: ccbuddyProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "ccbuddy_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
