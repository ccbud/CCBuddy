import {
  ...
  type CCbuddyActiveToolCall,
  type CCbuddyContextUsageBreakdownItem,
  type CCbuddyDeliveryKind,
  type CCbuddyPendingPermission,
  type CCbuddySessionContextUsage,
  type CCbuddySessionEvent,
  type CCbuddySessionGoal,
  type CCbuddySessionGoalVerification,
  type CCbuddySessionGoalVerificationTimeline,
  type CCbuddySessionGoalStats,
  type CCbuddySessionInfo,
  type CCbuddySessionKind,
  type CCbuddySessionProjection,
  type CCbuddySessionRuntimeState,
  type CCbuddySessionSettingsState,
  type CCbuddySessionStateSnapshot,
  type CCbuddySessionTodoGroup,
...
import type { CCbuddyApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: CCBUDDY_PROTOCOL_NAME,
      version: CCBUDDY_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<CCbuddyApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 ccbuddy-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("ccbuddy-artifact://") ? artifactUri : undefined;
}
...
): Promise<CCbuddySessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<CCbuddyApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as CCbuddySessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is CCbuddySessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = ccbuddyApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const ccbuddyMeta = asRecord(meta.ccbuddy);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = ccbuddyApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): CCbuddySessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<CCbuddyApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: CCbuddyDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): CCbuddySessionContextUsage | undefined {
...
): CCbuddySessionGoalVerification[] {
...
): CCbuddySessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: CCbuddySessionContextUsage["cache"] | undefined,
): CCbuddySessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): CCbuddySessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): CCbuddyPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 ccbuddyPendingPermissionSchema（shared/src/ccbuddy-protocol/index.ts:1139）和
  // ccbuddyPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 ccbuddyPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): CCbuddyActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): CCbuddySessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): CCbuddySessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): CCbuddySessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, CCbuddySessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, CCbuddySessionTodoGroup>,
  input: {
  ...
  const group: CCbuddySessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): CCbuddySessionEvent["type"] {
  switch (type) {
