import { BUILTIN_CCBUDDY_SLASH_COMMAND_HELP_ENTRIES, type CCbuddySlashCommand } from "@ccbuddy/shared";
import { BUILTIN_WORKFLOW_COMMAND_NAME } from "../builtin-workflow-command.js";
import {
  listCCbuddyCustomCommands,
  type ListCCbuddyCustomCommandsOptions,
} from "../custom-commands.js";
import {
  APP_PROTOCOL_APP_ONLY_BUILTIN_SLASH_COMMANDS,
  APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES,
  isReservedCCbuddySlashCommandName,
} from "../slash-command-surface.js";

export interface ListProtocolSlashCommandsOptions extends ListCCbuddyCustomCommandsOptions {
  /**
...
export async function listProtocolSlashCommands(
  ...
  let customCommands: Awaited<ReturnType<typeof listCCbuddyCustomCommands>>["commands"] = [];
  try {
    const outcome = await listCCbuddyCustomCommands(options);
  ...
  return [
    ...
    ...customCommands
      ...
      .filter((command) => !isReservedCCbuddySlashCommandName(command.name))
      .map((command) => ({
...
function listAppProtocolBuiltinSlashCommands(): CCbuddySlashCommand[] {
  const sharedBuiltins = APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES.flatMap((name) => {
    const command = BUILTIN_CCBUDDY_SLASH_COMMAND_HELP_ENTRIES.find((entry) => entry.name === name);
    if (!command) return [];
    return [
      {
