/* eslint-disable max-lines -- CCbuddy Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../ccbuddy-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@ccbuddy/adapters/exec";
import { inputIntentMetadata } from "../ccbuddy-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@ccbuddy/core";
import {
  CoreErrorType,
...
import {
  ...
  ccbuddyProtocolErrorCodes,
  ccbuddyProtocolMethods,
  ccbuddySessionCancelBackgroundTaskParamsSchema,
  ccbuddySessionCompactParamsSchema,
  ccbuddySessionCloseParamsSchema,
  ccbuddySessionCreateParamsSchema,
  ccbuddySessionEventsParamsSchema,
  ccbuddySessionForkParamsSchema,
  ccbuddySessionGoalParamsSchema,
  ccbuddySessionListParamsSchema,
  ccbuddySessionMessagesParamsSchema,
  ccbuddySessionReadParamsSchema,
  ccbuddySessionRuntimePreferencesResultSchema,
  ccbuddySessionResumeParamsSchema,
  ccbuddySessionSendParamsSchema,
  ccbuddySessionSetModeParamsSchema,
  ccbuddySessionSetModelParamsSchema,
  ccbuddySessionSetThoughtLevelParamsSchema,
  ccbuddySessionStopParamsSchema,
  ccbuddySessionSubscribeParamsSchema,
  ccbuddySessionSubagentsParamsSchema,
  ccbuddyTaskTokenUsageParamsSchema,
  ccbuddyUsageStatsParamsSchema,
  ccbuddyWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type CCbuddyAutomationBotDeliveryTarget,
  type CCbuddySessionCreateParams,
  type CCbuddyDeliveryKind,
  type IntegratedTerminalShellSelection,
  type CCbuddySessionRuntimePreferencesScope,
  type CCbuddySessionRuntimePreferencesResult,
  type CCbuddyModelContextBudgetStrategy,
  type CCbuddyProtocolTrace,
  type CCbuddySessionEvent,
  type CCbuddySessionHistoryTarget,
  type CCbuddySessionResumeParams,
  type CCbuddySessionPersistence,
...
import {
  ...
  type CCbuddyProtocolSessionRecord,
  type CCbuddyProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceCCbuddyApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<CCbuddyProtocolSessionRecord["app"]["readTarget"]>>>
>;

type CCbuddySessionRecordParams = (
  | CCbuddySessionCreateParams
  | (CCbuddySessionResumeParams & {
      mode?: CCbuddySessionCreateParams["mode"];
      model?: CCbuddySessionCreateParams["model"];
      parentSessionId?: CCbuddySessionCreateParams["parentSessionId"];
      thoughtLevel?: CCbuddySessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: CCbuddySessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: CCbuddyModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: CCbuddyProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<CCbuddyProtocolSessionRecord["app"], "listThoughtLevels">,
...
const liveProtocolStreamingDeltaBatches = new WeakMap<
  CCbuddyProtocolSessionRecord,
  Map<string, LiveProtocolStreamingDeltaBatch>
>();

function protocolEventSequenceKey(deliveryKind?: CCbuddyDeliveryKind): string {
  return deliveryKind ?? DEFAULT_PROTOCOL_EVENT_SEQUENCE_KEY;
}

function getProtocolEventSequenceState(
  record: CCbuddyProtocolSessionRecord,
...
function getProtocolToolInputTransmissionState(
  record: CCbuddyProtocolSessionRecord,
  deliveryKind?: CCbuddyDeliveryKind,
): CCbuddyProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: CCbuddyProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const ccbuddy = asRecord(meta.ccbuddy);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(ccbuddy.parentToolUseId) ??
    stringValue(ccbuddy.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: CCbuddySessionEvent,
  toolInputTransmissions: CCbuddyProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): CCbuddySessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("CCbuddy Protocol session event filtered", {
      event: "ccbuddy_protocol.session_event.filtered",
  ...
  // tool_input_delta 等高频模型中间态。CCbuddy Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<CCbuddySessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: CCbuddySessionEvent[] = [];
  const toolInputTransmissions: CCbuddyProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("CCbuddy Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "ccbuddy_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<CCbuddySessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<CCbuddyProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<CCbuddySessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: CCbuddyProtocolSessionRecord;
  sessionId: SessionId;
  createParams: CCbuddySessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("CCbuddy Protocol imported history persisted", {
    event: "ccbuddy_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: CCbuddyProtocolAgentServerContext,
  rawParams: unknown,
  trace: CCbuddyProtocolTrace | undefined,
  project: (
    record: CCbuddyProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(ccbuddySessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("CCbuddy Protocol session/create started", {
    event: "ccbuddy_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("CCbuddy Protocol session/create skipped unsupported thought level", {
          event: "ccbuddy_protocol.session_create.thought_level_skipped",
          module: "bootstrap.ccbuddy_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 CCbuddy session。
    ...
    context.logger?.info("CCbuddy Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "ccbuddy_protocol.session_create.completed",
    ...
    context.logger?.warn("CCbuddy Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "ccbuddy_protocol.session_create.failed",
      module: "bootstrap.ccbuddy_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: CCbuddyProtocolAgentServerContext,
  params: CCbuddySessionResumeParams,
  ...
  context.logger?.debug("CCbuddy Protocol session resume started", {
    activeBeforeWait,
    event: "ccbuddy_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("CCbuddy Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "ccbuddy_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.ccbuddy_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      ccbuddyProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("CCbuddy Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "ccbuddy_protocol.session.resume_completed",
    module: "bootstrap.ccbuddy_protocol",
...
export async function resumeSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "ccbuddy_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "ccbuddy_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.ccbuddy_protocol",
      sessionId: session.id,
...
export async function listSessions(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(ccbuddySessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("CCbuddy Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "ccbuddy_protocol.session.persisted_missing",
      module: "bootstrap.ccbuddy_protocol",
...
export async function getUsageStats(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddyUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionReadParamsSchema, rawParams);
...
export async function readMessages(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(ccbuddySessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("CCbuddy Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("CCbuddy Protocol session/send accepted", {
...
export async function compactSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("CCbuddy Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("CCbuddy Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("CCbuddy Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("CCbuddy Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("CCbuddy Protocol goal paused"));
    }
...
export async function forkSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: CCbuddyProtocolAgentServerContext,
  record: CCbuddyProtocolSessionRecord,
  fork: Awaited<ReturnType<CCbuddyProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: CCbuddyProtocolAgentServerContext,
  parentRecord: CCbuddyProtocolSessionRecord,
  forkRecord: CCbuddyProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("CCbuddy Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("CCbuddy Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("CCbuddy Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("CCbuddy Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("CCbuddy Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("CCbuddy Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("CCbuddy Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("CCbuddy Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(ccbuddySessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("CCbuddy Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(ccbuddySessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: CCbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(ccbuddySessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: CCbuddySessionPersistence,
  expectedPersistence?: CCbuddySessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(ccbuddyWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceCCbuddyApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: CCbuddyProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: CCbuddyProtocolAgentServerContext,
  _record: CCbuddyProtocolSessionRecord,
...
async function resolveForkTarget(
  context: CCbuddyProtocolAgentServerContext,
  record: CCbuddyProtocolSessionRecord,
  target: CCbuddySessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(ccbuddyTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: CCbuddyProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: ccbuddyProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "ccbuddy_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "ccbuddy_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // CCbuddyProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("CCbuddy Protocol runtime preferences request started", {
    event: "ccbuddy_protocol.runtime_preferences.request_started",
    module: "bootstrap.ccbuddy_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      ccbuddyProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      ccbuddySessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("CCbuddy Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "ccbuddy_protocol.runtime_preferences.response_received",
      module: "bootstrap.ccbuddy_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "ccbuddy_protocol.runtime_preferences.request_failed",
      module: "bootstrap.ccbuddy_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("CCbuddy Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("CCbuddy Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "CCbuddy Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_CCBUDDY_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: CCbuddyProtocolAgentServerContext,
  params: CCbuddySessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: CCbuddyProtocolTrace,
): Promise<CCbuddyProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("CCbuddy Protocol createRecord MCP config", {
    event: "ccbuddy_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: CCbuddyProtocolSessionRecord | undefined;
  const app = await createWorkspaceCCbuddyApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.ccbuddy/.agents 的 enabled MCP，
      ...
      // CCbuddy Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // CCbuddy Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: CCbuddyProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): CCbuddySessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isCCbuddySessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isCCbuddySessionMode(
  value: unknown,
): value is NonNullable<CCbuddySessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("CCbuddy Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "ccbuddy_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "ccbuddy_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "ccbuddy_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "ccbuddy_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "ccbuddy_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: CCbuddyStateUpdatedNotification = {
    patch,
    reason,
