...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@ccbuddy/contracts";
import {
  ccbuddyMcpListParamsSchema,
  ccbuddyMcpListResultSchema,
  type CCbuddyMcpListResult,
} from "@ccbuddy/shared";
import {
...
export async function listMcpServers(
  ...
  const params = parseParams(ccbuddyMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.ccbuddy_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.ccbuddy` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return ccbuddyMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return ccbuddyMcpListResultSchema.parse({
