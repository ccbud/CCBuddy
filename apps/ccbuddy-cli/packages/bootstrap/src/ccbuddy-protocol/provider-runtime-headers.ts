...
import {
  ...
  ccbuddyProviderRuntimeHeadersResponseSchema,
  type CCbuddyWorkspaceRef,
} from "@ccbuddy/shared";
import type { CCbuddyAppOptions } from "../app/types.js";
import {
...
): NonNullable<CCbuddyAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          ccbuddyProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: ccbuddyProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // ccbuddy-plan 的账号鉴权材料是每次请求刷新的运行时配置，
