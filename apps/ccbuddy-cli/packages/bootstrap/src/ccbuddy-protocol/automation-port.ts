...
import {
  ...
  ccbuddyAutomationCheckTaskBindingResultSchema,
  ccbuddyAutomationCreateResultSchema,
  ccbuddyAutomationDeleteResultSchema,
  ccbuddyAutomationListResultSchema,
  ccbuddyAutomationUpdateResultSchema,
  ccbuddyProtocolMethods,
  type CCbuddyAutomationProtocol,
} from "@ccbuddy/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => CCbuddyProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              ccbuddyProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              ccbuddyProtocolMethods.automationList,
              {},
              ccbuddyAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          ccbuddyProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        ccbuddyProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        ccbuddyProtocolMethods.automationDelete,
        { automationId: input.id },
        ccbuddyAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: CCbuddyAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: CCbuddyAutomationProtocol): CronAutomation {
  return {
