...
import {
  ccbuddyPluginsCancelOperationParamsSchema,
  ccbuddyProtocolMethods,
  ccbuddyWorkspaceCancelGenerateTextParamsSchema,
  ccbuddyWorkspaceHookTrustGrantParamsSchema,
} from "@ccbuddy/shared";
import type { BrowserControlPort } from "@ccbuddy/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@ccbuddy/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  CCbuddyProtocolError,
  CCbuddyProtocolMessage,
  CCbuddyProtocolMethod,
  CCbuddyProtocolNotification,
  CCbuddyProtocolRequest,
  CCbuddyProtocolRequestId,
  CCbuddyProtocolResponse,
} from "@ccbuddy/shared";
import {
...
} from "../ccbuddy-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type CCbuddyProtocolClientRequestOptions,
  type CCbuddyProtocolAgentDependencies,
  type CCbuddyProtocolAgentServerContext,
  type CCbuddyProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@ccbuddy/contracts";

export type { CCbuddyProtocolAgentDependencies, CCbuddyProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type CCbuddyProtocolOutboundMessage = CCbuddyProtocolNotification | CCbuddyProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, CCbuddyProtocolSessionRecord>;
}): Promise<void> {
...
interface CCbuddyProtocolPostResponseBatch {
  readonly messages: readonly CCbuddyProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class CCbuddyProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<CCbuddyProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: CCbuddyProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: CCbuddyProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    CCbuddyProtocolRequestId,
    CCbuddyProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: CCbuddyProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createCCbuddyApp);
    const resolvedDeps = {
      ...deps,
      createCCbuddyApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, CCbuddyProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: CCbuddyProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("CCbuddy Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "ccbuddy_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: CCbuddyProtocolRequestId): CCbuddyProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: CCbuddyProtocolRequestId): CCbuddyProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: CCbuddyProtocolMessage,
  ): Promise<CCbuddyProtocolError | CCbuddyProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("CCbuddy Protocol notification ignored", {
        event: "ccbuddy_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: CCbuddyProtocolRequest,
  ): Promise<CCbuddyProtocolError | CCbuddyProtocolResponse> {
  ...
  private async dispatchRequest(request: CCbuddyProtocolRequest) {
      ...
      case ccbuddyProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: ccbuddyWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? ccbuddyWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(ccbuddyPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(ccbuddyWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: CCbuddyProtocolRequestId, result: unknown): CCbuddyProtocolResponse {
    return { id, result };
  }

  private fail(
    id: CCbuddyProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): CCbuddyProtocolError {
  ...
  private requestClient<T>(
    method: CCbuddyProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: CCbuddyProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No CCbuddy Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: CCbuddyProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: CCbuddyProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
