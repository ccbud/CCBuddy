...
import {
  ...
  ccbuddyUserInputResponseSchema,
  type CCbuddyPermissionOption,
  type CCbuddyPermissionResponse,
  type CCbuddyUserInputQuestion,
...
import type { CCbuddyProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        ccbuddyProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → CCbuddyPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: CCbuddyPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 ccbuddyPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        ccbuddyProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): CCbuddyUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): CCbuddyUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): CCbuddyUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): CCbuddyUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "ccbuddy_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
