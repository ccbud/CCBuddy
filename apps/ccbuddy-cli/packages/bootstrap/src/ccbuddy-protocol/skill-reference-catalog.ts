...
import {
  ccbuddySkillsReferenceCatalogParamsSchema,
  type CCbuddySkillReferenceCatalogEntry,
  type CCbuddySkillsReferenceCatalogResult,
} from "@ccbuddy/shared";
import type { SkillLoadOutcome, SkillMetadata } from "@ccbuddy/contracts";
import { listCCbuddySkills } from "../skills.js";
import {
  parseParams,
  requireSession,
...
export async function getSkillReferenceCatalog(
  ...
  const params = parseParams(ccbuddySkillsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
  ...
  const outcome = await listCCbuddySkills({
...
function toResult(
  authority: CCbuddySkillsReferenceCatalogResult["authority"],
...
function toReferenceCatalogEntry(skill: SkillMetadata): CCbuddySkillReferenceCatalogEntry {
  const scope =
