...
import { openStartupSqliteSessionStore } from "@ccbuddy/adapters/storage";
import type { SessionInfo } from "@ccbuddy/contracts";
import type { ListCCbuddySessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listCCbuddySessions(
  options: ListCCbuddySessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
