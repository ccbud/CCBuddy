import { updateUiLocaleInFileConfig, type ConfigResult } from "@ccbuddy/adapters/config";
import type { AgentRuntime } from "@ccbuddy/core";
import { resolveLocale } from "@ccbuddy/i18n";
import { normalizeModelSelection, type ModelSelection } from "@ccbuddy/provider";
import {
...
import type { PrepareUserExecutionBoundary, CCbuddyApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<CCbuddyApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<CCbuddyApp["updateTargetStatus"]>
      >,
