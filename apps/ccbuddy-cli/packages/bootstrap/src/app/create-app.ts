...
} from "@ccbuddy/adapters/exec";
import { createNodeFileSystemAdapter } from "@ccbuddy/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@ccbuddy/adapters/http";
import { createJimpImageProcessorAdapter } from "@ccbuddy/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@ccbuddy/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@ccbuddy/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@ccbuddy/adapters/context";
import { createNodeSkillAdapter } from "@ccbuddy/adapters/skills";
import { createMcpAdapter } from "@ccbuddy/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@ccbuddy/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveCCbuddyRuntimeEnv } from "@ccbuddy/shared";
import {
...
import { resolveCCbuddyCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveCCbuddyBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadCCbuddyAgentProfiles } from "../subagents.js";
...
export async function createCCbuddyApp(options: CCbuddyAppOptions): Promise<CCbuddyApp> {
  if (!options?.providerRegistry) {
    throw new Error("createCCbuddyApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("ccbuddy").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveCCbuddyRuntimeEnv(options.env ?? process.env) === "development",
    );
    const ccbuddySubagentProfileOutcome = await loadCCbuddyAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const ccbuddySubagentProfiles = ccbuddySubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: ccbuddySubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: ccbuddySubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...ccbuddySubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          ccbuddySubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("CCbuddy runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("CCbuddy runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).CCBUDDY_MAILBOX_ROOT ?? "~/.ccbuddy/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const dynamicWorkflowRunPort =
      dynamicWorkflowJournal === undefined
        ? undefined
        : createDynamicWorkflowRunService({
            ...(runtimeConfig.memory?.workspaceIdentity === undefined
              ? {}
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveCCbuddyBuiltinPromptCommand(text, {
        ...
        return await resolveCCbuddyCustomCommandPrompt(text, {
          env: options.env,
          executionPort,
          logger,
          projectConfigPath: options.projectConfigPath,
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("ccbuddy-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("ccbuddy-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new CCbuddyAttachmentFaultError(CCBUDDY_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new CCbuddyAttachmentFaultError(CCBUDDY_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("CCbuddy app startup failed", error, {
