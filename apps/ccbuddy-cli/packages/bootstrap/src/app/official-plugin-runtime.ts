...
import {
  CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE,
  CCBUDDY_PLUGIN_HOST_COMMAND,
...
export function createBundledMcpRuntimeConfig(input: {
  ...
  return {
    ...
    env: {
      ...input.env,
      // 桌面打包态 process.execPath 是 CCbuddy Helper；缺少 Node 模式会误进 Electron main。
...
export function writeOfficialPluginRuntimeManifest(input: OfficialRuntimeManifestInput): void {
  const manifestPath = join(input.rootPath, ".ccbuddy-plugin", "plugin.json");
  ...
  // 把 seed 流程整个阻断，连带 listCCbuddySkills 拉不出 plugin skill。
  if (manifest.mcpServers === undefined) return;
  const mcpServers = asRecord(manifest.mcpServers);
  const hostPrefixArgs = officialPluginHostPrefixArgs();
  if (!hostPrefixArgs) return;

  // 保留对其他历史 official plugin MCP 的通用重写；ccbuddy-cua 当前是 skill/SDK-only，
  ...
  for (const [serverKey, serverRaw] of Object.entries(mcpServers)) {
    ...
    mcpServer.env = {
      ...mcpServerEnv,
      // 桌面打包态的 process.execPath 是 CCbuddy Helper。
      ...
      [CCBUDDY_PLUGIN_ID_ENV_KEY]: `${input.pluginName}@${CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`,
    };
...
export function officialPluginHostPrefixArgs(): string[] | undefined {
  if (isSeaRuntime()) return [CCBUDDY_PLUGIN_HOST_COMMAND];

  const entrypoint = process.argv[1];
  if (!entrypoint) return undefined;

  return [...process.execArgv, resolve(entrypoint), CCBUDDY_PLUGIN_HOST_COMMAND];
}
