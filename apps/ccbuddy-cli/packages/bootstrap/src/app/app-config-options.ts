...
import { detectLocale, resolveLocale } from "@ccbuddy/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@ccbuddy/contracts";
import type { CCbuddyAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.CCBUDDY_MESSAGE_ENABLED === "1" || env.CCBUDDY_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: CCbuddyAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: CCbuddyAppOptions): SupportedLocale | undefined {
