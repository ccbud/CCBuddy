...
import {
  ...
  CoreErrorType,
  CCBUDDY_DWF_CHILD_COMMAND,
  type CreateSessionTaskLinkInput,
  type DynamicWorkflowRunEvent,
  type DynamicWorkflowRunProgressPayload,
  type SessionId,
} from "@ccbuddy/contracts";
import type { AgentRuntime } from "@ccbuddy/core";
import { parseModelPickerValue, type ModelSelection } from "@ccbuddy/shared/model-selection";
import {
...
export function launchDynamicWorkflowRun(
  ...
  return runWorkflowScript({
    ...
    makeDriver,
    ...(deps.workspaceIdentity === undefined
      ? {}
 ...
 * `@ccbuddy/dynamic-workflow` 与 node 内建），既拿不到 contracts 的子命令常量，也不该知道
...
export function dynamicWorkflowChildSpawn(
  ...
  return isSea ? { argsPrefix: [CCBUDDY_DWF_CHILD_COMMAND] } : undefined;
}
 ...
 * `ccbuddy-protocol-v4/product-projection.ts` 的 `applyWorkflowRunEvent` 逐种类归约，其
