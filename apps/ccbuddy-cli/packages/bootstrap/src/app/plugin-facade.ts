...
import type { PluginLoadOutcome } from "@ccbuddy/contracts";
...
import type {
  CCbuddyApp,
  CCbuddyAppOptions,
  CCbuddyPluginSetResult,
  CCbuddyPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listCCbuddyPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<CCbuddyPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<CCbuddyPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === CCBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
