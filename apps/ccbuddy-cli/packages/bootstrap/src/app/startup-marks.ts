import type { ConfigResult } from "@ccbuddy/adapters/config";
import { discoverNodePluginsSync } from "@ccbuddy/adapters/plugins";
import type { AgentRuntimeConfig } from "@ccbuddy/core";
import type { Logger, McpServerConfig, PluginLoadOutcome } from "@ccbuddy/contracts";
...
import type { CCbuddyAppOptions } from "./types.js";
...
export function resolveStartupPlugins(input: {
  ...
  logger?: Logger;
  options: Pick<CCbuddyAppOptions, "officialPluginRoots" | "pluginStorageRoot">;
  ...
  const pluginOutcome = discoverNodePluginsSync({
    ...
    // (resolveCCbuddyPlugins) 和应用启动路径都要传，否则 `/skill skill-creator` 在会话里
    // 报 "Skill not found: skill-creator"，但 `ccbuddy plugins list` 却显示它是 enabled。
    officialPluginsEnabledByDefault: DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS,
    storageRoot: pluginStorageRoot,
    workingDirectory: input.workingDirectory,
  });
  input.startupTimer.mark("CCbuddy plugins resolved", {
    context: {
...
export function startAppStartup(input: {
  ...
  input.startupTimer.start("CCbuddy app startup started", {
    context: {
...
export function markConfigurationLoaded(input: {
  ...
  input.startupTimer.mark("CCbuddy app configuration loaded", {
    context: {
...
export function markStorageAdaptersInitialized(input: {
  ...
  input.startupTimer.mark("CCbuddy storage adapters initialized", {
    context: {
...
export function markMcpAdapterInitialized(input: {
  ...
  input.startupTimer.mark("CCbuddy MCP adapter initialized", {
    context: {
...
export function markRuntimeConstructed(input: {
  ...
  input.startupTimer.mark("CCbuddy runtime constructed", {
    context: {
      hasInjectedModelAdapter: input.hasInjectedModelAdapter,
...
export function completeAppStartup(input: {
  ...
  input.startupTimer.complete("CCbuddy app startup completed", {
    context: {
      sessionId: input.sessionId,
      workingDirectory: input.workingDirectory,
    },
