import type { CCbuddyToolExecResource, BackgroundBashOutputResult } from "@ccbuddy/shared";
import type { AiSdkModelAdapter } from "@ccbuddy/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { CCbuddyModelOption } from "@ccbuddy/shared";
import type { EffectiveModelSelectionResult } from "@ccbuddy/shared/model-selection";
export type { CCbuddyModelOption } from "@ccbuddy/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { CCbuddyInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@ccbuddy/contracts";
...
export type CCbuddyAppRuntimeConfigInput = AgentRuntimeConfig;

export interface CCbuddyAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: CCbuddyAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: CCbuddyToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@ccbuddy/contracts").TurnBackgroundAttribution;
...
export interface CCbuddyPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface CCbuddyPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: CCbuddyInstalledPluginData | null;
}
...
export interface CCbuddyApp {
  ...
  getCurrentModelOption?(): CCbuddyModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): CCbuddyModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@ccbuddy/shared").CCbuddySessionSubagentsResult>;
  ...
  listModels(): CCbuddyModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<CCbuddyPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<CCbuddyPluginUninstallResult>;
  /**
...
export interface RunCCbuddyProtocolAgentOptions {
...
export interface ListCCbuddySessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
