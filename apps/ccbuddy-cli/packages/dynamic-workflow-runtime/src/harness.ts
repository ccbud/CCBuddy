 ...
 * 本包**只**依赖 `@ccbuddy/dynamic-workflow` 与 node 内建——证明整条管线 app-free 可跑，
 * 绝不 import `@ccbuddy/core`/`@ccbuddy/contracts`/`@ccbuddy/bootstrap`/`@ccbuddy/adapters`。
 *
 * 时序（happy path）：
 *
 *   parent                         child(vm)
 *     │  write ~/.ccbuddy/workspaces/<key>/workflow-runs/<runId>.mjs（payload: lowered+args 内嵌）
 *     │  spawn(node <entry>)
...
import { writeChildEntryFile } from "./child-entry-file.js";
...
export interface RunWorkflowOptions {
  ...
  cwd?: string;
  /** 逻辑工作区身份只用于应用状态隔离；子进程 cwd 仍是物理路径。 */
...
export async function runWorkflowScript(options: RunWorkflowOptions): Promise<RunSettlement> {
  ...
  try {
    ...
    // running，随后的 resume_from 被「has not settled yet」拒绝。CCbuddy 状态根写不下同理。
    // 一律经引擎结算 stopped(interrupted)：宿主侧故障，可 resume。
