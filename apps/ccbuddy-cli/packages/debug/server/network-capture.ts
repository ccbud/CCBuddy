...
const traceHeaderNames = ["x-ccbuddy-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          CCBUDDY_HTTP_PROXY: url,
          CCBUDDY_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.ccbuddyCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.CCBUDDY_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.CCBUDDY_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.CCBUDDY_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.CCBUDDY_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.CCBUDDY_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
