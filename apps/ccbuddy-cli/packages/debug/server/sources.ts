...
export function defaultLogDir(): string {
  return join(defaultStorageRoot(), "cli", "log");
}

export function defaultDbPath(): string {
  return join(defaultStorageRoot(), "cli", "db", "db.sqlite");
}

function defaultStorageRoot(): string {
  const explicit = process.env.CCBUDDY_STORAGE_DIR?.trim();
  if (explicit) return resolve(explicit);
  const baseDir = process.env.CCBUDDY_DATA_BASE_DIR?.trim() || homedir();
  return join(resolve(baseDir), ".ccbuddy");
}
