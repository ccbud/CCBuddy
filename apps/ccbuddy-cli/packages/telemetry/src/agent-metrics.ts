...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "ccbuddy.agent.turn.duration"),
      agent_step: duration(meter, "ccbuddy.agent.step.duration"),
      tool_execution: duration(meter, "ccbuddy.tool.execution.duration"),
      command_execution: duration(meter, "ccbuddy.command.execution.duration"),
      context_compaction: duration(meter, "ccbuddy.context.compaction.duration"),
      detached_operation: duration(meter, "ccbuddy.detached.operation.duration"),
      model_call: duration(meter, "ccbuddy.model.call.duration"),
      model_attempt: duration(meter, "ccbuddy.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("ccbuddy.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("ccbuddy.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "ccbuddy.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "ccbuddy.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "ccbuddy.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("ccbuddy.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "ccbuddy.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "ccbuddy.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("ccbuddy.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("ccbuddy.telemetry.abandoned.count", {
