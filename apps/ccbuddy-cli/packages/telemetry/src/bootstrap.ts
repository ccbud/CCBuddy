...
import type { ModelStatusSink } from "@ccbuddy/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  if (!resolveOtlpTraceEndpoint(env) || isExplicitlyDisabled(env.CCBUDDY_MODEL_TELEMETRY_ENABLED)) {
    return env;
  }
  const existingInstallationId = normalizeTelemetryDeviceMid(env.CCBUDDY_TELEMETRY_DEVICE_MID);
  const installationId =
    existingInstallationId ?? (await resolveStandaloneDeviceMid(env.CCBUDDY_HOME?.trim()));
  const preparedEnv = installationId ? { ...env, CCBUDDY_TELEMETRY_DEVICE_MID: installationId } : env;
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.CCBUDDY_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.CCBUDDY_ENV ??
      env.CCBUDDY_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.CCBUDDY_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.CCBUDDY_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.CCBUDDY_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.CCBUDDY_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "ccbuddy-cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.CCBUDDY_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.CCBUDDY_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.CCBUDDY_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  ccbuddyHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = ccbuddyHome
    ? join(ccbuddyHome, "v2", "telemetry-state.json")
    : join(resolveCcBuddyDataRoot(), "v2", "telemetry-state.json");
