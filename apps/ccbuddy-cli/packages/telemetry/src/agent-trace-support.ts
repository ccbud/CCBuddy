...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@ccbuddy/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 CCbuddy Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "ccbuddy.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "ccbuddy.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "ccbuddy.execution.turn_id": safeId(correlation.turnId),
    "ccbuddy.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "ccbuddy.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "ccbuddy.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "ccbuddy.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "ccbuddy.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "ccbuddy.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "ccbuddy.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
