...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@ccbuddy/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@ccbuddy/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("ccbuddy.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("ccbuddy.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "ccbuddy.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "ccbuddy.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("ccbuddy.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("ccbuddy.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "ccbuddy.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("ccbuddy.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("ccbuddy.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("ccbuddy.model.attempt.tokens"),
    cardinalityView("ccbuddy.model.attempt.stream_stall.count"),
    cardinalityView("ccbuddy.telemetry.creation_drop.count"),
    cardinalityView("ccbuddy.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "ccbuddy.build.commit_id": resource.buildCommitId,
    "ccbuddy.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "ccbuddy.product.version": resource.productVersion,
    "ccbuddy.runtime.distribution": resource.runtimeDistribution,
    "ccbuddy.runtime.surface": resource.runtimeSurface,
    "ccbuddy.telemetry.schema_owner": "cli",
    "ccbuddy.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
