...
async function resolveSaveWorkflowInput(
  ...
  const shadowing = findSavedWorkflowShadowing({ cwd, name, scope, workspaceIdentity });
  return {
    result: true,
    input: {
      ...parsed.data,
      ...
      overwrite: savedWorkflowExists({ cwd, name, scope, workspaceIdentity }),
...
const saveWorkflowHandler: ToolHandler = async (input, context) => {
  ...
  const path =
    parsed.path ??
    savedWorkflowPath(
      savedWorkflowRoot(cwd, scope, { workspaceIdentity: context.workspaceIdentity }),
      parsed.name,
    );
