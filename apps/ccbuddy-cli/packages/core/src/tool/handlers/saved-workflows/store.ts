...
import { resolveCcBuddyDataRoot, resolveWorkspaceStateRoot } from "@ccbuddy/contracts/workspace-state";
...
/**
 * `savedWorkflowRoots` / 派生函数的可选参数。`homeDir` 是测试用的 base-directory
 * 覆盖；生产由共享 CCbuddy 路径契约决定。
 */
 ...
 *
 * 项目档落在 CCbuddy 的逻辑工作区状态根，全局档落在 `~/.ccbuddy/workflows/`。
...
export function savedWorkflowRoots(
  ...
  return [
    ...
    {
      scope: "global",
      dir: join(resolveCcBuddyDataRoot(options?.homeDir), SAVED_WORKFLOW_GLOBAL_DIR),
    },
  ];
}
...
export function resolveSavedWorkflow(options: {
  ...
  const { cwd, name, scope, homeDir, workspaceIdentity } = options;
  ...
  const roots =
    scope === undefined
      ? savedWorkflowRoots(cwd, { homeDir, workspaceIdentity })
      : [savedWorkflowRoot(cwd, scope, { homeDir, workspaceIdentity })];
...
export function findSavedWorkflowShadowing(options: {
  ...
  const otherRoot = savedWorkflowRoot(options.cwd, otherScope, options);
...
export function moveSavedWorkflow(options: {
  ...
  const { cwd, name, homeDir, workspaceIdentity } = options;
  if (!isValidSavedWorkflowName(name)) {
    return {
      ok: false,
      reason: "invalid_name",
  ...
  const fromRoot = savedWorkflowRoot(cwd, "global", { homeDir, workspaceIdentity });
  const toRoot = savedWorkflowRoot(cwd, "project", { homeDir, workspaceIdentity });
