...
export async function resolveCreateWorkflowInput(
  ...
  const found = resolveSavedWorkflow({
    cwd,
  ...
  if (!found.ok)
    return describeResolveFailure(model.saved.name, found, cwd, model.saved.scope, workspaceIdentity);
...
async function resolvePathSource(
  ...
  if (file.meta === undefined && model.args !== undefined) {
    return failure(
      `The workflow script file ${file.described} declares no arguments (it has no \`/* ccbuddy-workflow\` metadata block), so it takes none. Drop \`args\`, or add a block declaring them.`,
    );
  }
...
function describeResolveFailure(
  ...
  return failure(`${headline}\n\n${describeAvailableWorkflows(cwd, workspaceIdentity)}`);
}
 ...
 */
function describeAvailableWorkflows(cwd: string, workspaceIdentity?: string): string {
  const project = listSavedWorkflows({ cwd, scope: "project", workspaceIdentity }).entries;
