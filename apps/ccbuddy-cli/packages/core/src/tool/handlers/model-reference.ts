...
import type { ModelCatalogEntry, ModelSelection } from "@ccbuddy/contracts";
import {
  CCBUDDY_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@ccbuddy/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(CCBUDDY_MODEL_REASONING_SEPARATOR, searchFrom);
