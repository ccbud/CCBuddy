...
// 这里写一个。
//
// 落点 `~/.ccbuddy/workspaces/<key>/workflow-drafts/`，与保存定义、编译入口在同一
// 逻辑工作区状态根下。
//
// **尽力而为**：状态根写不进去时返回 `undefined`，模型可再内联提交。绝不回落到
// CCbuddy 数据根以外。
...
interface WriteWorkflowDraftInput {
  /**
   * 会话物理工作目录；参与状态根 key 的计算，缺席则无处可写。
   */
  cwd: string | undefined;
  /** 远程工作区的逻辑隔离身份；缺席时以 cwd 为 key。 */
...
export async function writeWorkflowDraft(
  ...
  try {
    const dir = path.join(
      resolveWorkspaceStateRoot(input.cwd, { workspaceIdentity: input.workspaceIdentity }),
      WORKFLOW_DRAFTS_DIR,
    );
    ...
    // 尽力而为：状态根写不进或盘满时当作「这次没有草稿」。调用方据此
    // 退回旧文案，工具调用本身照常完成。
    return undefined;
  }
}
