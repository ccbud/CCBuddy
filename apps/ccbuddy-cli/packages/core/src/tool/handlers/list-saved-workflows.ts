...
const LIST_SAVED_WORKFLOWS_DESCRIPTION = [
  `Lists the dynamic workflows saved for this project (\`~/.ccbuddy/workspaces/<key>/${SAVED_WORKFLOW_PROJECT_DIR}/\`) and the global archive (\`~/.ccbuddy/workflows\`). These are workflow DEFINITIONS you can run, not past runs — for the run history use ListWorkflowRuns instead.`,
  "",
...
const listSavedWorkflowsHandler: ToolHandler = async (input, context) => {
  ...
  const { entries, invalid } = listSavedWorkflows({
...
function formatListSavedWorkflowsModelContent(output: unknown): ModelMessageContent {
  ...
  if (workflows.length === 0 && invalid === undefined) {
    ...
    return [
      ...
      `No workflows are saved in this project yet. Saved definitions live in CCbuddy's workspace ${SAVED_WORKFLOW_PROJECT_DIR}/ directory.`,
      "</saved_workflows>",
    ].join("\n");
  }
