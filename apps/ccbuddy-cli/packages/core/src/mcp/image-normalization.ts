...
} from "@ccbuddy/ccbuddy-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@ccbuddy/ccbuddy-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[CCBUDDY_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
