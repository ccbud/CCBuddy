...
} from "./speculative-compaction.js";
import {
...
async function compactActiveConversationImpl(
  ...
  let preparedFirstPass =
    trigger === CompactTrigger.Auto && options.initialPromptTooLongCause === undefined
      ? takeSpeculativeCompaction(this, {
          entries: initialSelection.entriesForSummary,
          instructions: compactPrompt,
          model: compactModel,
        })
      : undefined;
  if (trigger !== CompactTrigger.Auto) discardSpeculativeCompaction(this);
  ...
  while (true) {
    try {
      ...
      while (true) {
        const summaryInputEntries = preparedFirstPass
          ? buildSecondPassEntries(
              entriesForSummary,
              countContextPrefixMessages(entriesForSummary),
              preparedFirstPass,
            )
          : entriesForSummary;
        ...
        const recordableEntries = filterOutputTokenContinuationEntries(summaryInputEntries);
        const recordableRequestMessages =
          recordableEntries === summaryInputEntries
            ? requestMessages
