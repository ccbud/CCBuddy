...
import type { FileSystemPort, Logger, TraceContext } from "@ccbuddy/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  const workspaceStateRoot = resolveWorkspaceStateRoot(input.workspaceRoot, {
    dataRoot: input.storageRoot,
  });
  return input.scope === "project"
    ? join(workspaceStateRoot, "agent-memory", agentKey)
    : join(workspaceStateRoot, "agent-memory-local", agentKey);
}
