...
import {
  CCBUDDY_CUA_OFFICIAL_MCP_NAMESPACE_NAME as CCBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME,
  CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
} from "@ccbuddy/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@ccbuddy/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(CCBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === CCBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(CCBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === CCBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === CCBUDDY_CUA_OFFICIAL_PLUGIN_ID,
  );
