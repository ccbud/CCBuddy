...
import type { ModelInputMessage } from "@ccbuddy/contracts";
import type {
...
export class ContextBuilder {
  ...
  build(): ContextBuildResult {
    ...
    // 「You are CCbuddy, an interactive coding agent」对一个
    ...
    if (!hasCustomSystemPrompt) {
      if (!isWorkflowActor && this.config.presentationSurface === "ccbuddy_desktop") {
  ...
  private assembleSystemMessages(sections: ContextSection[]): ModelInputMessage[] {
    ...
    if (dynamicSystemContent) {
      messages.push({
        role: "system",
        // CCbuddy by design：Main Agent 的 dynamic system block 自带左边界，所有 provider 保持一致。
