import { CompactTrigger } from "@ccbuddy/contracts";
import { ESTIMATED_TOKEN_CHAR_DIVISOR } from "@ccbuddy/shared";
import type {
...
import { modelMessageContentBlockToText, modelMessageContentToText } from "@ccbuddy/contracts";
...
function stringifyToolCallInputForTokenEstimate(input: unknown): string {
  try {
    ...
    // CCbuddy 的模型输入仍可能包含未知内容；异常输入不能让本地预算估算中断 compact。
