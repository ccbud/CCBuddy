...
import { OFFICIAL_CUA_PERMISSION_RULE_TOOL_NAME } from "@ccbuddy/shared";
...
export class PermissionService {
  ...
  checkPermission(
    ...
    // 修复原因：旧顺序先让 plan transition / yolo 等快捷分支返回 allow，导致显式禁用的
    // 工具或项目 deny 规则被跳过。硬拒绝必须在任何模式、会话授权和自动审批之前裁决。
   ...
   * 但**压不过"阻断"**。显式禁用和项目 deny 已由 checkPermission 的第一道门统一处理，
   * 因此这里仅处理 alwaysAsk 自身的会话授权与确认语义。
   */
