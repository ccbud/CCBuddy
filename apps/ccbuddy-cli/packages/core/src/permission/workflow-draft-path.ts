...
import { WORKFLOW_DRAFTS_DIR } from "@ccbuddy/contracts";
 ...
 *
 * 为什么安全：那个目录是 CCbuddy 数据树里的机器自有草稿目录，不在工作区版本库内；
 * 用户不会往里放需要保护的项目文件，改动一份草稿不影响项目里任何东西。而
...
/** `filePath` 是否落在当前逻辑工作区的 CCbuddy 草稿目录之内。 */
function isWorkflowDraftPath(input: WorkflowDraftPathInput): boolean {
  ...
  const draftsDir = path.resolve(
    resolveWorkspaceStateRoot(input.workingDirectory, {
      workspaceIdentity: input.workspaceIdentity,
    }),
    WORKFLOW_DRAFTS_DIR,
  );
  ...
  // 只看路径字符串，不看文件系统：相邻的 `workflow-drafts-other/x.ts` 不会因为前缀相同
  // 被误判为目录内——`relative` 给出的是 `../workflow-drafts-other/x.ts`。
  return (
