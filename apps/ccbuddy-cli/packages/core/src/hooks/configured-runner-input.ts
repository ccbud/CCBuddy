...
import { resolveWorkspaceHookTimeoutMs } from "@ccbuddy/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "ccbuddy-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 CCbuddy camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    CCBUDDY_PROJECT_DIR: input.cwd || workingDirectory,
    CCBUDDY_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      CCBUDDY_PLUGIN_DATA: plugin.dataPath,
      CCBUDDY_PLUGIN_ID: plugin.id,
      CCBUDDY_PLUGIN_NAME: plugin.name,
      CCBUDDY_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.CCBUDDY_PLUGIN_DATA = plugin.dataPath;
    replacements.CCBUDDY_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|CCBUDDY_PLUGIN_DATA|CCBUDDY_PLUGIN_ROOT|CCBUDDY_PROJECT_DIR|CCBUDDY_SESSION_ID|CCBUDDY_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "CCBUDDY_SKILL_DIR") {
