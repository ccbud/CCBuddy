 ...
 * 实现（ProtocolBrowserControlBroker）把它翻译成 CCbuddy Protocol 的
 * interaction/browserExecute 反向请求，由 app（host→main WebContentsView/CDP）执行。
 *
 * 类型说明：BrowserCommand/BrowserCommandResult 与 @ccbuddy/shared 的 browser-use 契约同构。
 * 此处定义结构镜像（不 import @ccbuddy/shared，避免 agent contracts 的 zod v3 与 shared zod v4
...
/** CCbuddy Protocol 使用包装结果；BrowserControlPort.list 会解包并直接返回 browsers。 */
...
// 与 @ccbuddy/shared 的 browserCommandSchema 各变体结构镜像同步。
...
/** 受控 tab 摘要（list 命令返回）；与 @ccbuddy/shared 的 browserTabSummarySchema 镜像同步。 */
