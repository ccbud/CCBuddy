...
/** 项目作用域在 CCbuddy 用户数据树中的相对目录。 */
export const SAVED_WORKFLOW_PROJECT_DIR = "workflows";

/**
 * 草稿目录（相对 CCbuddy 的逻辑工作区状态根）。模型在两次提交之间就地编辑的脚本文件
 * 落在这里，与该工作区的 `workflows/` 同级。
 */
export const WORKFLOW_DRAFTS_DIR = "workflow-drafts";

/**
 * 全局作用域的存放目录（相对 CCbuddy 数据根）。落点
 * `~/.ccbuddy/workflows/<name>.dwf.ts`，对所有项目可见。
 */
...
/**
 * 作用域。两档：`project` 落在 CCbuddy 的工作区状态根 `workflows/`，只在那个逻辑工作区
 * 可见；`global` 落在 `~/.ccbuddy/workflows/`，对所有项目可见。一个文件的作用域由它所在
 * 的目录推得，frontmatter 不存。
 */
