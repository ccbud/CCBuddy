...
readonly EXPECTED_APPLE_EVENTS_USAGE="CC Buddy uses automation to continue conversations in Terminal or iTerm."
...
APPLE_EVENTS_USAGE="$(/usr/libexec/PlistBuddy \
  -c 'Print :NSAppleEventsUsageDescription' "$INFO_PLIST")"
readonly BUNDLE_ID VERSION BUILD_VERSION MINIMUM_SYSTEM_VERSION EXECUTABLE_NAME APPLE_EVENTS_USAGE
readonly EXECUTABLE="$APP_PATH/Contents/MacOS/$EXECUTABLE_NAME"
readonly HELPER="$APP_PATH/Contents/MacOS/ccbud-gateway"
...
[[ "$APPLE_EVENTS_USAGE" == "$EXPECTED_APPLE_EVENTS_USAGE" ]] \
  || fail "Apple Events usage description is missing or changed"
[[ -x "$EXECUTABLE" ]] || fail "main executable is missing"
[[ -x "$HELPER" ]] || fail "ccbud-gateway is missing"
[[ "$(lipo -archs "$EXECUTABLE")" == "arm64" ]] || fail "main executable is not arm64-only"
"$ROOT/native/Scripts/verify-gateway-helper.sh" "$HELPER" arm64 >/dev/null
cmp -s "$APP_PATH/Contents/Resources/LICENSE" "$ROOT/LICENSE" \
  || fail "GPL-3.0 license resource is missing or changed"
cmp -s "$APP_PATH/Contents/Resources/CCBuddyGateway-NOTICE.txt" \
  "$ROOT/native/GatewayHelper/NOTICE" \
  || fail "cc-switch attribution notice is missing or changed"
cmp -s "$APP_PATH/Contents/Resources/CCBuddyGateway-THIRD-PARTY-NOTICES.txt" \
  "$ROOT/native/GatewayHelper/THIRD-PARTY-NOTICES.txt" \
  || fail "gateway third-party notices are missing or changed"
cmp -s "$APP_PATH/Contents/Resources/Wake-LICENSE.txt" \
  "$ROOT/native/Resources/Wake-LICENSE.txt" \
  || fail "Wake MIT license resource is missing or changed"
cmp -s "$APP_PATH/Contents/Resources/Zstandard-LICENSE.txt" \
  "$ROOT/native/Resources/Zstandard-LICENSE.txt" \
  || fail "Zstandard BSD license resource is missing or changed"
