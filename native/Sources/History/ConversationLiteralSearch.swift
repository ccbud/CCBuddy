...
struct ConversationLiteralSearch {
    struct Match {
        ...
        let count: Int
        /// Carries nonoverlap across independently decoded search blocks.
        let lastUTF16End: Int
    }
    ...
    init(query: String) {
        ...
        // Paths, identifiers, punctuation and mixed Han/ASCII phrases use the same literal
        // scanner. Escaping every scalar preserves regex metacharacters as literal input.
        let fastLiteral = !scalars.isEmpty && scalars.allSatisfy {
            $0.value < 128 || Self.isUnifiedHan($0.value)
        }
        var pattern = NSRegularExpression.escapedPattern(for: query)
        var maximumCandidateLength = queryUTF16Length
        if fastLiteral, queryUTF16Length <= 1_024 {
            pattern = ""
        ...
        expression = fastLiteral && queryUTF16Length <= 1_024
    ...
    /// A block owns only match starts before ownedUTF16Length; its right context can include
    /// adjacent blocks. startingAtUTF16 resumes after a previous block's crossing match.
    func match(in text: String, countingOccurrences: Bool = true,
               startingAtUTF16: Int = 0, ownedUTF16Length: Int? = nil) -> Match? {
        ...
        guard let expression else {
            return foundationMatch(in: text, countingOccurrences: countingOccurrences,
                startingAtUTF16: startingAtUTF16, ownedUTF16Length: ownedUTF16Length)
        }
        ...
        var count = 0
        var lastEnd = 0
        let ownedEnd = min(source.length, max(0, ownedUTF16Length ?? source.length))
        var cursor = max(0, startingAtUTF16)
        while cursor < ownedEnd {
            ...
            let chunkEnd = scalarBoundary(atOrAfter: min(ownedEnd, cursor + 65_536), in: source)
        ...
        return Match(range: range, count: count, lastUTF16End: lastEnd)
    }

    private func foundationMatch(in text: String, countingOccurrences: Bool,
                                 startingAtUTF16: Int, ownedUTF16Length: Int?) -> Match? {
        ...
        let ownedLength = min(length, max(0, ownedUTF16Length ?? length))
        guard startingAtUTF16 < ownedLength else { return nil }
        let ownedEnd = String.Index(utf16Offset: ownedLength, in: text)
        var cursor = String.Index(utf16Offset: max(0, startingAtUTF16), in: text)
        while cursor < ownedEnd {
            ...
            let chunkEnd = text.index(cursor, offsetBy: 16_384, limitedBy: ownedEnd) ?? ownedEnd
            ...
            while cursor < chunkEnd {
                ...
                if !countingOccurrences {
                    return Task.isCancelled ? nil : Match(range: range, count: count,
                        lastUTF16End: range.upperBound.utf16Offset(in: text))
                }
        ...
        return first.map { Match(range: $0, count: count,
            lastUTF16End: (lastEnd ?? $0.upperBound).utf16Offset(in: text)) }
    }
