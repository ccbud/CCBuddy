...
final class ClaudeCLIGatewayE2ETests: XCTestCase {
    func testRealClaudeCLIStreamsThroughCCBuddyGatewayAndResumesWithAssistantContext() async throws {
        ...
        let gatewayToken = "sk-gateway-ccbud-claude-cli-e2e"
        ...
        for request in requests {
            ...
            let target = try XCTUnwrap(URLComponents(string: "http://localhost\(request.target)"))
            XCTAssertEqual(target.path, "/v1/messages")
            XCTAssertEqual(
                target.queryItems?.first(where: { $0.name == "beta" })?.value,
                "true",
                "The gateway dropped Claude CLI's endpoint query: \(request.target)"
            )
    ...
    private func isolatedEnvironment(
        ...
        environment["ANTHROPIC_BASE_URL"] = "http://127.0.0.1:\(gatewayPort)"
